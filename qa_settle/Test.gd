...
# ═════════════════════════════════════════════════════════════════════════════
## ── СКОЛЬКО ЖДАТЬ, ПОКА СОШЕДШАЯСЯ КУЧА РАЗОЙДЁТСЯ НА ЛИЧНЫЙ ОБЪЁМ ─────────
## Разводка идёт шагами Unit.SEP_MAX_STEP раз в Unit.SEP_INTERVAL и
## заканчивается сама. Здесь стояло 3000 с пометкой «трёх секунд хватает и на
## 240 человек», и это перестало быть правдой ровно по двум причинам, обе —
## заказ владельца (авг. 2026):
##   • личный круг поднят (Unit.PERSONAL_RADIUS 0.175 → 0.25), то есть той же
##     куче нужно заметно больше места;
##   • бойцы РАЗНЫХ отрядов держатся ещё шире (Unit.SEP_CROSS_SQUAD), а куча в
##     блоке E собрана из ШЕСТИ отрядов — она и обязана расходиться дальше.
##
## Разница именно в СРОКЕ, а не в покое: замер прямой — при ожидании 15 секунд
## дрожание той же кучи равно 0.00000 м за 60 кадров, то есть куча встаёт
## насмерть, просто позже. Число выводится, а не подбирается: радиус, который
## куче нужен занять, делённый на скорость разводки.
##
## СКОРОСТЬ БЕРЁТСЯ С БОЛЬШИМ ЗАПАСОМ ВНИЗ (SETTLE_SPEED_FRAC). Номинально
## поправка даёт SEP_MAX_STEP за SEP_INTERVAL, но в середине плотной кучи
## соседи стоят со всех сторон и их поправки почти взаимно гасятся — реальная
## скорость расхождения там кратно ниже номинальной
const SETTLE_PILE := 240
const SETTLE_SPEED_FRAC := 0.5

static func settle_ms(n: int) -> int:
	var spacing: float = Unit.SEP_MIN_DIST * Unit.SEP_CROSS_SQUAD
	var need_r: float = sqrt(float(maxi(n, 1)) / PI) * spacing
	var speed: float = (Unit.SEP_MAX_STEP / Unit.SEP_INTERVAL) * SETTLE_SPEED_FRAC
	return int(ceil(need_r / maxf(speed, 0.01) * 1000.0))

@onready var SEP_SETTLE_MS: int = settle_ms(SETTLE_PILE)
