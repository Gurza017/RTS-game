...
# ═════════════════════════════════════════════════════════════════════════════
# H. СОН _process СО ЩИТОМ — ТРЕБОВАНИЕ РАЗВЁРНУТО (сент. 2026)
# Раньше стоящая армия мечников обязана была ЗАСЫПАТЬ по картинке. С переезда
# на общую отрисовку узел спрайта стоит на паузе (_look_detach_node), и ленту
# листает ТОЛЬКО визуальный тик: уснувший мечник замирал на случайном кадре
# idle — жалоба владельца «лучники и рыцари еле дышат». Теперь правило то же,
# что у копейщика: МНОГОКАДРОВЫЙ покой не усыпляется, стоящий мечник дышит.
# Проверяем и факт (не спит), и само право на сон (_process_can_sleep — false
# на многокадровой ленте): факт без права ловил бы случайное совпадение
# ═════════════════════════════════════════════════════════════════════════════
func _h_sleep() -> void:
	print("\n═════ H. СТОЯЩИЙ МЕЧНИК ДЫШИТ, А НЕ СПИТ СТАТУЕЙ ═════")
	var men: Array = []
	for i in range(20):
	...
	for m in men:
		...
		if mw.is_guarding():
			guarding += 1
	print("  из 20: не спит %d, щит поднят %d" % [awake, guarding])
	verdict("H1 стоящие мечники дышат (визуальный тик не спит)", awake >= 18,
		"не спит %d из 20" % awake)
	# Право на сон: у ленты покоя мечника больше одного кадра — спать нельзя.
	# Свойство, а не наблюдение: не зависит ни от LOD, ни от случайного тайминга
	var w0 := men[0] as Warrior
	verdict("H2 многокадровый покой не даёт права на сон",
		not w0._process_can_sleep() and w0._look_frames > 1,
		"can_sleep=%s кадров в ленте=%d" % [w0._process_can_sleep(), w0._look_frames])
...
# Опасное место механики: _process пересчитывает _guard_active, и выключенный
# _process оставил бы флаг щита взведённым — боец ходил бы на 65% скорости до
# конца партии. С сент. 2026 стоящий мечник по картинке НЕ СПИТ ВОВСЕ
# (многокадровый покой, см. блок H) — J1 стережёт именно это: вернись сон,
# и опасность вернулась бы вместе с ним. J2/J3 остаются защитой на случай,
# если сон когда-нибудь вернут (однокадровая лента, смена правила)
# ═════════════════════════════════════════════════════════════════════════════
func _j_wake_speed() -> void:
	...
	verdict("J1 стоящий мечник не спит (дышит, а щит не залипает)", not slept,
