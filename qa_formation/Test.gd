...
func _d_phalanx() -> void:
	...
	if times.is_empty():
		...
		# ── ПОРОГ ВЫВОДИТСЯ ИЗ КОНФИГА, А НЕ СТОИТ ЧИСЛОМ ───────────────
		# Здесь была жёсткая четырёхсотка, посчитанная под прежний разброс в
		# 1900 мс. Владелец разброс СОКРАТИЛ (жалоба «копья выставляют один-два
		# человека, приходится нажимать дважды» — см. Spearman.DROP_DELAY_MAX_MS),
		# и стенд покраснел не от поломки, а от разворота требования.
		# Проверяем СВОЙСТВО: волна есть, шлагбаума нет. Четверть заданного
		# разброса — это заведомо больше любого дрожания такта и заведомо
		# меньше самого разброса
		var spread_min: int = Spearman.DROP_DELAY_MAX_MS / 4
		verdict("D2 опускание НЕ одновременное", hi - lo > spread_min,
			"разброс %d мс при пороге %d" % [hi - lo, spread_min])
