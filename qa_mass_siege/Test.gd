...
func _m_siege() -> void:
	...
	for recS in five:
		for uS in (recS[1] as Array):
			...
			# Строка ядра — вслед за телепортом: стоящий отряд спит по физике,
			# и без sync_row центроид (по строкам) оставался на старом месте —
			# ось кольца смотрела на ПРОШЛУЮ позицию, сектора ложились за
			# зданием, и с фундаментами (ТЗ 19.09.2026) отряды шли вокруг дома
			(uS as Unit).sync_row()
	await pf(30)
