...
## Спринт 15 (11.09.2026): гноллы, залп, голос, мечники, тролль и пень, монах, свиноконница, Z-index, брод, стресс

Восемь блоков заказа владельца (сводное ТЗ пришло тремя письмами: основное,
уточнение по монаху, блок профилирования). Стенды: `qa_gnoll_fix` 36/36,
`qa_monk_healing` 19/19 (новый), `qa_monk_vfx` 26/26 (переписан под новый
контракт), `qa_river_crossing` 14/14 (новый), `qa_boar_rider_charge` 20/20
(новый), `qa_swordsmen_charge` 10/10 (новый), `qa_performance_stress` (новый),
`qa_troll_impact` 29/29, `qa_troll` 24/24, `qa_troll_hitbox` 20/20,
`qa_cavalry` 33/33, `qa_sprint13` 80/80, `qa_map` 35/35, `qa_visual_smoke`
26/26 (новый блок «боец поверх декора»), `qa_voice_cmd` 27/27.

### Блок 1. Гноллы

- Скорость 3.6 → 2.5, кулдаун 1.6 → 2.08 (+30 %), дуга броска 0.18 (положе
  стрелковой 0.20 — «бросок от руки»), кость на земле живёт 5 с
  (`Arrow.BONE_STUCK_LIFETIME`, свой срок у своего пула).
- **КУВЫРОК КОСТИ — КОЛОНКА ПОЛЁТА, А НЕ ФЛАГ СЛОЯ.** Признак «крутить»
  едет в ядро колонкой `_afAxK` (масштаб оси в instance-цвете), шейдер
  `mm_arrow` крутит квад вокруг оси взгляда при `spin_turns > 0` и малой длине
  оси. Тот же урок, что у `_afB` в спринте 14: всё, что описывает ОДИН полёт,
  лежит в колонке полёта.
- **ВЫЛЕТ КОСТИ ПРИВЯЗАН К КАДРУ ЗАМАХА** (`GNOLL_THROW_FRAME` 4 →
  `_throw_left` в тике гнолла, `_release_bone`). Отсюда две ловушки стендов,
  обе пойманы: замороженный (`set_tick(false)`) гнолл не бросает ВОВСЕ —
  стенд держит его на месте прикалыванием (`_pin`), а не заморозкой; и
  двадцать четыре вызова `_on_attack_fired` подряд дают ОДНУ кость (каждый
  следующий перебивает замах) — `qa_sprint13` B14-B16 ждёт каждый бросок.
  Счётчик промахов переехал в момент ВЫЛЕТА (`_throw_miss`): считать его при
  прицеливании значило делить на несостоявшиеся броски (13 из 17).
- Idle: `walk` только при `moved_recently() and state != IDLE` — «перебирает
  ногами на месте» давал разбор наложения у стоящего.

### Блок 2. Залп, голос «в атаку», фильтр стойки

- **ЗАЛП ЗАПИРАЛ ОТРЯД ПОРОГОМ ГОТОВНОСТИ, А НЕ САМИМ ЗАЛПОМ.** Пока окно
  закрыто, стрелок не бьёт (`Archer._may_strike_now`), а окно открывалось при
  доле готовых от ЧИСЛА ЖИВЫХ. В бою половина отряда цели в дальности не
  имеет, 0.7 от всего состава не набиралось никогда — отряд с купленным
  залпом молчал всю партию. Доля теперь считается от тех, кому ЕСТЬ В КОГО
  стрелять (`able`), плюс потолок ожидания `VOLLEY_FORCE_MS` 2500: готовые
  есть, доля не набирается — залп идёт всё равно. Вторая половина —
  гарантия «регулярно», без неё одного вечно неготового хватало, чтобы залп не
  случился никогда.
- **«В АТАКУ» — ПРИКАЗ АТАКИ, А НЕ МАРШ ВСЛЕПУЮ** (`VoiceControl._order_attack`).
  Марш идёт МИМО врага (авто-агро только в покое), лучники проходили сорок
  метров сквозь дальность выстрела не стреляя. Теперь ближайший чужой у
  центра отряда по сетке (`_nearest_foe`, радиус `ATTACK_M × 0.85` — запас на
  замок цели `_lock_sight_range` и глубину строя) получает тот же
  `command_attack`, что и ПКМ; стрелок встаёт и стреляет, пехота доходит.
  Врага нет — прежний марш.
- **ФИЛЬТР СТОЙКИ ИСПРАВЕН — ПРОВЕРЕНО ЗОНДОМ И СТЕНДОМ.** Сопоставление
  «копейщики держать строй» даёт `groups=["spearman"]`, `qa_voice_cmd` C1
  держит 12/12 копейщиков в обороне при 0 лучников и 0 мечников. Что
  наблюдал владелец — это, по всей видимости, распознаватель, потерявший
  слово рода войск (малая модель не знает «копейщикам» — см. раздел о
  голосе): без названного рода команда по построению идёт ВСЕМ. Менять
  умолчание «без рода — всем» не стал: оно нужно «держать строй» и «вперёд»
  без адресата. Вопрос владельцу: должна ли стойка БЕЗ названного рода
  игнорироваться вместо «всем».

### Блок 3. Мечники

- **ШИРОКАЯ ЛИНИЯ БОЯ НА МАРШЕ** (`Unit.MARCH_INTERCEPT_MELEE`): перехват на
  марше искал помеху в `attack_range + INTERCEPT_MARGIN` (2.4-2.8 м у пехоты —
  «упёрся телом»), отряд в четырёх метрах от чужого фланга проходил мимо.
  У РУКОПАШНОЙ пехоты (дальность ≤ 5 м, `pursues_target`) радиус свободного
  перехвата не ниже `AGGRO_RADIUS`; замок приказа игрока и выход из боя
  глушат его как прежде. Замер `qa_swordsmen_charge` C: фланг в 6 м — 3 из 6
  втянуты, в 14 м — 0, дошли.
- **СТЕНА ЩИТОВ — ПОСЛЕДНИЕ МЕТРЫ РАЗБЕГА** (`Warrior.SHIELD_WALL_RANGE` 8,
  `SHIELD_WALL_SPEED_MULT` 1.3 поверх `RAGE_SPEED_MULT`, `shield_wall_active()`
  = набег И цель ближе дальности стены). Замер: база 1.90, разбег 2.75, у
  стены 3.58 м/с, щит поднят на 94 из 94 кадров, враг отброшен на 0.37 м.

### Блок 4. Тролль и пень

- Спрайт −15 % (`TROLL_PIXEL_SIZE` 0.0261, `TROLL_SIZE_SCALE` 3.4, кольца,
  клик и прицел масштабированы вслед), запас −30 % (12 700).
- **ВСПЫШКА КРАСНАЯ И ЯРЧЕ** (`TROLL_FLASH_PEAK` 0.5, `TROLL_FLASH_COLOR`).
  Общая вспышка была белой ПО ПОСТРОЕНИЮ (`mix(rgb, vec3(1.0), COLOR.b)`);
  цвет стал uniform `flash_color` — свойство ленты, как привязка ног и пятна
  крови (`Bucket.set_flash` ← `Unit.hit_flash_color`). Туша на смерти
  краснеет (`TROLL_DEATH_TINT` в материале билборда тела).
- **СЕРИЯ ИЗ ТРЁХ, КАСАНИЕ ОТЛОЖЕНО** (`TROLL_SWINGS` 3, `TROLL_SWING_HIT_SEC`
  0.3, `TROLL_SWING_STEP` 0.6). Урон и разлёт списывались в момент НАЧАЛА
  замаха при ленте в 600 мс — пехота разлеталась раньше касания. Теперь
  `_strike_damage` только замахивается и делает полшага к цели
  (`push_smooth`), а `_release_swing` в тике (и на одышке — замах начат до
  неё) бьёт цель и дугу разом; базовый удар отключён `_damage_on_strike`.
- Пень: прицел стрелка — `TrollLair.aim_height()` 1.8 м (Archer спрашивает
  `aim_height` и у построек), овал 1.5 × 1.0 со сдвигом центра К КАМЕРЕ на
  0.45 радиуса (`ring_center` берёт направление у камеры — ракурс фиксирован,
  а угадывать ось карты нельзя; без камеры сдвига нет), стрелы на здании
  РАСТВОРЯЮТСЯ (`Arrow._land_on_building` = втыкание + `fade_out_in` 0.6 с —
  тот же путь, что у вытеснения лишних, второго «исчезнуть» не заведено).
  Стражи восполняются таймером (`TROLL_RESPAWN_SEC` 150 до `TROLL_GUARDS_MAX`
  3), пока пень жив.

### Блок 5. Монах

- Радиус поиска 10 м, дистанция каста 3.0 (`MONK_CAST_RANGE`), поводок 14
  (`MONK_HEAL_LEASH`), приказ подхода не чаще 0.6 с (`MONK_STEP_SEC`).
- **ЦЕЛЬ ДЕРЖИТСЯ ДО ПОЛНОГО ИЗЛЕЧЕНИЯ** (`_keep_or_pick`): прежний выбор
  самого раненого КАЖДЫЙ такт при двух одинаково побитых лечил обоих через
  такт, и «по одному» на экране не читалось. Монах — тоже пациент.
- **ПОДХОД — ПРИКАЗ, А НЕ ВТОРАЯ МЕХАНИКА ХОДЬБЫ** (`_walk_to_patient` →
  `command_move` рядом с пациентом, не в него; приказ игрока важнее).
- **VFX В ЦЕНТРЕ СПРАЙТА, РАЗМЕРОМ СО СПРАЙТ** (`_vfx_geom`: середина —
  `base_y × V_STRETCH`, тот же счёт, что у `qa_visual_smoke`; квад — высота
  рисунка × 1.05, потолок 6 м). Прежняя `aim_height()` — точка прицеливания,
  у копейщика 0.8 при середине 1.4: заклинание висело у колен.
- `qa_monk_vfx` переписан: раненый в дистанции каста (иначе стенд мерил бы
  подход), B9 — центр спрайта из ленты цели, C — шаги внутри дистанции
  каста, E2б/E3 — более раненый НЕ перехватывает, переезд после излечения.

### Блок 6. Свиноконница

- **10 М ПРОБЕГА НЕ НАБРАТЬ ПРИ РАЗГОНЕ С 10 М.** `charge_min_runup` 10, а
  `_charge_from` ставится на `charge_range` (было 10): касание на 2.2 м даёт
  7.8 м, и удар не проходил никогда — `qa_cavalry` F5/F8 покраснели, зонд
  показал IMPACT только после `charge_range` 14 (пробег 11.86). Шлюз A
  стартовал ДО этой правки, его красные — стар. состояние.
- **ПРЯМОЛИНЕЙНОСТЬ — ПУТЬ ПРОТИВ СМЕЩЕНИЯ** (`_charge_path`,
  `CHARGE_STRAIGHT_FRAC` 0.85): всадник, покрутившийся в свалке, набирал
  метры, не отъезжая, — это и был Double Charge Bug.
- **УДАР ПО РЯДАМ** (`_charge_rows`, `charge_row_kill` 1, `charge_row2_frac`
  0.3, `charge_row_depth` 1.1, `charge_splash` 3.0 — три ряда): первая ячейка
  гибнет, второй ряд — отлёт, падение 2 с и 30 % (через броню: медиана 21 %),
  третий — `push_smooth` на половину разлёта. Ряды ОКРУГЛЯЮТСЯ, а не берутся
  полом: веер проекций внутри шеренги иначе сажал третью в вторую.
- **КОНТРАТАКА** (`Unit._counter_charge_ready`, копейщик: «Защита» И
  `spearman_1d` «Плотный строй»): удар и толчок в ноль
  (`_push_after_cap` → 0 на `COUNTER_CHARGE_SEC` 4), копья бьют вдвое
  (17.24 против 8.62), всадник остановлен; фаланга давит своим напором.

### Блок 7. Z-index и брод

- **ОТХОД ДЕКОРАЦИИ ОТ КАМЕРЫ ТОПИТ ЕЁ В ГРУНТ — ЦЕНА СЧИТАЕТСЯ ТОЧНО.**
  Камера под 45°: на экранной высоте s метров нарисован грунт на s глубже,
  отход на P прячет все пиксели ниже P экранных метров. Дерево (5.0 м экрана)
  при отходе 7.05 исчезло целиком — `qa_visual_smoke` «объекта нет вовсе».
  Растительность сортируется по точке на земле (`ground_depth` в
  `veg_multimesh`, `depth_push` 0), а БОЕЦ ПРИПОДНЯТ К КАМЕРЕ (`depth_lift`
  0.35 в `mm_unit_sprite`): грунт того, кто ближе, перекрыть не может. Отход
  руины (половина высоты со спринта 12) и руды снят — та же скрытая поломка.
  Новый блок дыма «боец поверх декора» (копейщик в кусте 90.9 %, на руине
  97.0 %) гасит одни слои армии, декор стоит на обоих снимках.
- **ВОДА — ГДЕ ГРУНТ НИЖЕ ЗЕРКАЛА, А НЕ ПОЛОСА ВОКРУГ ОСИ** (`Main.wet_depth`
  = `RIVER_DEPTH × WATER_DROP − 0.02 − SHORE_DRY`, `SHORE_DRY` 0.10; зеркало
  на 11.85 м от оси, запрет был на 9.6). Знак запаса ошибся в первой версии
  (плюс отодвигал запрет ГЛУБЖЕ) — поймал `qa_map` B2б (24 точки под
  зеркалом). Ядро (`RiverWater`) считает той же мерой через `SetRiver(…,
  wetDepth)`. `slide_around_water` опознавал реку прежней шириной и шаг в
  новую полосу воды уезжал по радиусу от LAKE_CENTER — поймал `qa_map` B7.
- **БРОД — ДОРОГА ПРИКАЗОМ, У КАЖДОГО СВОЯ ПОЛОСА** (`Main.ford_route`,
  `Unit._ford_stage` 1 → 2, `_ford_lane` = смещение от центра отряда вдоль
  русла, `FORD_LANE_MARGIN` 4): гуськом шли потому, что скольжение вело всех
  в ОДНУ середину брода. Пост — конечная точка, `_march_target` — она же
  (бой посреди переправы иначе кончался у выхода из брода). Замер
  `qa_river_crossing`: 24 из 24, фронт 6.0 м до и на переправе, в воде 0.

### Блок 8. Профилирование и чистка

- Аудит: поиск целей у монаха, гноллов, тролля, голосового «в атаку» и удара
  с разгона идёт по сетке (`unit_grid.query_radius`); единственный обход
  группы в Unit — `_next_enemy_building`, по событию. Пулы: стрелы, кости,
  один квад VFX на монаха; картинка костей/стрел/колец/теней — по одному
  MultiMesh на слой. Пакетный толчок НЕ строился: по правилу проекта
  оптимизация без A/B не делается, а `qa_performance_stress` меряет тик
  замеса 50 × 50 с чарджем, лечением, дубиной и инъекциями смертей посреди
  действия (цель лечения, цель замаха, гнолл с костью, сбитый, сам пень).
  Ссылки на логово в Troll/Gnoll проверены — все под `is_instance_valid`.
- **ЧТО НАШЁЛ СТРЕСС-СТЕНД:** первый прогон — 13 × «Trying to cast a freed
  object» в `Monk._keep_or_pick` (`heal_target as Unit` на освобождённом
  пациенте, правило 5) и два снимка с эффектом лечения на мёртвом — та же
  ссылка роняла `_ride_heal_vfx` до `_stop_heal_vfx`. После починки: 11/11,
  ноль ошибок; физтик армии 2.56 мс на 100 бойцов при 14 инъекциях смертей,
  254 взмахах дубины, 19 костях в полёте разом и снесённом на 20-й секунде
  пне. Стенные интервалы между физкадрами (средний 16.7 = такт 60 Гц, p95
  29.5, худший 56 мс) — это пейсинг headless-движка на загруженной машине, а
  не тик: бюджет p95 30 мс намеренно щедрый (правило 12).
- Свиноконница в замесе не давила НИКОГО, пока стояла за спинами своей
  пехоты: 10 м по прямой из-за спин не набрать — это правило заказа, а не
  поломка; стенд ставит её на фланг с чистой дорогой к лучникам.

### Ловушки стендов этого спринта

- Плавный толчок и отлёт — СКОРОСТЬ, которую интегрирует тик жертвы:
  замороженный стоит на 0.00 м (`qa_boar_rider_charge` C5, `qa_swordsmen_charge` B5).
- Первый ряд, убитый на месте, к третьему кадру освобождён: типизированное
  `var u: Unit = men[i]` роняет корутину (правило 5).
- Шлюз, запущенный до правки конфига, показывает красные старого состояния —
  перед «починкой» перепроверять зондом (qa_cavalry F5/F8).
- **ФИНАЛЬНЫЙ ШЛЮЗ (46 сцен) — 0 регрессий спринта, четыре наследственных
  красных починены:** `qa_goblin` B4/C2 считал стаю гноллов у пня стартовыми
  отрядами орды (13 против 10 — со спринта 13); `qa_difficulty` G2/G3/H4/H5 —
  ЗАГРУЖЕННОЕ ЛОГОВО рожало вторую стаю поверх восстановленной (+3 отряда,
  +36 бойцов; `TrollLair._start_gnoll_pack` теперь молчит, если гноллы уже на
  карте); `qa_start_build` E3 читал снятые спринтом 14 гнёзда стрел
  (`troll._arrows`) и ронял стенд молча — переписан на «не втыкаются»;
  `qa_crowd` C ставил копейщика в угол деревни орды — с широкой линией боя он
  цеплял спящего гоблина в 10 м, будил деревню и гибнул до конца замера
  (площадка отодвинута от деревни, правило 5 в ожидании). `qa_mega_battle`
  B1-1 — известный ножевой порог (15.1 при 11.6), не регрессия.

## Спринт 16 (11.09.2026): залп по умолчанию, аура монаха и ветка кузницы, кольцо тролля, гноллы

Четыре блока заказа владельца (второе письмо расширило блок монаха: подключить
`heal effect`, ветка кузницы — темп/объём/радиус, AOE-перк). Стенды:
`qa_volley_fix` 8/8 (новый), `qa_gnoll_behavior` 14/14 (новый),
`qa_troll_ring/Shot` 11/11 (новый, оконный), `qa_monk_vfx/Shot` 6/6 (новый,
оконный), `qa_gnoll_fix` 37/37 (+D2б), `qa_monk_vfx` 26/26, `qa_monk` 13/13,
`qa_monk_healing` 19/19, `qa_volley` 23/23, `qa_archer_logic` 20/20,
`qa_sprint13` 80/80, `qa_forge` 48/48, `qa_hud_afford` 13/13, `qa_troll`
24/24, `qa_troll_impact` 29/29, `qa_visual_smoke` 26/26.

### Блок 1. Лучники: залп по умолчанию не стрелял

- **КОРЕНЬ — ДЕШЁВЫЙ ОТСЕВ В `_sweep_volleys`.** «Без единой купленной
  способности отряду тут делать нечего»: `ability_on` пуст — `continue`. Но со
  спринта 13 плата за способность снята, а режим ВКЛЮЧЁН ПО УМОЛЧАНИЮ
  (`squad_ability_on` без ключа — `true`); ключ в словарь кладёт только щелчок
  переключателя. Отряд с изученным `archer_1d`, которого никто не трогал, жил
  в режиме залпа: `Archer._may_strike_now` ждёт `squad_volley_open`, а такт
  залпов до отряда не доходил — окно не открывалось никогда. Ровно то, что
  видел владелец: выключил залп руками — «стрельба вернулась». Отсев снят;
  отряд без изученного узла и так отсеивается `squad_volley_mode` через
  `squad_has_ability`.
- **СИНХРОННОСТЬ ВЕРНУЛАСЬ К ДОЛЕ ОТ ЖИВЫХ.** Спринт 15 считал долю готовых
  от «способных» (в дальности), и залп рассыпался: `qa_volley` C1 «пик 3 при
  10». Теперь `whole` — от живых (`VOLLEY_READY_FRACTION` 0.7), а от способных
  — только принудительный залп по `VOLLEY_FORCE_MS` 2.5 с (`forced`), чтобы
  отряд, половина которого не достаёт, не молчал вечно.
- **«ПО ГОТОВНОСТИ» — ШИРЕ.** Разброс одиночной стрельбы умножен на
  `READY_SCATTER_MULT` 1.7 (множитель, а не число: кузница и выучка режут его
  так же). Залп кладёт стрелы золотым углом с `VOLLEY_JITTER` 0.10 — кучная
  туча.
- **СТЕНД `qa_volley_fix` СТАВИТ ЛОВУШКУ ДОСЛОВНО:** узел изучен, словарь
  пуст (A1 проверяет это как свойство), ПКМ по строю бессмертных копейщиков.
  A2-A4: окно открывается, ≥ 2 залпов за 20 с, пик за кадр ≥ половины
  отряда. B: переключатель выключен, стрелки в РАЗНОЙ фазе перезарядки
  (`_attack_timer = randf() × cooldown` — с одинаковыми таймерами они и «по
  готовности» били бы одним кадром), B2 «не одним кадром», **B3 — промахов
  больше: доля СТРЕЛ В ЗЕМЛЕ** (цели бессмертны и стоят; над квадратом 4×4
  залп 0-7 % в землю, по готовности 33-34 %). Первая версия сравнивала
  разброс точек втыкания — у залпа их ноль (все стрелы в цели), сравнивать
  нечего. **Чужой строй обязан быть ГЛУБОКИМ:** туча залпа раскладывается
  диском не уже `VOLLEY_MIN_SPREAD` 1.1 м, и над шеренгой в два ряда (0.7 м)
  треть её точек ложится за спины — залп «промахивался» на 30 % против 19-26
  у одиночной стрельбы; это накрытие, а не точность.
- **ТУЧА СХЛОПЫВАЛАСЬ В ТОЧКУ ПО ЖРЕБИЮ НОМЕРОВ УЗЛОВ.** В финальном шлюзе
  B3 покраснел зеркально: залп 54 из 60 в землю при разбросе втыканий
  0.07 м — весь отряд положил стрелы в ОДНУ точку в 1.1 м от центра чужого
  строя, мимо всех. Место стрелка в туче считалось как
  `get_instance_id() % VOLLEY_SLOTS` (97): у бойцов одного заказа номера
  узлов идут с одинаковым шагом (сцена рождает одно и то же число объектов),
  и при шаге, кратном 97, k у всех одно. Тот же жребий — `qa_volley` D2/E1,
  которые «мигали от прогона к прогону» со спринта 10. Теперь k —
  порядковый номер в составе (`GameManager.squad_member_ordinal`, линейный
  поиск по событию выстрела), одиночка без отряда — прежний путь. **Диск
  Фогеля заполняется на ЧИСЛО стрелков** (`squad_member_count`), а не на 97
  слотов: с номерами 0..9 при делителе 97 все десять точек легли в
  центральную треть радиуса, и залп задел двоих из шестнадцати (`qa_volley`
  D5 после первой версии правки).

### Блок 2. Монах: аура, ветка кузницы, AOE

- **VFX ЛЕЧЕНИЯ НЕ РИСОВАЛСЯ С САМОГО СПРИНТА 14, И ЭТОГО НИКТО НЕ ВИДЕЛ.**
  Headless `qa_monk_vfx` (26 проверок) стережёт узлы, привязку, лист, срок —
  всё зелёное; пикселей в headless нет. Оконный `qa_monk_vfx/Shot` дал
  «изменилось 0.0 %» при видимом узле в дереве, верной позиции и 3416
  непрозрачных пикселях текстуры. Зонд из четырёх квадов рядом: тот же
  материал с `ground_depth 1` — виден, с `ground_depth 0` — пуст;
  `StandardMaterial3D` — виден. **Причина в шейдере `cyl_billboard`: `DEPTH`
  присваивается в ветке `if (ground_depth > 0.5)`; шейдер, пишущий `DEPTH`
  хоть где-то, пишет `gl_FragDepth` ВСЕГДА, и ветка без присваивания отдаёт
  неопределённое число — фрагмент отбрасывается.** Ветка `else` теперь пишет
  `DEPTH = FRAGCOORD.z`. Это латентная грабля для любого будущего
  `ground_depth = 0`.
- **ГЛУБИНА VFX — ОТ ТОЧКИ НА ЗЕМЛЕ, ПОДТЯНУТАЯ К КАМЕРЕ** (`ground_depth` 1,
  `depth_push` −`HEAL_VFX_DEPTH_PULL` 0.6). Со своей глубиной (0) крест в
  центре ауры прятался бы за телом: спрайт бойца приподнят к камере на
  `depth_lift` 0.35, а центр спрайта на 0.43 м выше ног даёт лишь 0.3 м
  выигрыша. Подтяжка 0.6 > 0.35 — аура поверх тела, как и просили.
- **БЕЗ КОМПЕНСАЦИИ НАКЛОНА** (`v_stretch` 1 у обоих квадов). Растяжение
  `cyl_billboard` идёт ОТ НИЖНЕЙ КРОМКИ (правило наземных декораций): у
  квада, поставленного центром в центр спрайта, середина уезжала на
  0.207·h вверх — над голову. Единица рисует квад ровно вокруг узла.
- **АУРА СТРОИТСЯ КОДОМ** (`Monk._aura_texture`, статический кэш): лист
  `Heal_Effect.png` — 4 % непрозрачных пикселей со средней альфой 10/255,
  в бою невидим. Четыре кадра пульсирующего кольца (радиус 22 → 26 px из 64)
  с крестом, пиксель-арт без полутонов, `generate_mipmaps` (сэмплер
  `filter_nearest_mipmap`). Размер квада — `AURA_DIAM_PER_MID` 2.4 × высота
  центра спрайта над ногами (фигура ≈ 2·mid), пол `AURA_DIAM_MIN` 1.0: первая
  версия брала 0.9 высоты ЛИСТА — у копейщика 320 px при фигуре ~80 px, и
  кольцо в 4.6 м лежало далеко за силуэтом. Снимок: зелёное кольцо с крестом
  на груди копейщика, искры листа над ним.
- **ВЕТКА КУЗНИЦЫ:** `BONUS_KEYS` += `bonus_heal_rate` / `bonus_heal_amount`
  / `bonus_heal_radius`; узлы монаха 1a-3a «Псалом/Литания/Гимн» (+15/20/25 %
  к темпу — такт `MONK_HEAL_TICK/(1+rate)`), 1b-3b «Елей/Миро/Реликвия»
  (+15/20/25 % к объёму за такт), 1c-3c «Проповедь/Хор/Колокол» (+1.5/2/2.5 м
  к радиусу); прежние боевые бонусы этих узлов обнулены — узел лечит, а не
  бьёт. Цены и время — по ряду, как у всех. Подписи `MOD_SHORT_LABELS`
  добавлены. Читает `Monk.heal_tick_sec / heal_amount_mult / heal_radius`
  через `unit_bonus`.
- **AOE-ПЕРК «БЛАГОДАТЬ» (`monk_1d`, `"toggle": true`)** — тот же механизм,
  что залп: изучен → включён по умолчанию, HUD показывает переключатель через
  `_Forge.is_toggle_ability`. `Monk.aoe_active` → `_heal_pulse_aoe`: все
  раненые свои в радиусе, каждому `tick / MONK_HEAL_SEC_PER_MAN × mult ×
  MONK_AOE_RATE` (0.5 — «в 2 раза медленнее»); цель подхода и VFX остаётся
  самым раненым; `aoe_touched` — для стендов.

### Блок 3. Тролль: три кольца, серия, урон

- **ТРЕТЬЕ КОЛЬЦО — ЗАПИСЬ В ЧУЖОЙ СЛОЙ.** `set_hover_units` кладёт тролля в
  тонкий слой `_hover_b` (`wants_fine_ring`, 64-сегментный меш, масштаб
  `fine_ring_scale` 1.32 × овал 1.45/0.9), а покадровый догон
  `_update_hover_positions` писал ТОТ ЖЕ индекс в грубый слой `_hover` с
  `ring_scale()` 3.57: у ног тролля появлялось толстое кольцо чужого слота,
  а его тонкое стояло на месте постановки (второй тонкий овал — тот же слот
  с другого кадра). Теперь слой и масштаб берутся из `_hover_in_b`.
- **«СМЕЩЁННЫЙ ТОНКИЙ ОВАЛ» ПОСЛЕ ПРАВКИ ОСТАЛСЯ — И ЭТО БЫЛА ТЕНЬ.** По
  буферам ядра кольцо и квад тролля стоят в одной точке (x, z совпадают до
  сантиметра, стоя и на ходу), а на снимке кольцо лежало на ~0.8 м впереди
  ступней. `_anim_bottom_px` считал краской любую альфу > 0.02 — в арт
  тролля впечатана полупрозрачная тень 15 строк под ступнями, и на точке
  земли стояла ЕЁ кромка: 15 × 0.0261 × 1.41 ≈ 0.55 м экрана, 0.78 м по
  земле. У пехоты то же поле в 2-3 строки — незаметно. Ступни тролля
  ищутся по плотной краске (`TROLL_FOOT_ALPHA` 0.5, аргумент `min_alpha`);
  `base_y` 3.876 → 3.322. Остальные фракции не тронуты.
- **ЗАЖИМ ВЫРАВНИВАНИЯ ЛЕНТ — В ПИКСЕЛЯХ ЛЕНТЫ.** `BASE_Y_FIX_MAX` 0.45 м
  подобран под пиксель 0.0108 (~42 px); у тролля это 17 px. У тролля все
  ленты с одним полем (87 строк), сдвиг ноль, зажим не срабатывал — но
  правило общее, и для следующего крупного бойца с разным полем лент он бы
  сработал. `cap = BASE_Y_FIX_MAX × max(px / BASE_Y_FIX_PX, 1)`.
- **СЕРИЯ ПРИВЯЗАНА К ЦЕЛИ** (`Troll._series_target`): смена цели между
  ударами обнуляет `_swings`, серия [1]→[2]→[3]→одышка начинается заново.
  `TROLL_REST_SEC` 4.0 → 2.75 (заказ 2.5-3 с), `TROLL_SWEEP_DMG_MULT` 1.2 —
  множитель на `_strike_damage`, то есть и по цели, и по дуге.
- **`qa_troll_ring/Shot` (оконный, 11):** A — наведение на ИДУЩЕГО тролля
  (без движения покадровый путь не работает), живые слоты по всем слоям колец
  через `rb_slot` (ненулевой базис в 6 м): ровно 1, в тонком, ни в одном
  кадре больше; A5 — низ рисунка на ходу вправо; B — наведение + приказ: 2
  тонких, 0 грубых, снятие — 0; C — стоя и на ходу влево низ рисунка против
  экранной точки земли (разность снимков «слои армии погашены/показаны», как
  в `qa_visual_smoke`). Площадка — `_flat_spot` + вычистка леса: `(0, −40)`
  прежних снимков тролля лежит в русле, и кольцо там пряталось под зеркалом
  воды.

### Блок 4. Гноллы: кость замирает, поведение

- **КОСТЬ НА ЗЕМЛЕ УЖЕ БЫЛА НЕПОДВИЖНОЙ ПО ПОСТРОЕНИЮ** (`Arrow.axis_scale`:
  `GNOLL_BONE_AXIS_K` 0.4 в полёте, 1.0 у `_spent`; шейдер крутит при
  `axl < spin_gate`). Стенд этого не стерёг — добавлен `qa_gnoll_fix` D2б
  («лежащая кость помечена неподвижной»: ось полной длины у всех торчащих).
- **ПОВЕДЕНИЕ** (`Gnoll.gd`, числа в `goblin_config`): `take_damage` →
  обидчик становится целью, если своей нет (`aggro_answers`) — базовый ответ
  на удар работает только вплотную (`COUNTER_CHARGE_RANGE`), а гнолла бьют
  стрелой с двадцати метров, и он продолжал патрулировать под обстрелом;
  `_tick_flank` молчит, пока есть цель (фланговый `command_move` снимал цель,
  и гнолл уезжал на фланг, не бросив, — `qa_gnoll_behavior` B2 «брошено 0»);
  после вылета кости — `command_move` к пню на `GNOLL_RUN_BACK` 5 м
  (`run_backs`), только у настоящего броска по цели и не под кайт-блоком;
  `_run_back_t` = дорога + 0.4 с держит фланг и патруль — иначе фланговый
  приказ перебивал отход в следующий такт и гнолл бежал ОБРАТНО к стрелку
  (B3 «было 7.1, стало 6.9»); отход под тем же надзором, что кайт
  (`_kite_from`/`_kite_check_t`: не сдвинулся — встал). Ниже `GNOLL_HIDE_HP`
  0.45 — `hiding_to_lair`: бег к пню, в `GNOLL_HIDE_RANGE` 4.5 м
  `TrollLair.hide_gnoll` (идиом гарнизона: `garrisoned`, невидим,
  `leave_render`, без тика, вне групп и сетки, `set_off_map`), таймер логова
  раз в секунду лечит `GNOLL_HIDE_HEAL_FRAC` 0.12 и на полном запасе
  `release_gnoll`; гибель пня выпускает всех (`take_damage` логова).
- **ЗАМАХ НЕ ТЕРЯЕТ КОСТЬ.** Новый `_on_attack_fired` при `_throw_left > 0`
  сперва зовёт `_release_bone()`: прежде он ПЕРЕЗАПУСКАЛ отсчёт, и гнолл,
  получивший цель от авто-агро посреди чужого замаха, терял первый бросок —
  `qa_gnoll_fix` C6/E6 «в воздухе 0», «костей +0» (зонд по кадрам: кость
  вылетала на 24-м кадре в изоляции и не вылетала в шлюзе, где авто-агро
  успевало раньше). Прежняя запись спринта 15 «двадцать четыре вызова дают
  одну кость» больше не верна: дают двадцать четыре.
- **`qa_gnoll_behavior` (14):** A патруль без врагов; B укол стрелка
  (заморожен, бьёт стенд от его имени) → цель, бросок с ≤ 8.5 + 1 м, отход
  (ПИК дистанции за окно — снимок в конце застаёт гнолла на новом наскоке),
  второй укол → второй бросок; C просадка ниже порога (броня режет долей —
  дожать поле, правило спринта 9) → бежит, прячется (внутри 1, невидим, не в
  группе, не тикает), лечится до полного, выходит на карту; D пень снесён
  при гнолле внутри — вышел, жив, тикает.

### Ловушки стендов этого спринта

- **ОКОННЫЙ СТЕНД СТАВИТ ОКРУЖЕНИЕ САМ:** `_flat_spot` (перепад высот в
  круге 6 м, суша по `main.is_water`) плюс `queue_free` леса в 30-40 м —
  случайная крона прятала кольцо тролля целиком (снимок с троллем в лесу
  при зелёных вердиктах по буферам), а `PLAYER_BASE_ANCHOR + (60, −60)` из
  headless-стендов лежит ЗА КАРТОЙ (чёрная пустота на снимке).
- **РАЗНОСТЬ ДВУХ СНИМКОВ ИДУЩЕГО БОЙЦА ЛОВИТ ЕГО ЖЕ СДВИГ.** Кольцо едет с
  ним, и его смещение между снимками читалось как «низ рисунка» (+1.14 и
  +3.22 м). На время пары снимков кольцо снимается; сам боец в «фоне»
  погашен слоем — тогда разность равна его рисунку, где бы он ни был.
- **ЦВЕТОВОЙ ФИЛЬТР ПО ОДНОМУ СНИМКУ НЕ ГОДИТСЯ:** «тёмная тень под ногами»
  находилась и в тёмной траве в трёх метрах.
- **`_on_attack_fired` НАПРЯМУЮ ИЗ СТЕНДА ДЕЛИТ СЛОТ ЗАМАХА С АВТО-АГРО:**
  собственный удар гнолла по цели в 8 м перебивал стендовый — зонд по кадрам
  (`state`, `attack_target`, `_throw_left`, кости в воздухе) показал это за
  70 строк; чинить пришлось не стенд, а сам замах.
- **ШЛЮЗ, ЗАПУЩЕННЫЙ ДО ПРАВКИ, ПОКАЗЫВАЕТ КРАСНОЕ СТАРОГО КОДА** (`qa_gnoll_fix`
  в шлюзе E) — та же оговорка, что в спринте 15. **И ПАТЧ ВО ВРЕМЯ ШЛЮЗА ЛОМАЕТ
  СЦЕНУ, СТАРТОВАВШУЮ МЕЖДУ ДВУМЯ ЕГО ФАЙЛАМИ:** `qa_wall` загрузил новый
  `Troll.gd` со старым `Unit.gd` («Too many arguments for _anim_bottom_px»,
  SCRIPT ERROR: 2 при «провалов: 0» — тролля в стенде просто не было).
  Перегнан отдельно: 13/13 минус известный A1.
- **ФИНАЛЬНЫЙ ШЛЮЗ (63 сцены + перегоны):** 0 регрессий спринта. Красные —
  только известные: `qa_mega_battle` B1-1 (ножевой порог, 16.6-21.1 при 11.6),
  B2-1 под нагрузкой (2-8 рыцарей из 463 сквозь щель 1.10 м), `qa_wall` A1
  (3 в тылу). `qa_volley_fix` B3 в первом прогоне финального шлюза покраснел
  зеркально (залп 90 % в землю) — это и вскрыло жребий номеров узлов в туче;
  после правки 8/8 трижды подряд, `qa_volley` 23/23, `qa_archer_logic` 20/20,
  `qa_full_game_4k` 4/4. Оконные: `qa_troll_ring/Shot` 11/11,
  `qa_monk_vfx/Shot` 6/6, `qa_visual_smoke` 26/26 (шейдер `cyl_billboard`
  тронут — дым обязателен).

## Спринт 17 (11.09.2026): строй по клавишам, рудник орды, главный цикл ИИ, агро, лоадер и ложная победа

Четыре блока заказа плюс второе письмо с двумя критическими багами («первый
клик по Старту только меняет курсор», «Победа на 4:30-5:00»). Стенды:
`qa_formation_keys` 23/23 (новый), `qa_goblin_mine` 24/24 (новый),
`qa_goblin_loop` 33/33 (новый), `qa_ai_loop` 23/23 (новый), `qa_aggro_fix`
16/16 (новый), `qa_menu_loader` 11/11 (новый), `qa_ai_loop/Smoke` (новый зонд
длинной партии, см. ниже); переписаны под новое расписание `qa_goblin` 73/73,
`qa_difficulty` 58/58, `qa_squad` 52/52, `qa_ai` 26/26, `qa_panic` 19/19,
`qa_goblin_tactics` 25/25; `qa_visual_smoke` 26/26.

### Блок 1. Строй при растяге ПКМ: клавиши 1 / 2 / 3

- **КЛАВИШИ ПЕРЕХВАТЫВАЮТСЯ ТОЛЬКО ВО ВРЕМЯ ЖИВОГО РАСТЯГА** (`SelectionManager.
  _rmb_drag_active()` = ПКМ зажата, мышь уже двигалась, нажатие не над UI).
  Цифры 1-9 — группы горячих клавиш; без этих ворот «1» во время растяга
  переключала бы выделение на первую группу, а вне растяга клавиши строя
  крали бы группы. Перехват стоит ДО ветки групп и гасит событие
  (`set_input_as_handled`).
- **[1] ШИРОКИЙ — прежний строй** (`FORM_WIDE`: минимум глубины, весь фронт),
  **[2] КОРОБОЧКА** (`FORM_DEEP`: у каждого отряда блок в `DEEP_ROWS` 3
  шеренги, блоки кладутся слева направо, не влезшие — следующим рядом сетки
  через `DEEP_ROW_GAP`), **[3] — переворот авангарда** (`formation_flipped`:
  A копейщики → лучники → мечники → монахи, B мечники → лучники → копейщики →
  монахи; `Formations.RANK_ORDER_FLIPPED` и `rank_order(flipped)`).
  Раскладка — `_mono_formation_slots` → `_deep_formation_slots` /
  `_block_formation_slots`; эшелоны по типам (`_layered_formation_slots`) в
  глубоком режиме ставят следующий эшелон от САМОГО ДАЛЬНЕГО места
  предыдущего, а не от расчётной глубины ряда.
- **ПРЕДПРОСМОТР ПЕРЕРИСОВЫВАЕТСЯ В ТОТ ЖЕ КАДР:** клавиша сбрасывает
  `_fp_last` в «никогда» и взводит `_fp_pending` — та же дорога, что у
  движения мыши, без второго пути отрисовки. Режим и переворот ЖИВУТ между
  растягами (владелец выбирает строй один раз), счётчик
  `formation_key_presses` — для стендов.

### Блок 2. Рудники: базовый гарантирован, у орды свой

- **БАЗОВЫЙ РУДНИК ГАРАНТИРОВАН:** `_add_base_resource_clusters` при
  водяной точке берёт `land_target`, а после раскладки проверяет
  `_resource_pieces_near` и при пустом месте сажает один узел напрямую.
- **РУДНИК ОРДЫ** (`Main._spawn_goblin_mine`, `goblin_mine_spot` = деревня +
  направление к центру × (`VILLAGE_RADIUS` + `MINE_OFFSET` 25)): обычный
  `Mine`, переведённый в собственность орды `set_owner_faction`
  (гоблины теперь считаются захватчиками и получают доход — невидимый для
  игрока, у орды нет склада на экране). Стража `MINE_GUARD` — всадник и два
  копейщика ветеранского ранга 4 с четырьмя наградами, через тот же
  `spawn_goblin_squad` + `grant_squad_veterancy`; вожак держит их в
  `mine_guard_sids` и патрулирует кольцом из шести точек `MINE_GUARD_PATROL_R`
  (`_mine_guard_plan`), врага у рудника бьёт.
- **ТРЕВОГА — СИГНАЛ РУДНИКА, А НЕ ОПРОС ВОЖАКА** (`Mine.attacked`,
  `_raise_alarm`): любой чужой боец у рудника орды в `_tick_capture` и любой
  удар по нему (`take_damage`) поднимают сигнал не чаще `ALARM_GAP_SEC`;
  вожак (`attach_mine` → `_on_mine_attacked`) ставит `mine_alarm_until` =
  часы + `MINE_ALARM_SEC` 45 и на это время шлёт К РУДНИКУ ВСЁ поле
  (`ROLE_MINE_ALARM`), гарнизон деревни остаётся. **Ловушка первой версии:**
  тревога поднималась только при попытке ЗАХВАТА, а стража блокирует захват
  самим присутствием — сигнал не приходил никогда; теперь он идёт на любого
  чужого в радиусе.
- **Рудник на холме у брода** (`_hill_mine`: ближайший к центру карты не
  свой рудник) в фазе беспокойства держит один отряд (`EXPAND_MINE_SQUADS`,
  `ROLE_MINE_HOLD`, `hill_mine_sid`).

### Блок 3. Главный цикл орды (GoblinAI)

- **СПЯЧКИ НЕТ** (`DORMANT_UNTIL_SEC` 0): вожак просыпается в первый такт и
  входит в МИРНУЮ ФАЗУ (`PHASE_PEACE`) до `PEACE_SEC` 600 — деревня
  отстраивается, гарнизон стоит, поле патрулирует границу (кольцо
  `PATROL_BORDER_R` 26 из `PATROL_BORDER_POINTS` точек раз в
  `PATROL_BORDER_SEC`), угроза у деревни → `plan_defense`. Множитель
  сложности `goblin_dormant_mult` теперь крутит `PEACE_SEC`
  (`game_difficulty_config.goblin_peace_sec`): на Easy к центру выходят позже.
- **ФАЗЫ:** PEACE → CENTER (марш с дозором, прежняя `plan_advance`) → при
  пустом центре **HARASS** (новая фаза беспокойства): отряд на рудник холма,
  рейды (`_tick_raids`: чередование малый `RAID_SQUADS_SMALL` / большой
  `RAID_SQUADS_BIG` раз в `RAID_INTERVAL_SEC`, всадники первыми, добыча —
  `_raid_prey`: рабочий, иначе не-крепость в `RAID_HUNT_RADIUS`; отход при
  запасе ниже `RAID_RETREAT_HP`, при `RAID_FLEE_FOES` чужих в
  `RAID_FLEE_RADIUS` или по `RAID_MAX_SEC` — в хижину лечиться `ROLE_HEAL`,
  `raid_retreats`), разведка (см. ниже) → **HUNT** (общий штурм) только по
  `_assault_ready`: отрядов волны ≥ `army_squads()`, беспокойство длилось ≥
  `HARASS_MIN_SEC`, часы ≥ `ASSAULT_EARLIEST_SEC` 2100 (35-я минута) и цель
  известна. Цель — `_assault_target()`: разведанная база ближайшая к деревне,
  иначе центроид ближайших чужих построек; прежний «слабейший» снят.
  HUNT → DEFEND (волна выбита) → CENTER, как прежде.
- **РАЗВЕДКА — ОТДЕЛЬНЫЙ ОТРЯД ИЗ ДВУХ ВСАДНИКОВ** (`_scout_plan`,
  `SCOUT_UNITS`, рождается из деревни): состояния out → harass → home;
  по маршруту `_scout_waypoints` (неразведанные углы карты и чужие рудники),
  `_scout_look` пишет `known_bases[фракция]` по чужим постройкам в
  `SCOUT_SIGHT`; нашёл добычу — кусает `SCOUT_HARASS_SEC` и отходит домой
  (план отхода к деревне), новая вылазка через `SCOUT_SORTIE_SEC`.
  Гарнизон (`register_garrison`, первые `GARRISON_SQUADS` 3 ветеранских
  копейщика деревни), стража рудника, месть и разведка — `_is_special`: не
  входят ни в волны (`_wave_squads`), ни в планы командира, ни в найм
  (`_economy` считает волну без них).

### Блок 3б. Цикл ИИ людей (EnemyAI)

- **ЧАСЫ ПАРТИИ У ИИ СВОИ** (`clock` в `_process`); оборона до `_assault_time()`
  = часы ≥ `AI_ASSAULT_AT_SEC` 2100 и (армия набрана или волна уже в поле);
  `PEACE_SECONDS` 600 — мирная фаза стройки.
- **ЗАПИСИ ОТРЯДОВ ИИ НЕ ДЕРЖАТ ID** — `_sid_of(sq)` берёт `squad_id`
  первого живого бойца; записи одного типа сливаются пополнением
  (`_regroup`), и стенд, ждущий «три отряда — три записи», не дождётся.
- **РУДНИК И РЕЙД БЕРУТСЯ ДО ГАРНИЗОНА** (`_command_squads_defensive`: пул
  не-отходящих → `_assign_mine_squad` → `_assign_raid_squad` → дом/остальные).
  При `HOME_GUARD_PER_TYPE` 7 излишков у ИИ не бывает, и «экспансия первым
  делом» иначе не наступала. Отряд на рудник (`ROLE_MINE`, копейщик или
  мечник) держит СВОЙ `mine_node` как резерв и считает `mines_captured`,
  когда тот стал красным; прежняя версия каждый такт меняла цель на «ближайший
  не свой», и счётчик не рос никогда. Рейд (`ROLE_RAID`, мечник первым,
  `RAID_MIN_ARMY` 3 записи и `RAID_MIN_FIELD` 1) бьёт `_raid_prey` (рабочий
  игрока, иначе не-крепость в `RAID_HUNT_RADIUS`), отходит в замок через
  `request_garrison` при слабости, перевесе или `RAID_MAX_SEC`.
- **БАШНИ НА ГРАНИЦЕ** (`_tower_spot`): после стрелковой ИИ ставит
  `AI_TOWERS` 2 башни на `TOWER_BORDER_FRACTION` 0.3 пути к базе игрока,
  разведённые на `TOWER_SIDE` поперёк оси.

### Блок 4. Агро орды и ответ на дальний обстрел

- **РАДИУС ИНИЦИАТИВЫ ОРДЫ = ОБЗОР** (`Unit.aggro_radius()`: гоблины —
  `vision_radius(attack_range)`, люди — прежние `AGGRO_RADIUS` 10; поводок
  орды не короче обзора + 2). Стенд B: копейщик орды идёт на врага в 12.5 м
  сам, копейщик игрока на той же дистанции — нет.
- **ОБСТРЕЛ ИЗ-ЗА ОБЗОРА — ОТВЕТ, А НЕ СТОЯНИЕ** (`_answer_far_fire` из
  `take_damage`, только у `answers_far_fire()` — гоблины; у гнолла и тролля
  свой ответ, общий выключен): нет цели, не в отходе, свободен от приказа
  игрока, стрелок дальше `COUNTER_CHARGE_RANGE` — при запасе ниже
  `FAR_FIRE_FLEE_HP` 0.3 отход в лагерь (`GameManager.goblin_camp_point`:
  деревня, иначе логово), иначе отряд идёт на стрелка (`squad_counter_charge`).
  Счётчики `far_fire_answers` / `far_fire_flees` — для стендов.

### Второе письмо: лоадер и ложная победа

- **ПЕРВЫЙ КЛИК ПО «СТАРТ» — КУРСОР СТАВИТСЯ В МЕНЮ** (`UIAssets.install_cursor`
  из `MainMenu._ready`, а не при входе в партию; `Main._apply_custom_cursor`
  зовёт то же). Воспроизвести headless нельзя — курсор живёт в окне; логика
  такая: пока игровой курсор не установлен, первый клик тратился на его
  установку и смену `mouse_mode`.
- **СВОЙ ЭКРАН ЗАГРУЗКИ** (`MainMenu._show_loader`: CanvasLayer 100, тёмная
  плашка, кольцо `_LoaderSpinner` в `_process`, «Загрузка карты…»; курсор
  прячется на время смены сцены, `change_scene_to_file` после двух кадров).
  Ограничение: сборка карты синхронна, кольцо на это время замирает —
  зато нет системного «занято».
- **ПОБЕДА НА 4:30 — ОРДА ВЫРЕЗАЛА ПЯТЬ РАБОЧИХ ИИ ДО ЕГО КРЕПОСТИ.**
  `_faction_beaten` = «нет живых и (нет крепости или нет зданий)»; у ИИ без
  спячки орды в первые минуты не было ни того, ни другого. Теперь
  `_ai_beaten()`: победа — только если крепость ИИ УЖЕ БЫЛА
  (`_ai_castle_placed`), иначе после `AI_WIPED_GRACE_SEC` 300 подряд без
  живых и без построек (`_match_clock` копится в PLAYING). Плюс сама орда
  первые 10 минут не выходит из деревни (мирная фаза), разведка — два
  всадника с отходом, штурм не раньше 35-й минуты.
- **ЗОНД ДЛИННОЙ ПАРТИИ `qa_ai_loop/Smoke`:** живой Main без игрока,
  ускорение ×4 (`physics_ticks_per_second` × 4 и `time_scale` × 4 — дельта
  тика остаётся 1/60), печать раз в игровую минуту: численности сторон, фазы
  орды, рейды/вылазки/базы, башни и рудники ИИ; вердикт — конец партии не
  наступил до N-й минуты (`-- minutes=N`).
- **ФИНАЛЬНЫЙ ШЛЮЗ 69 СЦЕН + ПЕРЕГОНЫ: 0 РЕГРЕССИЙ**, красные только
  известные (`qa_mega_battle` B1-1/B2-1). Под нагрузкой параллельного зонда
  краснели и одиночно зелёные `qa_vet` (лучник погиб за 1200 кадров, SCRIPT
  ERROR на `_die` павшего), `qa_balance` C7, `qa_start_build` D4,
  `qa_cavalry` — перегнаны порознь 55/55, 21/21, 30/30, 33/33.
- **ТРИ ПРОГОНА ЗОНДА ДЛИННОЙ ПАРТИИ, КАЖДЫЙ НАШЁЛ СВОЁ.** Все три — 42
  игровые минуты без «Победы» и «Поражения» (S1-S3 3/3): мир до 10-й
  минуты, дальше рейды с обеих сторон (орда 7-10 рейдов, ИИ до 29), разведка
  орды находит базу ИИ на 13-й минуте, ИИ ставит башню и берёт два рудника,
  штурм орды (`hunt`) — на 37-й. Первый прогон: 11 SCRIPT ERROR на 34-35-й
  минуте — запись плана с павшей целью (см. ловушки). Второй: одна ошибка
  правила 5 в `FogOfWar._apply_enemy_building_visibility` (снесённая башня
  ИИ в кэше группы — `as Node3D` до `is_instance_valid`) и **ЗАМИРАНИЕ С
  25-й МИНУТЫ**: 16 минут численности обеих сторон не менялись, а орда
  переключала «центр»/«оборона» каждую минуту, не сходя с места. Причина —
  отряд, посланный в хижину лечиться (`ROLE_HEAL` по `request_garrison`),
  чей поход к воротам сторож хижины ОТМЕНИЛ (боец дерётся без режима
  отхода): роль оставалась HEAL навсегда, приказов такому отряду не раздают,
  в поле он не считается — `_field_squads()` = 0 при 18 отрядах в реестре.
  Лечение — `_sweep_heal()` в такте после `_retreat_broken`: отряд, которого
  нет ни в очереди, ни внутри ни одной хижины, снова полевой (`qa_goblin_loop`
  F1/F2). Третий прогон после обеих правок — **0 SCRIPT ERROR, 42 минуты, численности
  сторон меняются всю партию** (ИИ 251 → 841 → 646 бойцов, орда 1141 → 923 →
  1082, штурм `hunt` на 41-й минуте, ИИ ставит две башни и берёт рудник).
  `qa_vet` после шлюза перегнан отдельно ещё раз: 55/55, стрела убивает цель
  за 25 кадров (в шлюзе под нагрузкой — 1200, цель дожила).

### Ловушки стендов этого спринта

- **`ai.tick()` при выключенном `_process` НЕ РАЗДАЁТ ПРИКАЗЫ:** очередь
  `_order_queue` дренит `_drain_orders` из `_process`. Стенд обязан дренить
  сам и двигать `ai.clock` (`_tick_ai` / `think`).
- **Разведка съедает единственного всадника** — стенд, считающий всадников
  волны, ставит второго; рабочих у чужой базы разведка убивает — бессмертие.
- **Освобождённый обидчик в `take_damage`** (правило 5) — свежий замороженный.
- **Убить гарнизон ИИ из стенда** — через `GameManager._live_units`, группа
  узлов укрытых не видит.
- **B2 `qa_aggro_fix`:** цикл ждёт `attack_target` и выходил в тот же кадр —
  дистанция ещё не менялась; замер сдвига после ожидания.
- **Стража рудника и рудник орды в `qa_goblin`** считались хижинами и
  стартовыми отрядами (11 хижин из 10, 13 отрядов из 10, 8 ранговых из 5) —
  помощники стенда исключают `mine_guard_sids` и `building_id == "mine"`;
  блок E проверяет сон без такта вожака (срок 0 будит тут же).
- **Тот же рудник орды в других старых стендах:** `qa_squad` 8 считает
  стартовые отряды по конфигам — добавлен `MINE_GUARD.size()`; `qa_panic`
  A5/A6/B6 считали флажок владельца рудника (`Mine._refresh_flag`, тот же
  скрипт знамени) бесхозным флагом отряда — узел под `Building` исключён;
  `qa_ai` 5 ждал гарнизон до потолка, а рудник и рейд теперь берутся раньше
  — гарнизон вправе недосчитаться ровно их (`by_role` mine + raid);
  `qa_goblin_tactics` E3 считал «в атаке 3 из 3», а один отряд в фазе
  центра держит рудник холма (`hill_mine_sid`) — считается поле командира.
- **ПАВШАЯ ЦЕЛЬ В СЛОВАРЕ РАВНА null.** `plan.get("foe") != null` у записи с
  освобождённой целью — ложь, и запись атаки проваливалась в ветку марша
  без `goal` (11 SCRIPT ERROR за минуту боя на 34-й минуте зонда). Ветка
  выбирается по `plan.has("foe")`, живость — `is_instance_valid`.

## Спринт 18 (11.09.2026): аудио-система — реплики, орда, тролль, река

Четыре блока ТЗ владельца: реплики приказов и звуки людей, голоса орды и
горн, голос тролля, эмбиент реки. Стенды: **`qa_audio18`** 57/57 (новый),
**`qa_audio_env/Probe`** (новый зонд огибающей — режет составные файлы);
`qa_audio` 46/46, `qa_audio2` 44/44, `qa_voice` 50/50, `qa_voice_cmd` 27/27,
`qa_hud_afford` 13/13, `qa_goblin` 73/73, `qa_troll` 24/24, `qa_troll_impact`
29/29, `qa_helpbuild` 24/24, `qa_worker_sheep` 20/20. Шлюз — в конце раздела.

### Файлы

- **ИМЕНА ФАЙЛОВ ОРДЫ И ТРОЛЛЯ ПЕРЕИМЕНОВАНЫ В ASCII** до подключения: в
  них стояли знаки «ΓÇô» (искажённое тире из чужой кодировки) и `#` —
  правило 8 (PCK регистро- и кодировкозависим). Теперь
  `Goblins_voice/goblin_attack_1..4.mp3`, `goblin_death.mp3`,
  `goblin_laugh_1..4.mp3`, `horde_horn.mp3`; `Troll_voice/troll_growl_2..4.mp3`,
  `troll_victory.mp3`. Папки не были импортированы вовсе (ни одного
  `.import`) — импорт сделан.
- **СОСТАВНОЙ ФАЙЛ РЕЖЕТСЯ ПО ТАЙМКОДАМ, А НЕ НА РЕСУРСЫ.** AudioStreamMP3
  на куски не делится, а перекодировать чужой файл — держать вторую копию.
  Зонд `qa_audio_env/Probe` (AudioEffectCapture на dummy-драйвере, окна
  20 мс, отрезки громче 8 % пика с паузой ≥ 0.12 с) дал у «come on» три
  отрезка [0.30-0.78] [2.24-2.80] [4.24-4.64] при длине 5.07 с, у «go go go»
  — голос с 1.14 с (тишина в начале пропускается). Куски заданы
  `ORDER_REPLIES` парами from/to; `play_voice` стартует `play(from)`, а
  `_tick_voice_clips` из `_process` глушит голос по `get_playback_position()
  >= to` (два голоса, одно чтение позиции в кадр). Замер стенда: кусок
  0.20-1.10 жил 0.89 с.

### Реплики приказов

- **СОБЫТИЕ ОСТАЛОСЬ, ФРАЗА — ИЗ ПУЛА.** `mass_march` и `hold_line` (события,
  по которым считают стенды и окна повтора) со спринта 18 играют одну из
  пяти реплик `ORDER_REPLIES`: «Hold the line», «Go, go, go!» и три «Come
  on!». Выбор случайный без повтора подряд (`_pick_reply`, `_reply_last`);
  `reply_last_id` — для стендов. Окна повтора (9 с) и «через раз» у
  `hold_line` не тронуты: прежний заказ «реплики реже» в силе, новый —
  «не одна и та же фраза» — закрыт разнообразием. Условие массовости (5+
  боевых отрядов) сохранено: «выбор точки» и марш — то же событие
  `mass_march`.
- **«Forward march» из пула выведен** (в ТЗ пять реплик названы поимённо);
  константа файла осталась, `VOICE_BANK` его держит для стендов файлов.

### Люди: молоток и щелчок здания

- **Молоток строителя** — категория `build_hammer` («mine 5»), зовётся из
  `Worker._animate_chop` в момент касания на BUILDING при `_build_settled`
  (у стены; стройка и ремонт руин — одно состояние). Рабочая категория:
  прореживание по плотности, как у топора и кирки.
- **Щелчок по любому зданию людей** — `UI_BANK.pick_building` = «Chest Close
  1» из папки юнитов; банк интерфейса теперь принимает `res://` (`ui_path`),
  как и боевой (`sfx_path`).

### Орда

- **Крики атаки** `goblin_attack` (4 файла): pitch 0.90-1.10 и **дрожание
  громкости** `db_jitter` ±3 дБ на запуск (новый ключ SFX_LIMITS,
  применяется в `play_3d`); кричат на удар с шансом `GOBLIN_SHOUT_CHANCE`
  0.08 (у людей 0.03). Категории клича и смерти стали ВИРТУАЛЬНЫМИ
  (`Unit._sfx_shout/_shout_chance/_sfx_death`) — копейщик, всадник и гнолл
  орды переопределяют; тролль рычит.
- **Смерть** `goblin_death` — pitch 0.92-1.08 (±0.08), 5 голосов.
- **ГОРН** — `play_horn()` → событие `horde_horn` канала реплик, но на
  СВОЕЙ шине `Horn` → SFX с AudioEffectReverb (room 0.55, wet 0.28, dry
  0.85) — умеренный эмбиент; −8 дБ, окно 20 с. Триггеры в GoblinAI:
  `_check_first_contact()` раз за партию (`first_contact`): точка орды
  (деревня, рудник, центры отрядов) освещена туманом игрока ЛИБО боец игрока
  в обзоре гоблина (`nearest_of_side`, только FACTION_PLAYER); и переход фаз
  → CENTER / → HUNT (выход всей армии). Счётчик `horn_blows`.
- **ХОР СМЕХА** `play_chorus("goblin_laugh", at)`: все четыре файла разом,
  первый без задержки, остальные с микро-задержкой до `LAUGH_STAGGER_MAX`
  0.14 с (`create_timer`), окно между хорами 4 с. Триггер —
  `GameManager._on_squad_wiped` (зовётся из `remove_from_squad` ДО
  `_disband_squad`, словарь отряда цел): сторона игрока, тип не одиночный
  (рабочий — не воин), гоблин в `LAUGH_SIGHT` 18 м от точки гибели.
  **Слышимость проверяется ОДИН РАЗ, в момент гибели:** точка ещё освещена
  обзором павшего, маска пересчитывается раз в 0.15 с, и отложенные голоса
  туман не спрашивают — иначе половина хора молчала бы. Так и заказано:
  смех звучит, пока точка видна, туман закрывает её тактом позже.
- **Кто добил** — `Unit._slain_by` пишется в `take_damage` перед `_die`;
  читается в `_on_squad_wiped` на сырой ссылке (правило 5).

### Тролль

- **Рык** `troll_growl` (3 файла wolf/growl): выход из пня —
  гарантированный через `_spawn_growl_t` 0.15-1.4 с (разнесён, чтобы три
  стража не попали в одно окно категории); марш — раз в
  `TROLL_GROWL_MIN/MAX_SEC` 12-20 с хода (`moved_recently`) с шансом
  `TROLL_GROWL_MARCH_P` 0.35; бой — `_shout_chance` `TROLL_GROWL_FIGHT_P`
  0.12 на удар. Человеческий предсмертный стон у тролля снят.
- **Клич победы** `troll_victory`: в `_on_squad_wiped` убийца последнего —
  тролль (`stat_id`), жертва не орды: отряд ИЛИ рабочий (отряд из одного).
  На каждого павшего не звучит — только на последнего (стенд F8).

### Река

- `AMBIENCE_RIVER` = «River Stream Loop» (прежний «файл моря» в коде не
  использовался вовсе). Плеер на шине **Ambient** (новая шина → Master; туда
  же переехал лес; музыка отдельно). Громкость — от ТОЧКИ ФОКУСА камеры
  (`GameManager.view_point`, как у марша): `river_gain_at` = 1 в русле и
  на кромке (`RIVER_HALF_W`), smoothstep до нуля за `RIVER_FADE_M` 5 м;
  умножается на `march_zoom_gain()` (отдалённая камера воды не слышит);
  сглаживание `_tick_river` — вход 1.6 с, выход 2.2 с; пик `RIVER_DB` −20.
  Замер стенда: 0.35 с → доля 0.2, 2.5 с → 1.0; выход 0.5 с → 0.8, 3 с →
  0. Пауза глушит реку и горн вместе с остальным.

### Ловушки стендов

- **КАМЕРУ ЗАМОРОЗИТЬ И ПОСТАВИТЬ ЯВНО:** в headless курсор в углу экрана,
  край-панорама уводит камеру каждый кадр — вместе со слушателем (ребёнок
  пивота) и точкой фокуса; `pan_to` пишет только фокус, узлы двигает
  `_update_position()`. Без этого голоса пула не выдавались (слушатель за
  `SFX_CULL_DISTANCE`), а `update_view_point` стенда перетирался камерой.
- **Флаг `stream_paused` у незвучащего плеера не ставится** (движок):
  проверять паузу на звучащем горне.
- **`squad_is_combat` после гибели последнего отвечает «нет»** (считает
  живых) — «воин ли отряд» в `_on_squad_wiped` берётся по типу отряда.
- **Точка реальной деревни к концу стенда освещена** бойцами самого
  стенда — путь «игрок открыл туман» проверяется на тёмной точке с
  подменённым `village`.
- `qa_audio2` E-ДОП роняет один SCRIPT ERROR НАМЕРЕННО (cfg чужого типа),
  стенд это знает; C2 «звук не влияет на физшаг» — замер по стенным часам,
  мигает под нагрузкой (правило 12).
- **ОДИН ЛИШНИЙ `randf()` СДВИГАЕТ ВСЕ ЖРЕБИИ ПАРТИИ.** Финальный шлюз дал
  четыре новых красных (`qa_monk` B1 63.0 → 50.5 HP дважды подряд — то есть
  детерминированно, `qa_squad` 8 «+1 отряд», `qa_gnoll_fix` C6/F6, `qa_cavalry`
  E2): партия сеется `seed(world_seed)`, а таймеры рыка в `Troll._ready` и
  дрожание громкости брали из общего потока. Все НОВЫЕ случайности звука
  переведены на `AudioManager.rng` (свой RandomNumberGenerator), старые
  (питч в `play_3d`, прореживание топора/кирки, шанс клича 0.03 — у орды тот
  же) потребляют ровно столько, сколько до спринта; `qa_monk` вернулся к
  63.0. `qa_gnoll_fix` F считает кости только у площадки (стая у пня со
  спринта 17 стреляет по проходящим рейдам обеих сторон — её кости на
  другом конце карты замеру срока не принадлежат).
- **ФИНАЛЬНЫЙ ШЛЮЗ 74 СЦЕНЫ:** 0 регрессий аудио; красные — известные
  `qa_mega_battle` B1-1, `qa_cavalry` E2 (мигает и в шлюзе 17), `qa_audio2`
  C2 (стенные часы) и четыре сдвига жребия выше, все перегнаны зелёными
  после правки генератора: `qa_monk` 13/13, `qa_squad` 52/52, `qa_gnoll_fix`
  37/37 ×2, `qa_audio18` 57/57.

### Ребаланс ветеранства (спринт 18, второе письмо): «реже, но мощнее»

Заказ: набор опыта дороже на ~30 %, награды ощутимее (+2..+3 к атаке, заметная
броня/HP), сопутствующая компенсация, проверка `qa_vet` и `qa_balance`.

- **ПОРОГИ ×1.3** у всех родов войск, с округлением до 5 (регулярка по блоку
  `VET_CONFIG`): копейщик 40/80/120/180/240/300/350 → **50/105/155/235/310/
  390/455**; мечник и лучник 100…1300 → **130/260/520/780/1040/1300/1690**;
  монах 40…600 → 50/130/260/390/520/650/780; орда — гоблин как мечник,
  гнолл 80…1170, всадник 180…2080. Стенды порогов не хардкодят (правило 10):
  `qa_vet` 55/55 без правок.
- **ЛЕСТНИЦА НАГРАД:** лычки (1-3) главный **+3** (было +2), флаги (4-6) **+4**
  (было +3) с платой парным параметром **−2** (было −1); вторичный запас
  +8 / +10 (было +5 / +7), карточка здоровья +21 / +30 (было +14 / +20).
  Легендарные перки штандарта не тронуты — там награда правило, не цифра.
  Правило «ровно три карточки, ровно три параметра из тройки атака/броня/
  здоровье» сохранено (`qa_balance` D2/D2б/D3).
- **ПОТОЛОК СДВИНУТ СОЗНАТЕЛЬНО:** сумма по ветке атаки +25 вместо +13,
  копейщик с кузницей 47.5 урона (×3.4 базы; было ×2.5). Прежняя запись
  `BALANCE_MATH` «шаг вверх вернёт терминаторов» переписана: считать надо
  силу НА ОДНО УБИЙСТВО — 25/455 = 0.055 против 13/350 = 0.037 (+48 %), и
  ровно столько и заказано: ветеран режет новобранцев быстрее, а стоит
  дороже (30 % больше убийств на ступень). Затухающая броня (`ARMOR_SOFTNESS`)
  держит верх: очко после тридцатого вчетверо дешевле первого.
- **КОМПЕНСАЦИЯ ВРАГАМ — ГРЕЙД, А НЕ ЦЕНЫ.** Пре-выданные ветераны орды
  (`goblin_config.START_SQUADS` пять рядов, `MINE_GUARD` три) переведены с
  vet 4 / picks 4 на **vet 3 / picks 3**: при новой лестнице это те же +9 к
  главному (3+3+3 против 2+2+2+3), знамя на грейд ниже — сила стартовых элит
  и ранние тайминги не сдвинулись. Красный ИИ пре-выданных ветеранов не
  имеет (`START_SQUADS` пуст), качается по той же ×1.3-шкале, что игрок.
  Цены юнитов и улучшений не тронуты: удорожание опыта и рост шага
  симметричны для всех сторон, а спам плашек снимается самими порогами
  (первая ступень копейщика 50 убийств вместо 40, седьмая — 455 вместо 350).
- **СТЕНДЫ:** `qa_balance` +3 проверки — D3б лестница (+3/+21 HP на лычках,
  +4/+30 HP на флагах по каждой карточке), D3в пороги монотонны и первая
  ступень копейщика ≥ 50, D3г стартовые ветераны орды не сильнее прежних
  (+9 к главному); `qa_goblin_mine` B3 читает ранг стражи из `MINE_GUARD`,
  а не число 4. Итоги: `qa_vet` 55/55, `qa_balance` 24/24, `qa_difficulty` 58/58, `qa_vetui`
  45/45, `qa_goblin` 73/73, `qa_goblin_mine` 24/24, `qa_goblin_loop` 33/33,
  `qa_squad` 52/52, `qa_troll` 24/24, `qa_sprint13` 80/80.
- **`docs/BALANCE_MATH.md` раздел 5 переписан** (таблицы этапов были ещё от
  матрицы 3/4/5 с моралью и напором — устарели два спринта назад), потолок
  и пункт 6 «что сломается» — под новые числа.

### Спринт 18, третье письмо: щелчки зданий, «Стена копий», VFX монаха, голос по выделению, два логова, овцы и тролли, рудники ИИ, скалы

Стенд **`qa_sprint18b`** 49/49 (новый); переписаны `qa_boar_rider_charge` D
(новая механика), `qa_voice_cmd` (+C0а/C0б, 29/29), `qa_monk_vfx` B9б (26/26).
СТЕНДЫ_18B

- **ЩЕЛЧКИ ПО ЗДАНИЯМ ПО ТИПУ** (`GameManager.PICK_BY_BUILDING` →
  `pick_event_for(building_id)` в `_selection_click_sfx`): крепость —
  `bow_and_arrow_shot.mp3`, бараки — `lever_pull.mp3` (в паке лежал как
  «Lever_pull_ΓÇô_Mechani_#4», владелец прочёл его как «Liverpool G…4»),
  кузница — «Door Close 1.wav», остальные — «Chest Close 1». Марш ещё на
  20 % тише: `MARCH_WALK_DB` −16.0, `MARCH_RUN_DB` −14.1 (`qa_voice` F1в
  планка ×0.7×0.8).
- **«СТЕНА КОПИЙ» ВМЕСТО «ПЛОТНОГО СТРОЯ».** Старая контратака снята целиком
  (`_counter_charge_ready`, `COUNTER_CHARGE_DMG_MULT` — нет): она работала,
  только если всадник целил именно в копейщика, и не касалась ни тролля, ни
  бегущей пехоты. Новое: `forge spearman_1d` — режим-переключатель (как
  залп); в «Защите» отряд с ним смыкается в ТРИ шеренги
  (`GameManager.spear_wall_form`, зовётся из `on_squad_stance` — панель
  стоек и голос; не в бою), копья опускает сама стойка; налетевший с разгона
  — конница (`_charge_impact`), тролль (там же, топтание отменено), бегущая
  пехота (контакт солвера у бегущего в `_process_move`, раз за забег
  `_wall_struck`) — получает `spear_wall_strike`: **300 % удара копейщика**
  (`SPEAR_WALL_DMG_MULT`), разгон и напор сняты (`_counter_hit_until_ms`,
  `COUNTER_CHARGE_SEC` 4 с), бег остановлен, **замедление −30 %** на 5 с
  (`apply_slow`, множитель в `_effective_speed`). Обычный укол копий с разгона
  (`charge_counter_frac`) идёт помимо. Замер стенда: всадник 115 HP за
  контакт (36 стены + 79 укола), тролль остановлен, бегущий мечник встал.
- **VFX ЛЕЧЕНИЯ: РАЗМЕР СТРОГО ФИКСИРОВАН.** «Гигантский круг» был правилом
  спринта 15 «квад равен нарисованной высоте цели»: на копейщике оно давало
  кольцо 3.4 м, на лучнике — метр. Теперь `HEAL_VFX_SIZE_M` 1.3 (−20 % от
  прежнего минимума 1.6) и аура `AURA_DIAM_M` 0.8 одинаковы для всех; кольцо
  и крест по 2 px из 64 (было 4 и 6), крест короче; цвет (0.18, 1, 0.32) —
  ярко-зелёный. `_vfx_geom` от цели берёт только ЦЕНТР спрайта.
- **ГОЛОС — ТОЛЬКО ВЫДЕЛЕННЫМ** (`VoiceControl.squads_for` по
  `selection_manager.selected_units`, новый `SelectionManager.select_units`
  для стендов/программ): слово рода войск сужает выделение, «все» или без
  слова — всё выделенное боевое; пусто выделение — «некому», с места никто
  не срывается. Прежнее «по типу из всей армии» — причина жалобы.
- **ДВА ЛОГОВА** (`Main.troll_lair_center_ai`: от якоря ИИ по X зеркально
  `LAIR_OFFSET`, то есть к реке, по Z вниз; `GameManager.troll_lairs`,
  `register_lair`, `nearest_lair`, `lair_for(faction)`; `troll_lair` —
  первое, у игрока: месть орды и старые стенды не тронуты). Тролль без
  логова (слепок) берёт ближайшее.
- **ВСПЫШКА ТРОЛЛЯ** `TROLL_FLASH_PEAK` 0.5 → 0.07: едва заметна, красный
  проявляется окровавленностью шейдера по мере падения запаса.
- **ОВЦЫ И ТРОЛЛИ — САМОВОЗОБНОВЛЯЕМЫЙ РЕСУРС** (уточнение владельца):
  стадо пня вырезано/съедено → через `SHEEP_RESPAWN_SEC` 20 новая отара из
  `SHEEP_RESPAWN_COUNT` 5 (`TrollLair._arm_flock_respawn`, `kill_flip` овцы
  тоже сообщает логову); у стороны > `TROLL_RAID_SHEEP` 30 овец
  (`faction_sheep_count`) → раз в `SHEEP_PRESSURE_CHECK_SEC`
  `_sheep_pressure_check`: снесённый пень стороны восстанавливается на своей
  точке (`lair_spots`, `restore_lair`: руина снята, новое логово, у игрока —
  снова `troll_lair`), без живых троллей — выходит `TROLL_RAID_SPAWN` 1
  вор; сам тролль (`Troll._tick_raid`, фазы go → eat → home) идёт к стаду,
  съедает `TROLL_RAID_EAT` 3, берёт под пастьбу `TROLL_RAID_HERD` 5
  (`Sheep.herd_by`: привязка к загону снята, овца прыгает за пастухом) и
  ведёт к пню; дошёл — `adopt_sheep` (овцы — логова) и
  `TrollLair.on_raid_success` выпускает ещё `TROLL_RAID_REWARD` 3 тролля
  (голод при стаде > 5 без таймера — они принимаются за овец). По дороге
  ломает постройки стороны в `TROLL_RAID_SMASH_R` 14 м, **крепость —
  никогда** (`Troll.command_attack` отбрасывает `is_stronghold`).
- **РУДНИКИ ИИ И ОРДЫ ВОССТАНАВЛИВАЮТСЯ САМИ** (`Mine._die` →
  `GameManager.schedule_mine_restore`, `MINE_RESTORE_SEC` 60): руина в точке
  снимается, встаёт рудник прежнего владельца без цены; отстроил рабочий
  раньше — таймер холост. Игрока не касается.
- **СКАЛЫ** (`Main.CLIFFS_ENABLED`, `CLIFF_SLOPE` 0.45 = стена плато):
  поиска пути нет, а прежнее решение «непроходимых склонов быть не может»
  развёрнуто владельцем. Маска ячеек 1 м строится ядром
  (`ArmyCore.BuildCliffMask` по своей Height — 924 ячейки на карте), шаг в
  скалу СКОЛЬЗИТ вдоль неё по X или Z (`BatchMoveRows`; та же ветка в
  `Unit._move_blocked`), разведение на скалу не выталкивает, `land_target`
  уводит приказ кольцами `nearest_passable` (до 24 м). Как вода — правило
  партии: снятые границы мира снимают и скалы. Пологий спуск
  (`PLATEAU_RAMP_GENTLE`) остаётся дорогой.
  **ЧЕТЫРЕ ИТЕРАЦИИ, ПОКА ОТРЯД НЕ ВЫШЕЛ С ПЛАТО** (зонды
  `qa_cliff_probe/Probe2,3`; `qa_map` C1 и `qa_river_crossing` ставят отряд
  ПРЯМО НА ПЛАТО рудника на −58 по Z — и это же случится в партии с любым
  отрядом, взявшим рудник на холме): (1) маска в 1 м зубчата, скольжение по
  осям застревало в вогнутых зубцах → крутизна считается В ТОЧКЕ по четырём
  «сухим» высотам (`CliffGrad`, без просадки русла — иначе берег с
  гармониками становился стеной и боец не доходил до брода), маска расширена
  на ячейку и служит грубым фильтром `CliffNear`; (2) касательная к склону
  выбиралась «куда ближе цель» и в точке, где стена перпендикулярна цели,
  меняла знак — боец замирал → **память стороны обхода** `_cliffSide` на
  строку: выбирается на первом упоре, держится, пока прямой шаг упирается,
  забывается через `CliffForget` 45 свободных шагов (отступ от стены делает
  прямой шаг свободным на такт, и сброс на первом же свободном возвращал
  переворот); (3) хорды вдоль кривой стены сносили в склон → касательная с
  отступом `CliffBack` 0.35 НАЗАД ПО ШАГУ (не по градиенту: знак градиента не
  говорит, сверху боец или снизу); стоящий на стене (рождён, вытолкнут)
  вправе шагать «не круче» (`CliffEscape`), а в тупике скатывается вниз по
  склону; (4) `_cohesion_guard` возвращал «отставших» к медиане — отряд,
  растянутый по кольцу, бросал марш: идущий, у которого до своей точки
  дальше зоны отряда, — не отставший. Итог: 24 копейщика с вершины выходят
  кольцом через спуск за ~130 с (`qa_sprint18b` F6: ≥ 11 из 12, 0 замеров на
  скале), боец у обрыва скользит вдоль него (F5). Стенд `qa_river_crossing`
  переставлен на +Z (там плато нет): по −Z переправа честная, но не
  укладывается в 180 с бюджета.
- **ЛОВУШКИ СТЕНДОВ:** гибель стражи логова зовёт бонусную пару
  (`aggro_enabled` выключать перед зачисткой пня); укол копий с разгона идёт
  помимо стены — вычитать; парсер GDScript не проверяет вызов
  несуществующего метода у типизированной переменной (`Building.is_stronghold`
  есть только у Castle/Tower) — `has_method` + `call`.

### Спринт 18, письма 4-6: голосовая «защита» только копейщикам, зависшие кости, хотфикс 3-рядного строя

- **ГОЛОС «ЗАЩИТА / ДЕФ / ДЕРЖАТЬ СТРОЙ» — ТОЛЬКО КОПЕЙЩИКАМ** (четвёртое
  письмо; `VoiceControl._spearmen_only` / `_all_squads_of_type`). Прежний
  разбор фразы отдавал стойку ВСЕМ отрядам выделения, у которых не названо
  рода войск: лучники теряли марш, мечники меняли стойку, конница получала
  «оборону». Теперь намерение `defense` фильтрует список до копейщиков; в
  выделении их нет — команда «некому» и никого не трогает; выделение ПУСТО —
  все копейщики армии (остальные не получают ничего). Слово «деф» добавлено
  в словарь точных совпадений (`voice_commands_config`, короткое слово —
  только целиком, как «бей»). Стенд `qa_voice_cmd` C1в-C1е: 33/33.
- **ЗАВИСШИЕ КОСТИ — ОДИН СПИСОК СОБЫТИЙ НА ДВА СЛОЯ** (пятое письмо,
  скриншот с костями, крутящимися в траве). `ArmyCore.TakeArrowEvents`
  отдаёт события ВСЕХ полётов и очищает список; каждый слой
  (`ArrowRenderer.drain_events`) забирал его сам, и слой стрел, идущий
  первым, уносил и события костей: пока в воздухе была хоть одна стрела,
  кость НИКОГДА не получала `core_event` — узел оставался «в полёте»
  (`_flight_id ≥ 0`, `_process` выключен, срок `MAX_FLIGHT_SEC` не тикает),
  а слот MultiMesh стоял на последней точке дуги и крутился по оси вечно.
  Лечение: события берёт `GameManager` ОДИН раз и раздаёт по слоям
  (`ArrowRenderer.dispatch_events(ev, [arrows_mm, bones_mm])`, статический);
  `drain_events` оставлен для стендов с одним слоем. **Страховка сроком под
  ядром:** `ArrowRenderer._flight_since` + `sweep_flights(max_sec)` раз в
  секунду из `GameManager` (`_flight_sweep_t`): полёт без события старше
  срока гасится `_despawn` — стрелы по `MAX_FLIGHT_SEC` 10.5, кости по
  **`BONE_MAX_FLIGHT_SEC` 2.0** (бросок на 8.5 м длится меньше секунды;
  заказ «лимит 1.5-2 с»). Гибель стрелка / цели полёт не держит — событие
  приходит из ядра независимо от узлов. Стенд `qa_bones_fix` (8: стрелы и
  кости в воздухе одновременно → все полёты кончаются, ни один узел не
  «в полёте», страховка гасит фиктивный полёт старше срока, срок кости в
  земле 5 с). Побочно: `qa_gnoll_fix` E3 «сверок 338 → 97» — прежде в
  «летящих» считались зависшие кости; E6 переведён на пик прироста за окно
  (кость лежит 5 с, окно замера дольше — «конец минус начало» мигал).
- **ХОТФИКС: 3-РЯДНАЯ «КОРОБОЧКА» УТЕКЛА НА ВСЕ РОДА ВОЙСК** (шестое письмо,
  срочно). Клавиша `[2]` при растяге ПКМ (спринт 17) ставила режим
  `FORM_DEEP`, и режим ЗАПОМИНАЛСЯ между растягами: одно нажатие — и все
  последующие приказы строем раскладывали лучников, мечников и конницу
  блоками в три шеренги. Сама «Стена копий» по стойке (`on_squad_stance` →
  `spear_wall_ready`: тип копейщик + изучен `spearman_1d` + режим включён)
  была ограничена верно; протекала раскладка растяга. Теперь
  `SelectionManager._deep_for(movable)`: коробочка применяется ТОЛЬКО к
  эшелону, где все отряды — копейщики со «Стеной копий»; любому другому
  эшелону (и копейщикам без исследования или с выключенным режимом) —
  стандартный широкий фронт, слово в слово прежний код. В смешанном
  выделении под `[2]` копейщики с бонусом встают в три шеренги, остальные —
  как под `[1]`. Стенд `qa_formation_keys` B0/B0б/C7/C7б (27).
- **ПОПУТНО:** `AudioManager._load_settings` не роняет игру на битом файле
  настроек с чужим типом (словарь/вектор — `float()` бросал SCRIPT ERROR;
  qa_audio2 E-ДОП); `qa_ai2` `_h_frozen` — правило 5; стенды прежних
  требований переписаны на спринт 18: `qa_goblin` J8/K6б (щелчок по крепости
  — `pick_castle`), `qa_monk_healing` D3 (размер VFX фиксирован
  `HEAL_VFX_SIZE_M`, а не «≥ 60 % рисунка»).
- **ВТОРОЙ ПЕНЬ СТОЯЛ В 42 м ОТ КРЕПОСТИ ИИ** (нашёл `qa_ai` 4: у одного
  отряда лучников 27 из 30; A/B с `LAIR_AI_ENABLED = false` — все 21 отряд
  полные). `troll_lair_center_ai` брал `LAIR_OFFSET` «зеркально» (−28 по X,
  +104 по Z от якоря ИИ), а якорь ИИ стоит у НИЖНЕГО края карты: +104 по Z
  зажималось `GEN_HALF_Z`, и пень вставал в 31 м ниже и 28 м левее крепости —
  тролль на патруле (23 м) и гноллы (агро 10 м) резали новобранцев ИИ у
  ворот. Теперь своё смещение `LAIR_AI_OFFSET` (−104, −10): 104 м к реке
  (как у игрока до своего пня), чуть вверх от кромки, башни ИИ на 0.3 пути
  остаются в 45+ м; площадка резервируется от леса (`_reserve`), поэтому
  `WORLD_GEN_VERSION` 5 → 6. Стенды: `qa_ai` 4 судит по ПИКУ состава (ИИ
  нанимает полный штат; потери от нейтрального пня законны), 5 пропускает
  отряд с `issued = false` (пополнение ждёт такта); `qa_squad` 8 считает
  стражей обоих пней; `qa_monk` B1 — чужой копейщик заморожен `set_tick`
  (тик идёт из GameManager, `set_process` его не глушил, и «вылечено»
  зависело от жребия боя).
- **ПИСЬМО 7, ПУНКТ 1 («копейщики спавнятся 60 одиночками») — НЕ
  ВОСПРОИЗВОДИТСЯ.** Зонды `qa_squad_probe/Probe` (барак на стенде) и
  `Probe2` (настоящий флоу: `start_game`, крепость и бараки через
  стройплощадку, лимит населения, свободный выход шеренг): один отряд
  `type=spearman`, 60 из 60 в одном `squad_id`, клик по одному выделяет 60,
  `selected_squad_count` = 1. Единственное известное состояние, в котором
  барак выдаёт 60 бойцов без отряда, — SCRIPT ERROR в `GameManager.new_squad`
  (тогда `sid` = null и `squad_id` у всех 0): ровно это случается, когда
  редактор владельца перечитывает скрипты ПОСРЕДИ патча из нескольких файлов
  (патчи 02:03-02:09 легли в открытый с 01:05 редактор). Лечение — перезапуск
  игры на согласованном дереве; если повторится на чистом запуске — нужен
  вывод консоли (SCRIPT ERROR) и где нанимали.

### Спринт 18, письмо 8: пень ИИ подальше от крепости, орда не сносит крепость ИИ

- **Пункт 1 (пень ИИ)** — закрыт правкой выше (`LAIR_AI_OFFSET` (−104, −10),
  104.5 м от якоря ИИ, башни границы в 36.9 м от пня при патруле 23.4).
  Стенд `qa_lair_ai` A1-A7: расстояние больше патруля тролля + радиуса
  обороны гноллов + 30 м, не ближе пня игрока к его крепости, суша, сторона
  ИИ, низ карты, площадка без леса, стражи на месте.
- **Пункт 2 — ОРДА НЕ СНОСИТ КРЕПОСТЬ ИИ-ЛЮДЕЙ.** Одно правило
  `GameManager.goblin_may_raze(b)` (постройка ИИ с `is_stronghold` — нельзя;
  башня ИИ, дома, экономика и крепость ИГРОКА — можно) спрашивается из всех
  дорог, какими постройка становится целью: `Unit.may_attack_building`
  (виртуал; база — по фракции орды, тролль — «ни одну крепость», прежнее
  переопределение `command_attack` снято) — в `command_attack`, в авто-агро
  (`_find_nearest_enemy_in_range`, ветка построек) и в переходе замка на
  соседний дом (`_next_enemy_building`); у вожака и командира штурма — в
  выборе ближайшей постройки; и страховкой — в `Building.take_damage`
  (брызги, снаряд, дубина: урон от бойца орды по крепости ИИ не проходит).
- **НАЙДЕНО ПОПУТНО: ВОЖАК И КОМАНДИР НЕ ВЫБИРАЛИ ПОСТРОЙКИ ВОВСЕ.**
  `enemy_buildings_snapshot` со спринта 14 отдаёт ПАРУ `[узлы, xz]`, а
  `GoblinAI._nearest_enemy_building` и обе выборки командира обходили саму
  пару (`is_instance_valid(Array)` — ложь) и отвечали null: «орда бьёт
  постройки» (`BUILDING_HUNT_RADIUS` 70) молча не работала с тех пор, базы
  сносило только авто-агро бойцов вплотную. Обход переведён на `snap[0]`;
  стенд `qa_lair_ai` B8/B9 (вожак и командир у крепости ИИ выбирают дом).
- `qa_lair_ai` (22): позиция пня, приказ/авто-агро/прямой урон по крепости
  ИИ, дом ИИ и крепость игрока как прежде, удар игрока по крепости ИИ
  проходит, переход замка, тролль. Зонды `qa_squad_probe/Probe,Probe2` —
  проверка «60 одиночек» (не воспроизводится).
- **Известный красный вне шлюза:** `qa_world` 3b — 26 мокрых проб вне
  русла (низины рельефа ниже зеркала воды после «вода — где грунт ниже
  зеркала» спринта 15); к правкам спринта 18 отношения не имеет.
- **ЧАСЫ СТРАХОВКИ ПОЛЁТА — ФИЗКАДРЫ, НЕ СТЕНА.** Первая версия
  `sweep_flights` мерила возраст полёта `Time.get_ticks_msec()`; под
  нагрузкой шлюза физика отстаёт от стены (4000 бойцов), и честный полёт
  в 3 с игрового времени длится 10+ с стены — `qa_full_game_4k` A1 под
  шлюзом показал 1-2 «висящих» (одиночно — 0). Возраст теперь в
  `Engine.get_physics_frames()`, порог — секунды × `physics_ticks_per_second`
  (правило 12 стендов, перенесённое в игру). `qa_full_game_4k` печатает
  диагностику каждой висящей стрелы (узел, тело, гашение, полёт, кость).

## Спринт 19 (12.09.2026): фантомы и точка сбора, рабочие с топором, загоны и туши, навигация вокруг скал, экономика рабочих и Shift, монахи

Четыре письма владельца (9-12) плюс заказ на суб-агента. Стенды:
`qa_sprint19` (19), `qa_sprint19b` (48), `qa_sprint19c` (39), `qa_nav` (24),
`qa_archer_bench` (4 вердикта + таблица) — новые; `qa_formation_keys`,
`qa_keep`, `qa_sprint13`, `qa_rally2`, `qa_performance_stress` переписаны под
новый контракт. Шлюз спринта 18 закрыт перед началом (67 сцен, `SCRIPT ERROR:
0`, красные только известные плавающие).

### Письмо 9: фантом-спрайт, рабочие ждут крепость, строители у рисунка, ворота, точка сбора, прямоугольный выход, [1]/[2]/[3]

- **ФАНТОМ ПОСТРОЙКИ — ЕЁ ЖЕ СПРАЙТ** (`Main._create_building_ghost(size,
  building_id)`): полупрозрачная картинка здания вместо синей коробки с
  овалом; перекраска (`_ghost_mats`) как прежде — красный на запрете. У
  постройки без картинки (рудник) — прежняя коробка. `enter_building_placement`
  получил `building_id`, вызовы из `GameManager` его передают.
- **РАБОЧИЕ НА СТАРТЕ СТОЯТ** (`_spawn_starting_workers`: авто-добыча только
  при `castle_count(PLAYER) > 0`); достроенная крепость разводит бригаду по
  ресурсам, как и прежде (`_on_castle_built`).
- **СТРОИТЕЛИ — У НАРИСОВАННОГО ФУНДАМЕНТА.** Площадка мерила стену по
  КОРОБКЕ конфига (у замка 4 м от центра), а спрайт стройки стоит низом в
  центре пятна: на экране между строителем и срубом лежали три метра травы.
  `ConstructionSite.edge_distance` с лицевой стороны (+Z) отвечает
  `FRONT_EDGE` 0.6 м, полуширина — по рисунку (`_draw_half_w`). **Знак
  направления — ОТ площадки к рабочему** (как у `work_position`); Worker
  передавал вектор к площадке, и с первой правки лицевая сторона у него не
  распознавалась вовсе (`qa_sprint19` C2: 4.34 м) — исправлено переворотом
  в `_build_edge`.
- **ВОРОТА НЕ ДАЛЬШЕ `GATE_MAX_DEPTH` 2.5 м + зазор** (`Building.gate_depth`
  и переопределение у `Castle`: рисунок замка 8.66 м давал ворота в 4.8 м от
  центра, отряд появлялся «далеко от стены»). `qa_keep` A4 — нижняя граница
  через потолок.
- **ПКМ ПРИ ВЫДЕЛЕННОМ ЗДАНИИ — ВСЕГДА ТОЧКА СБОРА** (`SelectionManager
  ._rally_click` первым делом в `_handle_right_click`: только свои постройки в
  выделении — точка ставится в землю под курсором, даже поверх бойца или
  дома). **Маркер точки сбора виден у выделенного производящего здания и без
  назначенной точки** (`_refresh_rally_marker` + `effective_rally_point`).
- **ВЫХОД — ПРЯМОУГОЛЬНИК ПО ОСЯМ ФАСАДА.** `_spawn_one` разворачивал оси
  блока на курс «ворота → точка сбора»: при диагональной точке отряд выходил
  ромбом под 45°, и на экране это читалось как «два треугольника». Разворот
  снят; блок идёт к точке целиком (`qa_sprint19` F2: 30 лучников — колонн ≤ 6,
  шеренг ≤ 5).
- **КЛАВИШИ [1]/[2] — ВСЕМ РОДАМ ВОЙСК** (разворот хотфикса писем 6/7:
  `_deep_for` снят, `DEEP_ROWS` 3 для любого эшелона), **[3] крутит авангард
  по кругу** копейщики → мечники → лучники (`formation_front` 0/1/2 вместо
  `formation_flipped`, `Formations.RANK_ORDERS`). Память
  `project_deep_formation_spearmen_only` устарела и снята.

### Письмо 10: рабочие с топором, загоны, овцы внутри, туши, рейд тролля

- **РАБОЧИЙ ВООРУЖЁН** (`STATS.worker`: health 55, attack_1 4, range 1.3,
  cooldown 1.6 — уровень базового гнолла): авто-агро в покое, ответ на удар,
  приказ атаки по ПКМ; лента удара — топор (`chop`), звук — «chop».
  **Солдатом он не стал:** `Unit.is_combatant()` (виртуал; база — по урону,
  рабочий и монах — `false`) заменил `attack_damage > 0` в `squad_is_combat`
  и `squad_battle_cry`: клич, реплики, голосовое «все», массовость — без
  рабочих. Приказ атаки (и авто-агро) бросает работу (`Worker.command_attack`).
- **ЗАГОН С МЕСТОМ; ПКМ ПО ЗАГОНУ — НЕСТИ ТУДА** (`Worker.set_sheep_dest`,
  `_sheep_dest_forced`): назначенный полный загон → ближайший с местом,
  иначе нож у него.
- **ВНУТРИ ОГРАДЫ — ПЕРВЫЕ `INSIDE_CAP` 10 ПО ПОРЯДКУ ПРИВЯЗКИ**
  (`SheepPen.next_seq / inside_slot / graze_radius_for`, `Sheep.bind_seq`):
  внутренним радиус выпаса `INSIDE_FRAC` 0.62 полуширины, лишним —
  `OVERFLOW_RADIUS` 20 м; выбыла внутренняя — следующая по порядку встаёт
  внутрь сама. **ТОЧКА ПЕРЕБЕЖКИ — ВНУТРИ КРУГА, А НЕ НА КРОМКЕ:** прежний
  зажим на 0.85 радиуса сажал стадо кольцом по границе выпаса (у загона — по
  ограде, внутри никого: скриншот владельца); теперь точка равномерна по
  кругу (`AudioManager.rng`). Подпись загона — «внутри N / 10».
- **НАСТРОЕНИЯ НА ВЫПАСЕ** (`Sheep._pick_mood`): перебежка / прыжок на месте /
  лежит (последний кадр прыжка одним кадром, `_texture("rest")`) / медленно
  щиплет — из тех же двух лент; жребий из `AudioManager.rng`.
- **ТУША** (`Sheep.kill_by / butcher_join / take_meat / CORPSE_DESPAWN_SEC`):
  любой боец рубит живую овцу по ПКМ одним ударом (`Unit.command_hunt_sheep`
  — приказ на движение с проверкой в момент прибытия, `_hunt_strike_anim`, у
  копейщика — направленный лист удара), туша окровавлена (`modulate`
  `BLOOD_TINT`), мясо (`MEAT_TRIPS` 20) лежит У ТУШИ и делится артелью до
  `MAX_BUTCHERS` 4; ПКМ рабочим по туше — присоединиться; никто не режет
  120 с — истлевает. Ходка засчитывается по-прежнему складу.
  **`kill_by` снимает признак переноски:** овца, взятая рабочим на месте
  (`captured_by`), числилась «на руках», `kill_by` её не убивал, и рабочий
  резал живую по кругу (`qa_worker_sheep_loop` A3-A8).
- **РЕЙД ТРОЛЛЯ: СЪЕЛ → СЛОМАЛ ПОСТРОЙКУ → УВЁЛ ОТАРУ** (фаза `smash`,
  `TROLL_RAID_SMASH_SEEK` 45 м, `TROLL_RAID_SMASH_SEC` 60). **НАЙДЕНО
  ПОПУТНО: ДУБИНА НЕ БИЛА ПОСТРОЙКИ ВОВСЕ** — `_release_swing` искал жертв по
  сетке бойцов, здание в ней не состоит: приказ принимался, тролль махал,
  урона не было (со спринта 15). Постройка-цель теперь получает удар по
  дистанции до стены (`ring_radius`). **Перехват:** `Sheep.captured_by`
  снимает пастуха, `_raid_finish` отдаёт логову только тех, кто ещё в отаре.

### Письмо 11: навигация вокруг скал и воды

- **СЕТКА ПРОХОДИМОСТИ И A* В ЯДРЕ** (`ArmyCore.BuildNavGrid` из маски скал и
  воды реки вне брода, `NAV_CELL` 2 м; `NavPath` — A* 8-связный без срезания
  углов, натяжение нити по `NavLineBlocked` с зазором 0.7 м, отжим углов
  `NavCornerPush`; ответ — плоский массив промежуточных точек). GDScript:
  `GameManager.nav_route / nav_blocked / nav_unreachable / build_route` с
  **кэшем на физкадр по ячейкам 4 м** (`_nav_key`). Цена (`qa_nav` F): 133 мкс
  на маршрут через плато, 7 мкс на проверку прямой.
- **МАРШРУТ — ЭТО ПРИКАЗ, А НЕ КАДР.** `Unit.command_move` строит `_route`
  (брод своей полосой плюс обходы до и после; боковой сдвиг точек по смещению
  бойца от центра отряда — колонна, не нитка), `move_target` идёт по точкам
  (`ROUTE_WP_RADIUS` 1.6), раз в `ROUTE_LOS_SEC` пропускается угол, если уже
  видна следующая точка. Прежние два этапа брода (`_ford_stage`) стали частным
  случаем маршрута. **Подход к цели атаки** — раз в `NAV_RECHECK_SEC` 0.4
  (`_atk_waypoint`); автопилот подхода на обходе не взводится. **Авто-агро не
  идёт за обрыв** (`nav_unreachable` в `_check_auto_aggro`: цель без прямой и
  с обходом длиннее поводка — не повод сойти с места; стрелок, до которого
  дострелить, стреляет).
- Замер `qa_nav`: без навигации отряд, посланный с глухой стороны плато на
  вершину, за 35 с не подходит ближе 11.5 м (стена); с навигацией доходит за
  30 с, разброс отряда по дороге 1.7 м, на скале никто; враг над обрывом в
  9.5 м — никто не бежит в стену, лучники снизу стреляют; приказ атаки на
  плато — первый удар через 30 с, 10 из 10 наверху; через реку вдали от
  брода — 9 из 10 у цели, в воде 0 кадров.
- **СТЕНД `qa_rally2` C5 ПЕРЕПИСАН НЕ ПО КОДУ, А ПО СЦЕНЕ:** лагерь врага стоял
  на (0, 0) — в коридоре брода, через который и ведёт маршрут; с навигацией
  отряд срезает брод по диагонали и выходил на лагерь в упор. Лагерь
  сдвинут на x = −30 (те же 26 м от линии марша, вне брода). Bб/F2/F3 там
  красные и без навигации (A/B с `nav_cells_blocked = 0`).
- **В проекте нет `NavigationServer2D`/`PhysicsServer2D`** (заказ звал их по
  привычке): физтел у бойцов нет, столкновений нет, всё движение — пакетные
  проходы ядра; скольжение вдоль стены с памятью стороны — там же.

### Письмо 12: экономика рабочих, стройка, Shift, монахи

- **МЕДЛЕННЫЙ СТАРТ РАБОЧЕГО** (`STATS.worker`: 2.5 / 1.6 м/с, цикл 6 с,
  5 за ходку; найм 16 с) и **ШЕСТЬ ЛИНИЙ КУЗНИЦЫ**: колонка A — груз, B —
  темп кирки/топора, C — скорость, запас, **удар** (3c/4c/5c +1/+1/+2), D —
  **обучение** (`bonus_train`, `Building.queue_unit` делит время найма) и
  **стройка** (`bonus_build`). Ключи `bonus_train` и ауры — в `BONUS_KEYS` /
  `_MOD_TO_STAT` / `HUD.MOD_SHORT_LABELS`.
- **АВТО-ЗАЩИТА БАЗЫ АРТЕЛЬЮ** (`GameManager.workers_rally`): удар по
  рабочему или его собственный приказ атаки поднимает соседей в
  `WORKER_RALLY_RADIUS` 14 м (не строителей), только против малой угрозы
  (`WORKER_RALLY_MAX_FOES` 4 чужих у обидчика), не чаще раза в 3 с.
- **«БЕГ НА МЕСТЕ» У СТРОЙКИ — ДВЕ ПРИЧИНЫ, ОБЕ НАЙДЕНЫ.** (1) Точка
  стояния бралась лучом в ЦЕНТР площадки; у нарисованного основания все лучи
  сходились в одну точку, артель толпилась, четвёртый строитель в порог
  прихода не попадал (`qa_helpbuild` C2). Теперь `work_position` — ближайшая
  к рабочему точка стены (спереди отрезок рисунка, сзади и сбоку коробка), и
  рабочий идёт именно к ней: артель ложится вдоль стены сама. (2) **Признак
  «иду» (`_mv_moving`) переводит только базовый тик**, а в
  BUILDING/GATHERING/RETURNING Worker до него не доходит: окно замера тикало,
  а признак замирал на значении из марша — рабочий у стены числился идущим
  до конца стройки (`moved_recently` = true при нулевой скорости; поймал
  `qa_sprint19c` C2). Перевод признака добавлен в Worker.tick_physics.
- **АРТЕЛЬ ПО ОВЦАМ:** занятую соседом овцу не ждут — берут ближайшую другую
  (`_next_sheep` со `skip`), туша с мясом и местом в артели — тоже цель.
- **КЛИК ПО ИНТЕРФЕЙСУ В РЕЖИМЕ РАЗМЕЩЕНИЯ ФАНТОМ НЕ СТАВИТ** (`Main._input`:
  над панелью — `hud.point_over_ui` — клик отдаётся кнопке); **переключение
  типа постройки из режима размещения** (`enter_building_placement`
  отменяет прежний заказ с возвратом). **ОЧЕРЕДЬ SHIFT:** постановка с
  зажатым Shift повторяет заказ (`_placing_repeat` →
  `GameManager.try_worker_build`), а строителю площадка встаёт в очередь
  (`Worker.command_build(site, queue)`, `_build_queue`, `on_construction_finished`
  берёт следующую; любой другой приказ очередь снимает).
- **МОНАХ НЕ ДЕРЁТСЯ** (`STATS.monk` attack 0; `Monk.command_attack` — пустой;
  `is_combatant` false; тыловой напор — только вооружённым). **Дистанция между
  монахами** `MONK_SPACING` 5 м (`_tick_spacing`, свободный монах отходит).
  **Аура ярче и крупнее** (`AURA_DIAM_M` 1.4, кольцо 3 px, свечение дизером
  в ленте — срез альфы полутонов не умеет). **Баланс:** одиночное лечение
  `MONK_HEAL_SEC_PER_MAN` 14 с (было 10), «Благодать» — узел 2d (tier 2), каждого
  вдвое медленнее (`MONK_AOE_RATE` 0.5): отряд из четверых с половины —
  15.5 с. **ВОСКРЕШЕНИЕ** (3d): тело из слоя тел (`CorpseRenderer.Corpse` —
  сторона, род войск, отряд, `raisable`; `find_raisable`) поднимается через
  `GameManager.raise_fallen` бойцом с 1 HP в прежний отряд, тело снимается,
  вспышка (`Monk._res_flash`); канал `MONK_RES_SEC` 20 с только когда раненых
  рядом нет, откат 8 с; «Двойное попечение» (4d) — лечение во время канала и
  канал −30 %. **ОПЫТ — ЗА ЛЕЧЕНИЕ** (`MONK_XP_HP_PER_KILL` 150 HP = зачёт,
  воскрешение = 3) через тот же `credit_kills`. **АУРЫ ПО НАГРАДАМ**
  (`_VET_MONK_TEMPLATE`: броня / удар рукопашным / темп стрельбы стрелкам /
  радиус) раздаются раз в `MONK_AURA_TICK` союзникам в `MONK_AURA_BASE_R` +
  радиус и живут `MONK_AURA_HOLD_MS`; `Unit.aura_apply` читается в
  `_upgrade_damage_bonus`, `take_damage` и `_effective_cooldown`.
- **НАЙДЕНО ПОПУТНО: ЭФФЕКТ ЛЕЧЕНИЯ ВИСЕЛ НАД ТРУПОМ.** `_ride_heal_vfx`
  начинался с `if _heal_vfx_target == null: return`, а освобождённый объект в
  Variant РАВЕН null — павший и освобождённый пациент проходил как «никого не
  лечу», и квад светился над телом до следующего лечения
  (`qa_performance_stress` B7 при бессмертных монахах). Пустая привязка при
  видимом кваде теперь гасится.
- `qa_performance_stress` перестроен: гноллы — при своём пне (без него они
  подхватывали логово партии за тысячу метров и уходили патрулировать —
  «костей в полёте пик 0»), стая ведётся стендом с 15-й секунды (приказ с
  замком на цель в 40 м умирал сразу), B2 считает сам разгон (раздавленные —
  жребий свалки), B7 — с перепроверкой через два кадра.

### Суб-агент «Archer & FPS Optimizer»

`.claude/agents/archer-fps-optimizer.md` + `agents/archer_fps_optimizer/
SYSTEM_PROMPT.md` (роль, правила: интервальные тики, пул снарядов, геометрия
формаций, замер до/после, обязательный отчёт) + `README.md` (три шага).
Бенчмарк `qa_archer_bench`: 120 лучников против бессмертной стены, два
режима подряд — «по готовности» (перезарядки в случайной фазе: 9 выстрелов
за 0,1 с, 32 стрелы в воздухе) и «залп» (50 за 0,1 с, 120 в воздухе), физтик
и кадр логики, FPS — только в окне. **Порядок режимов значим:** залп
синхронизирует перезарядки, после него «по готовности» мерил бы хвост залпа.

### Стресс-отчёт «Big QA» (`qa_stress_report`, оконный измеритель)

Заказ владельца: пауза → стресс-сцена → снимок `Performance.get_monitor()` в
самый тяжёлый момент → отчёт по шаблону → снова к фиксам. Стенд собирает
сцену партии: замок, две стройки, 18 рабочих (стройка, лес, разделка), загон
с 8 овцами, 103 солдата игрока (лучники в залпе, мечники, копейщики, 3 монаха
с аурами и «Благодатью») против 112 орды (копейщики, свиноконница, гноллы,
два тролля со своим логовом); 300 кадров разогрева не считаются, 45 с замера.
Печатает отчёт в шаблоне владельца.

- **ИЗМЕРИТЕЛЬ ПО РОДАМ ВОЙСК — `perf_config.class_meter`** (пара
  `get_ticks_usec` вокруг `tick_physics` каждого бойца с раскладкой по
  `stat_id`, отдельная ветка цикла в `GameManager._physics_process`, чтобы
  штатный цикл не платил за проверку) плюс `nav_usec/nav_calls` вокруг
  `NavPath`. Дорого (два вызова часов на бойца) — включает только стенд.
- **ЧИСЛА (окно 1920×1080, ~210 живых):** минимум 72 FPS (средний 75), кадр
  13.5 мс в пике, худший интервал физкадра 38 мс — кадр с двумя тиками;
  монитор `TIME_PHYSICS_PROCESS` 21.0 пик / 17.8 p95 (он суммирует оба тика),
  честный тик армии по `tick_meter` 6.06 → **5.50 мс** после правки залпа;
  `TIME_PROCESS` 13.4 пик; кадр логики 2.76 мс; навигация 0.009 мс/кадр
  (2596 маршрутов A*, ~130 мкс каждый); пакетный шаг 0.16 мс; ≤ 127 вызовов
  отрисовки.
- **ПО КЛАССАМ (мс на тик / мс на кадр):** гоблины 0.006 / 1.97, гноллы
  **0.051** / 0.95, всадники 0.005 / 0.39, рабочие 0.021 / 0.19, копейщики
  0.013 / 0.16, мечники 0.020 / 0.12, лучники 0.009 / 0.09, тролли 0.050 /
  0.07, монахи 0.037 / 0.04. Гнолл вдесятеро дороже гоблина: кайт и фланг
  выдают `command_move` каждый такт.
- **ТОП ВЕТОК:** `vis_far` 0.77, `process_move` 0.64 (5.4 мкс на вызов —
  GDScript-часть шага: LOS маршрута, боковой сдвиг, окно смещения),
  `squad_corridor` 0.38, `grid_update` 0.37, `process_attack` 0.33.
- **ПРАВКА ПО ОТЧЁТУ: ТАКТ ЗАЛПОВ.** `_sweep_volleys` шёл КАЖДЫЙ физкадр по
  ВСЕМ отрядам и на каждый звал `toggle_ability_of` + `squad_ability_on` —
  0.42 мс/кадр (420 мкс на вызов) при 30 лучниках. Теперь раз в
  `VOLLEY_SWEEP_EVERY` = 3 тика и только по отрядам типа «archer» (отсев по
  типу до разбора способности). Ветка ушла из верхней двадцатки (< 0.08);
  окно залпа 200 мс это 12 тиков — опоздание на два тика синхронности не
  меняет. `qa_volley_fix` 8/8, `qa_archer_bench` 4/4; `qa_volley` C1 мигает
  (пик 4-5 из 10 при пороге 5) ровно как до правки (`gate_final18d`: тоже
  1 из 23) — личные 15 Гц бойцов размазывают залп на 3-4 тика, и стенд на
  десяти стрелках стоит на ноже.
- **КРАСНЫЕ ПОСЛЕ ШЛЮЗА, РАЗОБРАНЫ:** `qa_helpbuild` C2 ждал 240 кадров, а
  рабочий со спринта 19 ходит 2.5 м/с — десять метров до угла площадки плюс
  защёлка прихода в 4 с не укладываются (переписан на ожидание свойства
  `builder_count ≥ 4`); `qa_reform` B4 считал флажок рудника орды (тот же
  `SquadBanner`, ребёнок постройки) отрядным знаменем — считаются только
  знамёна в мире; `qa_sheep` C3, `qa_start_build` D4, `qa_archer_logic` C2
  одиночно зелёные (нагрузка шлюза). `qa_bugpass` 45, `qa_recon` 22,
  `qa_sprint18b` 49, `qa_worker_sheep` 20 — зелёные после правок стендов;
  `qa_visual_smoke` 26/26 и `qa_monk_vfx/Shot` 6/6 в окне (аура 1.4 м,
  свечение дизером).

## Спринт 20 (12.09.2026): комплексный пакет фиксов — монах и глушь, звуки, тролли и пень, клик и массовый приказ, стена копий и оборона, ремонт, перемирие и ИИ орды

Шесть модулей одним письмом. Ниже — что изменилось, почему именно так, и
что показали стенды. Новый стенд `qa_sprint20`, переписан `qa_troll_aggro`.

### Модуль 1: визуал монаха, глушь у пня, звуки убийств

- **Аура — только овал под ногами.** Лента ауры (`Monk._aura_texture`)
  раньше несла кольцо, крест и дизерное свечение; заказ оставляет «маленький
  зелёный овал строго под ногами» и саму ленту лечения. Овал рисуется
  контуром 2 px по эллипсу с коэффициентом `AURA_OVAL_K` 0.45 — на билборде
  под камерой 45° такой овал читается лежащим на земле, как кольцо
  выделения. Квад — 1.0 × 0.45 м, ставится в ТОЧКУ НА ЗЕМЛЕ цели
  (`draw_position`), глубина от той же точки без подтяжки к камере: спрайт
  бойца приподнят `depth_lift` 0.35 и честно перекрывает дальнюю дугу —
  овал под фигуркой, а не поверх. Лента лечения (`Heal_Effect`) — центр на
  половине своего квада над землёй: нижняя кромка у ступней при любом росте
  (тролль, пехотинец), а не «от пояса» через `sf[4] × V_STRETCH`.
  `qa_monk_vfx/Shot` B1 переписан: центр изменившегося теперь обязан лежать
  ближе к ногам, чем к центру ленты; порог изменившейся доли 12 → 5 %
  (заливки больше нет).
- **Глушь у пня** (`Main._dress_lair_glade`). Грибы (Deco 01-03), камни
  (04-06), кости (14-15) и кусты (07-09) по кольцу 9-22 м вокруг пня —
  общей отрисовкой `VegetationRenderer` (бакет по файлу, новых вызовов
  отрисовки нет), три рощицы по 5 деревьев на 26 м — обычные `ResourceNode`.
  Площадка пня зарезервирована от леса (`LAIR_CLEAR` 36), поэтому сажаем
  сами, минуя `_is_reserved`. **Жребий — свой `RandomNumberGenerator` от
  точки пня:** общий поток партии (`seed(world_seed)`) не тронут, одно зерно
  даёт тот же мир, `WORLD_GEN_VERSION` поднимать не пришлось. `qa_troll` A5
  («площадка чистая») сужен до внутреннего кольца 21 м.
- **Снесённый пень — тот же рисунок, затенённый.** `Building.ruin_tint()`
  (виртуал, WHITE) подаётся в `make_static_material` руины; у логова —
  `ruin_sprite_override()` = Dead Tree и `LAIR_RUIN_TINT` (0.42, 0.40, 0.46).
- **Хор смеха — только за убийство гоблином и только в малой стычке.**
  `_on_squad_wiped` раньше играл хор над любым выбитым отрядом игрока при
  одном гоблине в 18 м — и над добитым красным ИИ, и над добитым троллем.
  Теперь: добивший (`_slain_by`) — сторона орды и не тролль; отрядов орды с
  центроидом в `LAUGH_SIGHT` не больше `LAUGH_MAX_SQUADS` 3 (счётчик
  `laugh_suppressed`). **Рык тролля** (`troll_victory`) — на каждое его
  убийство из `Unit._die`, а не только над выбитым отрядом; окно категории
  2.5 с держит от пулемёта в свалке.

### Модуль 2: тролли и пень

- **Таймер восстановления пня 600 с.** Давление овец (`_sheep_pressure_check`)
  ставило пень заново на следующей же проверке после сноса. Теперь
  `TrollLair.take_damage` на гибели зовёт `GameManager.note_lair_fell(side)`,
  а `restore_lair` идёт только при `lair_regen_ready` — часы партии минус
  момент сноса ≥ `LAIR_REGEN_SEC`; отказы считает `lair_regen_refusals`.
- **Спавн −20 %, HP −20 %, гнолл +15 % темпа.** `TROLL_GUARDS_MAX` 3 → 2,
  `STATS.troll.health` 12 700 → 10 160, `STATS.gnoll.attack_cooldown` 2.08 →
  1.77. Стадо и так соответствовало заказу: `SHEEP_START` 5, сверх
  `TROLL_HUNGRY_FLOCK` 5 тролль ест без таймера, вырезанная отара
  возвращается по `SHEEP_RESPAWN_*` — не трогалось.
- **Солидарность вместо помощников.** Агро-цепочка 10.09.2026 выпускала из
  пня двух помощников по ПЕРВОМУ удару и бонусную пару по гибели группы.
  Заказ: задели одного — дерётся только он; задели двоих — все тролли
  логова. `TROLL_AGGRO_HELPERS`/`BONUS` = 0 (код цепочки оставлен, читает
  константы), `TrollLair.on_guard_hit` ведёт часы удара по каждому стражу
  (`_hit_at`); двое за `TROLL_SOLIDARITY_SEC` 10 → всем свободным
  `command_attack` на обидчика (`solidarity_calls`).
- **Малый радиус агро и поводок погони.** `Troll.aggro_radius()` →
  `TROLL_AGGRO_RADIUS` 12 м вместо обзора орды (15+). `_tick_chase`: цель
  держится не дольше `TROLL_CHASE_SEC` 10 без НАНЕСЁННОГО удара
  (`_release_swing` с попаданием сбрасывает часы); дальше — цель брошена,
  замок снят, тролль идёт к пню на патруль и `TROLL_CHASE_COOL_SEC` 3 сам
  никого не берёт (иначе тот же враг в двенадцати метрах взводил бы погоню
  заново тем же тактом). Удар по нему в окне агрит как обычно. Рейд за
  овцами поводком не считается (`_raid` не пуст — часы стоят).
- `qa_troll_aggro` переписан: B — солидарность (удар по одному → никто не
  выходит, остальные без цели; удар по второму → все на обидчика), C —
  поводок (60 м до приманки, через 10 с цель null и `chase_resets` 1,
  возврат к пню, приманка в 60 м агро не взводит, в 9 м — взводит), D —
  зачистка без бонусной пары.

### Модуль 3: клик, стрелки, массовый приказ

- **Точка земли под курсором — по рельефу.** `_pick_at` считал пересечение
  луча с плоскостью y = 0 («грунт плоский» — комментарий пережил плато и
  гармоники). Боец на плато +5 м уводил точку на 6.5 м, тролль у пня на
  +0.8 м — почти на метр; крупная цель проигрывала любому соседу. Теперь три
  итерации «высота в найденной точке → пересечение с плоскостью этой
  высоты» — сходится на любом склоне игры, стоит один клик.
- **Руина кликается по картинке.** У пепелища была только коробка
  `build_size` у основания, а ПКМ по верхней половине рисунка пролетал в
  землю. В `spawn_ruin` добавлена вторая форма — пластина по непрозрачной
  части спрайта (та же арифметика, что `_fit_pick_to_sprite`), а в
  `_pick_at` попадание луча в руину — факт (score 0), как у постройки.
  Сам ремонт (`_try_rebuild_ruin` → `rebuild_ruin`: руина уходит,
  стройплощадка того же дома со спрайтом стройки, рабочие строят) уже был —
  модуль 5 закрыт кликабельностью.
- **Стрелок откликается на приказ тем же кадром.** `command_attack` под
  `lock` у дальнобойного: `_facing` на цель и `_pose_dirty` (картинка
  разворачивается сразу), `_aggro_timer` 0, отряду в режиме залпа —
  `GameManager.squad_volley_prime`: окно залпа открывается немедленно, и
  готовые стреляют на своём тике вместо ожидания такта залпов и его порога
  готовности. Это и есть «приказ принят» глазами игрока.
- **Массовый клик по троллю.** `_big_target_spread` (цель — `Unit` с
  `fine_ring_radius() > 0`): выделенные отряды сортируются по дистанции,
  первые `BIG_TARGET_SQUADS` 3 бьют тушу, остальным цель переписывается на
  ближайшего врага в `BIG_SPILL_R` 5 м от отряда, а без такого — в 5 м от
  туши (тем же словарём `front`, каким раздаются участки чужого фронта).
- **Застрял на подходе — бьёт того, кто рядом.** Детектор `_tick_stuck`
  (тот же, что у марша и рабочих) теперь тикает и в подходе
  `_process_attack`: `STUCK_RETARGET_STREAK` 2 проверки без продвижения
  (1 с) → ближайший враг в `STUCK_RETARGET_RANGE` 5 м, `command_attack(alt,
  false)` — исходный приказ помнится в `_atk_pending`. Не для фаланги в
  обороне, не для отхода и бега. Лечит и «бег на месте» диверсантов орды,
  упёршихся в чужой строй за недосягаемой целью (модуль 6).

### Модуль 4: стена копий и оборона

- **Три шеренги — только копейщикам со «Стеной копий».** Жалоба «после
  исследования три ряда раздаются всем» — это запомненная между растягами
  коробочка [2] (спринт 17: режим живёт между растягами; спринт 18 хотфикс
  ограничил [2] копейщиками; письмо 9 спринта 19 вернуло [2] всем). Третий
  заход разводит их так: [2] по-прежнему доступна всем родам, но живёт ОДИН
  растяг — по отпусканию ПКМ `formation_mode` = широкий фронт. А ГЕОМЕТРИЯ
  стены — свойство отряда: в `_block_formation_slots` блок копейщиков с
  `spear_wall_ready` ложится в `SPEAR_WALL_ROWS` 3 при любой длине линии
  (интервал тянется до ×`WALL_STRETCH_MAX` 1.6 — растяг меняет ширину и
  поворот, не глубину); выключил — прямоугольник по линии, как у всех.
- **Фаланга в обороне подаётся целиком.** `GameManager._sweep_phalanx_press`
  раз в `PHALANX_PRESS_SEC` 0.3: у отряда копейщиков в стойке «Защита»
  (без приказа игрока, никто не идёт) ищется ближайший враг в
  `PHALANX_PRESS_RANGE` 5 м перед линией фронта; есть, и никто не достаёт —
  вся разметка и все бойцы сдвигаются на `PHALANX_PRESS_STEP` 1 м по курсу
  (`command_move` каждому на его же точку + сдвиг, курс сохранён). В
  контакте шаг не делается. **Одиночное подтягивание фланга снято**
  (`PHALANX_PULL_UP_SINGLE` false): разворот заказа авг. 2026 «весь отряд
  подтягивается» — тогда это делал каждый боец сам, теперь строй.
  `qa_sprint20` E5-E7: враг в 9 м — стоят (сдвиг < 0.4 м), в 4.5 м —
  фаланга шагнула ≥ 0.8 м строем, одиночек за врагом нет.

### Модуль 6: перемирие и ИИ орды

- **Перемирие 600 с** — одно число с мирной фазой орды (`TRUCE_SEC` =
  `PEACE_SEC`) и `AI_TRUCE_SEC` красного ИИ (рейды на экономику игрока —
  не раньше). Плашка «перемирие MM:SS» в таймере HUD
  (`GameManager.truce_left`).
- **Защита красного ИИ.** `GoblinAI._target_faction_allowed(f)`: база ИИ —
  цель рейдов, добычи и штурма не раньше `AI_PROTECT_SEC` 1800 и только при
  живой крепости ИИ. Игрок защитой не прикрыт.
- **Рейд без добычи возвращается.** `_raid_plan` при отсутствии добычи вёл
  отряд к базе и оставлял стоять до `RAID_MAX_SEC` — «пасутся в углу базы».
  Теперь база не найдена или `RAID_IDLE_SEC` 12 без цели — рейд снят,
  отряд в поле (`raid_returns`). Диверсии по 1-3 отряда (`RAID_SQUADS_BIG`
  3). Приоритет «брод и центр» (`PHASE_CENTER` → (0,0), рудник холма
  `hill_mine_sid`) и «кулак по лимиту армии» (`_assault_ready` требует
  `army_squads()`) уже стояли со спринта 17 — не менялись.

### Шлюз и цена спринта 20

- **Полный шлюз, 92 сцены, `SCRIPT ERROR` 0** (`gate_final20.txt`).
  Контрактные красные переписаны под заказ: `qa_gnoll_fix` A9 (кд гнолла
  1.77), `qa_audio18` E2/E3 (гоблин-добивший обязан быть В ОТРЯДЕ орды:
  хор судит по отрядам в обзоре) и F7 (рык идёт двумя запросами — из `_die`
  и над выбитым отрядом, окно категории пропускает один), `qa_sprint18b`
  B6 (фаланга не шагает навстречу набегающей коннице — `velocity` врага к
  строю или `is_charging`), C3/C4/E17б/E18, `qa_troll` A5 и `qa_lair_ai` A6
  (глушь у пня — чистое только внутреннее кольцо 21 м), `qa_troll_hitbox`
  A1 (HP 10 160), `qa_monk_vfx/Shot` (замер вокруг ленты, порог 0.5 %, у
  ног). Одиночно зелёные после шлюза: `qa_vet` 4г, `qa_full_game_4k` A1,
  `qa_voice` D6 (дважды), `qa_troll_impact` E5, `qa_start_build` D4.
  Известные: `qa_volley` C1, `qa_mega_battle` B1-1, `qa_rally2` Bб/F2/F3.
  Оконные: `qa_visual_smoke` 26/26, `qa_monk_vfx/Shot` 6/6.
- **Цена (qa_stress_report, окно, тот же сценарий, что в спринте 19):**
  минимум 68 FPS (было 72; разброс прогонов ±3-5), кадр 13.5 мс в пике (то
  же), честный тик армии 6.15 мс (было 5.50; `process_attack` 0.33 → 0.42
  мс/кадр — детектор застревания в подходе и такт продвижения фаланги),
  кадр логики 2.67 мс, навигация 0.009 мс/кадр. В бюджете 16.6 мс с запасом;
  честный A/B по `tick_meter` под такт фаланги не снимался — 0.3 с и десятки
  отрядов, а не тысячи бойцов.


## Аудит класса «гнолл»: скан обороны пня шёл каждый физтик (13.09.2026)

Заказ владельца: разобраться, почему тик гнолла стоит вдесятеро дороже тика
гоблина (стресс-отчёт спринта 20: 0.056 против 0.007 мс на юнит). Зонд —
**`qa_gnoll_bench`** (измеритель, вердиктов нет, в шлюз не входит).

- **ВИНОВАТ НЕ `command_move`, А СКАН, И ЭТО РАЗВОРОТ ПРЕЖНЕГО ДИАГНОЗА.**
  Столбец «Главный Bottleneck» в стресс-отчёте у гнолла был ЗАХАРДКОЖЕННОЙ
  СТРОКОЙ («кайт/фланг: command_move раз в такт»), а не замером, и он врал:
  кайт выдаёт приказ не чаще `GNOLL_KITE_SEC` (1.1 с) и глушится на
  `GNOLL_KITE_BLOCK_SEC`, если отход не дал смещения. Приказов гнолл шлёт
  МЕНЬШЕ, чем пехотинец. Строка исправлена на измеренную причину.
- **ИНТЕРВАЛ ВЗВОДИЛСЯ ТОЛЬКО НА УСПЕШНОМ ПУТИ.** И `_tick_kite`, и
  `_tick_flank` ставят свой таймер ПОСЛЕ проверки `foe == null`: врага в
  радиусе нет — таймер остаётся в нуле, и на следующем физтике скан идёт
  снова. То есть заявленное в комментарии «не чаще раза в `GNOLL_FLANK_SEC`»
  выполнялось ровно в том случае, когда враг НАЙДЕН, а в штатном (никого
  рядом) — не выполнялось вовсе. **Гнолл был дороже всего, когда вокруг
  пусто:** замер блока B против блока C — 62.6 мкс/тик в покое против
  49.0 мкс/тик с врагом у пня.
- **ЦЕНА СКАНА ИЗМЕРЕНА НА ЖИВОЙ СЦЕНЕ** (блок A, ячейка сетки ядра 1.00 м,
  85 140 ячеек): кайт `r = GNOLL_KITE_IN` 6 м — **10.3 мкс**, оборона пня
  `r = GNOLL_DEFEND_RANGE` 34 м — **32.4 мкс** за один заход из кода
  (`_nearest_foe_at` обходит ДВЕ чужие стороны, то есть два вызова
  `nearest_of_side`). `NearestOfSide` перебирает КВАДРАТ ячеек 2r × 2r:
  при 34 м это ~4761 ячейка на сторону, 9522 на заход, шестьдесят раз в
  секунду, на каждого гнолла. Цена растёт как r², и разница радиусов 6 и
  34 м — это не «вшестеро», а двадцать восемь раз по площади.
- **ЛЕЧЕНИЕ — ПРОМАХ ТОЖЕ ВЗВОДИТ ЧАСЫ** (`Gnoll._miss_gate`, ручка
  `perf_config.gnoll_scan_gate`): не нашли — ставим свой, КОРОТКИЙ интервал
  (`GNOLL_FLANK_MISS_SEC` 1.0, `GNOLL_KITE_MISS_SEC` 0.3), а не полный
  рабочий. Полные четыре секунды после промаха ставить нельзя — подошедшего
  к пню врага стая замечала бы с опозданием. Фазы разведены по номеру узла
  (`GNOLL_SCAN_PHASES` 17, множитель 0.75–1.25): иначе вся стая сканирует в
  один кадр и горб съезжает, а не исчезает.
- **ЗАМЕР A/B ЧЕРЕДОВАНИЕМ** (qa_gnoll_bench блок D, 3 раунда, порядок фаз
  переворачивается): **57.0 → 11.2 мкс/тик, −80 %**. На настоящей сцене
  (`qa_stress_report`, тот же сценарий): гнолл **0.055 → 0.016 мс/тик**,
  класс **1.02 → 0.30 мс/кадр**, честный тик армии **5.89 → 5.10 мс**,
  минимальный FPS 68 → 72, пик физтика 21.8 → 18.4 мс, худший интервал
  физкадра 44.7 → 38.8 мс. Поведение не тронуто: `qa_sprint13` 80/0,
  `qa_gnoll_fix` 37/0 (дважды), `qa_gnoll_behavior` 14/0,
  `qa_performance_stress` 11/0 с включённой ручкой.
- **ЛОВУШКА ЗОНДА: ПЛОЩАДКУ ДЕРЖАТЬ В ПОЛЕ КАРТЫ.** Габарит армии задаёт
  размер ячейки сетки ядра (`ArmyCore`: cell удваивается, пока w × h не
  влезет в `MaxCells` = 262 144). Стенд, поставивший бойцов за тысячу метров
  от карты — обычный приём у `qa_gnoll_behavior`, — получил бы ячейку 4 м и
  померил бы скан вшестнадцатеро дешевле настоящего.
- **ВТОРАЯ ЛОВУШКА, ТА ЖЕ, ЧТО В ПАМЯТИ ПРОЕКТА:** `_Opt.set("ключ", v)` на
  классе-скрипте — ошибка ПАРСИНГА («Cannot call non-static function set()»),
  сцена остаётся без скрипта и висит молча, а сторож-таймер живёт в этом же
  скрипте и потому не срабатывает вовсе. Статическую ручку присваивать
  напрямую.
- **ЧТО ОСТАЛОСЬ И НЕ ЛЕЧИЛОСЬ (замером не подтверждается как проблема):**
  `_nearest_foe_at` строит новый массив на каждый заход
  (`Constants.other_factions`, правило 4) — с гейтом это 3-4 вызова в
  секунду на бойца, то есть в шуме; готовый список по образцу
  `Constants.enemy_building_groups` имеет смысл заводить, только если скан
  снова станет частым. `_tick_flank` спрашивает логово через
  `has_method`/`call` по СТРОКЕ — тоже под гейтом.


## Центровой нод отряда (Squad Radar) и бенчмарк на 3000 лучниках (13.09.2026)

Заказ владельца: перенести скан целей с бойца на ОДИН центровой нод отряда
(2 Гц), завести упреждающий радиус 25 м при дальности лука 20 м с двумя
фазами (`TARGET_LOCKED` → `FIRE`), сбрасывать такт по клику игрока и снять
сравнительные метрики на 3000 лучниках (100 отрядов по 30).

Механика построена: `GameManager._sweep_squad_radar` + `_radar_scan` /
`_radar_face` / `_radar_open_fire`, ручка `perf_config.squad_radar`
(по умолчанию ВЫКЛЮЧЕНА — это изменение поведения, а не чистая оптимизация;
см. ниже, почему замер не даёт оснований включать её ради кадра).
Стенд-бенчмарк — **`qa_3000_archers_bench`** (8 вердиктов по поведению плюс
таблица; A/B чередованием, первый раунд — прогрев и в зачёт не идёт).

📊 **BENCHMARK REPORT: ARCHER SCANNING (3000 UNITS)**

| Метрика | Старая система (Индивидуальная) | Новая система (Squad Radar 25m/20m) | Разница / Профит |
| :--- | :--- | :--- | :--- |
| **Вызовов Scan / sec** | 1147 / sec | 1141 / sec | **−0.5 %** |
| **из них на стрелковых отрядах** | 399 / sec | 199 / sec | **−50.3 %** |
| **Process Time (Physics)** | 6.63 ms | 6.55 ms | **−0.08 ms** |
| **FPS (при 3000 юнитах)** | 89 FPS | 93 FPS | **+4 FPS (в шуме)** |
| **Задержка первого залпа** | 0.53–1.30 sec | 0.07–0.43 sec | **потолок 0.5 с вместо 2 с** |

Сцена: 100 отрядов по 30 лучников + 400 пехоты волной, 4548 живых, ячейка
сетки ядра 1.00 м; фазы по 8 с, порядок фаз переворачивается по раундам.

- **ИСХОДНАЯ ПОСЫЛКА ЗАКАЗА НЕВЕРНА, И ЭТО ГЛАВНЫЙ РЕЗУЛЬТАТ ЗАМЕРА.**
  «3000 лучников сканируют индивидуально на 3.5 Гц = 10 500 сканов/с» — в
  этом коде такого нет и не было. Скан УЖЕ отрядный, и держат его ДВА
  механизма, оба старые: отрядный кэш цели (`squad_target_get/put`, TTL
  `squad_target_ttl` 0.4 с, ищет знаменосец) и — что важнее — **отрядный
  ответ `_clear_enemy`** из `_sweep_corridors`: в габаритах отряда плюс
  радиус внимания чужих нет, значит их нет и у каждого бойца, и авто-агро
  не заходит внутрь ВООБЩЕ. Стоящий стрелковый отряд без врагов рядом стоит
  НОЛЬ сканов.
- **РАЗМЕТКА МЕСТ ВЫЗОВА (`perf_config.scan_sites`) НАЗЫВАЕТ ЦИФРЫ.** Из
  1147 сканов/с на сцене 3000 лучников: `blocker_reach` ~700/с — это
  МАРШИРУЮЩАЯ ПЕХОТА проверяет заслон на своём шаге, к лучникам отношения
  не имеет; `squad_cache_fill` ~390/с — собственно отрядный скан стрелков;
  остальное (`cache_miss_range`, `retarget_lost`, `blocker_march`) — единицы
  и десятки. Радар заменяет ровно вторую строку: 390 → 199/с (100 отрядов
  × 2 Гц, ровно как заказано). Общее число сканов при этом почти не
  меняется — две трети их дают не стрелки.
- **ТИК И FPS НЕ ДВИГАЮТСЯ, И ЭТО АРИФМЕТИКА, А НЕ НЕУДАЧА.** 200 сэкономленных
  сканов в секунду по ~25 мкс — это 5 мс в СЕКУНДУ, то есть 0.08 мс на кадр:
  ровно столько и намерено (6.63 → 6.55). FPS по раундам: 86.3 / 86.5 / 93.4
  у старого пути против 93.0 / 93.8 / 90.8 у радара — выборки перекрываются,
  разрешить разницу этим замером нельзя (документированный разброс ±3-5).
- **ЧТО РАДАР ДАЁТ НА САМОМ ДЕЛЕ — ЭТО ПРЕДСКАЗУЕМОСТЬ ОТКЛИКА.** Старый путь
  опрашивает по ЛИЧНОМУ таймеру бойца (`AGGRO_INTERVAL_CALM` 2.0 с /
  `HOT` 0.5 с), и задержка первого залпа — лотерея: пять независимых замеров
  дали 0.53, 0.80, 0.93, 1.03 и 1.30 с. У радара такт фиксирован, и задержка
  ограничена им сверху: 0.07-0.43 с. Плюс фаза прицеливания, которой у
  старого пути нет вовсе.
- **РАДИУС ОБНАРУЖЕНИЯ ВЫВОДИТСЯ, А НЕ СТОИТ ЧИСЛОМ** (`RADAR_MARGIN` 5.0 к
  `attack_range` бойца). Вписанные в код 25 м означали бы, что у отряда с
  прокачанной кузницей упреждение исчезло (дальность растёт до
  `attack_range_cap`), а у подрезанного стало бы больше самой дальности.
  Упреждение — это НАДБАВКА к дальности, а не абсолютная величина.
- **ЭТО СВОД, А НЕ УЗЕЛ-НА-ОТРЯД.** Сто отрядов узлами — это сто входов из
  движка в GDScript каждый кадр ради одного таймера; от этого в проекте уже
  отказались у торчащих стрел (`_sweep_stuck_arrows`) и у тел павших.
  Состояние «нода» живёт в записи отряда, обходит их один свод; снаружи это
  тот же центровой нод, только без платы за нотификацию. Фазы отрядов
  разведены по номеру (`(sid * 37) % RADAR_INTERVAL_MS`) — иначе сто отрядов,
  заведённых одним кадром, сканируют в один кадр, и горб съезжает, а не
  исчезает.

### Три поломки, которые нашёл собственный шлюз (все — на стыке с залпом)

- **ОДНА ЦЕЛЬ НА ВЕСЬ ОТРЯД СХЛОПЫВАЕТ ЗАЛП.** Первая версия
  `_radar_open_fire` давала всем тридцати ОДНУ И ТУ ЖЕ модель. Точка залпа
  считается по центру масс ЦЕЛЕЙ (`squad_volley_point`), и туча схлопнулась
  в одного бойца: `qa_volley` D5 — «ранено 2 из 16» вместо накрытия строя.
  Лечение — жертву внутри вражеского отряда разбирает тот же
  `squad_pick_member` («наименее обстрелянный»), которым её разбирает приказ
  игрока: один способ выбирать жертву на все источники приказа.
- **РАДАР НЕ ДОЛЖЕН ОТКРЫВАТЬ ОКНО ЗАЛПА.** Вторая версия звала
  `squad_volley_prime` на переходе в фазу огня — «чтобы стреляли тем же
  кадром». Окно, открытое мимо `_sweep_volleys`, уносит с собой и расчёт
  ТОЧКИ залпа (он живёт там же), а после такого залпа отряд больше не
  набирал «готовых» и замолкал НАВСЕГДА: замер показал `able=12, ready=0` на
  каждом такте. Окно — дело такта залпов, он идёт раз в `VOLLEY_SWEEP_EVERY`
  кадров (≤ 50 мс), и «мгновенно» от этого не страдает. Радар раздаёт цели,
  и только.
- **ТЕХ, КТО ПОТЕРЯЛ ЦЕЛЬ, НАДО ДОБИРАТЬ.** Раздача только на переходе
  оставляла бойца без цели навсегда: сам он больше не сканирует — в этом и
  смысл центрового нода. Добор идёт каждый такт, но ТОЛЬКО тем, у кого цели
  нет или она мертва (`_radar_open_fire(..., only_idle)`); у остальных
  личные цели, набранные боем, не перетираются.

### Ловушки бенчмарка (каждая давала неверный вывод)

- **ВОЛНА ОБЯЗАНА ДОЙТИ ЗА ФАЗУ.** Первая версия ставила пехоту в 55 м, и за
  шесть секунд (2.2 м/с) она не доходила вовсе — обе фазы мерили СТОЯЩУЮ
  армию, в которой никто никого не видит. Теперь 30 м: за фазу волна
  пересекает и 25 м, и 20.
- **ПЕРВЫЙ РАУНД — ПРОГРЕВ.** На одном и том же коде он дал 56 FPS в одной
  фазе и 281 в другой. В зачёт не идёт.
- **ПЛОЩАДКИ БЛОКОВ ПОВЕДЕНИЯ — ВНУТРЬ КАРТЫ.** `PLAYER_BASE_ANCHOR` лежит в
  углу, и «якорь минус 90 м» уходит за край: границы мира зажимают бойцов к
  кромке, дистанции схлопываются, и блок задержки залпа показывал «огня не
  было» в ОБОИХ режимах.
- **КЛИК ИГРОКА — ЭТО `lock`, А НЕ `forced`.** `command_attack(foe, true)`
  передаёт второй аргумент `forced`; ветка мгновенного отклика стрелка (и
  сброс такта радара) висит на ЧЕТВЁРТОМ, `lock`. Стенд с двумя аргументами
  проверял не то, что думал.
- **`qa_volley` D ЖДАЛ КАДРЫ ОТРИСОВКИ** (`frames(180)`), а не физкадры
  (правило 11). Переписано на `physics_frame`; на диагноз это не повлияло,
  но ждать там было нечего.

**Состояние:** ручка `squad_radar` по умолчанию false — игра не изменена.
С включённой ручкой зелены `qa_volley` (кроме известного мигающего E1),
`qa_volley_fix`, `qa_archer_logic`, `qa_target_lock`, `qa_mega_battle`
(кроме известного B2-1). Включать ли механику — решение владельца: замер
говорит, что кадра она не даёт, а даёт предсказуемый отклик и фазу
прицеливания.


## Трёхточечный РЛС пехоты на марше: −99 % сканов, ±0 мс (13.09.2026)

Заказ владельца: перевести проверку заслона на марше (`blocker_reach`) на
двухфазную схему — отряд щупает фронт ИЗ ТРЁХ ТОЧЕК (левый фланг, центр,
правый фланг) на 10 м раз в 2-3 с; пока впереди пусто, личный скан не зовёт
никто; помеха ближе 3-5 м — фаза 2, первая шеренга проверяет заслон как
раньше. Построено: `GameManager._sweep_march_radar` + `_march_radar_points` /
`_march_radar_scan` / `squad_march_clear`, ручка `perf_config.march_radar`
(по умолчанию ВЫКЛЮЧЕНА). Зонд — **`qa_infantry_radar`** (7 вердиктов,
headless).

📊 **INFANTRY PATH RADAR — 400 идущих против 400 в чужом строю**

| Режим | Тик армии | Сканов всего | из них перехват | щупов РЛС |
| :--- | :--- | :--- | :--- | :--- |
| A. Штатный (личный скан) | 3.95 ms | 327 / sec | **152 / sec** | — |
| B. РЛС 3 точки (10 м / 2.5 с) | 3.94 ms | 167 / sec | **1 / sec** | 18 / sec |
| Потолок: скана нет вовсе | 3.66 ms | 177 / sec | 0 / sec | — |

**Профит по сканам −99.1 %. Профит по времени — НОЛЬ (−0.01 мс/тик).**

- **ЦИФРА «~700 СКАНОВ/С» ОТНОСИЛАСЬ К ДРУГОЙ СЦЕНЕ.** Она снята на
  `qa_3000_archers_bench`, где 400 пехоты идут СКВОЗЬ строй из 3000 лучников
  — плотность контакта там предельная. На честном марше 400 против 400
  перехват даёт 150-200 сканов/с, то есть 0.4 скана на бойца в секунду при
  таймере 0.5 с: три четверти вызовов уже отсекает отрядный ответ
  `_clear_enemy` (`_sweep_corridors`), а внутри ядра пустую округу отсекает
  грубая сетка (`EnemyNear`) ДО перебора ячеек.
- **ЦЕНА ОДНОГО СКАНА — 4.5 МКС, И ОНА НЕ ЗАВИСИТ ОТ РАДИУСА** (замер на
  живой сцене: r = 3 м — 4.58 мкс, r = 10 м — 4.48). Отсюда арифметика:
  150 сканов/с × 4.5 мкс = 0.7 мс В СЕКУНДУ, то есть **0.011 мс на кадр**.
  Ровно столько РЛС и сэкономил (−0.01). Никакой схемой из этой статьи
  больше не достать: её там нет.
- **ПОТОЛОК −0.29 МС ОБМАНЧИВ, И ЭТО ГЛАВНАЯ ЛОВУШКА ЗАМЕРА.** Выключение
  скана целиком даёт 3.95 → 3.66 мс, и легко прочесть это как «вот сколько
  стоят сканы». Не стоят: 0.29 мс — это цена БОЯ, который перехват
  ПОРОЖДАЕТ. Без скана отряды меньше вступают в бой на ходу, в тике меньше
  дерущихся, и он дешевеет. РЛС поведение сохраняет (блок E: 39 из 40
  зацепивших против 39 штатных) — и потому сохраняет и цену. Это ровно
  правило проекта «мерить надо последствия, а не вызов», только развёрнутое:
  здесь последствие выключения — не экономия, а другая игра.
- **ПЕХОТА НЕ ПРОХОДИТ СКВОЗЬ СТРОЙ НИ В ОДНОМ ИЗ ТРЁХ РЕЖИМОВ** (блок D:
  за линией 0 / 0 / 0 из 400). Скан заслона за это НЕ ОТВЕЧАЕТ вовсе —
  тела держит пакетный шаг ядра (`ScanBlock`/`EnemyBlock`), а `blocker_reach`
  решает только, ВСТУПИТ ЛИ идущий в бой с тем, мимо кого идёт.
- **ЛБОМ В СТРОЙ ПЕРЕХВАТ НЕ НУЖЕН ВОВСЕ** (блок D: 400 из 400 в бою во всех
  трёх режимах, включая «скана нет»). Упор телом ставит `wall` — тот же
  пакетный шаг, — а дошедший до точки приказа встаёт в покой, где его
  подбирает обычное авто-агро. Первая версия блока D утверждала обратное и
  дала бы ложное зелёное на любом коде.
- **НАСТОЯЩИЙ СМЫСЛ ЭТОГО СКАНА — КАСАТЕЛЬНЫЙ МАРШ** (`MARCH_INTERCEPT_MELEE`,
  спринт 15: «цеплять юнитов с флангов / по касательной»). Блок E: отряд
  идёт МИМО группы в 6 м вбок. Штатный путь — 39 из 40 зацепили НА ХОДУ,
  первый контакт на 7.1 с; РЛС — 39 из 40, 7.5 с (отставание на один щуп);
  без скана — **0 из 40 на ходу**, первый контакт лишь на 10.2 с, уже после
  прихода в точку и через авто-агро. Мерить здесь надо МОМЕНТ, а не число:
  по числу через 22 секунды все три режима дают 40 из 40.

### Решения реализации

- **ТРИ ТОЧКИ, А НЕ ДВЕ** (уточнение заказа): точки берутся ПРОЕКЦИЕЙ мест
  бойцов на ось, поперечную курсу, то есть работают при любой ширине строя и
  любом повороте; центр — среднее мест (здесь это щуп, а не точка приказа).
- **ЩУП — ДИСК, А НЕ ЛУЧ.** Луч пускать нечем: физтел у бойцов нет
  (`collision_mask` = 0), и «трассировка» обернулась бы своим перебором
  сетки, то есть тем же сканом. Диск радиусом 10 м ещё и надёжнее — он не
  может пропустить помеху, стоящую чуть в стороне от оси.
- **ФАЗА 2 ВЗВОДИТСЯ С УПРЕЖДЕНИЕМ**, а не по факту контакта: между щупами
  проходит 2.5 с, за которые отряд идёт несколько метров. Порог — «контакт
  плюс путь до следующего щупа»; иначе помеха, замеченная на 6 м, была бы
  пропущена до самого упора телом.
- **ФАЗЫ ОТРЯДОВ РАЗВЕДЕНЫ ПО НОМЕРУ** (`(sid * 53) % step`): иначе сто
  отрядов щупают фронт в один кадр и горб съезжает, а не исчезает.
- **СТОЯЩИЙ ОТРЯД РЛС НЕ ОБСЛУЖИВАЕТ** (`mradar_phase` = «помеха»): его
  округу и так стережёт `_clear_enemy`, а «чисто» у стоящего означало бы
  снятый перехват у того, к кому противник подходит сам.

**Состояние:** ручка `march_radar` по умолчанию false — игра не изменена. С
включённой ручкой зелены `qa_wall` 13/13, `qa_melee` 14/14, `qa_disengage`
15/15, `qa_spear` 49/49, `qa_bugpass` 45/45. Включать ли — решение
владельца: механика работает и режет 99 % сканов, но кадр от этого не
меняется, а первый касательный контакт запаздывает на один щуп (+0.4 с).


## Приёмка РЛС, паника у края карты и элита Крепости (13.09.2026)

Пакет из трёх разделов: принять трёхточечный РЛС пехоты, вылечить «забивание
в угол» деморализованными и развести мечников Бараков и Крепости.
Стенды: **`qa_panic_edge`** (7, новый), **`qa_keep_elite`** (18, новый),
`qa_infantry_radar` 7/7.

### 1. РЛС принят, период удвоен

- `perf_config.march_radar` = **true** по умолчанию, `MARCH_RADAR_SEC`
  2.5 → **5.0 с**. Ручка оставлена ради A/B.
- **ЛИЧНЫЕ СКАНЫ СНИЖАТЬ НЕКУДА — ОНИ УЖЕ РЕЖЕ ЗАКАЗАННОГО.** Заказ просил
  «3-5 раз в секунду»; в коде опрос идёт по `AGGRO_INTERVAL_HOT` 0.5 с (2/с)
  и `AGGRO_INTERVAL_CALM` 2.0 с (0.5/с), то есть ВДВОЕ-ВШЕСТЕРО реже, чем
  просят. Поднимать частоту до 3-5/с означало бы ухудшить. С включённым РЛС
  перехватных сканов остаётся **1/с на 400 идущих** (было 152).
- **ШАХМАТНОЕ РАСТАЛКИВАНИЕ УЖЕ ПОСТРОЕНО, И ЭТО НАДО ЗНАТЬ ДО ПРАВКИ.**
  Заказ описывает «просчёт через кадр / по фазам / на узле отряда». В проекте
  ровно так и сделано: фаза бойца ставится при рождении
  (`Unit._sep_phase = get_instance_id() & 7`, восемь фаз), личный такт —
  `SEP_INTERVAL` 0.067 с (15 Гц, а не каждый кадр), сам расчёт идёт ПАКЕТНО в
  C# (`ArmyCore.BatchSeparation`) и многопоточно (`CoreThreads`, Parallel.For
  по диапазонам строк). Цена подтверждена замером: `sep_overlap` **0.17 мс на
  кадр** в стресс-сцене — статья десятого порядка. Ничего не переписывал:
  «заморозка внутренних рядов» (Rigid Offset) противоречит правилу 7 проекта
  («расталкивание монотонно и только наружу, сил избегания добавлять нельзя»)
  и стережётся `qa_crowd` A1/A3, а выигрыш там — доли миллисекунды.

### 2. Беглецы больше не забиваются в угол

- **ПРИЧИНА НАЙДЕНА И НАЗВАНА ЧИСЛОМ.** Точка бегства зажималась границей
  мира (`land_target` → `clamp_to_map`), и отряд, чей вектор бегства смотрит
  наружу, получал точку РОВНО НА КРОМКЕ — ВЕСЬ, до последнего бойца. Дальше
  туда же его возвращал подзыв связности (`_panic_regroup`). На скриншоте
  владельца это колонна у правого края.
- **ЛЕЧЕНИЕ — ТРИ ЧАСТИ.** `_panic_inside` держит все точки бегства не ближе
  `PANIC_EDGE_MARGIN` 18 м от кромки; `_panic_slide_from_edge` снимает у
  вектора бегства составляющую, направленную наружу (в углу — уходит вдоль
  той стороны, до которой дальше, то есть «беги по краю», а не «беги в
  угол»); подзыв связности тоже пропущен через `_panic_inside` — иначе он
  возвращал бы беглецов ровно в ту колонну, ради разгона которой всё
  делается. Замер `qa_panic_edge`: у бокового края ближайший к кромке
  **15.9 м** (было 0), в углу **15.1 м**.
- **РАЗБЕГ — КОНУС ±`PANIC_CONE_DEG` 30°, А НЕ ПОЛНЫЙ КРУГ.** Было: у каждого
  своё направление на все 360°, то есть половина отряда бежала НАВСТРЕЧУ
  своей же половине — на экране это «толпа сжалась и топчется». Замер: назад
  не пошёл никто (0 из 24), 24 из 24 убежавших уложились в конус, худшее
  отклонение 37°.
- **СТУПОР 20 → 14 с** (−30 %), мораль гнолла **40 → 55**: стая срывалась,
  потеряв около десятой части состава, теперь — около четверти.
- **ЛОВУШКА ЗАМЕРА, ДАВШАЯ ЛОЖНЫЙ КРАСНЫЙ ДВАЖДЫ:** угол бегства считался у
  всех, кто сдвинулся хотя бы на метр (потом — на три). Метр и три метра
  бойцу даёт и разбор наложения в куче, а точка бегства лежит в двадцати
  метрах: у почти не сдвинувшегося направления нет вовсе, и он давал
  «отклонение 84°» на исправном веере. Считать угол можно только у того, кто
  прошёл заметную долю пути бегства.

### 3. Элита Крепости: мечники с лычками

- **РАНГ, А НЕ ОТДЕЛЬНЫЙ РОД ВОЙСК.** Ветеранство уже умеет всё нужное:
  лычки на знамени, звание, бонусы, значок в панели, сохранение партии.
  Второй «элитный мечник» означал бы вторую таблицу статов, вторую ветку в
  найме и второй источник правды о том, что такое опытный боец.
- **ЧИСЛА.** Найм в Крепости 600 дерева / 1500 золота / 75 с против
  250 / 600 / 60 в Бараках (золото ×2.5). Ступени `KEEP_VET_UPGRADES`:
  II — 400/1200/200 камня, III — 800/2400/400, IV — 1600/4800/800; каждая
  следующая вдвое дороже. Потолок `KEEP_WARRIOR_VET_MAX` 4.
- **СОДЕРЖАНИЕ ПРИВЯЗАНО К РАНГУ, А НЕ К МЕСТУ НАЙМА** (`keep_food_mult` /
  `keep_gold_rate` в `_sweep_food`): +25 % еды и 0.05 золота в секунду за
  лычку. Правило накрывает и элиту Крепости, и отряд, доросший до лычек в
  бою, — платят за ОПЫТ. Нехватка золота при этом НЕ голод: мораль она не
  трогает (это дело еды), а бьёт по кошельку.
- **ГЛАВНАЯ ПОЛОМКА ЗАХОДА — ВЫДАЧА РАНГА ПУСТОМУ ОТРЯДУ.** Первая версия
  звала `grant_squad_veterancy` сразу при создании отряда в очереди найма.
  Отряд в этот момент ПУСТ (бойцы доезжают следующими кадрами через
  `_pending_spawns`), а раздача наград идёт через `squad_members()` — и ТОТ
  РАСПУСКАЕТ ПУСТОЙ ОТРЯД ПРЯМО В ГЕТТЕРЕ (известная ловушка проекта).
  Снаружи это выглядело так: заказ принят, очередь дотикивает до нуля, а
  отряда на карте нет вовсе (`qa_keep_elite` B1, sid 0). Теперь при создании
  пишется только НАМЕРЕНИЕ (`keep_vet_pending`), а раздача идёт в момент,
  когда в отряде появился первый боец.
- **РАНГ ПЕРЕЖИВАЕТ СОХРАНЕНИЕ** (`SaveLoadManager._capture_keep_vet`):
  одно число на фракцию, читается с умолчанием — старые файлы читаются как
  «ранг по умолчанию», и `FORMAT_VERSION` поднимать не нужно.
- **ЛОВУШКА СТЕНДА:** «дождаться нового отряда» по появлению нового ключа в
  реестре ловит ЧУЖОЙ отряд — логово тролля выпускает волны гноллов по своим
  часам, и проверка ранга мигала «0 через прогон» на исправном коде. Искать
  надо по ТИПУ и СТОРОНЕ.

**Регрессия:** `qa_panic` 19/19, `qa_balance` 24/24, `qa_gnoll_fix` 37/37,
`qa_wall` 13/13, `qa_melee` 14/14, `qa_disengage` 15/15, `qa_spear` 49/49,
`qa_bugpass` 45/45, `qa_squad` 52/52, `qa_tower` 47/47, `qa_food_economy`
16/16, `qa_vetui` 45/45, `qa_difficulty` 58/58, `qa_infantry_radar` 7/7,
`qa_panic_edge` 7/7, `qa_keep_elite` 18/18 (дважды).


## Большой гоблин и «скан по перезарядке»: механика построена, экономия ноль (13.09.2026)

Заказ: новый тяжёлый юнит Big Goblin со свипом и отбросом плюс глобальный
переход на событийный скан целей (Scan-on-Cooldown). Стенд —
**`qa_big_goblin`** (22 проверки, headless).

### Юнит

- **`scripts/goblin/BigGoblin.gd`** — обычный `Unit` третьей стороны на
  ЛЕНТАХ ГОБЛИНА-КОПЕЙЩИКА, увеличенных втрое (`BIG_PIXEL_SIZE` = пиксель
  человека × 3). Не тролль и не кабан: бой, строй, туман, ветеранство и общая
  отрисовка достаются от базы.
- **ЧИСЛА ВЫВЕДЕНЫ ИЗ ТАБЛИЦЫ ГОБЛИНА, А НЕ ВПИСАНЫ:** запас =
  `SQUAD_SIZE.goblin_spearman × health` (100 × 165 = **16 500**), шаг = 2.2 ×
  0.7 = **1.54**, удар = 10 × 3 = **30**. Владелец правит числа гоблина — туша
  едет за ними.
  **ВНИМАНИЕ К ЗАПАСУ:** буквальное прочтение заказа даёт тушу в полтора раза
  прочнее тролля (10 160) и отряд из пяти на 82 500 HP. Ручка
  `BIG_GOBLIN_HP_SQUADS` заведена ровно для того, чтобы поправить это одним
  числом, не трогая кода. Второе прочтение шага из того же письма («вдвое
  медленнее гоблина» = 1.1) разошлось с первым (−30 % = 1.54) — взято то, что
  названо точным процентом.
- **«КЛЕТКА» — ЭТО МЕТР.** Тайловой игровой сетки в проекте нет вовсе;
  ячейка сетки соседей в ядре равна 1 м. Поэтому «атака 4-5 клеток» =
  `attack_range` 4.5 м, «3 клетки в ширину» = сектор ±60° с полушириной 1.5 м,
  «отброс на 1-2 клетки» = 1.5 м.
- **КЛИКБОКС — НЕ Area2D.** Физтел у бойцов в проекте нет (`collision_mask`
  = 0), и попадание мыши считает `SelectionManager._pick_at` по `pick_body_h`
  / `pick_radius` — тем же способом, каким кликается тролль. Лучник целится в
  `aim_height` 1.4 м (в корпус, а не в ступни).
- **СВИП** (`_sweep_now`): ОДИН скан сетки на весь взмах, сектор перед
  взглядом, до `BIG_SWEEP_MAX` ближайших; каждому урон и отброс через
  `apply_knockback` — тот же канал затухающей скорости, что у конницы и дубины
  тролля. Своей механики отбрасывания не заводится.
- **РИТМ:** перезарядка 4 с — это очень долго, и между ударами туша выглядела
  бы замершей, поэтому каждый третий взмах ДВОЙНОЙ (второй удар через
  `BIG_COMBO_GAP`). Замер: 4 взмаха на 3 удара.
- **×2 ОТ СТРЕЛ** — `Unit.ranged_damage_mult()`, свойство ЦЕЛИ, читается ОДИН
  раз в `Arrow._strike`. В горячем пути рукопашной его нет вовсе: пехота за
  уязвимость крупных мишеней не платит ни одного сравнения.
- **СПАВН:** отряды по 5 (`SQUAD_SIZE.big_goblin`), три отряда на базе орды
  (три строки в `START_SQUADS`), один отряд с каждой волной логова и
  `BIG_DEFENSE_SQUADS` 2 МГНОВЕННО по удару в логово — без часов, в отличие от
  гнольских волн. Спавн идёт единственным в проекте путём
  (`Main.spawn_goblin_squad`).
- **ТЯЖЁЛЫЙ ШАГ** (`big_step`): редкий низкий удар ноги на ходу. Своих файлов
  туше не заведено — тяжесть делают ПИТЧ (0.55-0.68) и громкость поверх
  готовых сэмплов: новый сэмпл ради двух событий это лишний мегабайт в паке и
  лишняя строка в реестре прогрева.

### Scan-on-Cooldown: измерено, выигрыш около нуля

Заказ описывает четыре приёма. Три из четырёх В ПРОЕКТЕ УЖЕ ЕСТЬ, и это надо
знать до того, как их «внедрять»:

| Приём из заказа | Что уже стоит в коде |
| :--- | :--- |
| Заморозка скана на КД | `Unit._atk_snooze` + `ArmyCore.TickSnooze`: боец в упор к цели на перезарядке В GDScript-АВТОМАТ БОЯ НЕ ЗАХОДИТ ВОВСЕ; пробуждение — событие |
| Кэш прежней цели | Базовый `_process_attack` держит цель и ищет новую только когда та погибла или ушла; опрос округи идёт ТОЛЬКО в `State.IDLE` |
| Пакетный скан отряда | Отрядный кэш цели (`squad_target_get/put`), отрядный ответ `_clear_enemy` из `_sweep_corridors`, трёхточечный РЛС марша, центровой нод стрелков |
| Отрядный push-радиус | `Unit.sep_radius()` — личная колонка в ядре; расчёт пакетный, фазированный (8 фаз, 15 Гц) и многопоточный |

- **ЗАМЕР (`qa_big_goblin`, блок D): за 12 СЕКУНД БОЯ туша просит скан ЧЕТЫРЕ
  РАЗА** — два скана выполнены, два пропущены заморозкой. Во всей сцене за то
  же время сделано 14 сканов сетки.
- **ОТСЮДА ВЫИГРЫШ:** два сэкономленных скана по 4.5 мкс (цена скана измерена
  в `qa_infantry_radar`) — это **9 мкс за двенадцать секунд на бойца**, то есть
  ноль. Поллинга, который заказ предлагает убрать, в этом коде НЕ БЫЛО:
  боец с живой целью не сканирует вовсе, а стоящий опрашивает округу раз в
  0.5-2.0 с и только в покое.
- **ЧТО ЗАМОРОЗКА ДАЁТ НА САМОМ ДЕЛЕ — ПРЕДСКАЗУЕМОСТЬ, А НЕ КАДР.** Туша с
  четырёхсекундной перезарядкой не станет менять цель посреди КД: скан
  разрешён один раз, за `BIG_SCAN_LEAD` 0.6 с до готовности — ровно чтобы
  успеть развернуться и начать замах. Это поведение, а не оптимизация, и
  ворота стоят В BigGoblin, а не в базе: у пехоты свой ритм, и общий гейт по
  кулдауну сломал бы ей ответ на удар.

### Ловушки этого захода

- **ОТЛЁТ ИНТЕГРИРУЕТ ТИК ЖЕРТВЫ.** Первая версия проверки отброса
  замораживала цели (`set_tick(false)`) — и сдвиг выходил 0.00 у всех троих на
  исправном коде. Это уже ловилось в спринте 15 и поймалось снова.
- **ДВЕ КОМАНДЫ НА ОДНОЙ СТРОКЕ = ПАРС-ОШИБКА, А СТЕНД ПРИ ЭТОМ ВИСИТ МОЛЧА
  ДО КОНЦА ВРЕМЁН.** Скрипт не грузится, сцена остаётся без скрипта, СТОРОЖ
  ЖИВЁТ В ЭТОМ ЖЕ СКРИПТЕ и потому не срабатывает вовсе: прогон занял два
  таймаута по десять минут, прежде чем я догадался посмотреть первые строки
  вывода вместо последних. **`--headless --import` эту ошибку НЕ ПОКАЗАЛ** —
  парс-ошибки стендов он не ловит. Признак «висит без единой строки вывода» =
  смотреть ГОЛОВУ лога, а не хвост.
- **ЧЕТЫРЕ GODOT В ПАМЯТИ ОДНОВРЕМЕННО** (остатки убитых прогонов) — правило
  12 в чистом виде: замеры на такой машине врут, а прогоны тянутся втрое.
  Проверять `Get-CimInstance Win32_Process` перед замером.

**Регрессия:** `qa_big_goblin` 22/22, `qa_goblin` 73/73, `qa_troll` 24/24,
`qa_sprint13` 80/80, `qa_bugpass` 45/45, `qa_balance` 24/24, `qa_volley` 22/23
(известный мигающий C1).


## Режим построения снова запоминается — и это ТРЕТИЙ разворот (13.09.2026)

Жалоба: «после каждого растяга режим сбрасывается на широкий фронт, [2]
приходится жать заново». Сбрасывала его РОВНО ОДНА СТРОКА в обработчике
отпускания ПКМ (`SelectionManager._unhandled_input`), поставленная спринтом
20. Снята. Проверяет `qa_formation_keys`, блок E (7 проверок).

- **ИСТОРИЯ ТРЕБОВАНИЯ, И ЕЁ СТОИТ ПОМНИТЬ ЦЕЛИКОМ.** Режим запоминался →
  владелец прочёл это как «стена копий раздаётся всем войскам» (спринт 18,
  письмо 9: хотфикс «коробочка только копейщикам») → спринт 20 сделал режим
  живущим ОДИН РАСТЯГ → теперь жалоба ровно наоборот. Три захода на одну
  ручку.
- **ПРИМИРЯЕТ ИХ ВИДИМОСТЬ, А НЕ СРОК ЖИЗНИ, И ЭТО ГЛАВНЫЙ ВЫВОД.** Прежде
  запомненный режим был НЕВИДИМ: игрок не знал, каким строем пойдёт следующий
  приказ, и коробочка на чужом роде войск читалась как самоволка игры. Сброс
  лечил симптом. Теперь текущий режим написан в верхней панели
  (`HUD._formation_label`: «строй [1] широкий» / «строй [2] коробочка»,
  коробочка подсвечена ярче), и запоминание перестаёт быть сюрпризом.
- **ИНДИКАТОР ЧИТАЕТ КОНТРОЛЛЕР, А НЕ ДЕРЖИТ СВОЮ КОПИЮ.**
  `HUD` спрашивает `SelectionManager.formation_mode` в своём такте: второй
  источник правды о текущем режиме разошёлся бы с первым на первом же
  нажатии клавиши. Там же показывается и авангард [3], когда он не по
  умолчанию.
- **ХРАНИЛИЩЕ БЫЛО ПРАВИЛЬНЫМ С САМОГО НАЧАЛА.** Пункт заказа «перенеси
  хранение в контроллер ввода/выделения» выполнять не пришлось:
  `formation_mode` и так поле `SelectionManager`, а не временная переменная
  UI. Проблема была не в месте хранения, а в одной строке сброса.
- **ЧТО ИМЕННО ПРОВЕРЯЕТСЯ** (`qa_formation_keys` E1-E7): [2] включается при
  растяге; отпускание ПКМ режим НЕ сбрасывает; смена выделения, СНЯТИЕ
  выделения и обычный приказ на движение — тоже; второй растяг без единого
  нажатия идёт коробочкой; [1] возвращает широкий фронт, то есть менять
  режим по-прежнему может ТОЛЬКО игрок.
- **`qa_sprint20` E4 ПЕРЕПИСАН** — он стерёг снятое требование («после
  отпускания ПКМ коробочка сброшена»). Это правило 10 в чистом виде: стенд
  краснел не от поломки кода, а от разворота требования.
- **ЛОВУШКА ЗАМЕРА: НА КОПЕЙЩИКАХ ДВА РЕЖИМА НЕРАЗЛИЧИМЫ.** Проверка «второй
  растяг идёт коробочкой» сперва мерилась на копейщиках и краснела на
  исправном коде: к этому блоку стенда «Стена копий» уже изучена, а с ней
  копейщики ложатся в три шеренги при ЛЮБОМ режиме
  (`_block_formation_slots`, спринт 20). Мерить разницу режимов можно только
  на роде войск БЕЗ стены — блок переведён на лучников (глубина 1.2 против
  0.0 м).
- **ПАРС-ОШИБКА В СТЕНДЕ СНОВА СТОИЛА ДЕСЯТИ МИНУТ.** Дважды за заход:
  `sm` — ЛОКАЛЬНАЯ переменная стенда, а не поле (новый блок её не видел), и
  построчная замена оставила дубль хвостовой строки вердикта. Признак у обеих
  один: стенд висит БЕЗ ЕДИНОЙ СТРОКИ вывода, потому что сцена остаётся без
  скрипта, а сторож-таймер живёт в этом же скрипте. Смотреть ГОЛОВУ лога.

**Регрессия:** `qa_formation_keys` 35/35, `qa_sprint20` 35/35,
`qa_sprint18b` 50/50, `qa_squad` 52/52, `qa_sel2` 35/35, `qa_bugpass` 45/45,
`qa_reform` 16/16.


## Кузница монаха: 3×5 + бонусный столбец, и отход под уроном (13.09.2026)

Заказ: полностью переписать ветку монаха (три столбца по пять рядов плюс
бонусный столбец, открывающийся САМ при изучении всего ряда), переназначить
иконки, задать экономику и научить монаха отбегать под уроном. Дополнение
письмом: связать узлы с ЖИВОЙ логикой, а не только с интерфейсом.
Стенд — **`qa_monk_forge`** (36 проверок, headless).

### Форма и экономика

- **ТОПОЛОГИЯ СОВПАЛА С СУЩЕСТВУЮЩЕЙ, И ЭТО СЭКОНОМИЛО ВСЮ РАБОТУ ПО UI.**
  Сетка кузницы в проекте и так 5 рядов × 4 столбца, причём четвёртый уже
  описан как «открывается рядом A+B+C». Нового интерфейса не понадобилось
  вовсе: переписаны только ДАННЫЕ ветки (20 ячеек) плюс выдача бонуса.
- **ЦЕНА И ВРЕМЯ — ПО РЯДУ:** 100/15 с, 250/30, 500 + 50 дерева/45,
  1000 + 150 камня/60, 2000 + 300 камня/90. Бонусный столбец — ноль и ноль.
- **БОНУС ВЫДАЁТСЯ ПО ФАКТУ, А НЕ ПО СОБЫТИЮ** (`GameManager._grant_row_bonuses`):
  проверяется, изучены ли все три узла ряда. Событийная выдача («закончился
  третий») теряла бы бонус у того, кто изучал ряд не по порядку или загрузил
  партию с двумя узлами из трёх — замер B5 гоняет ряд в порядке c-a-b.
  **Платный узел даром не выдаётся вовсе:** нулевая цена здесь не
  предположение, а условие — впиши владелец туда число, и выдача стала бы
  подарком в обход склада.
- **ИКОНКИ: ЧЕГО НЕТ В СЕТЕ, ТОГО НЕТ.** В наборе 25 файлов, и второго щита,
  шлема и «монаха в капюшоне» среди них не существует. Взято ближайшее:
  «большой щит» — тот же `icon_shield`, «шлем/доспех» — `icon_broken_sword`
  (удар, который не убил), «монах в капюшоне» — `icon_trader` (им уже
  подписывался узел монаха), «поток огня» — `icon_rain_of_arrows`. Проверка
  A4 стережёт не КРАСОТУ, а существование файла: иконка, которой нет на
  диске, даёт пустую кнопку в кузнице.

### Связка с логикой (дополнение к заказу)

- **ВСЕ ЭФФЕКТЫ ЧИТАЮТСЯ ИЗ РЕЕСТРА ИЗУЧЕННОГО, ОДНИМ СПОСОБОМ**
  (`Monk._node(cell)`): радиус ауры, такт, объём, число целей, самохил,
  воскрешение, спасения. Двадцать отдельных функций-читателей не заводилось.
- **ЧИСЛА, КОТОРЫЕ НЕ ПРИБАВКИ, СТОЯТ ОТДЕЛЬНЫМИ ВЕТКАМИ.** «Непрерывный
  Поток» (4b) задаёт такт АБСОЛЮТНО, «Абсолютное Целительство» (5b)
  восполняет запас ЦЕЛИКОМ, «Глобальный Покров» (5a) отдаёт радиус числом в
  миллион метров — чтобы все, кто радиус читает, продолжали читать метры.
  **4b взят ПОТОЛКОМ, а не равенством:** к четвёртому ряду такт от прежних
  узлов уже быстрее трёх десятых, и «поставить ровно 0.3» означало бы
  ухудшить его этим исследованием (проверка C9 переписана на «не медленнее»).
- **ВОСКРЕШЕНИЕ ПЕРЕСТАВЛЕНО НА БОНУС РЯДА 2** (было 3d): 2d — один павший с
  30 % запаса раз в 15 с, 4d — двое с 50 % раз в 12 с, 5d — конвейер по
  четверо раз в 3 с. Приоритет подъёма — павшие МОНАХИ
  (`CorpseRenderer.find_raisable_priority`).
- **ПРИОРИТЕТ ПО РОДУ, А НЕ ПО ОТРЯДУ, И ЭТО УТОЧНЕНИЕ ВЛАДЕЛЬЦА.** «Свой
  отряд» у монахов — это Ctrl-ГРУППА игрока: сам монах есть отряд из одного
  (`SQUAD_SIZE_MONKS` 1), а Ctrl-группы живут списком ЖИВЫХ узлов, и у тела
  спросить его группу уже нечем. По роду войск ответ выходит тот же самый.
- **ПРАВИЛО «ВЫБИТЫЙ ОТРЯД НЕ ПОДНИМАЕТСЯ» НЕ КАСАЕТСЯ ОДИНОЧЕК.** У павшего
  монаха живых в его отряде НОЛЬ ВСЕГДА, и общее правило запретило бы подъём
  монахов насовсем — то есть убило бы тот самый приоритет, ради которого всё
  и делалось.
- **ВСЕ СПАСЕНИЯ ЛОВЯТ УДАР ДО БАЗОВОГО `take_damage`.** После него монах уже
  снят со строки ядра, из сетки и из отряда, тело уложено в слой павших — и
  «поднять его обратно» означало бы собрать бойца заново. Порядок цепочки:
  щит (4c) → «Второе Дыхание» (5c, 1 HP и 5 с неуязвимости) →
  самовоскрешение (бонус ряда 3, половина запаса, ОДИН раз за бой).

### Отход под уроном: три подряд гасителя, и все нашлись замером

Требование «отбегает на 5 м» не выполнялось трижды подряд, и каждый раз по
своей причине — все три видны только в динамике:

1. **Подход к пациенту перебивал отход** (0.5 м вместо пяти): пациент стоит
   рядом с обидчиком, и монах тем же тактом возвращался лечить.
2. **Гейт стоял на ОТКАТЕ, а не на факте прихода** (3.3 м): откат 1 с, а
   дорога на пять метров у медленного монаха занимает вчетверо больше.
   Отход теперь кончается ПРИХОДОМ (`_retreat_goal`), как выход из боя у
   пехоты; потолок выводится из шага (`_retreat_timeout`), а не стоит числом.
3. **Расстановка монахов перебивала отход**: её такт ловит `State.IDLE`, и
   монах, вставший на секунду, получал новую точку в полуметре.

Итог замера: пик отхода **4.5 м**, дистанция до обидчика 4.0 → 8.5 м, лечение
на ходу не прерывается, откат гасит спам (один отход на одиннадцать ударов).
**МЕРИТЬ НАДО ПИК, А НЕ КОНЕЧНУЮ ТОЧКУ:** монах отбегает и ВОЗВРАЩАЕТСЯ к
пациенту — это задуманное поведение, и замер в конце окна показывает уже
вернувшегося (0.5-0.9 м) и краснеет на исправном коде.

### Ловушки заходa

- **ИМЯ МЕТОДА СОВПАЛО С ПОЛЕМ БАЗЫ.** `Unit._retreat_from` — это Vector3
  (точка начала отхода, по ней считается билет прохода). Метод с тем же
  именем в наследнике GDScript ловит как «Member is not a function», и только
  при компиляции. Переименовано в `_step_away_from`.
- **СРЕЗКА ФАЙЛА `s[:i] + s[j:]` ПРИ j < i ДУБЛИРУЕТ КУСОК, А НЕ УДАЛЯЕТ.**
  Так в Monk.gd появилась вторая копия трёхсот строк вместе со вторым
  `_ready`. Лечится только откатом файла и повторным наложением правок одним
  проходом — что и сделано.
- **`qa_forge` A5 ПЕРЕПИСАН:** он требовал ненулевую цену у КАЖДОГО узла, а
  бонусный столбец теперь бесплатен по заказу. Название у такого узла
  по-прежнему обязательно, а цена обязана быть нулевой.

**Что НЕ реализовано и почему** (заказ перечисляет это в описаниях узлов,
но отдельной механики под них в проекте нет): «иммунитет к замедлению» (2c,
3c) — системы замедлений в игре не существует вовсе, замедлять монаха нечем;
«KnockbackResist +10 %» (2c) — отброс идёт общим каналом скорости
(`apply_knockback`), сопротивления в нём нет, и заводить его ради одного
узла значило бы трогать конницу, дубину тролля и свип большого гоблина.
Оба узла работают остальными своими числами (скорость и броня), их подписи
это называют.

**Регрессия:** `qa_monk_forge` 36/36, `qa_monk` 13/13, `qa_monk_healing`
19/19, `qa_forge` 48/48, `qa_bugpass` 45/45.

## ТЗ 14.09.2026: десять блоков одним заходом (14.09.2026)

Полный разбор — CLAUDE.md, раздел «ТЗ 14.09.2026: десять блоков». Здесь —
что стоило разобраться и что осталось красным.

**Замеры и ловушки этого захода.**
- Чистка конфигов скриптом (полотна комментариев → `docs/CONFIG_NOTES_2026-09-14.md`):
  диф КОДА до/после пуст, кроме литералов большого гоблина; правило подписи —
  первая строка блока, если она законченная фраза (иначе обрывок «Мечник стоял
  между копейщиком и кабаном: чуть» остаётся висеть над полем).
- Кросс-файловые `const X := preload("...").Y` в 4.7.1 работают — проверено
  зондом до того, как на них перевязаны 48 констант.
- `qa_wall` A3 «бой состоялся» краснел и на HEAD, и на правке: с отрядным
  радаром (13.09) стрелы ложатся «наименее обстрелянным», 48 рыцарей по 340 HP
  за 24 с не гибнут (−900 HP к 11-й секунде на обеих сторонах A/B). Критерий
  переведён на снятый запас.
- `qa_panic_edge` C2 краснел ЧЕРЕЗ РАЗ на неизменном коде: одиночный гоблин-
  угроза по своему таймеру агро шёл на отряд и уводил ось конуса (82° при
  угрозе, ушедшей на 8 м). Два прогона одного стеша: 37° и 84°. Угроза
  заморожена.
- `qa_goblin_loop` B4 «резерв держит рудник холма»: `_hill_mine()` брал
  ближайший к центру ЧУЖОЙ рудник, и после захвата резерв тут же уходил к
  следующему чужому за 76 м. На старой карте следующий стоял так же далеко —
  проходило по таймингу захвата. Теперь захваченный ордой рудник холма
  остаётся холмом (кроме рудника деревни).
- `qa_gnoll_fix` B3-B5: гнолл без пня раньше усыновлял ПЕРВОЕ логово партии
  и «отходил» к нему за тысячу метров — блок кайта срабатывал через возврат в
  зону. С домашней точкой отходы стали честными, и в кольце тел каждый из них
  «удавался» на метр (внутри кольца есть где качнуться) — заведена серия
  кайтов (`GNOLL_KITE_SERIES` 2 без ухода от якоря → блок).
- `qa_sprint18b` B10: поводок тролля от ДОМА ломал стенд, ставящий тролля
  за сотню метров от логова; поводок переведён на точку начала погони
  (`TROLL_CHASE_RADIUS` 45).
- `qa_ai_scenarios` C4: у пня между базами зона к броду — узкий клин из двух
  полуплоскостей; от точки выброса до зоны 50 м, стенд ждал 14 с.

**Известные красные после захода (не регрессии, на HEAD такие же):**
`qa_sprint19c` F7/F9, `qa_cavalry` E4, `qa_panic_edge` A2, `qa_mega_battle`
B2-1 (под нагрузкой), `qa_wall` A1, `qa_balance` C7 (нагрузка),
`qa_performance_stress` B10 / `qa_settle` E1 (перф под нагрузкой шлюза).

## Профиль замеса: бенчмарк `qa_melee_bench` и три утечки (14.09.2026, задача B)

Заказ: быстрый репрезентативный бенч (по два отряда каждого рода в 10-15 м),
профиль CPU/физика/логика/аллокации в фазах Idle / Move / Combat, аудит по
родам войск, критерий — стабильные 60 FPS без микрофризов. Измеритель —
`qa_melee_bench/Test.tscn` (описание в CLAUDE.md, список стендов).

**Как мерили.** Окно 1920×1080, V-Sync снят самим бенчем (иначе на 75-Гц
мониторе «прочее» в кадре — 19 мс ожидания монитора и FPS ср 74 при любой
сцене), `cap=60` для критерия и `cap=0` для запаса. `mult=2` — 578 бойцов
по ТЗ, `mult=5` — 1445 (масштаб партии владельца). Числа одного прогона
разнятся ±0.3 мс тика и ±10 FPS между прогонами.

| Сцена | Фаза | физтик армии | кадр логики | FPS мин / ср (cap 60) | FPS мин / ср (без потолка) |
|---|---|---|---|---|---|
| 578 | IDLE | 4.2 мс | 1.2-1.6 мс | 58 после разгона | 220 / 282 |
| 578 | MOVE | 6.7 мс | 1.8-2.2 мс | 58 / 60 | 116 / 164 |
| 578 | COMBAT | 4.8-5.5 мс | 3.1-3.6 мс | **59 / 60** | 68-82 / 126-215 |
| 1445 | IDLE | 3.6 мс | 1.4-1.6 мс | 58 после разгона | 88 / 230 |
| 1445 | MOVE | 5.6-5.8 мс | 2.0-2.1 мс | 53 / 60 | 139 / 161 |
| 1445 | COMBAT | 6.8-7.0 мс | 4.4-5.0 мс | **46-58 / 60** | 61 / 84 |

Аллокации C#: 27-68 КБ/кадр (упаковка PackedArray на границе языков —
TakeWoken / TakeArrowEvents / QueryRadius / позы), сборки gen0 2-6 за фазу,
gen2 ≤ 1; рост объектов GDScript в бою ОТРИЦАТЕЛЬНЫЙ (гибнут узлы). Ни
утечки объектов, ни лавины GC нет.

**Раскладка по родам (мкс на тик бойца, замес 578):** гоблины 5.8,
копейщики 6.5-7.3, конница 7-9, мечники 11, лучники 12, туши 11-12, гноллы
15, монахи 35-45, тролли 35. Худшие одиночные тики: конница 2.2-3.6 мс
(удар с разгона: гибель ряда + укладка сбитых телами + разлёт — событие),
туши 0.6-1.0 мс (свип), монахи 6-9 мс → после правок 0.7 мс.

**Что нашлось (по атрибуции пиковых кадров и колонке «худший вызов»):**

1. **Первый белый флаг — 25 мс кадра.** `tail_morale` в пике оказался
   `refresh_squad_banner` → `BannerArt._render`: знамя и белый флаг рисуются
   попиксельно в статический кэш при первом обращении, то есть в первый срыв
   отряда в бою. Сначала было приписано `build_route` у сотни беглецов
   (заведён и ОТКАЧЕН обход маршрута у паники — бакеты `panic_*` показали
   доли миллисекунды). Лечение — `Main._prewarm_art()` в `start_game`.
2. **Такт монаха до 5.8 мс.** `_keep_or_pick` брал `query_radius` (Godot-
   массив всех бойцов в 10 м — три сотни в свалке) и фильтровал в GDScript;
   `_try_resurrect` обходил ВЕСЬ слой тел (орда, тролли). Теперь
   `ArmyCore.MostWoundedOfSide` (по колонкам hp/hpMax, без массива) и
   реестр `CorpseRenderer._raisable` по фракции (ленивая чистка при
   обходе). Фазы такта монахов разведены по номеру узла.
   **Следствие:** монах судит о ране по колонке ядра — все пути лечения и так
   зовут `_soa_push_stats()`, добавлен выход из гарнизона
   (`Castle.release_unit`); стенд, пишущий `current_health` в поле, обязан
   пушить сам (`qa_monk_forge` E4, `qa_sprint13` H, `qa_monk_vfx`).
3. **Канал воскрешения звал `raise_spot` каждый кадр** (состав × слоты, до
   3600 пар с `global_position`) — 7.9 мс худший тик монаха. Кэш
   `Monk._res_spot` на 0.5 с.
4. **Первое лечение — 3.8 мс, и не там, где казалось.** Не `load()` ленты
   (он и был прогрет), а `BillboardUtil._detect_frame_count`: `get_image()`
   плюс скан ширины листа по пикселям в GDScript ради числа кадров; кэш по
   пути — прогрет тем же вызовом. **И ещё 1.7 мс — сам `load()`, потому что
   кэш ResourceLoader СЛАБЫЙ:** прогретая текстура без ссылки выгружалась в
   конце `_prewarm_art`, и лечение грузило её с диска заново. Ссылка держится
   в `Main._prewarm_hold`. Худший тик монаха 4.5 → 0.7 мс.

**Что осталось и почему (структурный потолок).** В устоявшейся рубке
`process_attack` 1.0-1.3 мс/кадр (3 мкс × 600 тысяч вызовов за прогон) и
визуальный проход `vis_far` 0.8-1.4 + `vis_fog` 0.3-0.45 + `vis_walk`
0.2-0.4 мс/кадр — это GDScript на КАЖДОГО бойца в каждом кадре, и точечно
не снимается (разбор потолка — `docs/PLAN_D_70FPS.md`). Пиковые кадры
(20-27 мс без потолка) — «кадр логики» 11-15 мс, где `vis_far` 4-13 мс
и `vis_pose` 1-3 мс: массовая смена позы/ленты одним кадром (контакт всей
линии, срыв отряда). Событийные ветки с худшим вызовом 3-5 мс:
`melee_calc` (первый контакт всех отрядов разом), `tail_morale` (срыв
отряда — сотня приказов одним кадром), `squad_corridor`, `pose_flush`,
`atk_strike`/`atk_damage` (свип туши, добивание с выбиванием отряда). Это
цена события, не кадра: при `cap=60` они укладываются в один-два кадра.
Один кадр в 130 мс в прогоне без потолка — «прочее 113 мс» вне кода игры
(драйвер/ОС), в прогонах с потолком не повторился. **Худшие одиночные
вызовы `pose_flush` 9 мс, `vis_core` 10 мс, `atk_snooze` 8 мс,
`squad_melee` 10 мс — кадр смены фазы самого бенча:** все бойцы
телепортируются в контакт одним кадром, у всех разом грязная поза и новые
соседи; в партии такого кадра нет.

**Инструмент, который остался:** `perf_config.prof_add` ведёт ещё и ХУДШИЙ
вызов бакета (`_prof_max`, пятый элемент строки `prof_report`), бенч
печатает «событийные ветки» по нему; `class_max` — худший тик рода войск и
состояние. Бакеты `die_*`, `tail_*`, `monk_*` гейтятся `profile_physics` и
без него стоят ноль. Среднее по бакету событие прячет: один вызов в 4 мс
среди двух тысяч по 9 мкс даёт «11 мкс» — искать надо по максимуму.

**Стенды после захода:** `qa_monk` 13/13, `qa_monk_healing` 19/19,
`qa_monk_forge` 36/36, `qa_monk_vfx` 26/26, `qa_panic` 19/19, `qa_corpse`
67/67, `qa_keep` 38/38, `qa_tower` 47/47, `qa_balance` 24/24,
`qa_goblin_loop` 33/33, `qa_gnoll_fix` 37/37, `qa_troll_impact` 29/29,
`qa_full_game_4k` 4/4, `qa_bugpass` 45/45, `qa_sprint13` 80/80,
`qa_sprint20` 36/36. Три стенда краснели на ЧИСЛАХ, которые владелец
поправил в конфиге 14.09.2026 (шаг гнолла 2.5 → 2.2, запас тролля 10160 →
8160, запас туши 8250 → 3250): `qa_sprint13` B1, `qa_gnoll_fix` A7,
`qa_sprint20` C2, `qa_fix_pack` B1, `qa_big_goblin` A1 переведены на
односторонние свойства («не медленнее орды», «не выше 2.5», «не выше
−20 %», «не выше половины отряда»). Известные красные без изменений: `qa_sprint19c` F7/F9,
`qa_cavalry` E4 (E2 мигает: 0 из 3 → зелёный на повторе), `qa_panic_edge`
A2.

## Древо лучника и снайперский выстрел (14.09.2026, задача C)

ТЗ «Archer Tech Tree & Snipe Shot». Разбор — CLAUDE.md, список стендов,
пункт `qa_archer_snipe`. Здесь — решения, которые не следуют из кода.

- **Сетка кузницы 5×4 осталась** (ряд = тир, колонка D = способности с
  воротами ряда): пять линейных пунктов ТЗ разложены так — A дальность ×3 +
  бронепробитие ×2, B темп ×3 + «Гроза великанов» ×2, C урон ×5; D: залп (1d,
  как был), снайпер (2d), частота снайперов (3d), 4d/5d — прежние заглушки
  «Отравленные»/«Стрелы-шипы» (механик под ними не было и нет). Цена/время
  — по тиру, как у остальных веток.
- **Проценты — отдельные ключи, а не перегрузка старых.** `bonus_cooldown`
  вычитает секунды (залп +0.12 с), новый `bonus_cooldown_pct` — долевой
  множитель поверх; `bonus_armor_pen` встроен в уже существующий хук
  `squad_armor_pierce` (перк «Пробитие» читался там же на каждый урон),
  `bonus_giant` — в путь стрелы рядом с `ranged_damage_mult`.
- **Снайпер — это статус по порядковому номеру, а не отдельный род войск.**
  Ни спрайта, ни строки в STATS: `members.find(u) < squad_snipers(sid)`.
  Дальность 25 у снайпера — личное поле `attack_range` (иначе FSM не даст ему
  цели дальше 20); статус пересчитывается раз в полсекунды. Проверка ТЗ
  «снайперы стреляют дальше отряда» держится, пока отряд не докачал 3a: с
  полным рядом A все бьют на 25.
- **Асинхронность — задержка по номеру, а не раздельные таймеры.** Выстрел
  снайпера идёт из того же `_on_attack_fired` (кулдаун и окно залпа — общие),
  но откладывается на k × 0.14 с; заявка на цель ставится СРАЗУ, при
  планировании, иначе три снайпера одного окна залпа выбрали бы одну цель.
  Замер стенда: кадры выстрелов пяти снайперов [0, 8, 16, 25, 33].
- **«Насмерть» = урон сверх запаса, а не обход `take_damage`:** иначе не было
  бы ни фрага, ни ветеранства, ни тела, ни хора орды. Щит и порог
  `MIN_DAMAGE_FRACTION` режут долей, и `max_health × 1000` они не удержат.
- **Голова павшего — геометрия тела, а не спрайта бойца.** Тело лежит квадом
  с базисом `(cos, sin | sin, −cos)` (CorpseRenderer.Bucket.write): верх
  рисунка (+Y квада) смотрит в (sin yaw, 0, −cos yaw). Точка головы —
  0.30 высоты квада вдоль этой оси от центра; стенд меряет 0.08 м между
  стрелой и расчётной точкой. У тролля тело — свой билборд (`_leave_corpse`),
  голова слева в ленте, при зеркале справа — `Troll.snipe_head_spot`.
- **Звук синтезирован** (`snd_snipe_shot.wav`: щелчок-шум 6 мс, свип тетивы
  1400 → 500 Гц, низкий «тук» корпуса): своего файла в паке нет, а взять
  «Bow Attack» с питчем значило бы не отличить снайпера от залпа.
- **Что вскрыл шлюз:** `qa_archer_logic` F — стенд ставил цель на
  0.7 × дальности вбок от маршрута; с базой 15 это 10.5 м, внутри ближнего
  марш-РЛС пехоты (10 м + полуширина), и перехват на марше был законным;
  цель отодвинута за РЛС и приближена по X, чтобы с точки приказа
  доставать. `qa_fix_pack` H1 утверждал число 20. `qa_fog` D3-D6а трижды
  краснели подряд на одном коде, затем 6 прогонов зелёные (в т.ч. на HEAD)
  — мигание, не регрессия.

**Стенды:** `qa_archer_snipe` 39/39 (новый), `qa_volley` 23, `qa_volley_fix`
8, `qa_archer_logic` 21, `qa_forge` 48, `qa_monk_forge` 36, `qa_tower` 47,
`qa_balance` 24, `qa_target_lock` 19, `qa_ai` 27, `qa_wall` 13,
`qa_big_goblin` 22, `qa_sprint13` 80, `qa_hud_afford` 13, `qa_corpse` 67,
`qa_bugpass` 45, `qa_fog` 42, `qa_difficulty` 58, `qa_archer_bench` 4,
`qa_recon` 22; второй пакет — `qa_fix_pack` 44, `qa_full_game_4k` 4,
`qa_sprint18b` 51, `qa_sprint20` 36, `qa_ai2` 30, `qa_ai_scenarios` 36,
`qa_keep_elite` 44, `qa_troll` 24, `qa_troll_impact` 29, `qa_gnoll_fix` 37,
`qa_infantry_radar` 7, `qa_boar_rider_charge` 20, `qa_performance_stress`
11, `qa_start_build` 30, `qa_lair_ai` 22, `qa_goblin` 73,
`qa_river_crossing` 14, `qa_disengage` 15, `qa_spear` 49, `qa_vet` 55,
`qa_hud5` 35, `qa_sel2` 35, `qa_vetui` 45, `qa_monk` 13, `qa_monk_healing`
19, `qa_gold_mine` 24, `qa_aggro_fix` 16. Известные красные без изменений:
`qa_sprint19c` F7/F9, `qa_cavalry` E4, `qa_mega_battle` B1-1/B2-0/B2-1
(нагрузка; B4-3 мигает: 0.91 / 0.80 / 0.73 при пороге 0.8 на одном коде,
на HEAD зелёный в одном прогоне). Попутно закрыты два правила 5:
`HUD._refresh_afford` передавал снесённое здание в типизированный аргумент
(SCRIPT ERROR в `qa_hud5`), разбор застрявших в `qa_mega_battle` B4
приводил освобождённый узел к `Unit`.

## Гарнизон на крышах: бараки, крепость, башня (14.09.2026, задача D)

ТЗ «Garrison System Overhaul: Barracks & Fortress Roof Support, Archer Buffs
& Visual Spawning». Разбор — CLAUDE.md, список стендов, пункт
`qa_roof_garrison` (26/26). Здесь — решения, которые не следуют из кода.

- **Модуль, а не третья копия башни.** Логика площадки в `Tower.gd` была
  написана под одно здание с одним рядом спрайтов; бараки и крепость
  потребовали бы двух копий с другой геометрией. `RoofGarrison` — RefCounted
  внутри Castle-наследника: хозяин отдаёт раскладку (`_roof_setup`), модуль
  ведёт спрайты, огонь, залп, снайперов, поглощение стрел и выгрузку. Tower
  переписан на делегирование с прежним API — стенды башни (`qa_tower` 47,
  `qa_tower_states` 23) прошли без переписывания кроме B8а (бафф).
- **Бараки стали Castle.** Единственный способ получить гарнизон
  (`request_garrison` / `absorb_unit` / `release_unit` / `_evacuate_on_death`)
  без второй реализации. Цена — обход всех `is Castle` в проекте: везде уже
  стоял `is_stronghold()` (введён ради башни 09.09.2026), пришлось поправить
  только порядок в HUD (`_portrait_icon_path`: бараки раньше замка) и ветку
  панели (башенная панель исключает Barracks). Золото — `_gives_gold()`.
- **Сортировка рядов: Z вместо Y.** Первая раскладка поднимала ряды по
  крыше (dy), и задний ряд рисовался поверх переднего — квад с мировой
  ориентацией берёт глубину по подножию, а под камерой 45° подъём
  приближает к камере на 0.71·dy. Ряды уходят назад по Z (−0.30) и почти не
  поднимаются (+0.05): на экране отход по Z читается как тот же подъём.
- **Бафф — в модуле, а не в поле бойца.** `attack_range` укрытого не
  трогается (его читают горячие ветки и снайперский `_apply_sniper_range`),
  дальность умножается в `tick` модуля, урон — множителем к `damage` при
  вызове `_on_attack_fired`. На выходе множители не «сбрасываются» — их
  просто негде применить. Снайпер читает `garrison_mult()` для радиуса
  поиска: у него цель своя, а не цель площадки.
- **Крепость: два отряда на крыше ПОВЕРХ лазарета.** `garrison_limit` у
  крепости прежний (5), но `roof_accepts` + `roof_squads_max` режут стрелков
  до двух; пехота идёт в лазарет как раньше. Авто-выпуск вылеченных обходит
  роды крыши (пост, не лазарет). ПКМ пустым выделением по крепости —
  `release_roof`, лазарет остаётся.
- **Лимит крепостей — по живым плюс площадкам.** Иначе три заложенные
  одновременно площадки давали бы шесть крепостей. Цена — 300 · 4^n золота
  (1200 за вторую, 4800 за третью); `build_cost_for` — единый источник для
  панели, карточки и списания.
- **Ловушки стенда.** «Застрял в здании» мерить от линии ворот по оси ворот
  (точка ворот крепости в 3 м от центра при коробке 8 — стоящий в проёме
  попадал в «стены»); залп с крыши узнаётся по `GameManager.volley_primes`
  и пиковому кадру (30 стрел за кадр); урон стрелы сверять с
  `_strike_damage() + _upgrade_damage_bonus()` × 1.3 по летящим стрелам
  укрытых (`shooter.garrisoned`), не по снайперским.
- **Шлюз (30 сцен, две пачки):** первая — `qa_tower` 47, `qa_keep` 38,
  `qa_start_build` 30, `qa_bugpass` 45, `qa_hud5` 35, `qa_worker_queue` 17,
  `qa_helpbuild` 24, `qa_food_economy` 16, `qa_archer_snipe` 39, `qa_volley`
  23, `qa_volley_fix` 8, `qa_difficulty` 58, `qa_ui9` 3/7 (известный),
  `qa_sel2` D2б-D4 под нагрузкой (одиночно 35/35 — клик по руине после
  `jump_to` камеры, правило «камеру замораживать»); вторая — `qa_ai2` 30,
  `qa_keep_elite` 44, `qa_fix_pack` 44, `qa_squad` 52, `qa_spear` 49,
  `qa_tower_states` 23, `qa_goblin` 73, `qa_ai_loop` 23, `qa_rally_zone` 13,
  `qa_spawnlane` 9, `qa_sprint20` 36, `qa_forge` 48, `qa_lair_ai` 22,
  `qa_full_game_4k` 4, `qa_archer_logic` 21. **Единственная настоящая
  регрессия — `qa_ai` 5:** `RedAI.towers()` считал башней любой не-столичный
  Castle, и с бараками-Castle пары периметра (копейщики впереди / лучники
  позади) вставали вокруг бараков ИИ — глубина копейщиков 20 м при лучниках
  7. Лечение — `is Tower` (preload, у `Tower.gd` нет class_name); после
  правки `qa_ai` 27, `qa_ai_scenarios` 36, `qa_ai2` 30, `qa_hud5` 35.

## Fix Pack 15.09.2026: крыши, бараки, упреждение, монах, лучники по ПКМ

ТЗ по семи скриншотам владельца. Разбор — CLAUDE.md, список стендов, пункт
«FIX PACK 15.09.2026». Здесь — что не следует из кода.

- **Кольцо башни рисовало задних поверх передних, и это была геометрия, а не
  слои.** `_ring_spot` поднимал дальнюю дугу по Y на 0.42·r и уводил назад
  по Z на 0.16 м. Под камерой 45° подъём на dy приближает к камере на
  0.71·dy, отход на dz удаляет на 0.71·dz: 0.42·1.1 > 0.16 — задние
  оказывались ближе. Эллипс теперь делается глубиной (0.62·r по Z, подъём
  0.04): на экране тот же овал. Проверка — глубина по камере партии в
  headless (`qa_roof_visual_fix` D), картинка — `Shot.tscn`.
- **Ноги на крышах подобраны глазом по оконным снимкам** (три итерации), а
  стенд стережёт «не выше прежних долей» и «внутри ската [0.30; 0.62]».
- **Упреждения не было — 8 %.** `ARCHER_LEAD_FACTOR` 0.08 при потолке 2.5 м
  давал вынос 0.16 м по бегущему всаднику при нужных 2-5 м. Полный вынос,
  потолок 14, одна функция `_lead_point` на обычную стрелу, залп и снайпера.
  Замер `qa_archer_lead` A2: средний снос точек падения вдоль хода
  −0.04…+0.30 м (позади было бы −2.1); снайперы по бегущей туше 21 из 21.
  `SCATTER_PER_SPEED` 0.34 → 0.20: при полном упреждении разброс — единственная
  причина промаха, и 3.3 м радиуса по всаднику 4.6 м/с означали 23 %
  попаданий против 62 % по стоящему (стало 31 % против 74 %).
- **Снайпер без отрядной цели молчал.** Его выстрел шёл только из
  `_on_attack_fired`, а отряд без цели с 20 м не стреляет — беглец в 20-25 м
  проходил мимо. `_snipe_autonomous`: стоящий снайпер раз в перезарядку
  снимает только ПРИОРИТЕТНУЮ цель (одиночка/паника/отход), отрядных не
  трогает.
- **«Лучники тупят по ПКМ» — две причины, обе в отметке «встали отрядом»**
  (спринт 14): её ставил снайпер на 25 м (луки на 20 бросали цель за 26 м),
  и её не снимал приказ атаки на другую цель (только приказ на движение).
  Теперь снайпер не объявляет, задние ряды подтягиваются на
  `SQUAD_SHOT_SLACK` 6 м сверх своей дальности, `command_attack` под `lock`
  снимает отметку. `qa_archer_lead` C1/C3.
- **Монах «игнорировал ПКМ»** потому что приказ по здоровому отряду
  считался исполненным в тот же такт — монах не двигался. Теперь приказ =
  сопровождение (`_follow_order_squad`), снимает только игрок. Фокус —
  начатый пациент до 100 %; многоцелевой такт — полный квант каждому.
  Ловушка стенда: `push_input` без `in_local_coords` — точка уезжала в
  (17928, 12209), и первая версия стенда «воспроизвела» жалобу на исправном
  коде.
- **Панель бараков** — `is_stronghold()` в ветке HUD крепости (та же форма
  ошибки, что у `RedAI.towers()` накануне: «Castle» больше не значит
  «крепость»).
- **Шлюз (35 сцен):** `qa_roof_visual_fix` 16, `qa_archer_lead` 12,
  `qa_monk_click_fix` 13, `qa_roof_garrison` 27, `qa_monk` 13, `qa_monk_forge`
  36, `qa_monk_healing` 19, `qa_monk_vfx` 26, `qa_sprint13` 80, `qa_fix_pack`
  44, `qa_performance_stress` 11, `qa_archer_snipe` 39, `qa_archer_logic` 21
  (B3/C1 переписаны под подтягивание), `qa_volley` 23, `qa_volley_fix` 8,
  `qa_target_lock` 19, `qa_disengage` 15, `qa_wall` 13, `qa_tower` 47,
  `qa_hud5` 35, `qa_sel2` 35, `qa_keep_elite` 44, `qa_hud_afford` 13,
  `qa_bugpass` 45 (B1-B3 переписаны: вынос полный), `qa_spear` 49, `qa_squad`
  52, `qa_orders` 9, `qa_combat_lock` 5 (одиночно; под нагрузкой 3 ждал по
  стенным часам), `qa_bldattack` 13, `qa_big_goblin` 22, `qa_knights_push`
  29, `qa_goblin` 73, `qa_ai` 27, `qa_full_game_4k` 4. Известные красные без
  изменений: `qa_ui9` 3/7, `qa_sprint19c` F7/F9 (F7 утверждал «Благодать
  вдвое медленнее одиночного» — развёрнуто ТЗ на полный квант), `qa_cavalry`
  E4, `qa_mega_battle` B1-1/B2-0/B2-1/B4-3.


## 16.09.2026 — BigStand, этапы 1-3 реализованы (приказы стоящим, расталкивание, сон по физике)

План — `docs/BIGSTAND_2026-09-16.md`; владелец выбрал «все четыре этапа»,
сделаны 1-3, этап 4 (порт `process_attack` в C#) — отдельный спринт.
Разбор по пунктам — CLAUDE.md, раздел «BigStand, этапы 1-3». Здесь — цифры
и то, что вскрыл шлюз.

### Замер `qa_bigstand` (3886 бойцов, headless, 4 прогона: база / этап 1 / 1+2 / 1+2+3)

| фаза | физтик, мс | кадр логики, мс | приказов/с | сканов/с |
|---|---|---|---|---|
| IDLE_FAR | 8.91 / 8.94 / 8.96 / **4.42** | 3.36 / 3.36 / 3.38 / **0.91** | 57 / 37 / 22 / 22 | 33 |
| MARCH | 14.74 / 15.01 / 15.32 / 14.42 | 3.13 / 3.08 / 3.15 / 2.99 | 36 / 34 / 38 / 37 | 41 |
| CLASH | 17.97 / 17.13 / 16.62 / 17.66 | 5.11 / 5.45 / 5.11 / 4.62 | 440 / 99 / 101 / 118 | 3584 / 1881 / 1414 / 4732 |
| AFTER | 13.16 / 13.19 / 12.57 / 11.53 | 6.02 / 5.96 / 5.47 / 4.09 | 117 / 97 / 80 / 37 | 786 / 610 / 487 / 644 |
| GNOLL_HEAP | 12.71 / 13.15 / 12.95 / 12.04 | 6.62 / 6.48 / 6.25 / 4.69 | 303 / 137 / 176 / 132 | 979 / 923 / 1074 / 872 |

- Этапы 1-2 — про дёргание, а не миллисекунды: приказов стоящим в бою 440 →
  ~100/с, в куче гноллов 303 → ~130, «стоящих с анимацией ходьбы» 58 → 9-15,
  пик `tail_reform` (12-39 мс) из кадров исчез, свободных дрейфующих отрядов
  0 во всех фазах. Тик не сдвинулся — ожидаемо.
- Этап 3 — миллисекунды покоя: стоящая армия 8.9 → 4.4 мс физтика и 3.4 → 0.9
  логики (спящих по физике ~3500 из 3886; IDLE-тиков 918 → 151 на кадр);
  после боя 13.2 → 11.5. В замесе структурный потолок прежний
  (`process_attack` 6.4 мс/кадр = 823 тыс. вызовов по 9.3 мкс за 20 с;
  дремлют на упоре лишь 235 из 2755 ATTACKING — остальные подходят и давят).
- CLASH стал длиннее и злее (сканов 3584 → 4732, погибших 332 → 215): дерущихся
  больше не выдёргивают подзыв и смыкание.

### Что вскрыл шлюз 40 стендов и A/B на HEAD

A/B делался честно: `git stash` + `dotnet build` (DLL ядра собирается из
stash-а!), те же стенды, `stash pop` + сборка обратно. Регрессий две, обе
закрыты:

1. **`qa_disengage` D2b** (HEAD 15/15, правки 14/15): `MOVE_ANIM_MIN_SPEED`
   0.6 → 1.0 читал не только ленту ходьбы, а и `moved_recently()`, по
   которому `wall` и `_may_answer_blow` отличают упор от «реально
   выбирается». Боец, протискивающийся из кольца, идёт медленнее метра в
   секунду. Лечение — свой порог ленты `WALK_ANIM_MIN_SPEED` / свой таймер /
   `walk_anim_recently()` у Unit, Spearman, Worker; логика на 0.6.
2. **`qa_full_game_4k` B1** (HEAD 0 внутри ядра, правки 230 = 3.67 %):
   бюджет коридора «отрядов / (TTL в кадрах) + 4» откладывал пересчёты в миг
   контакта, а ответ «чужих нет» — бит `FClearEnemy`, снимающий проверку
   чужих тел в шаге. Бисекция ручками (NodeWriteMin 0, SettledDeadzone 0,
   squad_phys_sleep off) — все красные; возврат бюджета 64 — 0.00 %. Пики
   коридора теперь глушит разнос сроков по sid (`CORRIDOR_TTL_JITTER_MS`).

Красные и на HEAD (не регрессии): `qa_settle` C2 (якорь 0.20 м при пороге
0.15 — детерминированно), `qa_guard` J5, `qa_squad` 8 (после reset 50 вместо
41), `qa_gnoll_fix` A3/A4 и `qa_gnoll_behavior` B2б (дальность броска 13.5 м
из конфига), `qa_big_goblin` A3-A5 (конфиг), `qa_tower` A5 (house3 max_hp),
`qa_fog` D3/D4/D6а, `qa_mega_battle` B1-1/B2-0/B2-1, `qa_cavalry` E4,
`qa_troll` B5/B6/D1/D2 (и отдельно, не только под нагрузкой). Мигают под
нагрузкой шлюза: `qa_mega_battle` B4-3, `qa_fog` C4-C6, `qa_troll`.

`qa_fog` D7б переписан под сон в тумане: визуальный тик либо жив, либо снят у
уже скрытого спящего, и тот числится в `_fog_sleepers` — будит его пересчёт
маски (`wake_hidden_sleepers`), а не обход раз в 15 кадров.

### Ловушки этого захода

- Стенд, запущенный рядом со шлюзом «чтобы быстрее», врёт по стенным часам
  и сам вешает шлюзу лишний процесс (`qa_fog` висел 15 минут).
- Прерванный по таймауту цикл стендов в фоне продолжает запускать следующие
  сцены — после kill проверять список процессов Godot.
- Git bash из `Start-Process` стартует без PATH (`mkdir: command not found`)
  — скрипту шлюза нужен `export PATH=/usr/bin:...`; `bash` внутри такого
  скрипта резолвится в WSL — звать по полному пути.

### Финальный прогон после починок (тихая машина, final3)

| фаза | физтик, мс (база → итог) | кадр логики | приказов/с | худший кадр |
|---|---|---|---|---|
| IDLE_FAR | 8.91 → **4.95** | 3.36 → **1.02** | 57 → 15 | 31.7 → 32.9 |
| CLASH | 17.97 → 17.76 | 5.11 → 5.70 | 440 → 79 | 59.4 → 50.7 |
| AFTER | 13.16 → 13.51 | 6.02 → 5.93 | 117 → 74 | 47.2 → 38.9 |
| GNOLL_HEAP | 12.71 → 13.27 | 6.62 → 6.13 | 303 → 189 | 43.2 → 34.6 |

После боя и в куче состав живых от прогона к прогону разный (потери в
CLASH 215-545), поэтому AFTER/HEAP сравнимы только по приказам и худшему
кадру; покой сравним напрямую. Два прогона до этого (final, final2) выкинуты:
машина была занята индексацией WebStorm (пики 104-188 мс с `atk_snooze` 36 мс
одним кадром — чистый C#-цикл, задушенный планировщиком).

## 16.09.2026 — BigStand, этап 4: ворота боя (подход к цели в ядре)

Заказ владельца: вынести регулярные проверки `process_attack` в C#, GDScript
звать по событиям, урон пакетно. Правила и что не делалось — CLAUDE.md,
раздел «BigStand, этап 4». Здесь — итерации и цифры.

### Почему не «весь автомат в C#»

Замер этапов 1-3 (`qa_bigstand`): в замесе 2755 ATTACKING, на упоре и в дрёме
235; ударов 9 на кадр. То есть 90 % входов в автомат — подход, а он однороден
(шаг к цели). Полный порт автомата уже дважды измерен и проиграл
(`batch_combat` −17 % в рубке; автопилот с воротами «чистый путь» +0.008 мс),
потому что в сплошной рубке все на упоре, а разнородные решения (заслон,
застревание, поводок, навигация, залп) переносить смысла нет. Ворота тика —
ядро ведёт шаг между событиями, автомат входит по расписанию — берут ровно
однородную часть.

### Итерации (qa_bigstand, физтик CLASH / вызовов process_attack за 20 с)

| итерация | что добавлено | CLASH | вызовов | qa_ab E2 (сближение) | qa_ab C2 (рубка) |
|---|---|---|---|---|---|
| этапы 1-3 | — | 17.76 | 823 тыс. | — | — |
| 4a | ворота без «чистого пути», темп подтягивания, обход своих, плановый возврат 0.5 с | 16.51 | 627 тыс. | +0.26 (4.8 %) | −0.09 |
| 4b | режим «стоять» для обороны, сцепка в ворота, замок цели без подтягивания | 15.44 | 540 тыс. | −0.04 | −0.06 |
| 4c | **упор в режиме подтягивания не будит**, марш стены к точке | 14.70 | 530 тыс. | +0.31 (6.8 %) | −0.01 |
| 4d | зазор взвода 0.5 → 0.05 | **14.43** | **475 тыс.** | +0.26 (5.8 %) | +0.02 |

Ключ — 4c: в GDScript ветка подтягивания заслон не сканирует и просто давит,
а первая версия ворот будила бойца на каждом заблокированном шаге, то есть
возвращала его в автомат каждый кадр; после снятия этого пробуждения
дремлющих на упоре стало 886 против 235. `qa_ab` в сплошной рубке даёт ноль
по построению (все на упоре), на сближении — 6 %.

### Финальная таблица (физтик / кадр логики, мс)

| фаза | база | этапы 1-3 | этап 4 |
|---|---|---|---|
| покой (IDLE_FAR) | 8.91 / 3.36 | 4.95 / 1.02 | **4.17 / 0.86** |
| марш | 14.74 / 3.13 | 15.59 / 3.12 | 14.24 / 2.80 |
| бой (CLASH) | 17.97 / 5.11 | 17.76 / 5.70 | **14.43 / 5.29** |
| после боя (AFTER) | 13.16 / 6.02 | 13.51 / 5.93 | 11.91 / 4.81 |
| куча гноллов | 12.71 / 6.62 | 13.27 / 6.13 | **11.24 / 5.00** |

Худшие кадры боя 59 → 51 → 46 мс, кучи 43 → 35 → 31. Приказов стоящим в бою
440 → 79 → 95/с. Состав живых в AFTER/HEAP от прогона к прогону разный,
сравнимы покой и бой.

### Пакетный урон (п. 3 заказа) — не делался, замер

Ударов 9 на кадр по 103 мкс (`atk_strike` 0.94 мс/кадр): это побочные
действия `take_damage` (мораль отряда, ветеранство, тело, ответ на удар,
звук), арифметика брони — доли микросекунды. Перенос арифметики в C# дал бы
~0.01 мс/кадр и второй источник правды о формуле (`damage_after_armor`
обязана жить в одном месте). Не сделано намеренно; если нужен пакетный
`take_damage` целиком — это вопрос инвариантов, а не производительности.

### Шлюз этапа 4 (38 боевых стендов) и A/B ручкой автопилота

Шлюз: 25 зелёных из 38. A/B «ворота вкл/выкл» по семи подозрительным
стендам отдельными прогонами:

| стенд | вкл | выкл | вердикт |
|---|---|---|---|
| qa_clash A2 | зелёный | зелёный | шум шлюза (интервал 0.29 на границе 0.33) |
| qa_cavalry E3/E4 | E4 | E3+E4 | не этап 4 (мигает: −36 м / +0.8 м) |
| qa_troll_impact C4 | красный | красный | не этап 4; мигает и при NodeWriteMin 0 (1 из 3) — момент снятия pos0 относительно шардированного тика жертв |
| qa_crowd A1 | красный | красный | **этап 3**: коридор усыплял отряд до первого тика — строка без FPosValid не в сетке и не в разведении |
| qa_fix_pack B6 | красный | красный | **этап 3**, та же причина: стрела не находит строку туши без позы |
| qa_mega_battle B7-2 | 1.40 м | 1.14 м | **этап 4**: упор в режиме подтягивания не будил, боец давил в тела до планового возврата |
| qa_full_game_4k A1/A3 | A3 | зелёный | мигание стрел (160 потолок против 260 в реестре / одна висящая) — оба и раньше мигали |

Починено: `Unit.sleep_physics` не усыпляет бойца без поданной позы
(`_sent_st < 0`) и подаёт позу явно перед сном; `ArmyCore.WritePose` /
`WritePoseBatch` / `AdvanceMatrix` обнуляют накопитель сдвига узла (иначе
узел получал «долг» после прямой записи позы из GDScript); пробуждение по
упору возвращено и режиму подтягивания (цена — вход в автомат на каждом
упоре, как у GDScript-ветки). Итерация 4c в таблице выше описывает то, что
было откачено этим пунктом.

После починок (пробуждение по упору в подтягивании, сон только после поданной
позы) `qa_mega_battle` B7-2 зелёный (1.26 м), `qa_crowd` 8/8, `qa_fix_pack`
44/44. Контрольный `qa_bigstand` после починок дважды снят на занятой машине
(марш, где ворот нет, уехал 14.2 → 16.9 мс, вызов `process_attack` 9.7 →
11.6 мкс на том же коде) — эти прогоны не сравнимы; числа в таблице выше —
из stage4d на тихой машине (до починок; починка упора возвращает часть
входов автомата: дремлющих 886 → 919-1102, вызовов 475 → 535-571 тыс.).
Честный перезамер — первым делом на тихой машине.

## 16.09.2026 — Монах по ТЗ: отряд-пациент, редкий скан, VFX на каждом

Правила — CLAUDE.md, «Автономный монах (ТЗ 16.09.2026)». Что вскрыли стенды:

- Полный сон физики монаха (set_tick(false) из _monk_step) пробовался и снят:
  раненый, появившийся рядом со спящим, не событие — его видит только скан
  (`qa_monk_vfx` B2 «монах взял раненого в лечение»). Дремота — редкий скан
  раз в 1.5 с; цена — одно вычитание в тике и обход реестра отрядов
  (`qa_monk_auto` C: 0.7 скана/с, приказов 0, сдвиг 0.000 м).
- Одиночный каст обязан лечить ТОЛЬКО фокус: первая версия при cap == 1
  лечила первого в списке «кто в дистанции», и `qa_monk_click_fix` B1
  краснел «первым вылечен другой»; `break` вместо `continue` там же давал
  «весь отряд за 7200 кадров не доведён».
- Отход не должен сдвигать такт скана на свой конец: `qa_monk_forge` E4
  «лечение на ходу не прерывается» — на отходе лечим тех, кто в дистанции
  каста, без подхода (`may_walk = false`).
- Стенды ставят раненых БЕЗ отряда — путь одиночки (`_care_lone`) обязателен,
  иначе `qa_monk_healing` B1-B3 (монах не идёт к раненому за 16 м).
- Расстановка монахов (`_tick_spacing`) вернулась в скан: без неё
  `qa_sprint19c` F3 «два монаха разошлись» краснел.
- Цена в `qa_bigstand` (12 монахов): 0.10-0.12 мс/кадр до и после, худший
  тик 251 → 648 мкс (скан реестра из 113 отрядов раз в 1.2-1.5 с).
- Ловушка: `sed -i "${L}a …"` с пустым `L` вставил блок после КАЖДОЙ строки
  (Monk.gd 1788 → 12516); восстановлено awk-фильтром по сигнатуре блока.

## 17.09.2026 — BigStand-5, этапы 0-1: честный измеритель и ведомые ядром вне GDScript-тика

Заказ и разбор до правок — `docs/BIGSTAND_2026-09-17.md`; правила — CLAUDE.md,
«BigStand-5, этапы 0-1». Здесь — что сделано и числа.

### Этап 0 — измеритель

- `qa_bigstand`: ручка `detail` (умолчание 0 — `profile_physics` и
  `class_meter` выключены), `sephist`; в каждой фазе печатаются стена
  среднего кадра и её раскладка (физтик + логика + рендер/движок/прочее),
  распределение кадров (≤16.7 / ≤33.4 / дольше, p50/p95/p99), GC (КБ/кадр,
  сборки gen0/1/2), стрелы, ведомые ядром (автопилот, напор, матрица, дрёма,
  сон физики, `TickSkipped`/`TickListed`). Раскладка по мониторам
  `Performance.TIME_*` была добавлена и снята: при снятом ограничении кадров
  «хвост физшага» показывал 15-35 мс при стене 17 (CLAUDE.md, «Разбор
  кадра» — мониторы для раскладки не годятся).
- `qa_arrow_bench` (новый измеритель): 510 лучников против 300 бессмертных,
  фазы A/B (залп по радару / по готовности), C (все стреляют одним кадром,
  три раза), D (`spawn_arrow` ×500 тёплый/холодный пул, кадр полёта).

### Этап 1 — список строк на тик

`ArmyCore.TickRows(shards, phase, attacking)` → `PackedInt32Array` строк
этого шарда: `FTickOn` (зеркало `Unit.tick_on`), `_st != DEAD`, не
(`FAutopilot | FRearPress | FAtkSnooze` при `_st == ATTACKING`), не
`FMatrixLed`; строки с `FTickAlways` (may_sleep_physics = false: лучник,
монах, рабочий, гнолл, тролль, туша) — всегда в списке. GDScript берёт узел
из `GameManager._row_units[row]`. Ведомый не входит в тик, поэтому: окно
ходьбы считает `tick_visual` (`_mv_tick`), `apply_knockback` / `push_smooth`
/ `knock_down` зовут `_core_release()` (иначе разлёт и лежание, которые
интегрирует только физтик, не шли бы), `_rear_wake(elapsed)` доводит
`_reform_permit` и `_rank_timer`. Ручка `perf_config.core_tick_list`.

### A/B headless, честный режим (одна сборка, ручка выкл → вкл; 3886)

| фаза | физтик выкл | физтик вкл | Δ | логика выкл → вкл | ядро оставило себе строк/кадр |
|---|---:|---:|---:|---:|---:|
| IDLE_FAR | 4.33 | 4.00 | −0.33 | 1.01 → 1.03 | 0 (спят 3045) |
| MARCH | 10.20 | 10.45 | +0.25 | 2.48 → 2.66 | 0 |
| MARCH обратно | 10.20 | 10.46 | +0.26 | 2.56 → 2.62 | 0 |
| **CLASH** | **12.55** | **10.71** | **−1.84** | 4.86 → 4.52 | 424 (в тик 445) |
| AFTER | 10.37 | 8.57 | −1.80 | 5.06 → 4.00 | 250 |
| GNOLL_HEAP | 10.75 | 8.68 | −2.07 | 5.56 → 4.06 | 161 |

В марше ведомых нет, и +0.25 — цена самого списка (скан 3900 строк, маршалинг
3.7 КБ, `for lr in lrows` вместо шага по массиву) плюс шум прогона (покой в
ту же сторону −0.33). Выигрыш в бою меньше оценки отчёта (−2.5…−3.0): в этом
прогоне автопилотных 1128 против 1460, дремлющих 1022, а не-ведомых ATTACKING
(полный автомат, стрелки в окне залпа, гноллы) — ~445 строк в кадр.

### Этап 1б — ожидание такта агро в ядре

После этапа 1 в списке на тик оставалось ~445 строк в кадр, из них ~140 —
стоящие в бою без цели (IDLE, чужие рядом — сон физики не положен): между
тактами авто-агро (0.5-2 с) их тик — преамбула и декремент таймера. Таймер
переехал в колонку `_aggroT` (`IdleWaitArm`), строка помечена `FIdleWait`
(1 << 26) и вне списка, пока `TickRows` не досчитает его до нуля (дельта —
`delta × shards`, как у самого тика). Не взводится у стойки «оборона»
(`_phalanx_advance` каждый такт), паникующих, летящих, лежащих и родов войск
с `FTickAlways`. Снимают ожидание события: `command_move`, `command_attack`,
`set_attack_target`, `take_damage`, `_core_release` — с возвратом остатка
таймера (`IdleWaitClear` → `_aggro_timer`), иначе ударенный ждал бы полный
срок заново. **Ловушка:** `set_attack_target(null)` у стоящего без цели
выходит первой строкой — снимать ожидание пришлось в самих `command_*`,
иначе строка с новым состоянием (MOVING) ждала бы в ядре до истечения
таймера (`qa_army` C3: 10 расхождений состояния).

| фаза | физтик выкл | физтик вкл (1а + 1б) | Δ | ядро оставило себе строк/кадр |
|---|---:|---:|---:|---:|
| IDLE_FAR | 4.38 | 4.26 | −0.12 | 40 |
| MARCH | 11.03 | 11.24 | +0.21 | 41 |
| MARCH обратно | 11.32 | 11.11 | −0.21 | 48 |
| **CLASH** | **14.23** | **11.56** | **−2.67** | 496 (в тик ~370) |
| AFTER | 11.94 | 9.77 | −2.17 | 294 |
| GNOLL_HEAP | 11.50 | 9.91 | −1.59 | 299 |

Пара снята на машине под чужой нагрузкой (браузер, IDE: CPU 50 %) — абсолютные
числа выше пары этапа 1а на ~1 мс в обе стороны, разница внутри пары честная
(марш совпал ±0.2). `qa_army` 23/23 (новые B3-B3г: список строк, реестр
строка → узел, ожидание агро).

### Окно (честный режим, одна сборка, ручка вкл ↔ выкл; 3886)

| фаза | физтик выкл → вкл | логика | FPS мин / ср выкл → вкл | стена ср. выкл → вкл |
|---|---:|---:|---:|---:|
| IDLE_FAR | 4.13 → 3.93 | 0.91 → 1.00 | 84/233 → 129/219 | 16.7 → 16.7 |
| MARCH | 10.52 → 11.16 | 2.55 → 2.63 | 43/49 → 44/49 | 20.2 → 21.8 |
| **CLASH** | **13.09 → 11.12 (−1.97)** | 5.06 → 4.98 | **30/38 → 34/41** | **26.6 → 24.6** |
| AFTER | 10.29 → 9.10 | 4.51 → 4.57 | 43/45 → 39/48 | 21.8 → 21.0 |
| GNOLL_HEAP | 10.61 → 9.03 | 5.05 → 4.92 | 42/45 → 46/48 | 22.0 → 20.6 |

Оконный выигрыш в бою меньше headless (−2.0 против −2.7): в окне рендер и
физика делят один поток, и марш в этом прогоне тоже уехал на +0.6 — прогоны
шли в разное время на нагруженной машине, цифры окна читать как порядок.
Кадр логики от этапа не зависит — там следующий этап (ведомые в BatchVisual).

### Шлюз этапа 1 (30 стендов, `gate.sh on`, ручка включена)

Зелёные: `qa_clash`, `qa_wall` (даже A1), `qa_full_game_4k`, `qa_crowd`,
`qa_army` ×3 (SelfTest 23/23 с новыми B3), `qa_target_lock`, `qa_reform`,
`qa_push`, `qa_bugpass`, `qa_spear`, `qa_goblin`, `qa_monk_forge`,
`qa_archer_snipe`, `qa_infantry_radar`, `qa_knights_push`. Красные, известные
на HEAD (список «Известные красные» и записи этапов 1-4): `qa_cavalry` E4,
`qa_mega_battle` B1-1 / B2-0 / B2-1, `qa_troll_impact` C4, `qa_squad` 8,
`qa_settle` C2 (0.20 м), `qa_big_goblin` A3-A5, `qa_troll` B5/B6/D1/D2,
`qa_gnoll_fix` A3/A4, `qa_volley` C1 (мигает на 10 стрелках). Под A/B
ручкой (выкл/вкл ×2) — `qa_disengage` A4, `qa_fix_pack` C4/C5/C5б,
`qa_combat_lock` 3, `qa_formation` B2 — см. ниже.

### `qa_arrow_bench` (первый прогон, профиль включён)

510 лучников, 300 бессмертных целей. A (залп по радару): 60 выстрелов/с,
`arrow_core` 0.18 мс/кадр, 140 мкс на прилёт, `atk_strike` 129 мкс на выстрел.
**C (все 510 стреляют одним кадром, ×3): цикл запуска 34-41 мс = 67-80 мкс на
выстрел (`_on_attack_fired` напрямую, без автомата), прилёты до 36 в кадр,
`arrow_core` в кадрах прилёта 1.4-1.5 мс средне и до 5.2 мс, 124-139 мкс на
прилёт, худший физтик хвоста 5.2-6.1 мс; управляемые аллокации первого залпа
421 КБ (рождение узлов сверх пула), второго и третьего — 0.** D:
`spawn_arrow` 47.5 мкс тёплый пул / 58.6 холодный (0.5 КБ на свежий узел);
кадр полёта 680 стрел — 0.12 мс (0.17 мкс на стрелу). Это база для этапа
«снаряды без узла» (отчёт § 5.3).

### A/B четырёх подозрительных стендов (по два прогона в каждую сторону)

| стенд | без ручки (вкл) | `knob=core_tick_list:0` | вердикт |
|---|---|---|---|
| `qa_fix_pack` C4/C5/C5б | красный ×2 | красный ×2 | не этап 1 (монах по ТЗ 16.09 — подъём павших до изучения узла) |
| `qa_formation` B2 | красный ×2 | красный ×2 | не этап 1 |
| `qa_combat_lock` 3 | зелёный ×2 | зелёный ×2 | мигает под нагрузкой шлюза |
| `qa_disengage` A4 | красный ×3 | зелёный ×3 | **сам стенд не читает аргументы** — обе стороны шли на одном коде |

**`qa_disengage` A4 мерил конец, а не пик.** Бюджет ожидания втрое щедрее
дороги (77 с при 30 с пути): отряд лучников доходил до точки, вставал, его
догоняли копейщики, ответ на удар взводил forced-цель, и к концу бюджета
двое-трое из шести честно возвращались к точке боя (после прихода — «другая
история» по комментарию самого стенда). Разница «вкл/выкл» оказалась
разбросом хаотичной фазы после прихода: положение на последнем кадре в семи
прогонах одного кода — 6, 4, 0, 6, 5, 6, 6 из шести. A4 переписан на ПИК
удаления за окно (правило «мерить пик, а не конец»): 6/6, 5/6, 6/6, 6/6,
6/6, 5/6, 6/6. Один прогон из восьми дал пик 0 из 6 — отряд не ушёл вовсе за 77 с стенных
часов; не воспроизвёлся, причина не установлена (машина под нагрузкой либо
настоящее редкое зависание — стенд ждёт по стенным часам, правило 12).
Побочное наблюдение: лучник под forced-приказом ответа на удар подходит к
цели автопилотом (`_arm_autopilot` в ветке подхода не отсекает стрелков) —
поведение прежнее, этап 1 его не менял.

## 17.09.2026 — BigStand-5, этап 2: отрядные пересчёты по колонкам ядра

Правила — CLAUDE.md «BigStand-5, этап 2». Что перенесено: коридор отряда
(`ArmyCore.SquadCorridor`: габариты + стволы + чужие одним вызовом, ответ
плоским массивом), бухгалтерия боя (`ArmyCore.SquadMelee`: engaged / free /
голосование по целям / три списка строк для GDScript), ряды без пересборки
строк, обходы радара и залпа по кэшу стрелковых отрядов без `squads.keys()`
и без `squad_members()`. Ручка `perf_config.squad_core_recalc`, прежний
путь — `_recalc_melee_legacy` и три вызова коридора.

**Где на самом деле лежали деньги.** Не в микросекундах обхода, а в GC:
`_recalc_melee` снимал `army.px` и `army.pz` (копии колонок ёмкости 4096 →
16 КБ каждая) на каждый пересчёт — 4.8 пересчёта в кадр × 32 КБ ≈ 150 КБ/кадр,
то есть ~75 % всех управляемых аллокаций замеса из отчёта 17.09 («~100 КБ в
покое не атрибутированы» — в покое пересчёты боя тоже идут, отряды стоят в
ATTACKING/IDLE с целями). Плюс `SquadBounds` возвращал
`Godot.Collections.Array` (финализируемая обёртка → gen1) 8 раз в кадр.

### Ветки CLASH (детальный прогон, профиль включён; сравнимы между собой)

| ветка | до (мс/кадр, худший вызов) | после |
|---|---|---|
| `squad_corridor` | 0.81, 3.9 мс | 0.70, 2.6 мс |
| `squad_melee` | 0.56, 8.0 мс | 0.37, 1.7 мс |
| `melee_calc` | 0.47, **7.0 мс** | 0.27, **0.5 мс** |
| `squad_radar` | 0.21, 6.2 мс | ниже порога печати (< 0.05) |
| `squad_volley` | 0.29, 1.8 мс | 0.26, 1.4 мс |
| GC, КБ/кадр (честный режим) | 177-204 | 48-52 |
| сборок gen0 / gen1 / gen2 за 20 с CLASH | 14-16 / 9 / 1 | 4 / 3 / 1 |

Худший `melee_calc` 7.0 → 0.5 мс: пик был сборкой GC, спровоцированной
снимками, а не счётом. Остаток коридора (0.70) — GDScript: сборка `live` из
`members` (20 мкс/отряд), раздача `_clear_*` и рангов, сплочённость, сон.
Единичная сборка gen2 в CLASH осталась (один раз за фазу, пауза попадает в
случайную ветку — в детальном прогоне это был `batch_move` 31.8 мс) — цель
этапа 4.

### A/B этапа 2 (headless, честно, ручка `squad_core_recalc` выкл → вкл, две пары)

| фаза | пара 1: выкл → вкл | пара 2: выкл → вкл |
|---|---:|---:|
| IDLE_FAR | 3.89 → 3.73 | 3.78 → 3.72 |
| MARCH | 10.10 → 10.19 | 10.20 → 9.98 |
| **CLASH** | **10.47 → 9.94 (−0.53)** | **10.78 → 10.02 (−0.76)** |
| AFTER | 8.53 → 8.35 | 8.91 → 8.93 |
| GNOLL_HEAP | 8.96 → 8.04 (−0.92) | 8.96 → 8.26 (−0.70) |
| GC CLASH, КБ/кадр | 169 → 48 | 180 → 49 |
| сборок gen0/gen1/gen2 за 20 с CLASH | 13/11/1 → 4/3/1 | 15/10/2 → 4/3/1 |

Меньше оценки отчёта (−1.0…−1.3): в C# ушёл счёт, а раздача по бойцам
(`_clear_*`, ранги, кандидаты в напор, `live` из `members`) осталась в
GDScript — там и лежит остаток `squad_corridor` 0.7 мс. Окно (вкл 36/43 FPS,
выкл 33/44) снято при разной фоновой нагрузке (IDLE 235 против 389 FPS) и как
A/B не годится.

### Шлюз этапа 2 (21 стенд) — регрессий нет

Зелёные: `qa_target_lock`, `qa_clash`, `qa_wall`, `qa_full_game_4k`,
`qa_infantry_radar`, `qa_reform`, `qa_crowd`, `qa_bugpass`, `qa_spear`,
`qa_goblin`, `qa_knights_push`, `qa_disengage` (переписанный A4),
`qa_3000_archers_bench`, `qa_ai`, `qa_ai2`, `qa_army` 23/23. Красные,
известные: `qa_mega_battle` B2-0/B2-1, `qa_squad` 8, `qa_volley` C1.
`qa_combat_lock` 3 (мигает; A/B по ручке 4 × зелёный) и `qa_cavalry` B3/E2/E4
(красные и с выключенной ручкой — B3/E2 мигают на одном коде, E4 известный).
**Ручки `knob=имя:значение` теперь читает `GameManager._ready`
(`_apply_cmdline_knobs`) для любого стенда** — A/B по ручке на qa_cavalry и
qa_combat_lock сравнивал уже разный код (в логе печатается «ручка
perf_config.… = …»); до этого их читал только qa_bigstand.

## 17.09.2026 — BigStand-5, этап 3: снаряд без узла, промах и торчание в ядре, прилёт пачкой

Заказ: убрать Node на время полёта, урон пачкой, звук и декали агрегатно.
Опора — аудит § 5 `docs/BIGSTAND_2026-09-17.md`: полёт (0.18 мкс/стрела) и
картинка (один MultiMesh на слой) уже были в ядре; дорого было ДВЕ ГРАНИЦЫ
СОБЫТИЯ — запуск и прилёт. Именно они перенесены.

### Что сделано

| слой | было | стало |
|---|---|---|
| запуск | `spawn_arrow`: узел из пула, 10 `set()`, репарент, `global_position`, три записи слота, `arrow_launch` через границу, два словаря `register_flight` | `fire_projectile` → `ArmyCore.ProjectileFire` (один вызов): слот из `Rb.Free` ядра, запись полёта с уроном/стрелком (instance id)/целью/флагами/сроками |
| полёт | `BatchArrows` (без изменений) | то же + возраст и потолок полёта в записи (`sweep_flights` остался legacy-узлам) |
| промах | событие → узел → `_stick_into_ground`: `play_3d`, jitter, `get_terrain_height`, запись слота, `note_stuck_arrow` (O(n) за потолком) | целиком в ядре `ProjectileLand`: доворот, jitter той же формулой, `Height` ядра, слот, `StuckAdd` (потолок по порядку вставки) |
| торчание | `_stuck_arrows` (узлы), `_sweep_stuck_arrows` → `tick_stuck` у каждого узла каждый кадр | `StuckTick` — один проход по колонкам, растворение в альфу слота, освобождение слота по сроку |
| попадание | событие Godot-массивом → `core_event` → `_strike` → `take_damage` + `play_3d` на каждую стрелу | `PackedInt64Array` + `PackedFloat32Array` пачкой → `_pj_consume`: агрегация по жертве, один `take_damage`, слоты дропаются/остаются в теле |
| стрела в теле | узел в `Corpse.arrows`, `fade_out_in` / `despawn_now` | id записи ядра в `Corpse.arrows`, `stuck_fade` / `stuck_remove` (узлы тоже принимаются — legacy) |
| звук | `bow_block` / `bow_impact` на каждую стрелу | одна `play_3d` на группу в клетке 32 м за кадр (ядро копит промахи `MissNote`, GDScript — попадания) |

Ручка `perf_config.projectile_core` (false — узловой путь целиком, включая
`spawn_arrow`, `dispatch_events`, реестр торчащих узлов) — A/B на одной
сборке. Ядро нумерует свои полёты с 10⁹, чтобы `ArrowCancel` legacy-узлов не
снимал чужие.

### Почему агрегация урона по жертве не меняет исход боя

Броня режет долей (`damage_after_armor` = `урон × A / (A + защита)`), щит и
`MIN_DAMAGE_FRACTION` — множители: сумма после них равна сумме до. Множители
самой стрелы (`ranged_damage_mult` цели, «Гроза великанов» стрелка,
снайперский one-shot как `max`) считаются на стрелу до суммирования. В одном
кадре ядро и раньше отдавало все касания одной жертвы до того, как GDScript
разбирал первое — `_st` DEAD ставится в `_die` синхронно, но EnemyRowAt уже
собрал события; так что «второй стрелой по мёртвому» не было и раньше.
Отличие одно и осознанное: убийство при попаданиях двух стрелков в один кадр
приписывается первому в пачке, а не последнему добившему.

### `qa_arrow_bench` (A/B, одна сборка, профиль включён — нужен `arrow_core`)

| статья | ручка выкл (узлы) | ручка вкл (ядро) |
|---|---:|---:|
| C: цикл запуска 510 выстрелов одним вызовом | 35.7-45.6 мс (70-89 мкс) | 11.7-13.5 мс (23-27 мкс) |
| C: прилёт, мкс на стрелу | 123-132 | 45-57 |
| C: худший кадр `arrow_core` с прилётами | 3.5-4.4 мс | 1.5-2.2 мс |
| C: сборок gen0/gen1 за залп | 0/0 | 0/0 |
| D: `fire_projectile` ×500, тёплый пул | 49.1 мкс, 0 КБ | 8.0 мкс, 0 КБ |
| D: ×500, холодный пул | 67.6 мкс, 264 КБ | 8.2 мкс, 0 КБ |
| A: `atk_strike` на выстрел | 123 мкс | 74 |
| A: прилёт на стрелу | 137 мкс | 73 |
| узлов Arrow в дереве | 170-300 | 0 |

Зонд `qa_arrow_bench/LaunchProbe` (что осталось в GDScript на выстрел, мкс):
анимация 0.57, `play_3d(bow_attack)` 1.47, `notify_incoming_fire` 0.33,
упреждение 0.50, словари залпа 0.52, `unit_bonus` 0.35, `archer_drill` 0.37,
`_UStats.stat` по строкам 0.75 (закэширован статикой), `fire_projectile`
8-18 (18 — когда слой рос шагом 64 на каждые 64 выстрела: 31 перевыделение
буфера в сервере отрисовки; рост стал геометрическим). Целиком
`_on_attack_fired` — 21.5 мкс с ростом, ~13 без.

Гейт «спайк < 3 мс на залп»: прилёт — да (1.5-2.2 мс на кадр с 33-41
прилётами, включая `take_damage`); запуск 510 стрел ОДНИМ вызовом — 12-13 мс,
но в игре окно залпа 200 мс = 12 тиков, то есть ~1 мс на тик против ~3.5-4
прежних. Остаток запуска — GDScript стрелка (анимация, звук, упреждение,
разброс), не снаряд.

### `qa_bigstand` (3886, headless, честный режим, две пары выкл → вкл)

| фаза | физтик, пара 1 | физтик, пара 2 | логика, пара 1 | логика, пара 2 | стена, пара 1 | стена, пара 2 |
|---|---:|---:|---:|---:|---:|---:|
| IDLE_FAR | 3.98 → 3.81 | 4.12 → 3.99 | 1.07 → 0.84 | 1.08 → 0.89 | 16.73 → 16.71 | 16.66 → 16.68 |
| MARCH | 10.14 → 10.05 | 11.29 → 11.04 | 2.54 → 2.33 | 2.84 → 2.52 | 17.21 → 16.97 | 19.05 → 18.20 |
| **CLASH** | **10.54 → 9.96** | **12.10 → 11.06** | **4.69 → 4.33** | **5.74 → 4.93** | **20.45 → 19.77** | **23.42 → 21.45** |
| AFTER | 9.14 → 8.03 | 10.74 → 9.02 | 4.65 → 3.98 | 6.10 → 4.32 | 18.56 → 17.22 | 21.76 → 18.25 |
| GNOLL_HEAP | 8.81 → 8.49 | 9.91 → 9.37 | 5.12 → 4.52 | 5.88 → 4.90 | 17.96 → 17.36 | 19.78 → 18.55 |

Вторая пара снята на машине под большей нагрузкой (обе стороны выше) —
сравнивать внутри пары. В BigStand стрел мало (40-60 в полёте, 50-87 выстрелов
в секунду), поэтому −0.4…−0.8 мс логики и −0.6…−1.0 физтика в бою — это
верхняя граница того, что снаряды там стоили; массовые залпы — по бенчу выше.
GC в CLASH 49.5 → 46.9 КБ/кадр (стрелы были не главным источником — тот
закрыт этапом 2).

### Окно (честный режим, одна пара выкл → вкл)

| фаза | физтик | кадр логики | стена ср. | FPS мин / ср |
|---|---:|---:|---:|---:|
| IDLE_FAR | 3.57 → 3.72 | 0.92 → 0.91 | 16.68 → 16.70 | 207/259 → 168/234 |
| MARCH | 10.00 → 10.50 | 2.58 → 2.48 | 19.60 → 20.61 | 37/49 → 44/50 |
| **CLASH** | 10.08 → 10.31 | **4.79 → 4.28** | 22.58 → 22.77 | **36/45 → 39/45** |
| AFTER | 8.79 → 8.70 | 4.63 → 4.24 | 20.27 → 20.17 | 44/50 → 45/50 |
| GNOLL_HEAP | 8.61 → 8.57 | 5.13 → 4.71 | 19.56 → 19.37 | 45/51 → 48/52 |

Физтик в окне — в шуме (IDLE без единой стрелы разошёлся на 0.15), кадр
логики −0.4…−0.5 в боевых фазах — там и живёт прилёт. Окно снято одной парой
на машине под чужой нагрузкой; headless-пары выше — опорные.

### Стенды

Переведены с узлов на записи ядра (`GameManager.stuck_arrow_records`,
`flight_records`, `stuck_arrow_pos`, `clear_stuck_arrows`,
`projectile_hit_now`, журнал `pj_hit_log`): `qa_volley_fix` (точки и счёт
торчащих), `qa_archer_snipe` (полёты, стрелы в голове, удар «здесь и
сейчас»), `qa_bones_fix` C2, `qa_troll_hitbox` B3, `qa_full_game_4k`
(`_arrow_stats`; A2 переписан — «занятых слотов ≤ в полёте + в земле +
узлов», прежнее «слотов ≤ узлов + 64» без узлов не имело смысла),
`qa_gnoll_fix` (хелперы отдают словари с прежними ключами узла),
`qa_roof_garrison` A9/A11/B5 (стрелок и урон полёта из записи),
`qa_archer_lead` (стрела ведётся по `src` торчащей и журналу попаданий; B0
получил проверку живости — всадник за три секунды у строя срубает пару
лучников, и `(u as Archer)` падал на освобождённом — так было и с
выключенной ручкой). `qa_corpse` K/O и `qa_fix` испытывают сам узел `Arrow`
и оставлены на нём.

Красные, проверенные A/B по ручке (одинаковы в обе стороны): `qa_volley` C1
и `qa_volley_fix` A4 «пик 4 из 10» (личные 15 Гц размазывают залп десяти
стрелков — известный миг), `qa_archer_lead` A3 (доля попаданий по бегущему
— лотерея разброса: 8-24 % на одном коде), `qa_gnoll_fix` A3/A4 и
`qa_sprint13` B3 (дальность броска в конфиге владельца 13.5 против лука 15),
`qa_troll_hitbox` A1 (запас тролля в конфиге 8160), `qa_tower` A5 (house3),
`qa_3000_archers_bench` B2 (радар 0.35 против 0.30 с — порог сравнения двух
режимов по времени).

### Шлюз этапа 3 (38 стендов, ручка включена) — регрессий нет

Зелёные: `qa_target_lock`, `qa_combat_lock`, `qa_clash`, `qa_wall`,
`qa_full_game_4k` (A1 подъём торчащей 0.22 м, A2 буфер, A3 потолок),
`qa_infantry_radar`, `qa_reform`, `qa_crowd`, `qa_bugpass`, `qa_goblin`,
`qa_knights_push`, `qa_disengage`, `qa_ai`, `qa_ai2`, `qa_army` 23/23,
`qa_corpse` 41, `qa_archer_snipe` 39, `qa_bones_fix`, `qa_roof_garrison` 26,
`qa_bldattack` 13, `qa_archer_lead` 12, `qa_archer_logic`, `qa_vet`,
`qa_start_build`, `qa_performance_stress`. Красные — известные на HEAD:
`qa_mega_battle` B2-1, `qa_cavalry` B3/E4, `qa_squad` 8, `qa_spear` C9
(5 из 6, под нагрузкой), `qa_fix_pack` C4/C5/C5б (монах по ТЗ 16.09),
`qa_troll` B5/B6/D1/D2, `qa_tower` A5, `qa_troll_hitbox` A1, `qa_gnoll_fix`
A3/A4, `qa_sprint13` B3, `qa_volley` C1, `qa_volley_fix` A4 (обе — «пик 4 из
10», в обе стороны ручки), `qa_3000_archers_bench` B2 (лотерея задержки
первого залпа: 0.383 против 0.350 с и с выключенной ручкой). **Сеяные
жребии сдвинулись законно:** `play_3d` берёт файл через `randi()` общего
потока, а вызовов стало на два порядка меньше — стенд, утверждающий число
из лотереи агро на конкретном зерне, мог поменять исход.

### Ловушки захода

- Умолчание параметра C# (`float reliefAmp = 0.0f`) до GDScript не доезжает:
  «Too few arguments for BatchArrows()» — фасад передаёт явно.
- `\` в конце строки внутри `'''…'''` Python-патча схлопывает перенос вместе
  со слэшем: GDScript-продолжение уезжает в одну строку (валидно), а следующий
  патч этого якоря уже не находит. Писать `r'''` либо `chr(92)`.
- В `'''…'''` без `r` последовательность `\n` внутри GDScript-строки патча —
  это перевод строки в Python, а не два символа.
- Heredoc bash на Windows портит `×` и другие не-ASCII символы в python-коде —
  патчи с кириллицей писать файлом (Write), не heredoc-ом.
- Одноимённая переменная в стенде (`var fr: float` и `for fr in …`) — Parse
  Error всего стенда, в сводке шлюза — «провалов: ; SCRIPT ERROR 1».

## 17.09.2026 — BigStand-5, этап 4: паузы сборщика и остаток коридора

Заказ: «gen2-пауза и остаток `squad_corridor`». Гейт плана 17.09 для этапа
«GC и хвост»: gen1 в CLASH 0, худший кадр `tail_morale` ≤ 2 мс.

### Диагностика: пауза была не gen2, а gen1 — и её делали обёртки узлов

`qa_bigstand` научен считать сборки ПО КАДРАМ (`ArmyCore.GcInfo` →
`GC.GetGCMemoryInfo`: поколение, пауза, продвинуто, фоновая ли, размеры
поколений ДО сборки, `FinalizationPendingCount`). Первый прогон (после
перевода `take_woken`/`take_press_woken`/`fog_refresh_rows` на
packed-массивы) показал:

| фаза | сборки gen0/1/2 | что видно |
|---|---|---|
| MARCH | [1, 1, 0] | gen1 блокирующая 5.1 мс, ждущих финализации 8857 |
| CLASH | [2, 0, 0] | — |
| AFTER | [2, 2, 1] | gen2 ФОНОВАЯ, пауза 3.4 мс; gen1 блокирующие |
| GNOLL_HEAP | [1, 1, 0] | **gen1 блокирующая 18.5 мс, ждущих финализации 43 406** |

То есть заказанная «gen2-пауза» — на деле блокирующая gen1, и её длину
задаёт очередь финализации: финализируемый объект gen0 не проходит никогда
(сперва очередь), значит, набивает gen1 по построению.

Зонд **`qa_bigstand/GcProbe`** гоняет каждый вызов ядра ×N между двумя
принудительными сборками и печатает, сколько финализируемых объектов он
оставил (по `FinalizationPendingCount`) и сколько байт выделил:

| вызов | финализируемых | Б/вызов |
|---|---:|---:|
| `army.enemy_near`, `enemy_block`, `pos_or`, `height_at`, `set_flag`, `sync_row`, `nearest_of_side`, `best_enemy_w`, `trunk_near` | 0 | 0.1 |
| `army.tick_rows` | 0 | 24.7 |
| `army.squad_corridor` / `squad_melee` / `unit_grid.squad_ranks` | 0 | 6.5 / 7.6 / 10.7 |
| **`unit_grid.query_radius`** | **5901 на 100 вызовов** | 5.4 |
| `GameManager.queue_pose+flush` (60 поз) | 0 | 237 |
| `take_damage`, `command_move`, `play_3d`, `global_position` | 0 | 0.1 |

Единственный источник — `Godot.Collections.Array` узлов из `QueryRadius`:
обёртка с финализатором на КАЖДЫЙ элемент (~59 на вызов при радиусе 8 м).
Кто звал: `Troll._foes_around()` — **каждый физтик на каждого тролля** (и на
одышке тоже), `_shove_ring`, `Mine._tick_capture`, `Archer._snipe_pick`, аура
и расстановка монахов, свип туши, счётчики `EnemyAI`/`GoblinAI`,
`workers_rally`, `Building._lane_occupied`, командир орды.

### Что сделано

- `ArmyCore.QueryRadiusRows(x, z, r)` → `int[]` (тот же обход сетки, тот же
  порядок), `EnemyCount(x, z, r, fac, dead)` — счёт живых чужих без массива.
  `SpatialGrid.query_radius` собирает массив узлов В GDSCRIPT по строкам из
  `GameManager._row_units` — все вызывающие целы; `ArmySoA.query_radius`
  ведёт туда же. `Troll._foes_around` → `enemy_count`. `GcProbe`: 5901 → 1.
- `perf_config.gc_low_latency` = true → `GCSettings.LatencyMode =
  SustainedLowLatency` из `GameManager._ready` (`ArmyCore.GcSetLatency`):
  gen2 только фоновые. A/B на прогоне невозможен — после снятия обёрток
  gen2 в 80 с не случается; ручка на длинную партию.
- Хвост тика: `_sweep_morale` — очередью по кадрам (реестр снимается раз в
  `MORALE_SWEEP_SEC`, доля на кадр = размер × delta / период; каждый отряд
  раз в период, кванты те же); `_start_panic` ставит состояние сразу, а
  `panic_flee` раздаёт `_panic_queue` по `PANIC_ISSUE_PER_FRAME` 24 в кадр
  (`_drain_panic_queue(force_all)` стендам); `_sweep_food` без обхода
  бойцов (павший снят с отряда в тот же кадр — непустой состав и есть живые;
  поштучно только одиночки-рабочие). **Ловушка первой версии очереди
  морали:** таймер такта убывал только при пустой очереди — период
  удлинялся на время разбора, и голод тянул мораль вдвое медленнее
  (`qa_food_economy` C3: 76.4 при поле 40). Часы идут каждый кадр, остаток
  прежнего такта дорабатывается с новой очередью.
- Коридор: `_push_corridor` только при смене ответа/состава (`cor_dirty`,
  `cor_ct`, `cor_ce` в записи отряда; ставит `add_to_squad`, боец без строки
  держит признак), `_squad_sleep_decide` не будит дерущийся отряд, если
  усыплять было некого (`phys_sleepers`; спящий, переведённый в другой
  отряд, ставит признак новому). Метка `cor_sleep` и печать `cor_*`/`tail_*`
  в детальном отчёте `qa_bigstand`; зонд `qa_bigstand/CorProbe` — цена
  пересчёта по подветкам на уставном отряде (60 копейщиков: 56-59 мкс, из
  них сборка состава 19, ядро 12-18, сон 8-11, раздача 4.6).

### Замер

`qa_bigstand` детальный (idle=2 march=3 secs=12 after=5 heap=5), CLASH:

| ветка | до | после |
|---|---:|---:|
| `squad_corridor`, мс/кадр | 0.72 | 0.51-0.53 |
| на пересчёт отряда, мкс | 91 | 67 |
| `cor_harvest` / `cor_core` / `cor_sleep` / `cor_ranks` / `cor_push` / `cor_cohesion`, мс/кадр | 0.188 / 0.132 / 0.115 / 0.065 / 0.055 / 0.019 | 0.151 / 0.103 / 0.036 / 0.071 / 0.008 / 0.017 |
| `tail_morale`, мс/кадр (худший кадр) | 0.39 (10.99) | 0.38 (1.36-1.9) — среднее то же, работа размазана |
| `tail_morale` худший, AFTER / HEAP | 5.87 / 6.58 | 0.85 / 1.88 |
| `tail_food_audio` худший, CLASH | 2.80 | 2.47 (марш-аудио и давление овец, не еда) |

Честный режим (80 с, 3886), сборки по фазам до → после: MARCH [1,1,0] →
[1,1,0] (gen1 5.1 → 3.2 мс, ждущих 8857 → 2194), CLASH [2,0,0] → [2-3,0,0],
AFTER [2,2,1] → [0,0,0], HEAP [1,1,0] → [1,0,0]; блокирующей gen1 в 16-19 мс
больше нет, gen2 не случается. КБ/кадр в CLASH 34-48 → 26.9-28.3. Физтик
CLASH честный 9.35-9.69 при 9.96-11.06 у этапа 3 — в разбросе, среднее
этап не трогал.

### Шлюз (43 стенда)

Регрессий нет. Красные — известные на HEAD: `qa_troll` B5/B6/D1/D2,
`qa_troll_hitbox` A1, `qa_big_goblin` A3-A5, `qa_panic_edge` A2, `qa_balance`
E2, `qa_fog` D3/D4/D6а, `qa_fix_pack` C4/C5/C5б, `qa_settle` C2, `qa_squad` 8,
`qa_mega_battle` B2-0/B2-1/B4-3, `qa_volley` C1. Под нагрузкой мигнул
`qa_big_goblin` C4 («5 взмахов на 3 удара» — туша сама бьёт в окне ожидания;
одиночно зелёный). Два красных были настоящими и оба — стендовые:
`qa_food_economy` C3 поймал часы очереди морали (починено в коде),
`qa_worker_sheep` B3 ждал два физкадра при трёх шардах тика на живой карте
(стенд ждёт свойство с потолком). Зелёные: `qa_troll_impact`, `qa_troll_aggro`,
`qa_gold_mine`, `qa_monk`, `qa_monk_auto`, `qa_monk_forge`, `qa_archer_snipe`,
`qa_panic`, `qa_clash`, `qa_wall`, `qa_crowd`, `qa_army/SelfTest`,
`qa_goblin_tactics`, `qa_keep`, `qa_sprint19`, `qa_sprint19b`, `qa_full_game_4k`,
`qa_bugpass`, `qa_spear`, `qa_guard`, `qa_ai`, `qa_ai2`, `qa_disengage`,
`qa_sprint20`, `qa_home_guard`, `qa_ai_scenarios`, `qa_helpbuild`,
`qa_target_lock`, `qa_goblin`, `qa_hold`.

### Ловушки захода

- `Godot.Collections.Array` из C# — финализатор на каждый элемент-узел, и
  дорог он не аллокацией, а тем, что переживает gen0: 43 тысячи ждущих
  финализации к моменту сборки — блокирующая gen1 на 18 мс. Любой ответ
  ядра чаще раза в секунду — packed-массив или число.
- Зонд, зовущий несуществующий фасад (`army.enemy_at`), печатает
  SCRIPT ERROR и честно идёт дальше — читать голову лога, а не только итог.
- Диск C: под ноль (0 ГБ свободно) роняет `sed` («couldn't flush stdout: No
  space left on device») и вывод PowerShell — «ошибка в стенде» на деле
  бывает диском.

## 17.09.2026 — BigStand-5, этап 5: тихие строки — GDScript-тик картинки только по поводу

Заказ плана: кадр логики CLASH ≤ 3.5 мс. После этапов 1-4 физтик в бою
взят (11.5 → ~10 мс), а кадр логики стоял на месте (4.3-4.9 мс): в
`_process` на каждого из 3886 бойцов раз в `vis_shards` кадров входил
`Unit.tick_visual` — преамбула, туман, LOD, замер хода, такт вспышки, сверка
позы — при том, что позицию, кадр ленты, кольца и полоски привязанной строки
и так вёл `ArmyCore.BatchVisual`.

### Потолок измерен до правки

Ручка-зонд `vis_probe_skip` (пропуск `tick_visual` у ВСЕХ привязанных строк,
картинка ведётся ядром как есть): кадр логики CLASH 4.09 → 1.53, AFTER
5.43 → 1.08, GNOLL_HEAP 6.06 → 0.96, MARCH 2.12 → 1.85, покой 0.71 → 0.59
мс. То есть тик бойцов — 2.5-5 мс кадра логики в бою, а остаток (метки,
подача буферов, тела, стрелы, хвосты) — ~1-1.5 мс. Ручка снята, число
записано в шапку `perf_config.vis_quiet_rows`.

### Что сделано

- **Строка объявляет себя тихой** в конце `Unit.tick_visual`
  (`ArmyCore.VisQuiet`, бит `FVisQuiet` = 1 << 27): условия — привязана к
  отрисовке (`_rb_bound`, `_far_slot`), лента в ядре (`anim_core`), не
  летит, поза и зеркало свежие, урон отдан ядру. Вместе с тишиной ядру
  отдаётся то, ЧТО уже нарисовано — взгляд позы, «идёт» по двум порогам
  (лента 1.0 / зеркало 0.6), в поле LOD, освещён (`FVisMoving` / `FVisSeen`
  / `FVisLit`, колонки `_vq*`) — и срок пробуждения `wake_at`: страховочный
  пересчёт позы по прежнему расписанию (`anim_every` тактов), замок
  анимации удара (`_anim_lock_until_ms`), таймеры подкласса
  (`_vis_extra_wake_ms`: копьё вниз у копейщика `_spear_ready_ms`, щит по
  угрозе у мечника `_threat_until_ms`).
- **Список визуального прохода даёт ядро** (`ArmyCore.VisRows(shards,
  phase)` → `PackedInt32Array`; `GameManager._process` идёт по нему через
  `_row_units`, как физтик по `TickRows`): живые, не спящие по картинке
  (`FSleepDraw`), не тихие. Шард — по номеру строки. Прежний обход
  `_live_units` по шардам — под `vis_quiet_rows = false`.
- **Надзор за тихой строкой — в `BatchVisual`** (каждый кадр отрисовки, в
  том же потоковом проходе, чистая математика по колонкам): истёк
  `wake_at`; лит по маске тумана ядра сменился (`_fogLit`, только чужие);
  вошёл/вышел из LOD (`viewR2`); ходьба — тот же замер окном 200 мс, что
  `Unit._sample_movement`, по обоим порогам, и направление хода против
  взгляда позы (`POSE_TURN_COS2`); у ведомого ядром (автопилот, напор,
  дрёма, матрица) — взгляд `_fx/_fz` против взгляда позы. Повод → снимается
  `FVisQuiet`, и строка попадает в следующий `VisRows`.
- **GDScript будит себя сам на своих событиях** (`Unit._vis_wake` →
  `ArmyCore.VisWake`): все прямые `_pose_dirty = true` / `_flip_dirty = true`
  переведены на `mark_pose_dirty()` / `mark_flip_dirty()` (те и снимают
  тишину), `_wake_process` (смена состояния, приказ, удар, `enter_render`),
  смена живого ряда из пакетного `_push_squad_ranks` (там поле пишется
  снаружи — метится поза).
- **Вспышка попадания и доля жизни у привязанной строки — в ядре**
  (`ArmyCore.VisHit` ← `take_damage`; `BatchVisual` гасит `_vFlash` и пишет
  слот 14/15): у тихой строки такт вспышки не идёт, а 0.07 с обязаны
  догореть и без него. Пик и доля пишутся в буфер ТЕМ ЖЕ кадром, как
  прежний `Slot.set_damage`. Снаружи (без привязки, `vis_core_path` выкл.)
  — прежний путь.
- **Скрытый пеленой ждёт света тихо** (ветка тумана `tick_visual` при
  `fog_core`): лит по маске сторожит `BatchVisual`, а не тик каждого из
  2800 скрытых. Вместе со сном по картинке скрытого стоящего (этап 3) в
  покое остаётся ~215 строк тика на кадр.

### Замер (`qa_bigstand`, 3886, headless, честный режим, одна сборка, ручка выкл → вкл)

| фаза | физтик | кадр логики | стена среднего кадра | худший кадр |
|---|---|---|---|---|
| IDLE_FAR | 3.97 → 3.46 | 1.04 → **0.45** | 16.60 → 16.71 (пол headless) | 32.0 → 24.6 |
| MARCH (туда / обратно) | 12.39 / 11.94 → 10.29 / 10.12 | 2.93 / 2.77 → **0.52 / 0.74** | 20.96 / 19.89 → 16.80 / 16.62 | 37.2 → 29.6 |
| CLASH | 11.28 → 10.43 | 4.53 → **3.03** | 21.50 → 19.34 | 59.7 → 38.2 |
| AFTER | 9.03 → 9.38 | 4.86 → 4.54 | 18.87 → 18.31 | 43.3 → 35.4 |
| GNOLL_HEAP | 9.87 → 9.03 | 5.81 → 4.85 | 20.22 → 17.51 | 34.3 → 30.2 |

Гейт плана (кадр логики CLASH ≤ 3.5) взят: 3.03. Марш — почти потолок
(0.52 при 1.85 у зонда пропуска — вместе с ядром дешевле, чем пропуск с
GDScript-заглушкой), покой 0.45. **AFTER и GNOLL_HEAP до потолка (1.08 /
0.96) не дошли — 4.5-4.9 мс:** там перемешанные отряды, у половины бойцов
цель в шаге за дистанцией удара, «стоят/идут» переключается по окну хода
каждые секунды, а каждое переключение — повод и полный тик. Следующий
кандидат — гистерезис порога ходьбы у надзора и печать `vis_listed` /
`vis_quiet_count` в отчёте стенда (сейчас ядро их считает, стенд не
печатает). Физтик в марше −2 мс — не этап: машина под редактором
владельца (два процесса Godot рядом), сравнивать только внутри пары.

### Шлюз

57 стендов, SCRIPT ERROR 0, регрессий нет. Красные — известные на HEAD:
`qa_fog` D3/D4/D6а, `qa_cavalry` E4, `qa_troll_hitbox` A1, `qa_troll_impact` E5
(под нагрузкой), `qa_panic_edge` A2, `qa_balance` E2, `qa_big_goblin`
A3-A5/C4, `qa_troll` B5/B6/D1/D2, `qa_mega_battle` B1-1/B2-0/B2-1, `qa_volley`
C1, `qa_tower` A5. Единственный новый — `qa_troll` B8 «вспышка взводится»
читал поле `_hit_flash`, а у привязанной строки вспышку теперь ведёт ядро
(`VisHit` → слот 14): стенд переписан на чтение буфера отрисовки. A/B
снимался при двух процессах Godot владельца — числа сравнивать только
внутри пары.

### Ловушки захода

- **Стенды, прогнанные ДО последнего патча, врут арностью:** `qa_clash` и
  `qa_disengage` (20:29) поймали «Invalid call to function 'batch_visual'…
  Expected 17 argument(s)» по 2-28 тысяч раз — `GameManager` уже звал новую
  сигнатуру, `ArmySoA` ещё нет (патч 20:33). После перепрогона — 0 ошибок.
  Сводка обязана печатать время лога рядом с временем последнего патча.
- `_pose_dirty = true` встречалось в `Unit.gd` десятками мест — переводить
  на `mark_pose_dirty()` только скриптом с проверкой счётчика замен, иначе
  одно забытое место оставляет тихую строку с грязной позой навсегда.

## 17.09.2026 — Монах и лучники: граница ауры, канал 5 с параллельно, стрелок с места не сходит

ТЗ владельца после этапа 5. Стенды: **`qa_monk_fix`** (22, headless),
**`qa_archer_fix`** (19, headless); переписаны под новые свойства
`qa_monk_auto` A2/B2, `qa_monk_healing` A2/A4/B, `qa_monk_click_fix` A4,
`qa_monk_forge` C12, `qa_sprint19c` F9/F10/F15.

### Монах

- **Каст — из любой точки ауры** (`Monk.cast_range()` = `heal_radius()` 20 м);
  `MONK_CAST_RANGE` 3 м больше не читается (константа осталась стендам).
  Подход — только к пациенту ЗА аурой, точка стояния — `stand_dist()` =
  `MONK_STAND_FRAC` 0.7 × R от ЦЕНТРА отряда пациента в сторону монаха
  (снаружи строя); тот же закон у марша к фронту. Прежние 0.7 × 3 м вели
  монаха внутрь строя, и там его толкала каждая шеренга.
- **Перебежки 1-5 м** — переиздание приказа подхода при сдвиге точки на 1 м;
  теперь слабина `MONK_MOVE_SLACK` 3 м, а подход к точке подъёма — только
  стоя (идущему второй приказ с точкой в паре метров от первой — те самые
  перебежки). Замер `qa_monk_fix` A: подошёл к отряду за 1.6 R, встал в
  14.4 м от центра (полугабарит строя 1.3), за 12 с стояния сдвиг 0.00 м,
  приказов подхода 0, лечит с границы.
- **VFX мигал между тактами:** окно эффекта было `MONK_HEAL_TICK` 0.5 с +
  0.25 при такте лечения `heal_tick_sec()` 1.2 с. Окно = такт + запас;
  «лечить некого» гасит эффект. B: 40 % → 100 % за 8.5 с, VFX на пациенте
  100 % кадров (433 из 433), снят через 2 кадра, доля жизни в буфере
  отрисовки 0.4 → 1.0.
- **Канал воскрешения 5 с** (`MONK_RES_SEC` 20 → 5; откат 15/12/3 → 5/4/2 —
  `MONK_RES_COOLDOWN` / `_4D` / `_5D`), **параллельно лечению всегда**: гейт
  `parallel_care()` в тике снят (узел 4d только укорачивает канал), этап B
  (павшие) в `_care_squad` открывается и при живых раненых. **Свой круг
  канала `_res_aura`** — гаснет в `_stop_res` безусловно и в `_die`:
  «зависший зелёный круг без модели» был кругом ЛЕЧЕНИЯ, который канал
  одалживал и возвращал только при пустом `_heal_vfx_target`. C: канал
  открылся при раненых, 5 тактов лечения за канал, поднят за 5.0 с, к телу
  не бежал (11 м), круг погашен; D: тело снято посреди канала — круг
  погашен за 2 кадра, модель не появилась. Побочно позеленели известные
  красные `qa_fix_pack` C4/C5 («подъём при раненых рядом»).
- **Ауры наград (`MONK_AURA_BASE_R`) 4 → 20 м** (заказ владельца вслед):
  монах стоит в ~14 м от отряда, 4 м не доставали ни до кого.

### Лучники

- **«Стрельба по трупам» не воспроизвелась** (`qa_archer_fix` C/D: после
  гибели всех целей и тролля 0 выстрелов, мёртвых целей ни у кого, скан у
  туши пуст) — ворота `is_dead()` в ядре, радаре, снайпере и кэше цели
  целы. Добавлена лишь страховка в `Archer._on_attack_fired` (павший в тот
  же кадр — тетива не спускается).
- **«Рассыпание на одиночные ноды» поймано в покое: две причины.**
  (1) Подтягивание задних рядов (`_should_pull_up`) и авто-агро с `far`
  (радар отдаёт цель всем тридцати, задние не достают и идут к ней каждый
  сам). Рост дальности от кузницы переводит чужих через порог радара — так
  жалоба и привязалась к исследованиям. `Unit.holds_ground_on_aggro()`
  (Archer → true) глушит оба пути: стрелок бьёт то, что достаёт с места, к
  дальнему его ведёт только приказ игрока. (2) Смыкание после стычки
  («вышли из боя — собрать строй», `force`) пересчитывало разметку по
  медиане и раздавало места жадно «ближайшему»: сосед на пару сантиметров
  ближе — и цепочкой 3-5 лучников уезжали на 1-3 м (36 приказов смыкания
  за минуту). `SquadFormation.close_ranks` сперва закрепляет стоящих в
  своей ячейке, а `squad_close_ranks(force)` не трогает ЦЕЛЫЙ строй
  (`_formation_intact`: все стоят на постах в допуске прибытия). Замер: 20
  узлов ветки подряд в покое и под приказом — ушедших 0, сдвиг 0.00 м.
- Ловушка стенда: площадка у базы игрока — в зоне патруля тролля, он вырезал
  8 лучников и утянул отряд за собой (читалось как «рассыпание под
  приказом»); дикие замораживаются на старте стенда.

### Шлюз

MA_GATE_PLACEHOLDER

## 18.09.2026 — ТЗ, десять блоков: VFX монаха, подъём до штата, хитбоксы, агро/бег/касание, лучники, тексты, бараки, обход скал, пни, рудники

ТЗ владельца с семью скриншотами. Стенд — **`qa_tz0918`** (headless, блоки
A-L), зонд `qa_probe_bpanel` (ширина панели бараков), `qa_monk_healing` D4
переписан под разворот. Ниже — что нашлось, что изменено, чем проверено.

### 1. VFX лечения (Monk)

- Жалоба: «зелёные круги остаются на земле, если отряд начал движение».
  Две причины. (а) Лента и овал ЕХАЛИ за целью по `draw_position()` весь
  такт лечения (1.2 с + запас): ушедший отряд уносил их с собой, а гаснущий
  по сроку эффект читался как «остался на земле». (б) Монах, ушедший в
  крепость лечиться (`Castle.absorb_unit` → `set_tick(false)`), терял тик,
  а гасит картинки только `tick_physics` — овал под последним пациентом
  жил до конца партии (зелёное кольцо у башни на скриншотах 1-2: пациент
  зашёл на крышу, монах — в крепость).
- Лечение: `_vfx_caster_still()` / `_vfx_target_still(t)` (не `MOVING`, не
  `moved_recently()`, монах — не на отходе) в `_ride_heal_vfx`,
  `_ride_extra_vfx`, `_bind_heal_vfx_many` (идущие цели квада не получают)
  и у круга канала в `_tick_res`; `Monk.set_tick(false)` → `_stop_heal_vfx`
  + `_stop_res`. Замер `qa_tz0918` A: VFX снят через ≤ 3 физкадра после
  приказа отряду, после приказа монаху, после `set_tick(false)`; вернулся
  по остановке. `qa_monk_healing` D4 (прежнее «эффект едет за лечимым»)
  переписан: погашен все 40 кадров хода, вернулся тактом после остановки.

### 2. Воскрешение до штата

- `_squad_corpse(sid)` брал `find_raisable_priority` (монахи первыми, иначе
  БЛИЖАЙШЕЕ тело стороны) и сверял `squad_id`. В замесе ближайшим лежит
  чужой по отряду павший → «тела нет» → `_squad_needs_care` false → пациент
  переключается → подъём кончался одной моделью. Плюс на откате канала
  (5 с) `_care_squad` без раненых отвечал «работы нет» → приказ ПКМ
  сбрасывался, монах уходил к фронту.
- `CorpseRenderer.find_raisable_of_squad` / `raisable_count_of_squad`;
  `Monk._squad_short(sid, alive)` держит отряд пациентом и приказ живым на
  откате, при уходе отряда за ауру — подход к первому живому. Фокус
  одиночки (`_care_lone`) — в пределах `max(MONK_HEAL_LEASH 14, cast_range
  20)`: с кастом из ауры (17.09) раненый в 16 м терял фокус на каждом такте
  (`qa_monk_healing` C3 «+29 при ожидаемых 73», E1 «тронулось 1»).
- Замер `qa_tz0918` B: отряд из 8 с тремя павшими в 12 м, тело чужого
  отряда в 3 м от монаха — подняты все трое, приказ ПКМ не сбрасывался.

### 3. Хитбоксы и кольца

- **Пень**: красный овал на скриншоте 3 — это `TROLL_LAIR_RING_SHIFT` 0.45
  (спринт 15: «овал шире и ниже, пень в центре»): при радиусе 11 м центр
  уезжал на 5 м к камере. Разворот: 0.0, овал (1.0, 0.62) — под корнями.
- **Башня**: `Building.aim_height()` был 0.8 у всех построек — у башни 7.5 м
  это подножие. Теперь середина рисунка `(_draw_base_y + _draw_top_y) / 2`
  (новое поле `_draw_top_y` из `_fit_marker_to_sprite`); пень — своё.
- **Пехота**: `_pick_at` считал бойцу счёт от якоря у СТУПНЕЙ: круг
  попадания симметричен вокруг ног, половина — в пустой траве под спрайтом,
  верх фигуры (1.5-2 м) — вне круга при `unit_slack` 1.35. Якорь смещён на
  `UNIT_PICK_MID` 0.6 × lean (как `pick_body_h` у тролля). `qa_tz0918` C3:
  клик по туловищу переднего бойца при втором в 1.6 м позади — выбран
  передний; C4 клик по ногам — по-прежнему он.

### 4. Агро, бег, конница, звук, касание

- **Бег гоблина ×1.3** (`GOBLIN_RUN_MULT`) — по событию `set_attack_target`
  (`GoblinSpearman._sync_run_speed`), не в тике: ведомый ядром в тик не
  входит. D1/D2: 2.2 → 2.86 с целью, назад без неё.
- **Всадник**: охота `_tick_hunt` (0.5 с, 30 м, поводок 40, агро 20), цель
  держится, марш возобновляется (списано с `BigGoblin._tick_hunt`). Приоритет
  целей — **вторая колонка веса в ядре** `_tgtWc` (`SetCavWeight`,
  `BestEnemyPrio(row, r, crowd, prio)`; `BestEnemyW` — обёртка prio 0/1):
  вес — свойство цели (`cav_target_weight`: лучник 2.2, мечник 1.6, рабочий
  1.4, копейщик в «Защите» 0.4, пушится `Spearman.set_stance`), кто читает —
  `target_prio_mode()` (стрелок 1, всадник 2). D7: копейщик в строю в 6 м и
  лучник в 12 м — выбран лучник; D8: тот же копейщик в «Атаке» — ближайший.
- **Радиусы**: `Warrior.aggro_radius` 5, `Spearman` 5 в «Защите» (в «Атаке»
  прежние 10 — `qa_aggro_fix` A3), всадник 20 (`RIDER_AGGRO_RADIUS`).
- **Клич тролля втрое реже**: `TROLL_VICTORY_CHANCE` 0.34 в `Unit._die` и
  `_on_squad_wiped`, жребий из `AudioManager.rng`.
- **Касание по кадру замаха**: `Unit._hit_delay_ms()` (150 мс у рукопашной
  до 3 м), запись `_hit_pend_*` в ветке удара, `_release_hit` из преамбулы
  `tick_physics` (после окна смещения, до ворот лежащего); промах — цель
  дальше `attack_range + pad + 1.0` или пала. Дрёма не взводится, пока
  касание не отпущено. E: замах на кадре N, урон на N+7…9 (0.12-0.15 с),
  промахов 0. Цена: 2-4 GDScript-тика на удар при кулдауне 1-2 с.

### 5. Лучники

- `_lock_sight_range()` у стрелка = `max(3 × дальность, 60)`: ПКМ по отряду
  за 40 м (2 × 20) считался исчерпанным сразу, замок падал, авто-агро
  выдавало соседний отряд — «игнорируют клик, стреляют по случайным».
  Пехоте — прежние 2 × агро (`qa_knights_push`, `qa_target_lock`). Трупы:
  ворота `is_dead()` целы во всех четырёх выборах цели (17.09).

### 6. Тексты и таблица

- «Стена копий» — строки про залп были ОБЩИМИ для любого переключателя в
  `HUD._add_ability_toggle`; теперь `toggle_lines` у узла `archer_1d`.
  `cooldown` → «Кулдаун»; `STAT_PANEL_W` 232, отступы 6/3, шаг 0.
- Таблица только по наведению на портрет / карточку награды; прячется
  `modulate.a`. Пробовал `visible = false` — у скрытой панели контейнеры не
  раскладывают детей (`qa_vetui` C3: иконка награды на x = 6). Ряд наград в
  прозрачной таблице глушится `mouse_filter` (`_bonus_holders`); `qa_upd4`
  C5 эмитит `mouse_entered` мимо фильтра — подсказка на месте.

### 7. Бараки

- Зонд `qa_probe_bpanel`: пусто 430; после 6 заказов **456**; с лучниками
  на крыше **511** при стандарте 430. Виновник — живой такт панели
  (`_refresh_selected_hp`-ветка): `if _selected_node is Castle and not
  is_stronghold(): info_label.text = _tower_info_text(...)` — бараки-Castle
  получали текст башни, `_sync_panel_grid_widths` оживлял InfoColumn (132 px).
  Исключение `Barracks` там же, где оно стояло в сборке панели (2141).
- «Найм лучников» — кнопка «Выгрузить» с `UNIT_ICONS.archer` (Archer.png)
  и укрупнением найма; теперь `icon_rain_of_arrows.png`, boost 1.0. H1/H2:
  содержимое 430 после найма и с крышей.
- Знамя ветеранов на крыше: `RoofGarrison.roof_index_of` / `banner_point`,
  ветка `garrisoned` в `_update_squad_banners`. I1: знамя видно, снос от
  центра площадки ≤ 0.6 м.

### 8. Обход скал

- `Unit._nav_dir(mp, tp, delta, straight)` поверх `_atk_waypoint`; зовут
  `_process_move` (ход без маршрута) и `Worker` (стройка, склад, четыре фазы
  овцы). `command_attack` сбрасывает `_atk_route`/`_nav_t`. J: прямая
  сквозь плато рудника блокирована, `_nav_dir` даёт cos < 0.98 к прямой.

### 9. Пни

- `_sheep_pressure_check`: порог у любой стороны → цикл по всем `lair_spots`
  (восстановление по таймауту, тролль-вор при пустом пне). K3: 32 овцы у
  игрока — вор вышел и у пня игрока, и у пня ИИ.

### 10. Рудники

- Рудники стояли с рождения (`_spawn_gold_mines`: два ничейных + рудник ИИ),
  «из воздуха» — туман: рудник ИИ в `enemy_buildings` прячется по `is_seen`,
  ничейные — под пеленой. `ArmyCore.FogStampSeen` (штамп в `_fogSeen`),
  `ArmySoA.fog_stamp_seen`, `FogOfWar.add_explored` (обе копии маски),
  `Main._reveal_gold_mines` после спавна, `GOLD_MINE_REVEAL_R` 14. L: 3
  рудника, все «разведаны» и видны на старте.

### Шлюз

Шлюз 78 сцен (`qa_monk_* … qa_full_game_4k`), затем перепроверка 15 + 10 + 4
сцен после правок по итогам. **Регрессий нет.** Новый стенд `qa_tz0918`
48/48, `qa_nav` 24/24, `qa_monk_healing`, `qa_vetui`, `qa_upd4`, `qa_recon`,
`qa_sel2`, `qa_keep`, `qa_pivot`, `qa_knights_push`, `qa_helpbuild`,
`qa_crowd`, `qa_sprint20`, `qa_fog`, `qa_full_game_4k`, `qa_wall`,
`qa_cavalry`, `qa_balance` (кроме известной E2) — зелёные;
`qa_visual_smoke/Shot` (окно) 26/26.

Стенды, переписанные по итогам: `qa_monk_healing` D4 (разворот: эффект
гаснет у идущего), `qa_helpbuild` B2 (рабочий, нарисованный поверх низа
фундамента, законно перехватывает клик — курсор на его теле), `qa_nav`
(орда замораживается: плато рудника в 20 м от стартовых свино-всадников, а
те с этого дня охотятся — отряд стенда вырезался ударом по рядам на подходе),
`qa_mega_battle` B1 (правило 5 на списке павших), `qa_upd4`/`qa_vetui`
вернули таблицу к прозрачности (см. п. 6).

Известные красные, те же на HEAD: `qa_troll_hitbox` A1, `qa_balance` E2,
`qa_ui9` 3/7, `qa_gnoll_fix` A3/A4, `qa_big_goblin` A3-A5, `qa_tower` A5,
`qa_sprint13` B2/B3, `qa_sprint19c` A1/F7, `qa_gnoll_behavior` B2б/B3,
`qa_volley` C1, `qa_settle` C2, `qa_guard` J5, `qa_squad` 8,
`qa_mega_battle` B2-0, `qa_goblin_tactics` D1, `qa_hud5` A6 (чужой боец под
дымкой остаётся нарисованным — краснеет и с `vis_quiet_rows:0`, та же
семья, что `qa_fog` D3/D4/D6а; к ТЗ не относится). **`qa_troll_impact`
B1/B2/C2/C3/C4/D2б и `qa_sprint18b` B10 — конфиг владельца:**
`unit_stats_config.gd` правился во время шлюза (13:10 и 13:24; `attack_cooldown`
тролля 2.7 при ожидании стенда 1.4), `qa_spear` из-за этого не загрузился
вовсе (Parse Error на полузаписанном файле, 142 SCRIPT ERROR) — перепроверять
после того, как владелец закончит с балансом.

Ловушки захода: heredoc Git Bash схлопывает `\` и в python-строках —
патчи с продолжениями строк только файлом через Write; `queue_unit` требует
цену и время (стенду — `train_from_config`); `_flat_spot`, отбирающий
«ровное», отбирает и ПЛАТО — клик по бойцу на плато уводит точку земли на
высоту × lean за спину (`_pick_at` сходится к y = 0, если первое пересечение
легло вне плато) — площадка обязана быть ещё и низкой; `set_tick(false)` у
монаха теперь гасит VFX — замораживать пациентов, не монаха; окно смещения у
замороженного бойца не ведётся — стенд, переставляющий цель телепортом,
ловится сдвигом от точки привязки (`VFX_MOVE_TOL`), а не `moved_recently`.


## 19.09.2026 — Аудит FPS на ~1000 у пня: пожиратели кадра, стенд владельца, раздача приказов орды

**Заказ.** «FPS падает с 60 до 15-20, когда на карте ~10 отрядов (~1000
юнитов), даже если войска просто стоят рядом с гноллами или тушами, и в
бою. Цель 60-70 FPS при 1000+.» Четыре раздела: профиль (IDLE-сканы, спящие,
гноллы и туши), бой/агро, навигация (путь на отряд, спавн без пересчёта
сетки), стресс-стенд в трёх режимах с прогоном всех стендов.

### Стенд сценария владельца — `qa_bigstand/Owner.tscn`

`qa_melee_bench mult=5` (1445) жалобу не воспроизводил (IDLE 234 FPS, бой
58-85), `qa_bigstand` (3886, ИИ остановлены) — тоже не тот сценарий. Новый
стенд наследует `qa_bigstand/Test.gd` (те же `_measure`/`_summary`) и ставит
ЖИВУЮ ПАРТИЮ КАК ЕСТЬ: все ИИ работают, туман включён, стартовые бойцы партии
на месте (3414 бойцов на карте), армия игрока — 32 отряда, 1086 своих — в
32 м от пня партии (36 гноллов, тролль), две туши рядом, вся армия выделена
(кольца), камера над ней. Фазы: A IDLE (стоят), B MARCH (25 м строем и
обратно), C CLASH (8 отрядов гоблинов + 4 конных + туши + стая пня против
армии). Запуск: `godot --path . res://qa_bigstand/Owner.tscn -- secs=15
idle=10 march=10 cap=0 detail=0` (headless — мс и пики, окно — FPS).

### Измерители, заведённые в этом заходе

- **`perf_config.sys_meter`** — часы вокруг `_process` каждой подсистемы
  (`goblin_ai`, `red_ai`, `gnoll_ai`, `reserve`, `home_guard`, `fog`, `hud`,
  `main`, `selection`, `gm_process`; каждая обёрнута парой
  `get_ticks_usec`, в выключенном виде — одно сравнение bool) и под ним
  разбор такта вожака (`ga_regroup/ga_issue/ga_drain/ga_cmd_move/ga_cmd_attack/
  ga_form`), приказа (`cm_pre/cm_land/cm_route/cm_tail`) и маршрута
  (`br_ford/br_nav/br_spread`). `qa_bigstand` печатает мс/кадр, ХУДШИЙ вызов,
  число вызовов и цену вызова — пики ищутся по худшему, не по среднему.
- **Часы A* всегда включены** (`GameManager._nav_query`: `nav_usec`,
  `nav_calls`, `nav_worst_usec`, `nav_routes_failed`, `nav_miss_key`,
  `nav_miss_reuse`; ядро — `NavUnreach`, `NavCompCount`); стенд печатает строку
  «НАВИГАЦИЯ: A* вызовов … мимо кэша: ячейка / концы …, худший вызов».
- **`qa_bigstand/NavProbe.tscn`** — цена маршрута по частям на живой карте:
  `army.nav_line_blocked` 10-13 мкс, `nav_free` 7 мкс (**переход
  GDScript → фасад → C# здесь ~7 мкс, а не 2**), попадание в кэш маршрутов
  15-25 мкс, `build_route` 16-36 мкс, **A* через плато 160 м — 1.47 мс**
  (a→b без препятствий — 15 мкс).

### Что нашлось (по убыванию цены)

1. **ПИКОВЫЕ КАДРЫ 60-100 мс — РАЗДАЧА ПРИКАЗОВ ОРДЫ** (`GoblinAI._drain_orders`,
   150 бойцов за кадр). Профиль сперва показывал `nav_route 38-60 мс в кадре`,
   и подозрение пало на A*; часы по этапам (`ga_*`) назвали
   `ga_drain` (худший 58-75 мс), внутри — `ga_cmd_move` 90-155 мкс на приказ
   при 40-60 % бойцов с маршрутом, внутри `command_move` — `cm_route`
   (`build_route`) 64-102 мкс, внутри — `br_nav` 60-80 мкс: **каждый боец
   строил свой A*** (кэш маршрутов жил 30 кадров — сбрасывался между двумя
   тактами вожака, — и ключевался ячейкой 4 м по точке САМОГО бойца: места
   толпы из ста гоблинов лежат в двадцати ячейках). Один A* через плато —
   1.47 мс, сто на такт — вот и кадр.
   Снято четырьмя правками:
   - **путь один на отряд с обоих концов** (`NAV_KEY_FROM_CELL` 16 м для
     старта и цели, `NAV_CACHE_FRAMES` 600) — чужой ответ бойцу выдаётся
     только после проверки, что прямая от его точки к первому углу и от
     последнего угла к его цели свободны (`ArmyCore.NavReusable`, один
     переход границы); не сошлось — точный запрос по своим ячейкам 4 м
     (`_nav_cache_x`); стенд считает промахи по ячейке и по концам;
   - **компоненты связности** (`ArmyCore.NavBuildComponents` при сборке
     сетки, связность по рёбрам — диагональ A* без срезания угла): цель за
     обрывом — «пути нет» мгновенно, без заливки до `NavMaxExpand` 60 000.
     В сценарии владельца ни разу не сработали (A* «не найдено 0») —
     оставлены страховкой на цели за рекой и рудник ИИ на плато;
   - **`NavSpread` в C#** — боковой разнос точек маршрута одним вызовом
     вместо двух переходов на каждую точку у каждого бойца (`br_spread`
     10 мкс);
   - **бюджет раздачи режет и ВНУТРИ отряда** (`_issue_squad(plan, budget)`
     продолжает с `plan["_k"]`, разметка ставится один раз и кладётся в
     `plan["_slots"]`), `ORDER_BUDGET_MEMBERS` 150 → 24.
   Итог `ga_drain` худший вызов: **61-75 → 8-13 мс**; A* в фазе IDLE 295 → 52-86.
   Дальше не пошёл: остаток `cm_route` — сам переход границы (три вызова
   ядра ≈ 20 мкс) плюс редкие честные A*.
2. **РЕГРЕССИЯ ТЗ 18.09 п. 8 — ОБХОД СКАЛ НА КАЖДЫЙ ТИК ИДУЩЕГО.**
   `_nav_dir` → `_atk_waypoint` (два вызова и два чтения автозагрузки) шёл у
   каждого идущего каждый тик: `qa_bigstand` MARCH 9.3 → 12.9 мс. Теперь —
   инлайн в `_process_move`, проверка прямой раз в `NAV_RECHECK_SEC`, при
   свободной прямой — раз в `NAV_RECHECK_FREE_SEC` 1.2 с, `Unit.nav_active`
   — снимок `nav_on()` раз в кадр, `nav_blocked` — кэш на физкадр по ячейкам
   4 м (160 вызовов/кадр через границу → десятки). Ручка
   `perf_config.nav_detour_move` (A/B: MARCH 12.0 вкл / 10.95 выкл до кэша).
3. **ОТЛОЖЕННОЕ КАСАНИЕ ДЕРЖАЛО БОЙЦА В GDSCRIPT-ТИКЕ** (ТЗ 18.09 п. 4): пока
   `_hit_pend_ms` не отпущено, дрёма не взводилась — 2-4 такта GDScript на
   каждый удар (CLASH +1 мс). Теперь дрёма взводится КОРОТКИМ таймером до
   кадра касания, а по пробуждению настоящий кулдаун восстанавливается от
   момента замаха (`_hit_pend_cd`, `_hit_strike_ms`, `_restore_attack_timer`).
   Ручка `perf_config.melee_hit_delay`.
4. **`draw_orders` 1.7-2.0 мс/кадр** при 32 выделенных отрядах: на каждый —
   состав (новый массив) и МЕДИАНА центра (две сортировки) каждый кадр.
   Списки меток пересобираются раз в `ORDER_MARK_EVERY` 6 кадров, пульс — по
   прошлому списку.
5. **Коридор спящего отряда без чужих — вдвое реже** (`CORRIDOR_SLEEP_TTL_MULT`
   2): тысяча стоящих у пня — 100 отрядов по 67 мкс пять раз в секунду впустую.

### Что НЕ подтвердилось

- Гноллы, туши, резерв, охрана, красный ИИ, туман, HUD, выделение — все
  `_process` вместе < 1 мс/кадр (`gnoll_ai` 0.01, `reserve` 0.05, `fog` 0.2).
  «Стоят рядом с гноллами или тушами» само по себе не дорого: сканы отрядные и
  глушатся `_clear_enemy`, `rider_hunt` 360-380 сканов/с по `EnemyNear` — доли
  миллисекунды.
- Сетка навигации строится ОДИН РАЗ при генерации карты
  (`Main._ready` → `refresh_map_bounds` → `build_nav_grid`); спавн её не
  пересчитывает — пункт ТЗ «спавн без пересчёта сетки» уже выполнен.
- `PhysicsServer` / `NavigationServer` / Raycast в проекте не используются
  вовсе — оптимизировать там нечего.
- «Прочее» кадра (стена минус физтик и логика) 8-10 мс в окне — это рендер и
  движок (121-148 вызовов отрисовки, 3400 спрайтов), GDScript'ом не снимается.

### Замер

Окно, `Owner` (1086 своих, 3414 на карте), три прогона одного кода — разброс
между прогонами по FPS ±20 %, читать пики и физтик:

| режим | физтик | логика | физкадр p95 / худший | FPS мин / ср |
|---|---|---|---|---|
| A IDLE (до) | 5.6-5.8 | 1.1-1.4 | 27-28 / **71-95** | 70-94 / 90-125 |
| A IDLE (после) | 6.0 | 1.2 | 27.7 / **43** | 63 / 81 |
| B MARCH (до) | 6.8-7.4 | 1.2-1.7 | 27-29 / **72-86** | 40-76 / 58-95 |
| B MARCH (после) | 7.3 | 1.4-1.7 | 29.7 / **40-47** | 43-46 / 55-67 |
| C CLASH (до) | 10.0-10.4 | 4.8-5.2 | 33-39 / **98-139** | 29-33 / 39-43 |
| C CLASH (после) | 10.1 | 4.4 | 36.2 / **48** | 35 / 40 |

Цели ТЗ (A 70, B 60+, C 50-60) в среднем взяты по A и B, C — 40 при
2000 бойцов в замесе у пня; худший кадр боя 98-139 → 48 мс. Против
ПОСЛЕДНЕГО КОММИТА (stash A/B, headless `qa_bigstand` 3886, одна машина, один
час): физтик IDLE 9.9 → 4.5, MARCH 17.5 → 11.9, CLASH 20.0 → 12.8, AFTER
13.3 → 10.3, HEAP 14.6 → 10.4 мс; кадр логики 3.9 → 0.6 / 5.7 → 4.3.

### Ловушки

- `ga_drain` худший вызов не равен 40 × средняя цена приказа: цена приказа
  считает `command_move`, а в кадре ещё `squad_set_formation` (145 мкс на
  отряд), `land_target` и приказы атаки — пик = 24 бойца × худший A*.
- Профильная ветка `nav_route` в `qa_bigstand` — это дельта `nav_usec` за
  кадр; «nav_route 40 мс в кадре» означало сумму A* всех приказов кадра, а не
  один вызов.
- `owner2` показал «MOVING 941 в IDLE»: армия честно стояла, но гноллы
  кайтили копейщиков (`squad_counter_charge` → цель ушла → `_resume_march`
  → `command_move` в свою точку) — живая партия, не поломка стенда; мерить
  с несколькими прогонами.
- `qa_goblin_tactics` D1 «веер: группы [-1, 0]» — красный уже в шлюзе ТЗ
  18.09 (сводка `gate_summary`), к раздаче отношения не имеет.

## 19.09.2026 — Два ТЗ: кольца монаха, обход скал, лучники по зданиям и по клику, крыши, ремонт, засадка, спавн строем, дальность, анимация туш

Стенд — **`qa_tz0919`** (41 проверка, headless, блоки A-I). Первое ТЗ — пять
блоков, второе (тот же день, по трём скриншотам) — четыре. Ниже — только то,
что нельзя вывести из кода.

### ТЗ-1

- **П. 1, кольца монаха.** `Monk.GROUND_RINGS_ENABLED = false`: `_make_aura_node`
  возвращает null, `_heal_aura` / `_res_aura` не заводятся, все читатели и так
  проверяли `is_instance_valid`. Осталась только лента лечения над моделью.
  Стенды прежнего требования переписаны: `qa_monk_fix` D1 (канал без круга),
  `qa_sprint18b` C1, `qa_sprint20` A4, `qa_monk_vfx/Shot` (меряет одну ленту).
- **П. 2, обход скал / гуськом.** Три причины и три правки: (1) A* считал
  шаг у самой скалы таким же дешёвым, как в поле — теперь у ядра карта
  дистанции до стены (`ArmyCore._navWall`, `NavBuildWallDist` при сборке
  сетки) и штраф шага `NavWallCost` {1 ячейка: +2.5, 2 ячейки: +0.8};
  (2) нить натягивалась с зазором 0.7 м — теперь `NavThreadClearance` 1.6 м
  (`NavLineBlockedPad`), отжим угла 1.5 клетки от стены; (3) **обход у
  подхода к цели и у хода без маршрута шёл БЕЗ бокового разноса** — каждый
  боец отряда получал один и тот же угол (`nav_route` теперь кэшируется на
  отряд — тем более), отсюда нитка через угол; `_atk_waypoint` и инлайн в
  `_process_move` зовут `_nav_spread(…, _lat_offset())`, `NAV_LAT_MAX` 3.5 →
  6.0 (половина ширины отряда). `GameManager.NAV_WALL_MARGIN` — из ядра,
  стенд B меряет: угол через плато в 5.0 м от скалы, разнос двух бойцов
  3.85 м. Проверка `NavLineBlocked` для «упирается ли прямая» осталась 0.7 м.
- **П. 3, лучники по зданиям.** «Шесть отрядов не стреляют» стендом НЕ
  воспроизвелось (C1: 6 из 6, 15 тыс. стрел; ринг-раскладка у постройки
  цела). Сделано остальное: **тела с крыши — врассыпную** (`RoofGarrison.
  absorb_ranged_hit`: чередование `FALL_STAY_EVERY` — половина павших
  ОСТАЁТСЯ лежать на площадке (`Unit.corpse_ground_override` — тело на
  высоте крыши, а не грунта), половина падает к подножию по золотому углу на
  0.85-1.35 радиуса рисунка), резервист встаёт (было). `corpse_spot` больше
  не веер у ворот.
- **П. 4, рассадка.** `ROOF_VISIBLE` бараки 30 → 15, крепость 60 → 40; шаг
  сетки `GRID_DX` 0.44 → 0.62, ряды `ROW_DZ` −0.55; крепость — `KEEP_CENTRE_MEN`
  22 двумя половинками по `KEEP_HALF_MEN` 11 (4 колонки, просвет
  `KEEP_CENTRE_GAP` 1.3 м), фланги по 9 (3 колонки), ноги фланга
  `CASTLE_FLANK_FOOT_FRAC` 0.54 → 0.60. Стенды `qa_roof_garrison` A4/B2/B3 и
  `qa_roof_visual_fix` переведены на новые числа.
- **П. 5, ремонт.** `Building` получил утиный контракт стройки:
  `work_position` / `edge_distance` / `add_builder` / `remove_builder` /
  `builder_count` / `needs_repair` / `repair_done`, тик `_tick_repair`
  (`REPAIR_RATE` 0.06 запаса в секунду на рабочего; `_process` просыпается по
  `add_builder`). `Worker._process_build` считает `repair_done()` концом
  работы наравне с `_done`. `SelectionManager._try_repair` — ПКМ рабочим по
  своему повреждённому зданию (после рудника и руины, до стройплощадки):
  рабочие — `command_build(здание)`, прочие выделенные — марш к стене.
  **Ловушка стенда:** площадка, сдвинутая от базы НАРУЖУ, уходит за кромку
  карты (z = −148 при пределе 125), и рабочий, зажатый границей, «не доходил».

### ТЗ-2

- **П. 1, кольца при засадке.** Сам вход (`absorb_unit` → `forget_on_map`)
  кольца снимал верно (F1/F3: слоты буфера пусты). Дыра была в
  **`SelectionManager._select`: клик разворачивался на весь состав
  (`squad_of`) — включая УЖЕ УКРЫТЫХ**, и им ставилось кольцо по замёрзшей
  нарисованной точке у ворот. Теперь `_select_one` и
  `SelectionDecalRenderer.register` пропускают `garrisoned` (F4).
- **П. 2, спавн строем.** `Building.SPAWN_WHOLE_SQUAD`: заказ выходит ОДНИМ
  кадром (прежде шеренгами по `ROW_RELEASE_SEC`, и первые ряды получали
  смыкание по частичному центру), сетка появления ×`SPAWN_SPACING_MULT` 1.35,
  и та же сетка кладётся отряду в разметку (`_commit_spawn_formation` →
  `squad_set_formation`) — смыканию нечего стягивать (G4: сдвиг центра 0.00 м
  за 6 с). С флажком точки сбора разметку ставит сам марш. `qa_spear` A8
  («между шеренгами есть пауза») переписан на новое требование.
- **П. 3, фокус и дальность.** (1) Раскладка по фронту (`frontline_targets`) и
  разлив у крупной цели (`_big_target_spread`, спринт 20) переписывали цель
  части отрядов — для стрелков это и было «распыление»: у стрелкового отряда
  цель-боец больше не подменяется (кольцо у постройки осталось). (2) Снайпер
  под замком игрока (`_snipe_pick`) не выбирает одиночек — цель без отряда
  идёт как есть, у отряда — только его модели. (3) **Строгий потолок
  дальности** (`Archer._on_attack_fired` / `_snipe_launch`): цель дальше
  `reach() × garrison_mult()` + `SHOT_RANGE_SLACK` 0.75 — снаряд не рождается
  (`shots_refused_range`); точка прицела (упреждение до 14 м, точка залпа,
  разброс) режется по кругу дальности (`_clamp_reach`). H2: 0 стрел дальше
  досягаемости из 15 тыс. **Ловушка стенда:** мерить дальность от `start`
  записи полёта, а не от нынешней точки стрелка (он мог сдвинуться).
- **П. 4, анимация туш.** `_play_attack_anim` на ТОЙ ЖЕ ленте выходил из
  `_set_anim` без перезапуска кадра: у тяжёлых (туша, тролль) следующий
  взмах приходил до истечения замка прошлого, незацикленная лента стояла
  на последнем кадре — «застывает в позе, пропускает кадры». Теперь та же
  лента перезапускается с нулевого кадра (`_look_frame`/`_anim_phase` = 0,
  `_push_row_anim` — фаза в ядре с нуля) и будит тихую строку (`_vis_wake`).
  I2: фаза 0 у 3 из 3 взмахов.
- **Попутно:** `SquadFormation.close_ranks` закрепляет стоящих в ячейке
  ТОЛЬКО в первых `alive_n` ячейках — выбитая первая шеренга закрывается
  задними (`qa_spear` E2 краснел с 17.09), целый строй по-прежнему не
  пересаживается (`qa_archer_fix` зелёный).

## 19.09.2026 — ТЗ-3: панель рабочего, ответный удар и подкрепление ИИ, бег и агро, фокус на троллях, направленный чардж, QA_CombatGate

Четыре блока ТЗ плюс модуль `CavalryImpactHandler` по отдельному ТЗ того же
дня. Стенд — **`qa_combat_gate`** (23 проверки, headless, блоки A-E, ключ
`-- only_e` гоняет один блок чарджа). Ниже — только то, что нельзя вывести
из кода.

### Блок 1 — панель рабочего

- **Панель сжата по содержимому**: `WORKER_PANEL_W_BOOST` 1.2 → 1.0, иконки
  построек ×1.25 (было 1.5), ряд отцентрован той же правой распоркой, что у
  Крепости (`_sync_panel_grid_widths`: `EXPAND_FILL` и при `_worker_boost`).
- **Фон кнопки постройки — два цвета заказа** (`HUD.BUILD_OK_BG` #2D5A27 /
  `BUILD_LACK_BG` #5A2727, `_build_bg`), а не подмес к цвету рода войск, как
  у найма (`_afford_color`). `_style_cmd_button` темнит заливку на четверть —
  `_build_bg` делит на 0.75, чтобы на экране стояли числа заказа. Живость —
  тот же реестр `_afford_watch` (`_watch_build_afford`: запись с `build`,
  цена крепости прогрессивная и перечитывается через `build_cost_for` на
  каждую проверку). Открытая карточка пересобирается по сигналу склада
  (`_card_anchor` / `_card_data` в `_refresh_afford`) — красное число цены
  перекрашивается без перенаведения.
- Тексты: дома — «Генерируют еду, дают +N слота для рабочих и +N слота для
  армии» (числа из `HOUSE_*_SLOTS`), крепость — «Главное здание. Вмещает
  гарнизон из 40 лучников (2 отряда) на крыше и 20 в резерве» (из
  `ROOF_SQUADS`/`ROOF_VISIBLE`/`squad_size`), загон — одна строка.

### Блок 2 — красный ИИ: TOTAL_RETALIATION и подкрепление

- **Потери стороны считает `GameManager.note_loss`** из `Unit._die`
  (счётчик по фракции + точка последнего убийцы `last_loss_from`), а не обход
  реестра: читает такт ИИ раз в две секунды.
- `EnemyAI._update_retaliation`: с прошлой отметки потеряно
  `RETALIATION_LOSSES` 60 моделей → `retaliation_until` = часы +
  `RETALIATION_SEC` 120, вся полевая армия (все роли, кроме отхода разбитых)
  получает `ROLE_ASSAULT` к точке последнего убийцы (нет — к базе игрока),
  раскладка волны та же, что у штурма (`_command_retaliation`). Охрана
  крепости (`HomeGuard`) в удар не идёт — это и есть DEFEND_BASE. Откат
  `RETALIATION_COOLDOWN_SEC` 180 с после конца; по окончании `issued` снят у
  всех — планы переиздаются.
- **Подкрепление одноразовое** (`_tick_reinforcement` на `REINFORCE_AT_SEC`
  120 часов ИИ → `Main.spawn_ai_reinforcements`): 8 отрядов копейщиков
  блоками в углу ЗА замком (глубже колонны охраны, вбок от оси, две колонны
  блоков), уходят под `HomeGuard.adopt` (спят до штурма). Признак
  `reinforced` едет в слепок (`meta.ai_reinforced`), и **часы ИИ теперь
  догоняют часы партии при загрузке** — иначе подкрепление и час штурма
  считались бы с нуля после каждой загрузки.
- Все ручки — в `spawn_config` (`AI_RETALIATION_*`, `AI_REINFORCE_*`),
  ссылки в `ai_start_army_limit`.

### Блок 3 — боевая логика

- **Авто-агро уже отрядное и интервальное** (радар 2 Гц на стрелковый отряд,
  `_clear_enemy` коридора, личный таймер 0.5-2 с) — стенд A меряет: 120
  стоящих под обстрелом дают **~50-90 сканов/с и 0 маршрутов/с**, порог
  заказа «< 1000 запросов/с» выполняется с запасом.
- **На дальний урон отвечает любой стоящий отряд** (`Unit.answers_far_fire`
  → true, было только у орды): `_answer_far_fire` идёт через
  `squad_counter_charge`, но НЕ у фаланги в «Защите» (её подаёт
  `_sweep_phalanx_press`), не у бегущего, не у стрелка на марше (ТЗ 14.09
  п. 10) и не у отряда, уже втянутого в рукопашную (`squad_engaged > 0`);
  отход в лагерь при малом запасе остался только у орды. `qa_aggro_fix` C7
  переписан на новое свойство.
- **Множителей урона в спину у копейщиков нет** — проверено:
  `_incoming_damage_factor` у `Spearman` только `PHALANX_WALL_TAKE`, у
  `Warrior` щит по сектору; общий `SPRINT_DAMAGE_MULT` — цена бега, не спины.
- **Двойной ПКМ: фаланга не бежит** (`command_move`: `_set_sprinting(run and
  not _stance_holds_ground())`), обычные бегут. **Бегущий по приказу игрока
  дерётся с первого касания чужого тела**: `wall` в `_process_move` =
  `_enemy_contact and (not moved_recently() or (sprinting and _player_run))`
  — скольжение вдоль строя держит `moved_recently()` истинным, и «упор» у
  бегущего не наступал никогда (D3: 0 из 36 в бою при 12 с у чужой шеренги).
  Бег ИИ в обход (ROLE_FLANK, `_player_run` false) чужой строй по-прежнему
  обтекает. Удар в упор по бегущему тоже кончает бег боем (`take_damage`).
- **Фокус десяти отрядов на тролле:** (1) `_big_target_spread` больше не
  разливает лишние отряды на «ближайшего врага в 5 м» — это уводило их за
  гноллом-кайтером к пню («убегают за карту»); теперь только на СОСЕДНИХ
  гигантов (`_giants_near`, `BIG_SPILL_R` 30 м), нет соседей — все на туше
  (`qa_sprint20` D2 переписан, добавлен D2б). (2) **Дистанция удара по
  гиганту — до края туши** (`Unit._pad_of`: `sep_radius()` у цели с
  `fine_ring_radius > 0`), как у постройки по рисунку — иначе 120 копейщиков
  лезли к центру и вдавливались в тело. (3) **Тело гиганта блокирует чужой
  шаг своим радиусом** (`ArmyCore._bodyR` / `SetBodyRadius` ←
  `Unit._ready` для гигантов: добавка `sep_radius − SEP_MIN_DIST`; список
  `_giants` обходится в `ScanBlock`/`EnemyBlock` после ячеек, коарс-гейт
  `EnemyNear` расширен на `_bodyMax`) — раньше `ScanBlock` брал общий
  `blockR` 0.55 и для тролля с телом 1.7 м. Замер B4: внутри 1.1 м от центра
  туши **27 → 0** бойцов, кольцо в досягаемости 120 из 120.

### Направленный чардж — `scripts/units/CavalryImpactHandler.gd`

- RefCounted без тика; зовётся ОДИН раз из `Unit._charge_impact` на касание:
  `profile(dirn, target)` → сектор по `dot(направление конницы, направление
  строя жертвы)` с порогом `FRONT_COS` 0.34 (тот же, что `CHARGE_FRONT_COS`
  / `PHALANX_FRONT_COS`): < −0.34 лоб, > +0.34 тыл, между — фланг.
  **Направление строя — отрядное число** (`squad_enemy_dir` в бою, иначе
  `squad_course`; у одиночки — взгляд), без обхода состава.
- Таблица: фланг ×1.5 урона / ×1.25 отлёта / −6 морали, тыл ×2.0 / ×1.5 /
  −15; лоб — множители 1.0 и прежние правила копий (`repels_charge`,
  контрукол, «Стена копий»). Множители едут в `_charge_rows(…, dmg, kb)` и в
  общую ветку брызг; мораль — `apply_morale` раз на касание через
  `squad_add_morale`. Стенд E: тыл dmg 2.0 / dot 1.0 / мораль 75 → 59.8,
  фланг dmg 1.5, лоб — профиль не пишется.
- **Ловушка стенда:** настоящий разгон шести всадников с 30 м срабатывал
  через раз — `charge_min_runup` 10 м не набирался, когда разгон взводился
  поздно (шард тика); это правило игры, его стережёт `qa_boar_rider_charge`.
  Блок E зовёт `_charge_impact` с честным `_charge_from` за 15 м — меряет
  сектор, а не спуск разгона.

### Мораль и стенды

- **Отдых возвращает СВОЮ мораль, а не потолок** (`_sweep_morale`: регенерация
  до `_squad_base_morale`, не до `MORALE_MAX`). Постоявший копейщик (база 75)
  доходил до 100, и относительный порог `PANIC_RATIO` 0.5 срывал лучника
  (база 45) с первого выстрела — `qa_target_lock` C2 «в зоне огня 0 из 5»
  (паника на 69-м такте) и `qa_hold` C3.
- `qa_hold` C3: беглец ставился в 50 м — внутри замка стрелка 60 м (ТЗ
  18.09), приказ держался, новую цель брать было нельзя; теперь за
  `_lock_sight_range() + 15`.
- `qa_fog` C4: поза бойца в строку ядра явно (`sync_row`) — источники тумана
  ядро берёт по `FPosValid`, а первый тик шардирован.
- `qa_fog` E4-E6 / `qa_ai` 3/4/7: **ремонт (ТЗ 19.09-1) сломал стройку** —
  базовый `Building.repair_done()` (полный запас) отвечал true на свежей
  площадке и руднике, и рабочий бросал стройку первым тиком.
  `ConstructionSite.repair_done` → `_done`, `Mine.repair_done` → `_dead`.
- `qa_start_build` C1 хардкодил `POP_BASE_WORKERS == 5` — владелец поставил
  10/3; проверка на свойство.

## 19.09.2026 — ТЗ «Обход гор и углов» и ТЗ «60 FPS в долгой партии»

Два ТЗ одного вечера. Стенды: **`qa_cliff_bypass`** (QA_CliffBypassGate, 24,
headless), **`qa_long_battle`** (QA_LongBattle_90Min, измеритель с четырьмя
вердиктами, ключи `-- minutes= size= mult=`). Разбор с числами — ниже.

### Обход гор: отступ по габариту, единая дорога отряда, тупик — не в стену

- **Отступ от скал — свойство рода войск** (`Unit.nav_clearance()`: пехота и
  гоблины 1.5, конница 2.2, туша 3.5, тролль 4.0; константы
  `unit_stats_config.NAV_CLEARANCE_*`, кэш `_nav_clear` в `_ready`). Число
  идёт во ВСЕ запросы бойца к сетке (`nav_blocked`, `nav_route`,
  `build_route`, `nav_unreachable`, `_nav_spread`) и в ядре меняет четыре
  вещи: штраф шага у стены (`NavCellPenalty`: масштаб `clear/1.6`, гигантам
  платит и третья клетка; дистанция стены считается до 4 клеток), зазор
  нити (`max(1.6, clear/2)`), отжим угла (`NavCornerPush` ищет ближайшую
  непроходимую клетку в радиусе отжима и отводит точку РОВНО на отступ —
  прежний отжим смотрел восемь соседей, а угол нити на 2-3 клетке от стены
  не отжимался вовсе) и видимость прямой (`NavLineBlockedC`, pad `clear/2`).
  Класс отступа входит в ключи кэшей (`_nav_cls`, старшие биты).
- **Лес — стоимость, а не стена.** Слой `_navTree` (стволов на нав-ячейку из
  реестра стволов, пересчёт по флагу рубки `NavRefreshTrees`): штраф на ствол
  1.0 пехоте / 1.0 коннице / 3.0 гигантам, для гиганта ячейка от 3 стволов
  непрозрачна нити и прямой. Скриншот 1 (тролли у угла в лесу) — ровно это.
- **Дорога — ОДНА НА ОТРЯД, от центра** (`GameManager.squad_leg`, кэш
  `_squad_route_cache` по отряду и ячейке цели на 120 кадров). Ячейка 16 м
  унифицировала маршрут, только пока отряд не стоял на её границе: круглое
  плато обходится слева и справа почти одинаково, и половина отряда шла
  налево, половина направо (стенд: ширина отряда 5 → 43 м — «затор у
  выступа»). Боец получает нить центра, разнесённую своим смещением
  (`_nav_spread`); первый угол не виден с его точки — личный обход до него.
  **Нить и угол отжимаются ещё и на полуширину строя** (`NavPath(..., halfW)`,
  `SQUAD_ROUTE_HALF_W` 3.5): нить — хорда, касательная к скале, и ряд на 2 м
  ближе к стене свою хорду терял — `NavSpread` половинил сдвиг, блок 4 в
  ряд сжимался до 2.4 м за углом. Замер: малый габарит блока при огибании
  46-59 % → **91 % (гоблины), 88 % (конница)** при лесе у угла.
- **«Заворот» колонны (место в ряду по первому колену на все колена)
  пробован и замерен хуже** (55 → 46 %): цель бойца всё равно стоит по
  мировому смещению, и второе колено сводило строй клином. Оставлен перенос.
- **Угол маршрута пропускается, только когда боец поровнялся с ним по
  колену** (`_route_beside_wp`): внешний ряд видел цель за углом раньше
  внутреннего, срезал и уходил вперёд — блок вытягивался в две нитки
  (4.1 × 2.7 → 5.5 × 1.2). Пропуск по одной видимости оставлен вытолкнутому
  назад. Отрядный маршрут подхода (`_atk_waypoint`) при переспросе
  пропускает углы, чей следующий виден, — тролль качался между двумя углами
  у стены.
- **Тупик — не в стену** (`_nav_dead_end` ← `GameManager.nav_last_ok`):
  «пути нет» прежде означало «иди прямо», и тролль с целью на плато вставал
  лбом в угол. Инициатива бросает цель и стоит; приказ игрока держится —
  боец стоит и переспрашивает раз в NAV_RECHECK_SEC.
- **Ловушки стенда:** тролль без логова УСЫНОВЛЯЕТ логово партии лениво и
  уходит в патруль/обед за сотню метров (`GameManager.troll_lair = null` на
  время стенда); поводок погони тролля (`TROLL_CHASE_SEC` 10) снимается в
  цикле замера каждый кадр по `attack_target` (цель выбирает
  `squad_pick_member`, не переданный tgt); ширина «поперёк хода» мерит
  разворот блока, а не гуськом — retention считается по МАЛОМУ габариту
  блока (ковариация XZ); героический `sed`-патч с `\` сломал переносы
  (правило памяти), а срез python по индексам задвоил функцию — Parse Error
  ловится только головой лога.

### 60 FPS в долгой партии: зонды, паника один раз, сон в тумане

- **`scripts/TelemetryLogger.gd`** (узел под Main, `perf_config.telemetry`):
  раз в 10 с строка в `user://performance_log_<дата>.json` — fps, кадр,
  честные физтик/логика (если счётчики включены), живые/идущие/дерущиеся,
  спящие резервы, скрытые на низкой частоте, тела, стрелы в полёте/в земле,
  A* в секунду (`nav_routes_built`), проверок прямой, сканов, тиков,
  застрявшие (MOVING без `moved_recently`), паникующие, очередь в замок,
  отряды, GC. FPS < 30 — срез: бойцы по родам и состояниям, A* по родам с
  прошлого среза (`GameManager.nav_who` ставит боец, `nav_req_class` считает
  `_nav_query`), фазы ИИ, `tick_listed/skipped/hidden`, худший A*. Срезы не
  чаще 30 с. Покадрового пути нет.
- **Паника — строго один раз** (`PANIC_MAX_RUNS` 1, `panic_runs` в записи
  отряда). Цепочка «сорвался → отбежал к кромке → сорвался снова» (морали
  после ступора 55 %, относительный порог) забивала край карты столбом
  беглецов с белыми флагами (скриншот 1:18:49) и слала маршруты сотнями.
  Отбег `PANIC_FLEE_DIST` 20 → 12, разброс `PANIC_SPREAD` 6 → 4. После ступора
  — **очередь на лечение** (`_enter_heal_queue`, только игроку): к ближайшей
  своей крепости; место есть — штатный `request_garrison`, нет — стоит в
  `PANIC_HEAL_WAIT_R` 3 м от кольца замка и раз в `PANIC_HEAL_RETRY_SEC` 10 с
  переспрашивает, без приказов и маршрутов между проверками. Пороги морали
  отряду в очереди не считаются. Снимают: приказ игрока
  (`squad_forget_combat`), вход в гарнизон; выход из лазарета
  (`_release_members` → `squad_note_healed`) обнуляет счёт паник. Красному
  ИИ и орде очередь не даётся — у них свои отходы.
- **Скрытый туманом тикает реже** (`ArmyCore.TickRows(..., armyTicks,
  playerFac, fogSlowDiv)`, `perf_config.fog_slow_div` 6): чужой боец под
  пеленой (маска ядра, та же что у картинки) и не в бою (`st != ATTACKING`)
  посещается раз в 6 своих тактов и отдаётся отрицательным номером —
  диспетчер тикает его с дельтой × 6 (часы, патруль и шаг честные, шаг
  0.2-0.5 м). Освещённый — полная частота со следующего такта. Полный сон
  (тик раз в 2 с) не годится: патруль обязан ходить, а шаг 4-6 м прошёл бы
  сквозь скалы. Счётчики `TickHidden` / `TickSlowed`.
- **Блок «очистка мусора» отменён владельцем** в тот же вечер: трупы, стрелы
  и их сроки (15 мин, потолки) НЕ трогались.
- **`qa_long_battle`**: две армии по SIZE на живой карте, раз в 60 с
  пополнение до штата (новые отряды у кромки, живым полный запас) и новый
  приказ атаки; ускорение как у `qa_ai_loop/Smoke`; раз в минуту строка
  телеметрии; вердикты по последней минуте (физтик + логика ≤ 16.7 мс, A*
  < 100/с, застрявших < 3 %, срезов по FPS нет — только окно). Прогон 3 мин
  400 на 400 при живой орде (2700 живых): физтик 2.1 + логика 2.0 мс, A*
  12/с, застрявших 0.

## 19.09.2026 — Туши, гноллы, сон резерва, профиль 5000

Два письма: ТЗ «Фикс больших гоблинов» (5 блоков) и «Оптимизация и баланс по
свежему логу» (кап гноллов, сон резерва, профиль 4500+). Стенды
**`qa_big_goblin_pair`** (17) и **`qa_reserve_sleep`** (13), оба headless.

### Большие гоблины

- **Звук.** Шаг туши брался из категории ударов по доспеху («ведро»). Свой
  банк `big_step` (два синтезированных thud 62/74 Гц, 22 кГц, 0.28 с),
  −14 дБ, питч 0.92-1.08, три голоса, gap 0.25. Играет по ленте ходьбы
  (`walk_anim_recently()`), не чаще `BIG_STEP_SEC` 1.1: замер 8 вызовов за 8 с.
- **Агро.** `_tick_hunt` сканирует и в IDLE (раньше — только MOVING), пехота
  в приоритете (`_hunt_scan`: боевой ближайший, нет — любой); цель без
  оружия уступает боевому в `BIG_AGGRO_RADIUS` тем же тактом. Замер: туша в
  покое берёт копейщика за 0.52 с при рабочем вдвое ближе; марш мимо врага в
  9 м прерван боем.
- **Связка.** `_escort_target` (цель живого врага у своего боевого не-гиганта
  в 30 м), `_escort_follow` (в покое без приказа — к центру отряда ближайшего
  своего идущего/дерущегося гоблина дальше 22 м); после гибели цели — скан
  28 м → цель соседей → возврат на марш. Стенд C: 30 гоблинов по приказу
  атаки, 5 туш — только марш за стенку: все пять в контакте (подхватов цели
  5), цель у всех — стенка, сквозь стенку 0, худшее отставание от первого
  гоблина 28.5 с при разнице хода по конфигу 14.4 с (туша 1.54 м/с против
  2.2 — владелец), контакт / чистый ход 1.36.
- **Анимация и замедление.** (1) `Unit._walk_min2` — порог ленты ходьбы поле,
  у туши `BIG_WALK_ANIM_FRAC` 0.4 × шаг: общий порог 1.0 м/с при шаге 1.54 на
  шардированном окне давал «идёт без ленты». (2) `ArmyCore.BatchSeparation`:
  гигант (`_bodyR > 0`) на ходу мелких союзников не видит, стоящий
  толкается долей `sepR_j / sepR_i`; мелкий от гиганта отходит на радиус
  гиганта. Замер D: сквозь 30 стоящих гоблинов 1.59 м/с = чистое поле,
  рывков 0; рассинхрон ленты 2 из 391 проб.

### Потолок гноллов

`spawn_config.GNOLL_MAX_ALIVE` 72 на пень (ручка владельца), волна выходит в
свободные места (`TrollLair.spawn_gnoll_squads`, `gnoll_capped`). В логе
партии гноллы копились до 312 (две волны по 144 внахлёст с откатом) и давали
~половину A*. `qa_sprint13` B10-B12 зелёные (старт 36 + волна 36 = 72).

### Сон резерва и охраны — два разных бага

Стенд `qa_reserve_sleep` на живой карте воспроизвёл оба.
- **Резерв орды уводил вожак.** `GoblinAI._issue_orders`: список `field`
  выбрасывает `reserve_sids`, но общий хвост («все отряды, кроме tactical и
  handled») идёт по `squads` целиком — резерв, проснувшийся на штурм,
  получал `ROLE_CENTER` и уходил к центру карты. Зонд `cmd_sid_report`:
  12 075 приказов `horde_ai` резерву за 150 с, все 12 отрядов в 60-140 м от
  постов, все MOVING «к центру». Правка — пропуск `reserve_sids` вне
  `raid_sids` в хвосте.
- **Охрана крепости держала тревогу «есть цель».** `_threatened()` спрашивал
  `squad_in_combat`, а тот отвечает «да» по наличию `attack_target`: под
  форсированным приказом бойцы держат цель, ушедшую на 56 м, не двигаясь
  (стенд C: 5 отрядов, 60 из 60 ATTACKING, реально идут 0). Тревога у обоих
  модулей теперь по удару (`GameManager.squad_hit_recently`) и чужим в зоне;
  `_go_home` через `command_move` снимает цели. Плюс приказ домой
  переиздаётся раз в `RESERVE_HOME_RETRY_SEC` / `HOME_GUARD_HOME_RETRY_SEC`
  20 с, если отряд не на посту (прежнее «когда все в покое» не срабатывало
  при одном уведённом бойце).
- Итог: резерв спит через 9-24 с после ухода/гибели противника, охрана —
  через 27-33 с (откаты 8 и 12 с + дорога). Телеметрия — поля `reserve` /
  `guard` в каждой строке.

### Профиль 5000 бойцов и 130-140 отрядов — `qa_bigstand/Owner.tscn -- scale=2.5`

Сцена владельца ×2.5: своих 2715 (80 отрядов), всего на карте 5043 (орда,
резерв, охрана, гноллы, туши, рабочие), отрядов 130 → 141 в бою, ИИ и туман
живые. Честный режим (`detail=0`), headless:

| фаза | физтик | логика | прочее | стена ср. / p95 / худший | ≈ FPS |
|---|---|---|---|---|---|
| A IDLE (стоят у пня) | 8.78 | 8.71 | 7.9 | 25.4 / 35.1 / 73.2 | ~40 |
| B MARCH (строем) | 12.4 | 4.8 | 8.5 | 25.7 / 32.4 / 44.5 | ~39 |
| C CLASH (стенка на стенку) | 15.8 | 9.4 | 12.4 | 37.7 / 48.5 / 66.6 | ~26 |

Раскладка по веткам (`detail=1`, ранжирование, не бюджет), мс/кадр:

| фаза | тик бойцов (GDScript на каждого) | отрядные проходы и расталкивание | логика кадра |
|---|---|---|---|
| IDLE | process_move 1.0, auto_aggro 0.6, grid 0.5, attack 0.3 ≈ **2.4** | sep_overlap 2.3, corridor 1.6 + ranks 0.7, reform 1.0, morale 0.3, melee 0.3 ≈ **6.2** | vis_far 3.9 (197 строк/кадр по 19.7 мкс), vis_pose 0.8 |
| MARCH | process_move 5.4, grid 1.3, mv_speed 1.25, intercept 0.5 ≈ **8.5** | corridor 1.8 + ranks 0.8, sep 1.1, reform 0.3 ≈ **4.1** | vis_far 1.4 |
| CLASH | process_attack 5.2, atk_find/strike/damage 1.9, process_move 0.7, grid 0.8 ≈ **8.6** | corridor 2.3 + ranks 1.2, sep 1.7, melee 0.7 + calc 0.6, morale 0.65 ≈ **7.4** | vis_far 3.5, vis_pose 1.3 |

- **Ответ на вопрос ТЗ:** в покое нагрузку держат ОТРЯДНЫЕ ПРОХОДЫ и
  расталкивание (6.2 против 2.4), на марше и в бою — ТИК БОЙЦОВ (8.5 и 8.6
  против 4.1 и 7.4), отрядные проходы в бою почти вровень.
- **В ПОКОЕ ЭТО ШТОРМ СМЫКАНИЯ.** 657 приказов/с при стоящей армии: 47 из
  130 отрядов держат дрейф > 0.7 м (33 в откате), `close_ranks` шлёт
  копейщикам 229/с, лучникам 96/с, мечникам 77/с — соседние отряды стоят
  вплотную, расталкивание разных отрядов (×1.6) толкает крайних, смыкание
  возвращает, и так по кругу (та же петля, что в BigStand 16.09, откат ×2 до
  8 с её только прореживает). Каждый приказ будит строку из «тихих»
  (`vis_quiet_rows`) — отсюда 197 полных визуальных тиков в кадр и логика
  8.7 мс в ПОКОЕ. Следующий шаг, если брать: смыкание не выдаёт приказ
  отряду, чей дрейф создан соседним отрядом (чужие тела в ячейках), либо
  порог дрейфа от плотности соседей. Не делалось — не в этом ТЗ.
- **Пики** 60-76 мс в покое — GC gen1 (пауза 9.5 мс, 7.4 МБ продвинуто) плюс
  такт коридора/корни; в бою — `process_attack` 13-17 мс одним кадром
  (после потерь все ATTACKING разом входят в автомат).
- Навигация 0.00 мс/кадр (79-187 A* за фазу), подсистемы ИИ < 0.3 мс/кадр —
  как в аудите 19.09.

### Шлюз 45 стендов (gate10, 19.09.2026 ночь) — регрессий нет

Новые стенды `qa_big_goblin_pair` 17/17, `qa_reserve_sleep` 13/13, `qa_home_guard`,
`qa_ai_scenarios`, `qa_goblin`, `qa_goblin_loop`, `qa_ai_loop` (кроме известного
C1), `qa_fix_pack`, `qa_full_game_4k`, `qa_sprint13` (B2/B3 — числа гнолла в
конфиге владельца) — зелёные. Три стенда краснели ОТ СТЕНДА, все переписаны
на свойство и перепрогнаны зелёными:
- `qa_tz0919` I1 «3 взмаха за 8 с» при кулдауне туши 2.7 = 2.96 взмаха —
  ножевой порог; с IDLE-агро туша берёт цель до приказа стенда, и приказ
  перезапускает серию. Окно — три кулдауна по конфигу + 2 с.
- `qa_gnoll_fix` E1/E2/E6 без единой стрелы: цель в 16 м при базе лука 15,
  а с ТЗ 19.09 снаряд не рождается дальше досягаемости (`_shot_reach`).
  Цель — `attack_range − 2`.
- `qa_big_goblin` D3 «счётчики скана считают»: новый скан покоя
  (`_hunt_scan`) шёл мимо `scans_done` — теперь считает (D1 «≤ 4 за бой»
  цел: 1).
Мигающие под нагрузкой шлюза и одиночно зелёные: `qa_goblin_tactics` D2
(конница «» вместо «attack»), `qa_cliff_bypass` C5 (retention 61 / 64 / 77 %
в трёх прогонах против 91 % в прошлом — метрика гуляла 57-91 % и до этих
правок; порог 70 % ножевой). Известные красные без изменений: `qa_troll_impact`
B1/B2/C4/D2б (конфиг тролля владельца, с 18.09), `qa_big_goblin` A3-A5,
`qa_balance` E1/E2, `qa_mega_battle` B2-0/B2-1, `qa_gnoll_fix` A3/A4,
`qa_gnoll_behavior` B2б, `qa_goblin_tactics` D1, `qa_ai_loop` C1.

## 19.09.2026 (вечер) — ТЗ «Град стрел по укреплениям, монах, смыкание, пень за рекой, волны, разведка, UI ветеранства»

Четыре блока одного ТЗ плюс три уточнения владельца в тот же час (параллельный
урон без ×2, ручное управление гарнизоном, разведка полными отрядами). Разбор
причин — в CLAUDE.md, раздел «ТЗ 19.09.2026 (вечер)». Здесь — замеры и что
вскрыли стенды.

### Блок 1: стрельба по зданиям

- **Фокус.** Зонд `qa_garrison_archers_test` A: 10 отрядов лучников (120
  стрелков) по башне с гарнизоном, бараки и крепость в 18-21 м. До правки —
  разлив кольца: 3 отряда у башни, остальные по соседям. После: A2 «замок на
  башне у 10 из 10», A3 «за 8 с потерь замка 0, чужих целей 0», A4 «стрел в
  башню 224, мимо 0», A5 «башня 8000 → 3666, бараки и крепость целы».
- **«Стреляют по трупам» — воспроизведено и названо числом.** Первый прогон
  A4: 24 стрелы из 192 легли в точку (40, −22) — где за 2 кадра до приказа
  стоял и пал гоблин-приманка. Виновник — `volley_aim` отряда, записанный
  тактом залпа по живому гоблину и НЕ обновлявшийся по зданию (такт считал
  только цели-бойцы), а `squad_volley_prime` открывал окно на эту точку.
  Второй прогон после правки: 0 мимо. (В первом же прогоне были ещё стрелы,
  выпущенные ДО клика и долетевшие в окно замера, — стенд ждёт 90 кадров.)
- **Параллельный урон.** B1 на трёх зданиях: стены −100.0, гарнизон −48.1 =
  броня(50) — точно; B2 копьё: стены −50, гарнизон −0.0. C: 4 павших, 4 тела
  на площадке (y = 5.29 при грунте 0.31), у подножия 0, резервист встал.
- **Ручной приказ.** D4: 5 с огня по указанному дальнему отряду — 0 кадров с
  чужой целью, ближний нетронут (1457 → 1457), дальний 1650 → 1485; D5 после
  гибели цели — авто-агро по ближнему (окно 7 с: залп синхронный, перезарядка
  3 с). E: приказ по чужим баракам за дальностью центра (стена в 19.1 м при
  19.5) — 7000 → 6514.
- **Правки прежних стендов:** `qa_start_build` D4 (стена −400 И лучник),
  `qa_tz0919` C2 («потери запаса, стены тоже» — залп 180 стрелков за 14 с
  никого не добивает при срезе 0.5 и лечении в лазарете; тела — прямыми
  попаданиями), C4/C5 (все на крыше, места разные), `qa_tower` B8в цел
  (прямой удар по укрытому — только стены).

### Блок 2: монах

`qa_monk_move`: A1-A3 клик между отрядами (ближайший свой 2.1 м) — марш, пост
в 0.19 м от клика, приход 0.27 м за 335 кадров; B1 у чужого строя — марш;
C1 37 м при раненых за спиной — дошёл (пик 0.60 м), маршей к фронту 0; C3
автономия вернулась (14.5 м до раненых); D1 клик в фигуру — лечить.

### Блок 3: смыкание, пень, волны, разведка

- `qa_reform_idle`: 6 отрядов × 30 с потерями 30 % — за 20 с покоя приказов
  нашим отрядам **0** (до правки — сотни: центр разметки по всем ячейкам);
  20 чужих тел вплотную — 0; сдвинутый на 2.2 м вернулся (0.49 м), приказов
  смыкания 5 за 10 с (было бы по всему составу по кругу).
- `qa_lair_freeze`: старт — у пня за рекой 0 нод, всухую 1/3/5; потолки
  держатся числом (2 стража, 10 овец); разморозка по бойцу на берегу —
  первой порцией отара, дальше по одной порции за шаг (7 шагов), в конце 2
  тролля и 60 гноллов, frozen = false.
- `qa_wave_wake`: резерв 12 отрядов — первая волна 4, через срок +4; охрана
  5 — волна 3, затем 2; ударенный спящий — вне очереди.
- `qa_scout_raid`: 2 полных конных отряда (50), дорожки разные, точки в 72.7
  м друг от друга; налёт на рабочего → run с отходом → out; одинокие лучники —
  добыча, с пехотой в 18 м — нет; потери 45 % — домой.
- Стенды, зависевшие от живого пня `troll_lair` (25 файлов), получили
  `GameManager.call_deferred("thaw_lairs_now")` после `add_child(main)`.
  `qa_home_guard` C1/C2б/D1, `qa_ai_scenarios` A6/A6б, `qa_goblin_loop` B3/C3
  переписаны под волны и полные отряды.

### Блок 4: UI

`qa_vet_ui2`: A — панель башни показывает 3 карточки и таблицу, клик применяет
награду (pending 0), значок алерта выделяет башню; B — 8 полосок, база 14,
после `spearman_1a` зелёный 1.5; C — наведение: preview атаки > 0, минус
брони < 0, уход — 0, после выбора фиолетовый отрезок и красный минус; D —
VetMark у награды, Forge_spearman_1a с плашкой «Калёный наконечник | +1.5».
`qa_recon` C1 читал числа в средней колонке — карточка разведки оставлена
числами (bar только у своих).

### Шлюз

Гейт 1 (16 стендов после блока 1) и гейт 2 (53 стенда после всех блоков):
регрессий не найдено; красные — известные на HEAD (`qa_tower` A5, `qa_volley`
C1 / `qa_volley_fix` A4, `qa_mega_battle` B2-0, `qa_settle` C2, `qa_squad` 8,
`qa_ai_loop` C1, `qa_goblin_tactics` D1/D2, `qa_gnoll_fix` A3/A4,
`qa_gnoll_behavior` B2б/B3, `qa_upd4` D4/D5, `qa_hud5` A6) либо от заморозки
пня (стенды переведены на `thaw_lairs_now`, гейт 3). `qa_mass_siege` S1
(отставание секторов 13.9-47.7 м) — не сверен A/B: стенд не в шлюзе, чужие
незакоммиченные правки в дереве не позволяют `git stash`; см. отчёт.
Попутно: `_recalc_melee` — типизированное присваивание освобождённой цели
(правило 5, ловил `qa_sprint19b`), `_squad_base_morale` — очередь такта с
распущенным отрядом (`qa_squad`, 5 SCRIPT ERROR).

## 19.09.2026 (ночь) — Срочный багфикс паники, ТЗ-B (кузница рыцаря, подсказки, панель статов), ТЗ-C (рудники в тумане)

### Паника лучников до боя (запись партии, 08:47)

**Что было.** Такт морали (`GameManager._sweep_morale`) переходил к порогам
паники, как только `squad_in_combat(sid)` отвечал «да» — а тот отвечает «да»
уже на ПРИКАЗ атаки (у бойцов есть цель). Дальше относительный порог сравнивал
долю морали отряда с долей морали противника: лучник с базой 45 против конницы
орды на 100 давал 0.45 < 1.0 × `PANIC_RATIO` 0.5 — и отряд срывался в первый
же такт после ПКМ, без единого выстрела и без единой потери. Телеметрия
(`performance_log_2026-09-19_1528.json`) показывает всплески `panicking`
30-91 ровно на росте `attacking` — в момент начала стычек.

**Что сделано.**
- «В бою» для морали — это «нас бьют»: регенерация идёт и у отряда с целью,
  которого не бьют (`squad_hit_recently`, окно `RECENT_HIT_WINDOW_MS` 3 с).
- Порог паники один — критические потери: живых ≤ `PANIC_ALIVE_FRAC` 0.30
  полного штата, отряд бьют, и жребий `PANIC_CHANCE` 0.5 на каждую НОВУЮ
  потерю ниже порога (`panic_roll_alive`), из `AudioManager.rng`. Аура
  легенды делит шанс (своя ×1.5 → шанс ÷1.5, чужая новобранцу — умножает).
- Абсолютный порог по морали, «осталось шестеро» и относительный порог сняты
  как триггеры (константы оставлены, `qa_food_economy` читает
  `PANIC_THRESHOLD` как ориентир пола голода). Мораль по-прежнему считается.
- Стендам: `panic_chance_override`, `panic_rolls`, `squads[sid]["panic_rolls"]`.

**Стенд `qa_panic_guard` 13/13:** 30 лучников с приказом на бессмертную
замороженную конницу — 8 с без паники и без жребиев; обстрел без потерь —
ноль; 50 % потерь с шансом 1.0 — ноль (мораль 24 < 30, раньше сорвались бы);
9 живых — жребий брошен, с шансом 0 паники нет, без новой потери повторного
жребия нет; новая потеря при шансе 1.0 — паника и белый флаг; мораль 1/100 у
полного состава — паники нет, вне боя растёт к базе. `qa_balance` C1-C7
(12 копейщиков против 36 мечников) — зелёные с `panic_chance_override = 1`
(живых 3 из 12 в миг срыва).

### ТЗ-B: древо рыцаря, подсказки, панель статов

| было | стало |
|---|---|
| 1d «Яростный Набег», после рывка отряд стоял (агро мечника 5 м, лотерея такта) | «Яростная Атака», `_rush_aggro` по прибытии / истечению: ближайший чужой в 14 м тем же тиком (`qa_knight_forge` D2: 12 из 12 с целью за 0.75 с) |
| 2d-5d — заглушки без кода | 2d «Рассечение» (2 соседа × 50 %), 3d «Неистовство» (+2 удара → 7), 4d «Стальной вихрь» (3 × 75 %), 5d «Кровь берсерка» (×1.25 ниже половины) |
| 3a «Подкова» +0.2 скорости | «Тяжёлые латы» +1.5 брони +0.4 напора; напор 0.6/0.8 → 0.9/1.2, броня 5a 2 → 2.5 |
| колонка D монаха даром по ряду (`_grant_row_bonuses`) | платная (150…2500 золота, 20…100 с), 2d требует 1d у ВСЕХ веток, `_grant_row_bonuses` не зовётся |
| у монаха ни одной горизонтальной связки | 10 связок рядов 2-4, стрелка C → D в каждом ряду, вертикаль по D |
| подсказка: бонусы ключами, «Действует», «Исследование: N с», «Способность отряда» + движковый пузырь поверх | описание, «Статус: Доступно/Недоступно» (зелёный/красный), причина серым, цена; `QuietTooltipButton` |
| панель статов во всю ширину нижней панели, полоски ~350 px, наведение только над бейджами | `STAT_COMPACT_W` 250 над левым краем, `BAR_W` 64, `ColorRect` подложки IGNORE, предпросмотр по наведению на узел кузницы |

Рассечение — хук `Unit._after_melee_hit(tgt, dmg, dir)` после касания (в
обоих путях: мгновенном и по кадру замаха); база пуста, мечник сканирует
сетку один раз на удар. Замер стенда: по цели 19.2 за удар, соседям 9.6 (50 %)
/ 14.4 (75 %), одиночка — задетых ноль. Серия из семи: мощных 3.

### ТЗ-C: рудники сквозь туман

Причины две: `neutral_buildings` не входила в обход
`FogOfWar._apply_enemy_building_visibility` (рудник никогда не прятался), и
`Main._reveal_gold_mines` штамповал «разведано» вокруг каждого (ТЗ 18.09
п. 10). Теперь группа в обходе, штамп под `GOLD_MINE_REVEAL_ENABLED` = false,
свой рудник (`Mine._capture` игроком) снимает `_fog_hidden`. `qa_mine_fog`
10/10: на старте 4 из 4 рудников не разведаны и невидимы (слой клика 0),
рабочий в обзоре — виден, ушёл — силуэт, контрольный — спрятан, захваченный —
виден. `qa_tz0918` L переписан под разворот.

### Шлюз

Гейт 4 — 67 стендов, SCRIPT ERROR 0, регрессий нет. Переписаны под новые
свойства и зелёные: `qa_forge` A5/A6 (бесплатных узлов нет; способность
только в D, «весь столбец или ничего» снято — у рыцаря 1 способность + 4
прокачки), `qa_vetui` B4 (карточка компактная, не шире панели),
`qa_monk_forge` A3/B/C (платная D, цепочка), `qa_archer_snipe` A6 (2d после
1d), `qa_sprint13` F2 (имя), `qa_tz0918` L, `qa_balance` C (шанс 1.0),
`qa_monk_vfx` C (эффект гаснет у идущей цели и возвращается к вставшей —
прежняя C1 утверждала снятое 18.09 «эффект едет за целью»). Под нагрузкой
шлюза мигнули и одиночно зелёные: `qa_tz0919` C5, `qa_monk_vfx` B3/B4/D1.
Красные — известные на HEAD: `qa_sprint13` B2/B3, `qa_hud5` A6, `qa_ui9`
3/7, `qa_upd4` D4/D5, `qa_fix_pack` C5б, `qa_sprint19c` A1/F7,
`qa_panic_edge` A2/C2, `qa_goblin_tactics` D1, `qa_sprint18b` B10/B11,
`qa_squad` 8, `qa_balance` E1/E2.

## 19.09.2026 (поздний вечер) — Аудит двух партий, коллизии зданий и Y-sort тролля, фиксы ИИ по аудиту, телеметрия

### Аудит партий (отчёт `docs/GAME_ANALYSIS_2026-09-19.md`)

Две партии (`performance_log_2026-09-19_1528` — 51 мин, до багфикса
паники; `_1747` — 41 мин, после). Главное: багфикс паники подтверждён
живой партией (пик паникующих 494 → 49, срывы целыми отрядами без потерь
15 → ≤ 4); оба ИИ на Hard проигрывают до своего «часа X» (красная крепость
пала 36:50 / 15:49, орда 27 минут в `harass` без HUNT, резерв 12 → 2);
потолок кадра на 7-8 тысячах — «прочее» 20 мс (рендер и движок), не тик.
Найдено кодом: ложные тревоги резерва (6 за партию B) — `_raid_plan` →
`return_squad` снимал `lent` у конницы под обстрелом за сотню метров, и
`_threatened` считал это «бьют резерв»; `goblin_squads = −1` в телеметрии;
накопители навигации и потерь не сбрасывались между партиями. Партия A
шла параллельно со шлюзом стендов — её FPS занижен чужой нагрузкой.

### Коллизии зданий (ТЗ «Building Collisions», стенд `qa_building_block` 21/21)

ТЗ говорит на языке 2D (`CollisionShape2D`, `NavigationServer2D`,
`y_sort_enabled`) — в проекте физтел и навигационного сервера нет, тела
бойцов держит пакетный шаг ядра. Постройка регистрирует в ядре свой
ФУНДАМЕНТ рядом кругов (`Building.block_circles` →
`ArmyCore.RegisterObstacle`): ширина — нарисованная полуширина (кольцо на
земле, минус `BLOCK_INSET`), глубина — от передней стены `BLOCK_FRONT` 0.35
до задней грани коробки; круг радиусом по глубине, ряд с шагом не шире
радиуса (крепость — 3 круга r 2.17, хижина — 2, дом — 1). Ворота
(`gate_depth` ≥ 1.7 м), точка строителя (`FRONT_EDGE` 0.6 + 0.25), площадка
сбора и кольцо подхода атакующих (`ring_radius` + рука) лежат снаружи по
построению — стенд A3-A5 это меряет. Шаг в круг (`BatchMove`, после ствола,
до чужого строя; та же логика в запасной дороге `Unit._move_blocked`)
скользит вдоль, лобовой отклоняется вбок по номеру строки; стоящий ВНУТРИ
(площадка заложена поверх отряда, боец из слепка) выходит наружу — шаг
разворачивается по нормали (C1: 55 физкадров). Разведение в фундамент не
выталкивает. Своя запись, не `RegisterTrunk`: ствол для застрявшего
отключается (`FTrunkIgnore`) и в сетке — цена, фундамент не отключается и
в сетке — стена. Коридор отряда без построек рядом снимает проверку
(`FClearTrunk` считает и `BldNear`). Правило партии, как вода и скалы
(`boundsOn`); ручка `perf_config.building_obstacles`. Не держат: руина
(`_dead`), загон (`SheepPen.blocks_movement` false), пень — ствол
`LAIR_TRUNK_R` 2.2 (гноллы прячутся с 4.5 м, тролли выходят).

Сетка навигации: базовый слой скал и воды (`_navBase`) плюс слой построек
(ячейка стены = 2, центр ячейки в r + `NavBldPad` 0.3), пересборка стен и
компонент лениво при первом запросе после смены реестра (`NavSync` в
каждом публичном `Nav*`). Крепость — 54 ячейки, прямая сквозь неё
упирается, A* даёт обход; кэши маршрутов GameManager рвутся при смене
реестра (`_nav_caches_drop`). Регистрация — отложенно из `_ready` (позиция
задаётся после `add_child`) и заново по `NOTIFICATION_TRANSFORM_CHANGED`;
снятие — `_exit_tree` и `_die`.

Замер B: 24 копейщика сквозь крепость — тел внутри фундамента 0 кадров из
1018, дошли 22 из 24; башня принимает гарнизон (30 за 339 физкадров),
рабочий встаёт на стройку снаружи; снос снимает запись.

### Y-sort тролля (та же ТЗ, п. 2)

Спрайт лучника на крыше — `cyl_billboard` со своей точкой на земле НА
ВЫСОТЕ КРЫШИ: под камерой 45° каждый метр высоты приближает к камере на
0.71 м, и крыша башни (4-6 м) рисовалась поверх тролля в трёх метрах
ПЕРЕД башней (скриншот). Новые uniform'ы `depth_anchor_on` /
`depth_anchor`: точка сортировки берётся у хозяина (низ рисунка постройки),
`depth_push` разводит ряды крыши к камере (`ROOF_SORT_LIFT` 0.30 −
`ROOF_SORT_ROW` 0.10 за метр глубины, не ниже 0.05) — крыша поверх стены,
тролль перед стеной поверх крыши (F2: 396.69 против 399.42 по камере),
позади — под ней. `qa_visual_smoke/Shot` 26/26, снимок
`qa_building_block/Shot`.

### Фиксы ИИ по аудиту (ТЗ «Фикс ИИ и телеметрия», стенд `qa_ai_audit` 26/26)

- **Резерв:** удар по отряду считается тревогой только в
  `RESERVE_WAKE_RADIUS + 20` от деревни (`_threatened`); ниже
  `RESERVE_LEND_FLOOR` 8 отрядов конница взаймы не даётся.
- **HUNT:** `_assault_ready` — после часа X и `HARASS_MAX_SEC` 480 с
  беспокойства наступление идёт тем, что есть, не меньше
  `ASSAULT_MIN_SQUADS` 6; `assault_gate()` — ворота телеметрии.
- **Красный ИИ:** `_retaliation_refusal` — полевых отрядов ≥
  `RETALIATION_MIN_SQUADS` 8, своих в поле ≥ `ADVANTAGE` 1.0 × чужих в
  `SCAN_R` 45 у источника потерь; у своей крепости (`HOME_R` 45) — без
  проверки. Отказ — переспрос через `RECHECK_SEC` 30, счёт потерь цел.
- **Телеметрия:** `goblin_squads` — размер массива; `GameManager.tm_reset`
  на старте партии (навигация, потери, накопители); в каждом образце `ai`
  (фаза, отряды, ворота HUNT, часы, удары и отказы красных), `economy`
  (склад, приток, расход по стороне и ресурсу — `ResourceManager.tm_*`),
  `damage_in` / `damage_out` (сторона → род, урон после брони из
  `Unit.take_damage`), `abilities` (залп, снайпер, яростная атака, натиск,
  стена копий, чардж, воскрешение — `tm_ability`); `events` с таймкодом
  (смена фазы орды, ответный удар и его отказы, тревоги резерва и охраны,
  паника, падение крепости). Цена — одно сравнение bool на удар.

### Шлюз коллизий (74 стенда, SCRIPT ERROR 0) и две правки по нему

- **Точка приказа внутри фундамента недостижима, как в воде** —
  `GameManager.nearest_passable` теперь выносит её и из фундамента
  (`in_building`, зазор `BLD_TARGET_CLEAR` 0.4): клик по дому, флажок сбора
  на крыше — иначе «вечная ходьба» у стены.
- **К постройке обход ведёт до точки остановки, а не до центра**
  (`Unit._process_attack`, `nav_goal`/`ntp`): центр здания лежит в его же
  фундаменте, прямая к нему «упиралась» в само здание, `NavNearestFree`
  цели отвечала ячейкой с любой стороны дома, и обход уводил бойца от
  сектора кольца к случайной стене (`qa_mass_siege` S1: отставание 8-26 м).
  Цель обхода — сектор кольца, пока он ведёт, иначе точка перед стеной по
  лучу от бойца. S1 после правки: отставание 1.3-2.9 м.
- **Ловушка стенда:** `qa_mass_siege` телепортировал СПЯЩИЕ по физике
  отряды без `sync_row()` — центроид (по строкам ядра) оставался на старом
  месте, ось кольца смотрела назад, сектора ложились за зданием; без
  фундаментов отряды проходили сквозь замок и «доходили». Поправлен.
- Красные шлюза — известные на HEAD или мигающие: `qa_rally2` Bб/C5/F2/F3/
  F7/F8 (Bб краснеет и с выключенной ручкой), `qa_mass_siege` B1,
  `qa_tower` A5, `qa_roof_garrison` A5/B3, `qa_gnoll_fix` A3/A4,
  `qa_gnoll_behavior` B2б/B3, `qa_goblin_tactics` B2/D1, `qa_ai_loop` C1,
  `qa_mega_battle` B2-0, `qa_guard` J5, `qa_fog` D3/D4/D6а, `qa_tz0919` C5,
  `qa_sprint19c` A1/F7, `qa_balance` E1/E2, `qa_settle` C2, `qa_disengage`
  A4, `qa_sprint18b` B10/B11, `qa_volley` C1, `qa_squad` 8,
  `qa_cliff_bypass` C5; `qa_sheep` C3 и `qa_sprint19b` B6 — одиночно
  зелёные в обе стороны ручки.

### Audio System Update (ТЗ 19.09.2026, стенд `qa_audio_update` 19/19)

- **Нарезка пака** `voicebosch-monster-grunts-174559.mp3` (88 с, 44 сэмпла
  с паузами ~2 с) — `qa_audio_env/Slice.tscn`: headless-захват PCM через
  `AudioEffectCapture` (dummy-драйвер микширует в реальном времени),
  отрезки по тишине (окно 10 мс, порог 6 % пика, пауза 0.16 с), нормировка
  к −3 дБ, фейды 6 мс, `AudioStreamWAV.save_to_wav`. Первая половина файла
  → `Troll_voice/troll_grunt_1..21.wav`, середина → `Goblins_voice/
  big_grunt_1..8.wav`, завершающая треть → `goblin_grunt_1..15.wav`.
- **Банк:** `knight_angry` (angry1-4), `knight_victory` (excited2),
  `troll_grunt`, `big_grunt`, `goblin_grunt`; у всех `"shuffle": true` —
  файл берётся из `AudioManager.rng` без повтора подряд (`_cat_last_file`);
  прочие категории по-прежнему из общего `randi()` (сеяные стенды).
- **Шанс и кулдаун на отряд — `AudioManager.squad_voice(cat, key, at, chance,
  gap)`:** окно открывается раз в gap на ключ (отряд; одиночка — свой узел),
  жребий ОДИН на окно (иначе сотня бойцов одного приказа выбивала бы 30 %
  наверняка). `GRUNT_CHANCE` 0.30, `GRUNT_SQUAD_GAP` 1.75, марш —
  `GRUNT_MOVE_GAP` 4.0, рыцарь — `KNIGHT_ANGRY_GAP` 2.0.
- **Хуки — свойства рода войск** (`Unit._sfx_grunt / _sfx_hurt /
  _sfx_move_grunt`, база молчит): удар состоялся (обе дороги касания и свипы
  тролля/туши), урон принят (`take_damage`, живой), приказ марша дальше 6 м
  (`command_move`). Тролль — `troll_grunt` на удар и урон; туша —
  `big_grunt` на свип, урон и клич (`_sfx_shout`); гнолл, гоблин, всадник —
  `goblin_grunt` на удар, урон и марш.
- **Рыцари:** `Warrior._angry_pending` взводится `start_rage_dash`, снимается
  первым состоявшимся ударом (`_after_melee_hit`) — `knight_angry` один на
  отряд в окне; добил последнего в чужом отряде мечник —
  `knight_victory` из `GameManager._on_squad_wiped`
  (`knight_victory_events`).
- **Ловушки стендов:** `play_3d` вдали от слушателя отсекается до пула
  (SFX_CULL_DISTANCE) — shuffle мерить у слушателя; голосов у категории
  2-4 и сэмплы 2-3 с — 16 запусков подряд не прозвучат, мерить на коротком
  пуле с паузой; **`qa_audio18` F7 утверждал исход лотереи 0.34** — новый
  потребитель `AudioManager.rng` (грюнты) сдвинул жребий; переписан на
  серию добиваний (событие на каждое, клич хотя бы раз).

## 19.09.2026 (ночь-2) — ТЗ «Фикс обтекания зданий и навигации»: вплотную, единым строем, ворота свободны

Два скриншота владельца через час после ввода фундаментов: отряд копейщиков
у башни разошёлся надвое, лучники наверху топчутся, у левой башни отряд
«бегает по кругу», гружёные рабочие дёргаются у крепости, гарнизон входит и
выходит с задержками. Стенд — **`qa_building_navigation`** (25 проверок,
headless; ключи `-- skip=abc trace=N`): застройка (крепость, 2 барака, 4
башни), золотая жила слева-сзади крепости (прямая к воротам — через
фундамент), два массива деревьев, 10 рабочих, 2 отряда лучников, 6 отрядов
копейщиков; блоки A рабочие (120 с ходок), B гарнизон (башня и крыша
крепости, дважды), C марш 8 отрядов сквозь застройку вправо и обратно.

**Базовая линия на прежнем коде (тот же стенд): 15/25.** Расщепление
отряда — 129 проб, выход из башни 12/20 и из крепости 5/20, ни один из 8
отрядов не пришёл за 45 с, отставших 4.

### Что было не так — пять причин, все найдены стендом

1. **Сетка навигации не видела башен.** Ячейка 2 м помечалась стеной, только
   если её ЦЕНТР лежал внутри круга фундамента (`NavBldPad` 0.30): башня
   радиусом 0.9 между центрами четырёх ячеек не попадала в сетку вовсе. A*
   не строил обход, прямая считалась свободной, и башню обходил ШАГ —
   скольжением вдоль круга, сторона по ЧЁТНОСТИ СТРОКИ ядра: соседи по
   шеренге расходились по разные стороны, отряд рвался надвое (скриншот 1).
   Теперь ячейка — стена, если её прямоугольник ЗАДЕВАЕТ круг (зазор
   `NavBldPad` 0.25 = тело), а сторона скольжения у шага — ПАМЯТЬ
   (`_bldSide`: держится, пока боец трётся об этот фундамент; первый
   лобовой контакт — по отряду, не по строке).
2. **Обход зданий строился скальным отступом.** Угол нити отжимался от
   ближайшей стены на `max(3.0, clear) + halfW` = 6.5 м, нить — на
   1.6 + 3.5 = 5.1 м, и шаг у стены платил штраф скалы: вокруг башни в 2 м
   выходил крюк в 13 м. Здание — не скала: у `_navWall` (дистанция до стены
   для штрафа) остались только скалы и вода, сосед ячейки дома платит
   `NavBldAdjCost` 0.5, боковой зазор нити у дома `NavBldSidePad` 0.9, угол
   отжимается на `NavBldCornerPush` 1.0 и **подтягивается к настоящему
   кругу** (`BldHug`: шагами к ячейке стены, пока круг не в зазоре
   `NavBldHug` 0.8 — по точным кругам, не по ячейкам 2 м). Замер: отряд по
   оси крепости проходит в 0.2-0.6 м от края фундамента, крюк центра ×1.00.
3. **Фланг строил свою дорогу.** `squad_leg` считал дорогу центра прямой,
   пока свободна ПРЯМАЯ центра; боец, чья прямая упиралась в башню, получал
   личный A* — с той стороны, где ему короче. Теперь у `NavPath` ранний
   выход проверяет ПОЛОСУ ±halfW против ячеек построек (`NavBandBlockedBld`),
   полуширина — по смещению бойца двумя ступенями (3.5 / 6.0, кэш по
   ступени): полоса задела дом — A* от центра, весь отряд огибает с одной
   стороны, а `NavSpread` гасит смещение, легшее в дом (сжатие в гармошку у
   угла, за углом — прежняя разметка).
4. **Личный обход дописывался В КЭШ.** `squad_leg` строил личный обход до
   первого угла отрядной нити и делал `pre.append_array(pts)` — а Packed-
   массивы в GDScript идут ПО ССЫЛКЕ, `pre` был объектом `_nav_cache`.
   Каждый следующий боец получал маршрут длиннее на нить: 3, 5, 7 … 25
   точек `[угол, угол, угол …]`, и, дойдя до цели, шёл по «второму кругу»
   назад — это и есть «бегают по кругу вокруг здания» и «топчутся» со
   скриншота (трасса стенда: центр отряда уехал на 40 м назад без единого
   command_move). `duplicate()`. Баг был и до фундаментов — с ними стал
   виден на каждом доме.
5. **Подзыв сплочённости выдёргивал идущих.** Ворота «идущего далёким маршем
   не зовём» мерили дальность по `move_target` — у бойца на обходе это
   ближайший угол, а не цель; воронка у угла растягивала отряд до 14.6 м
   при зоне 14, передние в трёх метрах от своих точек получали приказ назад
   к медиане, медиана уезжала за ними — каскад. Теперь идущий по приказу
   (MOVING) подзывом не трогается вовсе, стоящий на своём посту — тоже
   (`Unit.march_goal()` — конечная точка хода, читают внешние).

### Ворота и сдача ресурсов

- **Сдача — у ворот, а не в центре.** `Worker._process_return` шёл к
  `drop_off.global_position` и сдавал в 1.8 м от него — центр крепости лежит
  ВНУТРИ фундамента, с боков и с тыла до него не дойти никогда (скриншот 2:
  гружёные у стены). `Building.deposit_point()` = ворота, `at_deposit(p)` —
  круг `DEPOSIT_RADIUS` 2.4 у ворот либо полоса `DEPOSIT_WALL_PAD` 0.6 у
  нарисованного края (толчея выдавила к стене — тоже сдал). Те же точки у
  ходок с мясом и переноски овцы в замок (`_dest_point` / `_at_dest`).
- **Видимость прямой прощает концы у стены.** Ворота и точка сдачи лежат в
  ЯЧЕЙКЕ постройки (сетка грубее фундамента): проба нити в пределах ячейки
  от конца, который сам вне кругов, не считается — иначе к воротам «дороги
  нет» никогда, и боец у ворот переспрашивал A* каждые 0.4 с.
- Вход в башню и крышу крепости: 20 из 20 за 0.4-10 с (бюджет 13-25),
  выход 20 из 20 за 0.3-1.3 с, дважды. Рабочие: 38 ходок за 120 с, худший
  простой 0.5 с, разворотов 2.

### Итог стенда: 25/25 (было 15/25)

Марш 8 отрядов сквозь застройку: пришли за 38-39 с из 60, отставших 0, тел
в фундаменте 0, расщеплений 0 (было 129), поперечник после прохода —
исходный, крюк ×1.00-1.01, ближайший подход к стене 0.2-0.6 м.

### Ловушки

- **Packed-массив из кэша — по ссылке.** Любой `append_array` / `push_back`
  на ответе `nav_route` / `squad_leg` правит кэш; сперва `duplicate()`.
- Метрика «крюк ×0.97 < 1» у пришедшего отряда — норма: центр меряется до
  прихода в 4 м, длина хорд по 15 кадров.
- Стенд ставит жилу СЗАДИ крепости: с жилой перед воротами сдача проходила
  и на старом коде (базовая линия: A2 зелёный), и баг не ловился.

### Шлюз 54 стенда после правок (20.09.2026, 00:30-02:00) — регрессий нет

SCRIPT ERROR 0 (после правки стенда `qa_gnoll_fix`, см. ниже). Красные —
известные на HEAD: `qa_cliff_bypass` C5 (69 % при мигании 57-91),
`qa_mass_siege` B1 (дерущиеся вдали, мигал и до правок: 1/631 и 7/539),
`qa_tower` A5, `qa_squad` 8, `qa_settle` C2, `qa_rally2` Bб/C5/F2/F3/F7/F8
(+A1/A4 — блок A бимодален на одном коде: пик с целью 20 → 8 в двух
прогонах подряд), `qa_roof_garrison` A5/B3 (рассадка ТЗ 19.09-2),
`qa_ai_loop` C1, `qa_mega_battle` B1-1/B2-1, `qa_sprint19c` A1/F7,
`qa_guard` J5, `qa_goblin_tactics` D1, `qa_gnoll_fix` A3/A4;
`qa_river_crossing` B5 (вытянутость ×5.0) — только под нагрузкой шлюза,
одиночно ×1.2 и 14/14.
**`qa_gnoll_fix` E ронял «previously freed» три прогона из четырёх:**
площадка блока E лежит в 30 м от «красного пня», гноллы патрулируют
кольцом `GNOLL_PATROL_RADIUS` 26 м и с 13 м бросают кость в ЗАМОРОЖЕННОГО
лучника стенда (`_slain_by` — гнолл sq48 в 13-16 м). Прежде везло по
фазе патруля; стенд теперь замораживает диких пня на время блока.
