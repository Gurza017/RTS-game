...
func slot_radius() -> float:
	if resource_type == Constants.RESOURCE_WOOD:
		# ВПЛОТНУЮ К СТВОЛУ, НО СНАРУЖИ ЕГО ЗАПРЕТНОГО КРУГА. Было 1.1 + зазор
		# = 1.65 м от центра дерева, то есть рабочий вставал под кроной и рубил
		# воздух в метре от комля. Кольцо считается от толщины СТВОЛА, а не от
		# габарита спрайта, и лезвие ложится ровно на дерево.
		#
		# ── ЗАЗОР БОЙЦА ВХОДИТ В ФОРМУЛУ, А НЕ ПОДРАЗУМЕВАЕТСЯ ──────────────
		# Ствол непроходим не сам по себе: солвер не пускает ЦЕНТР бойца ближе
		# чем TRUNK_RADIUS + Unit.TRUNK_CLEARANCE (0.35 + 0.22 = 0.57 м). Точка
		# рубки обязана лежать СНАРУЖИ этого круга — и с запасом, потому что
		# рабочего постоянно двигают разбор наложения (до метра в секунду) и
		# доводка к стволу.
		#
		# ЗДЕСЬ СТОЯЛО `TRUNK_RADIUS + SLOT_GAP` = 0.65 м, то есть запас ВОСЕМЬ
		# САНТИМЕТРОВ, и рабочий проскакивал его за один тик. Дальше он попадал
		# в положение, из которого не выбраться: обход стволов умеет только НЕ
		# ПУСКАТЬ внутрь, срабатывает на шаге и правит НОВУЮ точку, а рубящий
		# стоит на месте и шага не делает вовсе. Это и есть «зашёл внутрь
		# дерева и бесконечно дёргается» со скриншота владельца.
		#
		# Комментарий рядом при этом уже описывал 0.9 м («0.35 + 0.55»): SLOT_GAP
		# позже уменьшили с 0.55 до 0.30 ради РУДЫ, где та же константа, — и
		# запас у дерева молча упал втрое. Теперь он выводится, а не совпадает:
		# 0.35 + 0.22 + 0.30 = 0.87 м, то есть 0.30 м снаружи запретного круга.
		# Стережёт qa_helpbuild, проверка E0
		return TRUNK_RADIUS + Unit.TRUNK_CLEARANCE + SLOT_GAP
	# ── У РУДЫ ТО ЖЕ ПРАВИЛО, И ОНО ТЕПЕРЬ ЗАПИСАНО, А НЕ СОВПАДАЕТ ─────────
	# Точка добычи обязана лежать СНАРУЖИ запретного круга (радиус препятствия
	# плюс личный зазор бойца) — иначе рабочий встаёт внутрь собственной жилы и
	# дёргается, потому что вытолкнуть стоящего некому. У дерева это выведено
	# явно (строка выше), у руды же держалось на СОВПАДЕНИИ констант:
	# препятствие 0.5 × size_scale, а кольцо 1.0 × size_scale, то есть вдвое
	# дальше. Замер по трём классам кусков (0.62 / 1.05 / 1.60): запреты
	# 0.53 / 0.75 / 1.02 против колец 0.92 / 1.35 / 1.90 — сегодня безопасно.
	# Но ровно так однажды молча сломалось ДЕРЕВО: SLOT_GAP уменьшили с 0.55 до
	# 0.30 ради руды, и запас у дерева упал втрое, а комментарий рядом
	# продолжал описывать прежнее число. Пол по запретному кругу ничего не
	# меняет сегодня и не даст сломать это завтра
	var floor_r: float = _obstacle_radius() + Unit.TRUNK_CLEARANCE + SLOT_GAP
	return maxf(maxf(1.0 * size_scale, _visual_half_width) + SLOT_GAP, floor_r)
...
## `renew` — «дай ДРУГОЕ место, это не сработало». Просят его тогда, когда
## рабочий признан застрявшим по дороге и выключение стволов ему не помогло
## (см. Worker.STUCK_REPATH_TICKS): значит, держит его не ствол, и менять надо
## саму точку. Прежний слот при этом считается ЗАНЯТЫМ — иначе выбор
## «ближайшего свободного» вернул бы ровно его же, ведь рабочий стоит рядом
func claim_slot(who: Node3D, renew: bool = false) -> Vector3:
	...
	if _mine != null:
		return _mine.claim_slot(who, renew)
	if who == null:
	...
	if pick < 0:
		# Просили ДРУГОЕ место, а свободных нет вовсе — оставляем прежнее.
		# Выдать точку за внешним кольцом здесь было бы хуже: рабочий уехал бы
		# от жилы дальше, чем стоял, и перестал бы до неё доставать
		pick = avoid if avoid >= 0 else slot_total() + (id % SLOT_MAX)
	_slot_owner[id] = pick
...
func pick_body_h() -> float:
	...
	# центр рисунка — на его половине. ВЫВЕДЕНО из той же ручки, а не стоит
	# числом: правка размера кусков обязана доезжать и до разбора клика
	return PIECE_TARGET_H * 0.5 * size_scale
...
func hover_ring_lift() -> float:
	if resource_type != Constants.RESOURCE_WOOD or _veg_tex == null:
		return 0.0
	# ── ПОЛЕ МЕРЯЕТСЯ ТЕМ ЖЕ ОБМЕРОМ, ЧТО И У ПОСТРОЕК ─────────────────────
	# Здесь был свой скан по Image.get_used_rect(), и он считал непрозрачным
	# ЛЮБОЙ ненулевой пиксель. Шейдер растительности режет пустоту по
	# ALPHA_SCISSOR_THRESHOLD = 0.5, то есть всё, что бледнее половины, на
	# экране отсутствует вовсе — а обмер его засчитывал, и кольцо садилось
	# на 0.19-0.26 м НИЖЕ нарисованного комля (жалоба владельца по скриншоту
	# ели: «основание ствола не в центре эллипса»).
	# BillboardUtil.opaque_rect считает по ТОМУ ЖЕ порогу, что режет шейдер,
	# и заодно кэширует результат по пути ресурса — свой кэш здесь больше не
	# нужен. Заодно исчезла вторая реализация одного и того же обмера
	var r: Rect2 = _BBUtil.opaque_rect(_veg_tex)
	var frac: float = clampf(1.0 - r.end.y, 0.0, 0.4)
	# ── ПОЛЕ МЕРЯЕТСЯ В КАДРЕ, А РИСУЕТСЯ РАСТЯНУТЫМ ────────────────────────
	# Квад растительности тянет по вертикали шейдер (veg_multimesh, uniform
	# v_stretch — компенсация наклона камеры, см. BillboardUtil.V_STRETCH), и
	# тянет ОТ НИЖНЕЙ КРОМКИ: низ остаётся на грунте, всё выше него уезжает
	# вверх в V_STRETCH раз. Значит и прозрачное поле под стволом на экране в
	# V_STRETCH раз выше, чем в мировых метрах квада.
	# Без этого множителя кольцо поднималось на 71% нужного и всё равно
	# оставалось «на траве под деревом» — ровно та жалоба, ради которой поле и
	# начали мерить
	return frac * TREE_HEIGHT * _BBUtil.V_STRETCH
...
## ПЛОТНОСТЬ ПРИВЯЗАНА К ЮНИТАМ (заказ владельца, сент. 2026: «перерастянутый
## комок из гигантских пикселей»). Пиксель бойца — 0.0108 м (Spearman.PIXEL_SIZE);
## прежние 4.4/128 и 3.8/64 давали тексель руды 0.034 и 0.059 м — в 3-5.5 раза
## крупнее юнитского, и рядом с солдатом жила читалась крупнопиксельной кашей.
## Теперь 2.0/128 = 0.0156 и 1.4/64 = 0.0219: заметно ближе к бойцу (камень
## нарисован на холсте 64 px, мельче его пиксель не сделать без нового арта)
const FRAME_METRES_STONE := 1.55
const FRAME_METRES_GOLD  := 2.0

## Целевая высота куска класса 1.0 (см. Main.PIECE_CLASSES: big 1.6, mid 1.05,
## small 0.62 — то есть примерно 1.36 / 0.89 / 0.53 м). Это и есть «ручка размера».
## Подобрана так, чтобы при классовых вариантах рисунка (Main.ORE_VARIANT_POOLS)
## зум почти не работал: крупный класс получает крупно НАРИСОВАННЫЙ вариант и
## дотягивается до цели в пределах 1.25, а не надувается втрое
const PIECE_TARGET_H := 0.85
## Насколько разрешено отходить от родной высоты рисунка ради целевой.
## ZOOM_MAX держим тугим: именно многократное увеличение и превращало
## пиксельный камень в мыло. 1.25 — мягкое пересэмплирование, крупный кусок
## делается выбором крупного АРТА (см. Main.ORE_VARIANT_POOLS), а не зумом
const ZOOM_MIN := 0.55
const ZOOM_MAX := 1.25
...
func _maybe_load_sprite() -> void:
	...
	# ── ЖИЛА РИСУЕТСЯ ОДНИМ КВАДОМ, БЕЗ АДДИТИВНОГО БЛИКА ──────────────────
	# `*_Highlight.png` — НЕ слой блика поверх жилы, а САМА ЖИЛА, нарисованная
	# шестью кадрами с ходящим по слиткам глянцем. Её и рисуем, штатным кадровым
	# путём (см. выбор `path` в начале функции): один квад, один обычный
	# материал, никакого аддитивного смешивания и никаких пятен на траве
