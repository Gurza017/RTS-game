...
const STATS := {
	...
	"warrior": {
		"health": 200.0,                     # 150 HP — три с половиной десятка ударов копья
		...
		"armor": 5.0,                     # броня: доля снижения, см. ARMOR_SOFTNESS
		"morale": 75.0,                     # мораль 0..100 — тяжёлая пехота держится
		"push_force": 1.7,                   # напор: рыцарь продавливает строй
...
const PLAYER_STARTING_RESOURCES := {
	Constants.RESOURCE_WOOD:  22550.0,     # дерево: замок, рабочие, дома
	Constants.RESOURCE_GOLD:  22380.0,     # золото: найм, исследования, способности
	Constants.RESOURCE_STONE: 22340.0,     # камень: только постройки
	Constants.RESOURCE_FOOD:  2200.0,     # еда: капает с домов, тратится на найм
}
...
const BUILDINGS := {
	"castle": {
		...
		"worker_buildable": false,                # кнопки у рабочего нет: замок особый
	...
	"mine": {
		"name": "Рудник",                         # подпись в интерфейсе
...
## ДОМ — ЕДИНСТВЕННЫЙ ИСТОЧНИК ЕДЫ У ЛЮДЕЙ (у орды это хижина)
const HOUSE_FOOD_INCOME   := 4.0   # сколько еды даёт один дом за начисление
