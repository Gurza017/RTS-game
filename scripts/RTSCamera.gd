...
func _process(delta: float) -> void:
	...
	# ВМЕСТЕ С РАДИУСОМ ВИДИМОЙ ПОЛОСЫ: на отдалённом зуме камера показывает
...
## Радиус круга, накрывающего ВСЮ видимую на земле область, плюс запас на один
## габарит спрайта. Круг, а не прямоугольник, потому что отсечение по LOD
## считается расстоянием от точки фокуса — и делать его точнее незачем: ошибка
## идёт в сторону «посчитали лишнего», то есть в сторону правильной картинки
func _lod_ground_radius() -> float:
	return _visible_ground_half(size).length() + 4.0
