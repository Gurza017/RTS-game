...
## ── ПОРОГ ТОТ ЖЕ, ПО КОТОРОМУ РЕЖЕТ ШЕЙДЕР ─────────────────────────────────
## Здесь стояло 0.05, то есть «непрозрачным» считался любой еле заметный
## след альфы. А на экране пустоту отсекает ALPHA_SCISSOR_THRESHOLD, и во всех
## билбордах проекта (cyl_billboard, veg_multimesh, mm_unit_sprite, mm_corpse)
## он равен 0.5. Пиксели между 0.05 и 0.5 НЕ РИСУЮТСЯ ВОВСЕ — а обмер их
## засчитывал, и всё, что от него зависит, оказывалось ниже и шире рисунка.
##
## Замер (зонд по Tree1-4): нижняя кромка по альфе > 0 лежит на 0.027-0.125
## высоты кадра, а по срезу 0.5 — на 0.063-0.162. В метрах пятиметрового квада
## это 0.19-0.26 м, ровно тот отрыв кольца от комля, который видно на скриншоте
## владельца («основание ствола не в центре эллипса»).
##
## Число обязано ехать вместе со срезом: разойдутся — и обмер снова начнёт
## описывать не то, что нарисовано
const _OPAQUE_A    := 0.5
