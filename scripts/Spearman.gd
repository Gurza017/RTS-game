...
func _process_can_sleep() -> bool:
	...
	if _spear_ready_ms > 0 and now_ms < _spear_ready_ms:
		return false
...
## ── РАЗБРОС СОКРАЩЁН ПО ЗАКАЗУ ВЛАДЕЛЬЦА ────────────────────────────────────
## Было 1900 мс. Заказ звучал как «фаланга опускает копья не по-механически,
## раз-раз-раз», и разброс в две секунды его выполнял — но он же и породил
## следующую жалобу: «копья выставили один-два человека, приходится нажимать
## кнопку дважды». Второе нажатие ничего не меняло (перехода в стойку уже нет,
## личный срок не переписывается) — оно просто давало время дожить разбросу.
## 300 мс — это по-прежнему ВОЛНА, а не шлагбаум, но глазом она читается как
## «нажал — и строй выставил копья»
const DROP_DELAY_MAX_MS := 300
...
## ── РАЗВОРОТ: КОПЬЁ ОПУСКАЕТ ТОТ, КТО ДЕРЁТСЯ, А НЕ ТОЛЬКО ТОТ, КОМУ
## ПРИКАЗАЛИ ─────────────────────────────────────────────────────────────────
## ЖАЛОБА ВЛАДЕЛЬЦА: «копейщики идут в атаку с копьями строго вверх».
##
## Прежнее правило («копья выставляются только под ПРЯМОЙ приказ по конкретной
## цели») сохраняло старое «в стойке АТАКА копья подняты» — и было верным ровно
## наполовину. Отряд получает цель не только кликом: авто-агро, подтягивание
## фланга, ответ на удар, перехват заслоном. Во всех этих случаях `_charge_order`
## ложен, и копейщик честно ДРАЛСЯ с вертикальным древком.
##
## Разделять надо не «приказали или нет», а «дерусь или иду»: копьё поднято у
## того, кто ИДЁТ, и опущено у того, кто ВОЮЕТ. Признак боя берётся отрядный и
## уже готовый (ответ кэшируется на физический кадр, своего скана не заводится),
## а `attack_target != null` остаётся обязательным — без цели опускать копьё не
## перед кем.
##
## Правило «на бегу фаланги нет» это не трогает: `sprinting` проверяется выше и
## перебивает всё (см. _spear_leveled)
## ── КОПЬЁ ОПУСКАЕТ ТОТ, КТО ДЕРЁТСЯ, А НЕ ТОЛЬКО ТОТ, КОМУ ПРИКАЗАЛИ ───────
## ЖАЛОБА ВЛАДЕЛЬЦА: «копейщики идут в атаку с копьями строго вверх».
##
## Прежнее правило («копья выставляются только под ПРЯМОЙ приказ по конкретной
## цели») верно ровно наполовину: цель отряд получает не только кликом —
## авто-агро, подтягивание фланга, ответ на удар, перехват заслоном. Во всех
## этих случаях `_charge_order` ложен, и копейщик ДРАЛСЯ с вертикальным древком.
##
## Делить надо не «приказали или нет», а «дерусь или иду»: копьё поднято у того,
## кто ИДЁТ, и опущено у того, кто ВОЮЕТ.
##
## ── ПОРЯДОК РАБОТ ЗДЕСЬ РЕШАЛ ВСЁ, И ПЕРВАЯ ПОПЫТКА ЕГО НАРУШИЛА ──────────
## Ровно это правило уже вводилось и было ОТКАЧЕНО: qa_formation D1 показал
## «копья опустили 19 из 20, фронт 8, тыл 8» — древко опустил ВЕСЬ отряд.
## Причина была не в правиле, а в том, что ему не на что было опереться: кого
## считать передовым, знает `_live_rank`, а он не считался НИКЕМ вне стойки
## ЗАЩИТА и оставался нулевым у всех, так что гистерезис по ряду пропускал
## поголовно. Сперва ранг (GameManager._push_squad_ranks, фильтр по стойке
## снят) — и только потом это условие.
##
## Признак боя берётся отрядный и уже готовый (ответ кэшируется на физический
## кадр, своего скана не добавляется), а `attack_target != null` обязателен:
## без цели опускать копьё не перед кем. Правило «на бегу фаланги нет» не
## затронуто — `sprinting` проверяется выше и перебивает всё
func _charging() -> bool:
	if attack_target == null:
		return false
	if _charge_order:
		return true
	return squad_id > 0 and GameManager.squad_in_combat(squad_id)
...
func _spear_leveled() -> bool:
	...
	if holds != _was_holding:
		...
		_spear_ready_ms = (now_ms + _own_drop_delay_ms()) if holds else 0
	if not holds:
		_spear_down = false
		return false
	# ── ГИСТЕРЕЗИС ПО РЯДУ ───────────────────────────────────────────────────
...
## ── СРОК ЛИЧНОЙ ЗАДЕРЖКИ ИСТЁК — ПОЗУ ПЕРЕСЧИТАТЬ СЕЙЧАС ────────────────────
## Полный разбор позы идёт по расписанию (раз в anim_every визуальных тактов,
## на большой армии это до полусекунды). Личная задержка опускания копья
## истекает между двумя такими разборами, и без пометки копьё оставалось
## поднятым до следующего планового пересчёта — то есть к сокращению разброса
## прибавлялось бы всё то же ожидание.
##
