...
## РЕЖИМ ЗАПОМИНАЕТСЯ между растягами и переживает смену выделения: игрок
## выбрал коробочку — следующий растяг тоже коробочка, пока он сам не нажмёт
## [1]. Превью перерисовывается тем же кадром (см. _formation_key), а текущий
## режим показан в верхней панели — иначе запомненный строй читается как
## самоволка игры (разбор — у места, где раньше стоял сброс)
const FORM_WIDE := 1
const FORM_DEEP := 2
...
# ─────────────────────────────────────────────────────────────────────────────
## Во сколько раз ТЫЛОВОЙ эшелон шире своей честной доли фронта (заказ
## владельца). Действует только на эшелоны ПОСЛЕ первого, то есть только в
## смешанном выделении
const BACK_ECHELON_WIDEN := 2.0
## Глубже скольких шеренг тыловому эшелону вставать нельзя
const BACK_ECHELON_MAX_ROWS := 3

## Ширина линии, на которой эшелон уложится не глубже BACK_ECHELON_MAX_ROWS.
## Считается ТОЙ ЖЕ арифметикой, какой раскладывает места _block_formation_slots
## (доли по численности отрядов плюс просветы между блоками), иначе проверка
## судила бы об одном строе, а раскладка выдавала другой
func _echelon_flat_width(bucket: Array) -> float:
	var blocks := _split_into_blocks(bucket)
	if blocks.is_empty():
		return 0.0
	var men: int = bucket.size()
	var per_row: int = int(ceil(float(men) / float(BACK_ECHELON_MAX_ROWS)))
	# +1 к каждому блоку: per_row в раскладке считается ПОЛОМ от доли, и без
	# запаса последний человек шеренги уезжал бы в следующую
	return float(per_row + blocks.size()) * UNIT_SPACING 		+ float(maxi(blocks.size() - 1, 0)) * BLOCK_GAP
...
func _layered_formation_slots(line_start: Vector3, line_end: Vector3, movable: Array) -> Dictionary:
	...
	for bucket in _Formations.group_by_rank(movable, formation_front):
		...
		if not deep_b and b.size() < front_men and full_len > 0.001:
			...
			# Жалоба владельца (скриншот растянутого фронта): копейщики стоят
			# широкой линией, а лучники и рыцари за ними — «колбасами» по
			# четыре-пять шеренг в глубину.
			#
			# Так и выходило по построению: доля ширины бралась ЧЕСТНО по
			# численности (двадцать лучников за восьмьюдесятью копейщиками —
			# четверть фронта), а глубина строя от доли и обратна. Честная
			# доля даёт одинаковую ПЛОТНОСТЬ, но игрок просит не плотность, а
			# ЛИНИЮ: тылу положено стоять в две-три шеренги, иначе стрелять
			# может только его передний ряд, а рыцарям некуда выехать.
			#
			# Считаются ДВА требования, берётся большее:
			#   • удвоенная доля — прямой заказ («коэффициент x2 по ширине»);
			#   • ширина, при которой эшелон физически не глубже
			#     BACK_ECHELON_MAX_ROWS шеренг.
			# Оба зажимаются длиной самой линии: шире фронта тыл не встанет.
			#
			# ОТДЕЛЬНО ВЫДЕЛЕННЫЙ ОТРЯД СТРЕЛКОВ СЮДА НЕ ПОПАДАЕТ ВОВСЕ: он
			# первый и единственный эшелон, front_men равен его численности, и
			# ветка не срабатывает — раскладка у него прежняя, слово в слово
			var want: float = full_len * float(b.size()) / float(front_men) 				* BACK_ECHELON_WIDEN
			want = maxf(want, _echelon_flat_width(b))
...
func _pick_at(screen_pos: Vector2, mask: int) -> Dictionary:
	...
	var best_radius := INF
	var best_rank := 99
	## Насколько высоко по картинке победителя стоит курсор (см. разбор ниже)
	var best_height := -INF
	...
	## Длина наклона в квадрате: делитель в пересчёте «отрыв по земле → высота
	## по картинке». Считается один раз на клик, а не на кандидата
	var lean_len2: float = lean.length_squared()
	for node in cands:
		...
		if ground.x == INF:
			...
			# Здания попадают сюда ТОЛЬКО лучом, а ловит луч пластину, подогнанную
			# под непрозрачную часть рисунка (Building._fit_pick_to_sprite):
			# попадание в пластину и попадание в картинку — одно и то же событие.
			# Считать зданию «промах по земле» после этого незачем и вредно
			score = 0.0
		else:
			score = maxf(anchor.distance_to(ground) - radius, 0.0)
		# ── ГДЕ ИМЕННО ПО КАРТИНКЕ КАНДИДАТА СТОИТ КУРСОР ──────────────────
		# Луч идёт под наклоном, поэтому вдоль него высота и глубина связаны
		# жёстко: точка земли под курсором лежит ровно на lean × h от основания
		# того, чей рисунок курсор накрыл на высоте h. Значит h считается из
		# одного вычитания — и это ЕДИНСТВЕННАЯ величина, по которой можно
		# сказать, кто нарисован ПОВЕРХ: чем выше по своему спрайту накрыт
		# кандидат, тем ближе он к камере
		...
		if score < best_score - PICK_TIE:
			...
			# ── СЧЁТ РАВНЫЙ: РЕШАЕТ, КТО НАРИСОВАН ПОВЕРХ ──────────────────
			# ЖАЛОБА ВЛАДЕЛЬЦА: «клик по крепости срабатывает с пятого-десятого
			# раза, буквально пролетает сквозь спрайт; если рядом пробегают
			# крестьяне, клик перехватывают они».
			#
			# ЗАМЕР (зонд qa_keep) объясняет всё. Курсор на стене замка, на 78%
			# высоты рисунка: точка земли под таким курсором лежит в 6.28 м ЗА
			# зданием, а из счёта здания вычитался полугабарит его КОРОБКИ —
			# всего 4 м. Оставалось 2.28, тогда как рабочему, случайно стоящему
			# в этой точке, доставался ноль. Замок проигрывал рабочему, которого
			# сам же и загораживает.
			#
			# Ранг («боец важнее здания») этого не спасал, а усугублял: он и есть
			# то правило, по которому рабочий выигрывал. Оно верно ровно в одном
			# случае — когда боец СТОИТ ПЕРЕД стеной, и тогда курсор накрывает
			# его выше, чем здание (у бойца h ≈ рост, у здания h ≈ 0). Поэтому
			# высота идёт ПЕРЕД рангом, а ранг решает при равной высоте — то
			# есть ровно тогда, когда оба нарисованы в одном месте
			if height > best_height + PICK_TIE:
				better = true
			elif height >= best_height - PICK_TIE:
				# Высота одинаковая: решает сначала ранг, затем размер — по
				# мелкой цели попасть труднее, она и важнее
...
## ── ВЫЧЕРКНУТЬ БОЙЦА ИЗ ВЫДЕЛЕНИЯ ───────────────────────────────────────────
## Зовёт GameManager.forget_on_map, когда боец уходит с карты ЖИВЫМ (гарнизон).
## Кольцо снимает вызывающий: здесь только учёт, чтобы панель и приказы игрока
## не адресовались тому, кого на карте нет.
##
## СИГНАЛ ИДЁТ МОЛЧА (silent). Звук выделения означает КЛИК ИГРОКА, а вход в
## замок — событие игры (то же правило, что у закладки и достройки замка,
## см. GameManager.on_selection_changed)
func forget_unit(node: Node) -> void:
	if not _sel_set.has(node):
		return
	...
	if is_gather_cmd or is_attack_cmd:
		...
		if is_attack_cmd:
			...
			if target is Building:
				...
				# ТЕМ ЖЕ полем `front`, каким раздаются участки чужого фронта:
				# второго пути назначения цели заводить нельзя
				var spread: Dictionary = _ring_squads_around(target as Building)
				for sid_r in spread:
					front[sid_r] = spread[sid_r]
...
## Просвет между соседними блоками отрядов, метры.
##
## ИСТОРИЯ РУЧКИ: 3.0 → 1.0 (заказ «плотнее, группа расползается»)
## → снова вверх по новому заказу владельца: «выделить 2-3 отряда и
## отправить в точку — целевые точки накладываются, отряды наезжают
## друг на друга и дёргаются на месте».
##
## МЕТР БЫЛ МЕНЬШЕ, ЧЕМ ХОЧЕТ САМА ФИЗИКА. Бойцы РАЗНЫХ отрядов
## держатся шире своих — разбор наложения разводит их в Unit.SEP_CROSS_SQUAD
## раз сильнее (см. ArmyCore.BatchSeparation). Ставь блоки ближе, чем это
## расстояние, — и крайние шеренги соседних отрядов оказываются внутри
## чужого личного круга: каждый честно идёт в свою точку разметки, а
## расталкивание тут же выталкивает его обратно. Это и есть «дёргаются
## на месте» — цикл «сжатие → выталкивание», тот же самый, что ловился
## у заслона ИИ (см. EnemyAI._screen_step).
##
## ПОЭТОМУ ПРОСВЕТ ВЫВОДИТСЯ, А НЕ СТОИТ ЧИСЛОМ: он равен тому
## расстоянию, на котором чужие бойцы всё равно встанут сами, с небольшим
## запасом на шеренгу с каждой стороны. Правка баланса или плотности
## строя доедет сюда сама — точно так же, как шаг заслона у ИИ
const GROUP_CELL_GAP := Unit.SEP_CROSS_SQUAD * Unit.SEP_MIN_DIST + 2.0
...
## ── СКОЛЬКО ИЗ НИХ БОЕВЫХ ──────────────────────────────────────────────────
## ЖАЛОБА ВЛАДЕЛЬЦА: «выделяю рамкой пять крестьян — они издают громкий боевой
## клич, будто это фаланга копейщиков».
##
## САМ КЛИЧ (battle_cry) рабочим не доставался и раньше: GameManager.
## squad_battle_cry отсеивает отряды без оружия. Слышно было ДРУГОЕ — РЕПЛИКИ
## КОМАНД: «вперёд, марш», «держать строй», рог к атаке. Они раздаются, когда
## приказ считается МАССОВЫМ, а массовость считалась по ЧИСЛУ ОТРЯДОВ — и вот
## тут рабочие и пролезали: каждый из них числится отрядом ИЗ ОДНОГО ЧЕЛОВЕКА
## (учётная единица, иначе его не выделить и не показать в панели). Пять
## крестьян — это ровно пять «отрядов», то есть порог MASS_ORDER_SQUADS.
##
## Считаем поэтому БОЕВЫЕ отряды. Признак берём у GameManager (squad_is_combat)
## — тот же, по которому отсеивается и сам клич: два описания «кто здесь
## солдат» неминуемо разошлись бы
func selected_combat_squad_count() -> int:
	var seen: Dictionary = {}
	for u in selected_units:
		if not is_instance_valid(u) or not (u is Unit):
			continue
		var sid: int = (u as Unit).squad_id
		if sid > 0 and not seen.has(sid) and GameManager.squad_is_combat(sid):
			seen[sid] = true
	return seen.size()
...
## МАССОВЫЙ ЛИ ПРИКАЗ. Считаются только БОЕВЫЕ отряды: реплики команд — это
## голос войска, и обозу они не полагаются (разбор — у
## selected_combat_squad_count)
func _mass_selection() -> bool:
	return selected_combat_squad_count() >= MASS_ORDER_SQUADS
...
## ── СКОЛЬКО ОТРЯДОВ ВООБЩЕ МОЖЕТ БИТЬ ЭТО ЗДАНИЕ ОДНОВРЕМЕННО ──────────────
## Считается не по кольцу подхода, а по РАБОЧЕЙ дуге — той, с которой до стены
## реально дотягиваются оружием. Кольцо подхода лежит на RING_STANDOFF (4.5 м
## от стены), а бьют с длины руки: у хижины гоблинов это дуга примерно в
## десять метров, то есть три отряда по тридцать человек и ни одним больше.
## Раздавать сектора по кольцу ПОДХОДА бессмысленно — дойдя до своей точки,
## отряд всё равно идёт к стене, и там опять все в одной куче
const RING_FIGHT_REACH := 2.0
## Глубина между рядами кольца, метры. Заметно больше просвета между отрядами
## одного ряда: это ВТОРАЯ линия, и ей положено стоять за спинами первой
const RING_ROW_DEPTH := 4.0
## Просвет между соседними секторами одного ряда, метры
const RING_SQUAD_GAP := 1.0

## Шаг между секторами: САМЫЙ ШИРОКИЙ из посланных отрядов плюс просвет.
## Шаг один на все отряды (а не свой каждому) по той же причине, по какой
## ячейка групповой сетки берётся по максимуму: неравные доли дуги пришлось бы
## раздавать после сортировки по азимуту, и порядок «кто слева, тот и встанет
## слева» перестал бы выполняться
func _ring_step(by_squad: Dictionary) -> float:
	var widest := 0.0
	for sid in by_squad:
		var men: Array = by_squad[sid]
		# Квадрат из n человек с интервалом UNIT_SPACING — та же оценка ширины,
		# что и у зажима формы (см. SHAPE_CLAMP_SLACK): корень из численности
		widest = maxf(widest, sqrt(float(maxi(men.size(), 1))) * UNIT_SPACING)
	return widest + RING_SQUAD_GAP

## ── КУДА РАЗЛИВАТЬСЯ ЛИШНИМ ОТРЯДАМ ────────────────────────────────────────
## Радиус, в котором ищется СОСЕДНИЙ ДОМ ТОГО ЖЕ ВИДА для отряда, которому не
## хватило места у первого. Число то же самое, что у перехода замка на соседнюю
## постройку (Unit.BUILDING_CHAIN_RANGE): это «тот же лагерь», а не «любая
## постройка на карте», и оба правила обязаны понимать под лагерем одно и то же
const RING_SPILL_RANGE := 26.0

## ── ЛИШНИЙ ОТРЯД ИДЁТ НА СОСЕДНИЙ ДОМ, А НЕ СТОИТ В ОЧЕРЕДИ ────────────────
## ЖАЛОБА ВЛАДЕЛЬЦА ПО СКРИНШОТУ: «отправляю три отряда на хаты гоблинов — один
## бьёт, а два встают в кольцо и бездействуют».
##
## Так и было ПО ЗАМЫСЛУ, и замысел этот тоже был заказан: «мест у стены
## столько, сколько их есть, остальные ждут» — иначе десять отрядов сходились в
## одну кучу у одной хижины. Но ждать имеет смысл, только когда бить больше
## нечего; в деревне гоблинов хижины стоят сеткой с шагом шесть метров, и рядом
## всегда есть свободный дом.
##
## Поэтому очередь остаётся ПОСЛЕДНИМ средством: сначала лишние отряды
## разливаются по соседним домам ТОГО ЖЕ ВИДА, и только если свободных не
## осталось — встают вторым рядом, как и раньше.
##
## ВОЗВРАЩАЕТ РАСКЛАДКУ `sid → постройка`: цель отряду переписывает вызывающий,
...
func _ring_squads_around(b: Building) -> Dictionary:
	...
	# ── СКОЛЬКО ОТРЯДОВ ВООБЩЕ ПОМЕЩАЕТСЯ НА ДУГЕ ──────────────────────────
	# Жалоба владельца (скриншот): десяток отрядов, посланных на одну хижину,
	# «сбиваются в одну точку, дёргаются и бесконечно перебегают».
	#
	# Так и выходило по арифметике. Дуга делилась на РОВНЫЕ доли по числу
	# отрядов, а её длина от этого числа не зависела вовсе: у хижины радиус
	# кольца 6.5 м, дуга 150° — это 17 м, и на десять отрядов приходилось по
	# 1.7 м на сектор при ширине отряда из пятнадцати человек около трёх.
	# Соседние сектора перекрывались вдвое, отряды приходили друг на друга,
	# расталкивание их разбрасывало, подход вёл обратно — ровно тот же цикл
	# «сжатие → выталкивание → сжатие», что когда-то ловился у заслона ИИ.
	#
	# ЛЕЧИТСЯ ТЕМ ЖЕ, ЧЕМ ТАМ: шаг между секторами ВЫВОДИТСЯ из ширины отряда,
	# а не из числа отрядов. Сначала кольцо раздвигается (до RING_MAX_EXTRA —
	# дальше отряд просто не достанет до стены и будет топтаться), а кто не
	# поместился, уходит во ВТОРОЙ ряд на RING_ROW_DEPTH дальше: он держит
	# позицию за спинами первого, а не лезет в занятое место
	var step: float = _ring_step(by_squad)
	# Вместимость РАБОЧЕЙ дуги: сколько отрядов физически помещается там, где
	# они достают до стены. Всё, что сверх, — лишние (см. разбор в шапке)
	var per_row: int = _ring_capacity(b, step)

	# ── РАЗЛИВАЕМ ЛИШНИХ ПО СОСЕДНИМ ДОМАМ ТОГО ЖЕ ВИДА ────────────────────
	# Порядок обхода — тот же азимутальный: кто подошёл с краю, тот и уйдёт на
	# соседний дом, а серединные останутся на первом. Иначе отряды пересекали
	# бы друг друга, меняясь местами
	var plan: Dictionary = {}          # sid → постройка, которую он бьёт
	var here: Array = []               # кто остаётся у b
	var spill: Array = []              # кому места не хватило
	for i in range(n):
		var sid_i: int = int((order[i] as Array)[0])
		plan[sid_i] = b
		if i < per_row:
			here.append(sid_i)
		else:
	...
	if not spill.is_empty():
		var nearby: Array = _same_kind_buildings_near(b)
		var taken: Dictionary = {}     # постройка → сколько отрядов уже дали
		var left: Array = []
		for sid_s in spill:
			var got: Building = null
			for nb in nearby:
				var bb: Building = nb
				var cap: int = _ring_capacity(bb, step)
				if int(taken.get(bb, 0)) < cap:
					got = bb
					break
			if got == null:
				left.append(sid_s)      # свободных домов нет — встанет в очередь
				continue
			taken[got] = int(taken.get(got, 0)) + 1
			plan[sid_s] = got
		spill = left
	# У первого дома встают те, кто на нём остался, плюс не пристроенные —
	# этим достаются вторая и третья линии, как и раньше
	_ring_place(b, here + spill, per_row, step)
	# И каждому соседнему дому — свои отряды, тем же способом
	var by_house: Dictionary = {}
	for sid_p in plan:
		var house: Building = plan[sid_p]
		if house == b:
			continue
		if not by_house.has(house):
			by_house[house] = []
		(by_house[house] as Array).append(int(sid_p))
	for house2 in by_house:
		var hb: Building = house2
		_ring_place(hb, by_house[house2], _ring_capacity(hb, step), step)
	return plan

## Сколько отрядов помещается у стены этой постройки при таком шаге секторов
func _ring_capacity(b: Building, step: float) -> int:
	return maxi(1, int(floor(RING_ARC * (b.ring_radius() + RING_FIGHT_REACH) / step)))

## ── СОСЕДНИЕ ДОМА ТОГО ЖЕ ВИДА, ОТ БЛИЖНЕГО К ДАЛЬНЕМУ ────────────────────
## «Того же вида» — по building_id, а не «любая вражеская постройка»: отряд,
## посланный сносить лагерь, не должен вместо хижины уйти на замок, который
## игрок сносить не просил. Список короткий (деревня гоблинов — сетка с шагом
## шесть метров), считается ОДИН РАЗ НА КЛИК, покадрового пути здесь нет
func _same_kind_buildings_near(b: Building) -> Array:
	var out: Array = []
	var fac: int = Constants.FACTION_PLAYER
	for u in selected_units:
		if is_instance_valid(u) and u is Unit:
			fac = (u as Unit).faction
			break
	var r2: float = RING_SPILL_RANGE * RING_SPILL_RANGE
	for grp in Constants.enemy_building_groups(fac):
		for node in get_tree().get_nodes_in_group(String(grp)):
			if not is_instance_valid(node):
				continue
			var bb := node as Building
			if bb == null or bb == b or bb.is_dead():
				continue
			if bb.building_id != b.building_id:
				continue
			var dx: float = bb.global_position.x - b.global_position.x
			var dz: float = bb.global_position.z - b.global_position.z
			if dx * dx + dz * dz > r2:
				continue
			out.append(bb)
	out.sort_custom(func(p, q):
		var pp: Vector3 = (p as Building).global_position
		var qq: Vector3 = (q as Building).global_position
		return pp.distance_squared_to(b.global_position) \
			< qq.distance_squared_to(b.global_position))
	return out

## Разложить перечисленные отряды по секторам вокруг постройки.
## Порядок в `sids` значим: он уже отсортирован по азимуту подхода
func _ring_place(b: Building, sids: Array, per_row: int, step: float) -> void:
	if sids.is_empty():
		return
	var c: Vector3 = b.global_position
	# ── КОЛЬЦО ПОДХОДА СЧИТАЕТСЯ ОТ КОРОБКИ, А ДИСТАНЦИЯ УДАРА — ОТ РИСУНКА,
	# И ЭТО НЕ РАССОГЛАСОВАНИЕ ────────────────────────────────────────────
	# Соблазн свести их к одной величине был, и он проверен замером: с
	# ring_radius() кольцо у замка уезжает на 0.33 м дальше, и qa_mass_siege S1
	# краснеет (отставание от своего сектора 3.4 м против 2.8). Дело не в
	# пороге стенда: у двух чисел РАЗНЫЕ задачи.
	#   • ДИСТАНЦИЯ УДАРА отвечает на вопрос «дотягивается ли копьё до того,
	#     что игрок ВИДИТ» — значит, до края РИСУНКА (см. Unit._pad_of, там это
	#     и была жалоба «тыкают копьями в воздух»).
	#   • КОЛЬЦО ПОДХОДА отвечает на другой: «где отряду встать, не влезая в
	#     дом». «Дом» здесь — пятно РАЗМЕЩЕНИЯ, то есть build_size, по которому
	#     считается и сама постановка построек.
	# Зазор RING_STANDOFF = 4.5 м всё равно вчетверо больше их разницы, так что
	# на подходе она не значит ничего, а на дистанции удара значит всё
	var radius: float = maxf(b.build_size.x, b.build_size.z) * 0.5 + RING_STANDOFF
	# Ось подхода считается по ТЕМ, КОГО СЮДА ПОСЛАЛИ: у соседнего дома она
	# своя, и брать её у первого значило бы разворачивать дугу не туда
	var sx := 0.0
	var sz := 0.0
	var cnt := 0
	for sid in sids:
		var sc: Vector3 = GameManager.squad_centroid(int(sid))
		if sc == Vector3.ZERO:
			continue
		sx += sc.x - c.x
		sz += sc.z - c.z
		cnt += 1
	var base_ang: float = atan2(sz, sx) if cnt > 0 else 0.0
	var n: int = sids.size()
	var cap: int = maxi(per_row, 1)
	for i in range(n):
		var sid3: int = int(sids[i])
		var row: int = i / cap
		var in_row: int = i % cap
		# В последнем (неполном) ряду делим дугу на то, сколько в нём реально
		# стоит: иначе два отряда второго ряда жались бы к левому краю
		var row_n: int = mini(cap, n - row * cap)
		var r_i: float = radius + float(row) * RING_ROW_DEPTH
		var ang: float = base_ang + RING_ARC * ((float(in_row) + 0.5) / float(row_n) - 0.5)
		var spot := Vector3(c.x + cos(ang) * r_i, 0.0, c.z + sin(ang) * r_i)
		...
		# row > 0 — ВТОРАЯ ЛИНИЯ: дошёл до своего места и стоит, пока впереди
		# нет просвета. Без этого признака она честно доходила бы до кольца и
		# тут же шла к стене — то есть в ту самую кучу, ради ухода от которой
		# сектора и заводились
		GameManager.squad_set_attack_anchor(sid3, GameManager.land_target(spot), row > 0)
