...
## Насколько растение уступает бойцу по глубине, В СВОИХ РОСТАХ.
## ОТХОД ОТ КАМЕРЫ ТОПИТ ДЕКОРАЦИЮ В ГРУНТ, И ЦЕНА СЧИТАЕТСЯ ТОЧНО.
## Камера смотрит под 45°: на экранной высоте s метров над точкой земли
## нарисован грунт, который лежит ровно на s метров ГЛУБЖЕ (s / cos45 метров
## дальше по земле, и та же s по оси взгляда). Значит отход на P метров прячет
## под грунтом ВСЕ пиксели ниже P экранных метров — не «слегка притапливает», а
## отрезает низ рисунка. Первая версия спринта 15 толкала растение на его
## полный рост (дерево 5.0 м экранной высоты против отхода 7.05 м) — и
## qa_visual_smoke показал «объекта на экране нет вовсе».
##
## ПОЭТОМУ ДЕКОРАЦИЮ НЕ ТОЛКАЮТ ВОВСЕ, А БОЙЦА ПРИПОДНИМАЮТ К КАМЕРЕ
## (mm_unit_sprite, depth_lift): грунт того, кто БЛИЖЕ, перекрыть не может, и
## у подъёма нет этой цены. Ручка оставлена — она законна для того, у кого
## рисунок и так лежит на земле.
const DEPTH_PUSH_HEIGHTS := 0.0
## ═══════════════════════════════════════════════════════════════════════════
...
func _get_or_make(tex: Texture2D, frames: int, aspect: float,
	...
	# ── БОЕЦ РИСУЕТСЯ ПОВЕРХ ТРАВЫ И КУСТОВ (заказ спринта 15) ────────────
	# Глубина куста берётся у его ТОЧКИ НА ЗЕМЛЕ (см. veg_multimesh): до этой
	# правки её не было вовсе, и верхушка куста честно оказывалась ближе
	# собственного корня, перекрывая бойца ПЕРЕД собой. Отхода от камеры
	# сверху нет — цену такого отхода см. у DEPTH_PUSH_HEIGHTS
	mat.set_shader_parameter("ground_depth", 1.0)
	mat.set_shader_parameter("depth_push", DEPTH_PUSH_HEIGHTS)
