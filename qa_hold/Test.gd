...
func _a_wall() -> void:
	...
	# ── ТРЕБОВАНИЕ РАЗВЕРНУЛОСЬ ВТОРОЙ РАЗ (заказ владельца, авг. 2026) ─────
	# История этой проверки короткая и поучительная.
	#   1) Сначала здесь стояло «стойка держит позицию ДАЖЕ по прямому приказу».
	#   2) Потом владелец попросил обратного: «по приказу фаланга идёт вперёд
	#      единым забором», и проверка стала мерить, что строй ДОШЁЛ до точки
	#      приказа и там встал.
	#   3) Теперь требование развёрнуто снова и сформулировано жёстче прежнего:
	#      «в режиме Defense перемещение (Move/Charge) запрещено на уровне 0.0,
	#      разрешён только разворот строя и урон по вошедшему в дальность».
	#      Повод — фланговые копейщики, которые «всё равно срываются с места».
	#
	# Проверяем НОВОЕ требование: приказ атаки по врагу в трёх метрах не
	# сдвигает залоченный строй ВООБЩЕ. Число берём из конфига стойки, а не из
	# себя: вернёт владелец lock_position = false — проверка сама поедет за ним
	var locked: bool = bool(_UStancesRef().get_stance(
		_UStancesRef().STANCE_DEFENSE).get("lock_position", false))
	for u in men:
	...
	if locked:
		verdict("A2 залоченная стойка не идёт даже по прямому приказу атаки",
			chase < 0.25,
			"макс. сдвиг %.2f м (lock_position = true)" % chase)
	else:
		# Прежнее поведение: стена доходит до точки приказа и там встаёт
		var order_gap: float = 3.5
		verdict("A2 стена дошла до точки приказа, а за убежавшим не пошла",
			chase <= order_gap + 0.6 and chase > 0.5,
			"макс. сдвиг %.2f м при разрыве приказа %.1f м" % [chase, order_gap])
	# ── И ОТДЕЛЬНО: РАЗВОРОТ РАЗРЕШЁН, А ХОДЬБА НЕТ ────────────────────────
	# Без этого «замок позиции» нельзя отличить от «боец вообще ничего не
	# делает»: заказ владельца разрешает facing прямо
	var faced := 0
	for u in men:
		var uu := u as Unit
		var to_foe := Vector3(foe.global_position.x - uu.global_position.x, 0.0,
			foe.global_position.z - uu.global_position.z).normalized()
		if uu._facing.dot(to_foe) > 0.5:
			faced += 1
	verdict("A2б строй развернулся на врага",
		faced >= men.size() / 2,
		"смотрят на врага %d из %d" % [faced, men.size()])
	for u in men:
