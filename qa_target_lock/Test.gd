...
═════ F. ЛУЧНИК НЕ ПЕРЕВОДИТ ЗАЛП НА БЛИЖНИХ ═════")
	...
	# ── ПРИМАНКА ОБЯЗАНА СТОЯТЬ, А НЕ ПРИХОДИТЬ БИТЬ В УПОР (сент. 2026) ────
	# В стойке АТАКА её собственное авто-агро законно вело копейщиков на
	# лучников (7 м — в радиусе инициативы), они добегали и били с длины
	# руки — а удар В УПОР под замком законно получает ответ (приоритет №2,
	# см. take_damage: «помеха обязана быть физической»). Проверка мигала не
	# на поломке замка, а на гонке «успеет ли приманка дойти за окно замера».
	# Стойка ЗАЩИТА держит её на месте (авто-агро обороны берёт только то, до
	# чего достаёт копьё) — соблазн для НЕЗАКОННОГО перенацеливания остался,
	# законный ответ на удар из сценария ушёл
