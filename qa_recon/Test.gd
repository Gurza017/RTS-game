...
func _c_grades() -> void:
	...
	# ── ОКРУГЛЯЕМ ТАК ЖЕ, КАК ПАНЕЛЬ ───────────────────────────────────────
	# Здесь стояло int(bonus), то есть ОТБРАСЫВАНИЕ дробной части: узел на
	# +1.5 урона панель показывает как «+2», а стенд искал «+1» и краснел.
	# Дробные прибавки появились вместе с новой лестницей тиров кузницы
	# (см. docs/BALANCE_MATH.md), и округление обязано совпадать с панельным
	verdict("C1 грейд кузницы врага виден в карточке",
		before != after and after.contains("+%d" % roundi(bonus)),
