...
func _test_gold() -> void:
	...
	# ResourceNode удалили — второго, аддитивного квада у жилы в игре нет
	...
	# ── ТРЕБОВАНИЕ РАЗВЕРНУЛОСЬ ВМЕСТЕ С ПЕРЕДЕЛКОЙ РУДЫ (сент. 2026) ───────
	# «Золото плотнее камня» выводилось из «самородки мельче нарисованы»; с
	# привязкой варианта рисунка к классу (Main.ORE_VARIANT_POOLS) крупный
	# самородок КРУПНЕЕ булыжника по арту, а камню размах ужат сильнее
	# (LAYOUT_SPAN_STONE 0.75 против золота 0.82) — порядок радиусов законно
	# перевернулся. Свойство, которое надо стеречь: ОБЕ кучи остаются
	# компактным месторождением — не схлопнуты в точку и не разъехались
	# россыпью по поляне
	verdict("5 обе кучи компактны (не точка и не россыпь по поляне)",
		gr > 0.6 and sr > 0.6 and gr < 2.2 and sr < 2.2,
