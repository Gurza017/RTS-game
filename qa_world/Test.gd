...
func _t2_map() -> void:
	...
	verdict("2g игрок в НИЖНЕМ ЛЕВОМ углу", pa.x < 0.0 and pa.z < 0.0,
		"(%.1f, %.1f)" % [pa.x, pa.z])
	# ТЗ 14.09.2026, п. 3: ИИ переехал на ЛЕВЫЙ берег, в нижний угол
	verdict("2h ИИ в НИЖНЕМ ЛЕВОМ углу (левый берег)", ea.x < 0.0 and ea.z > 0.0,
		"(%.1f, %.1f)" % [ea.x, ea.z])
	...
	verdict("2i базы зеркальны по Z на одном берегу (x равны, z в сумме ноль)",
		absf(pa.x - ea.x) < 0.01 and absf(pa.z + ea.z) < 0.01,
		"игрок (%.2f, %.2f), ИИ (%.2f, %.2f) при сдвиге %.0f" % [pa.x, pa.z, ea.x, ea.z, shift])
