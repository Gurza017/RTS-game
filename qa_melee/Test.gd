...
func _run() -> void:
	...
	# ПЕРЕСЧЁТ — В ТОТ ЖЕ МИГ, ЧТО И РУЧНОЙ СЧЁТ (сент. 2026). melee_counts
	# читает КЭШ возрастом до MELEE_TTL_MS, а бойцы на границе дальности за это
	# время успевают её пересечь: стенд ловил расхождение в единицу (18/19) и
	# винил учёт, хотя проверяет он ФОРМУЛУ учёта, а не свежесть кэша — та
	# заложена в конструкцию нарочно. Пересчитываем принудительно и сверяем в
	# одном кадре
	GameManager._recalc_melee(mine, Time.get_ticks_msec())
	var cnt: Array = GameManager.melee_counts(mine)
	# Считаем то же самое вручную — ТОЙ ЖЕ метрикой, что и учёт: по плоскости
	# XZ, а не в трёх измерениях (учёт читает столбцы ядра, Y в них не входит)
	var eng := 0
	var fre := 0
	for m in _live(mine):
		var u := m as Unit
		...
		if t == null or not is_instance_valid(t):
			...
			var bdx: float = u.global_position.x - t.global_position.x
			var bdz: float = u.global_position.z - t.global_position.z
			if bdx * bdx + bdz * bdz <= u.attack_range * u.attack_range:
				eng += 1
			else:
				fre += 1
	...
	# player_order = true. Здесь стоял голый command_move(point) — то есть
	# ВНУТРЕННИЙ приказ (смыкание рядов, сплочённость, ИИ), а не клик игрока.
	# Разница существенная: приказ игрока взводит замок FORCED_MOVE_SEC и окно
	# прохода сквозь тела, и ровно они дают отряду выбраться из свалки, прежде
	# чем он снова начнёт считать себя упёршимся (см. Unit._forced_move_pass и
	# разбор у признака `wall` в _process_move). Все боевые пути игры
	# (SelectionManager._handle_right_click, _issue_formation_move) передают
	# этот флаг — стенд обязан ходить той же дорогой
	var z_before: float = _mid_z_of(m3)
	for u in _live(m3):
		(u as Unit).command_move(Vector3(70, 0, -50), false, Vector3.ZERO, false, true)
	# ── СРОК ПОДНЯТ, И ЭТО НЕ ПОБЛАЖКА ─────────────────────────────────────
	# Здесь ждали четыре секунды. Двенадцать на двенадцать к моменту приказа
	# СЦЕПЛЕНЫ телами, и первые секунды отряд физически выдавливается из
	# свалки: боец, чей шаг никуда не привёл, по новому правилу обязан
	# драться, а не стоять столбом (заказ владельца — «упёрся в коллизию,
	# значит вступай в бой»). Требование «приказ рвёт связку» от этого не
	# отменяется — оно про ИСХОД: отряд обязан УЙТИ и перестать драться.
	# Его и проверяем, дав отряду время выбраться
	await frames(720)
	var still_fighting := 0
	for u in _live(m3):
		if (u as Unit).attack_target != null:
			still_fighting += 1
	# ── ПОЧЕМУ ЗДЕСЬ НЕТ ПРОВЕРКИ «ОТРЯД УШЁЛ НА N МЕТРОВ» ─────────────────
	# Пробовал добавить и убрал: она мерит не то. Двенадцать на двенадцать
	# СЦЕПЛЕНЫ телами, и пока противник жив и держит, отряд физически не
	# уходит никуда — сколько бы ни ждали. Проверяемое требование другое и
	# оно выше: приказ РВЁТ СВЯЗКУ, то есть ни у кого не остаётся цели.
	# Дистанцию отхода честно меряет qa_disengage (блок A4), где противник
	# отряд не держит
	print("  смещение отряда за время ожидания: %.1f м" % (z_before - _mid_z_of(m3)))
...
## Средняя Z живых бойцов ОТРЯДА: по ней меряется «отряд ушёл»
func _mid_z_of(sid: int) -> float:
	var live := _live(sid)
	if live.is_empty():
		return 0.0
	var acc := 0.0
	for u in live:
		acc += (u as Node3D).global_position.z
	return acc / float(live.size())
