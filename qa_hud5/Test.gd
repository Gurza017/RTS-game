...
func _d_tooltips() -> void:
	...
	# ── D4 ПЕРЕПИСАН: ТРЕБОВАНИЕ РАЗВЁРНУТО ВЛАДЕЛЬЦЕМ ──────────────────────
	# Было «окно отцентровано по наведённой иконке». Жалоба со скриншотом панели
	# Замка: кнопки найма прижаты к ПРАВОМУ краю панели, карточка вдвое шире
	# кнопки — и центрированная по ней, она вылезала на середину экрана, ни к
	# чему не привязанная. Новое правило: левая кромка окна совпадает с левой
	# кромкой нижней панели (см. HUD._tip_anchor_geometry).
	# Проверяется СВОЙСТВО, а не число: сравниваем две живые кромки
	verdict("D4 окно выровнено по левому краю нижней панели",
		absf(tr.position.x - pr.position.x) < 2.0,
		"левый край окна %.0f, левый край панели %.0f" % [tr.position.x, pr.position.x])
