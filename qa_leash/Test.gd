...
func _run() -> void:
	...
	# ── ПОЛОВИНА ЗАПАСА, А НЕ ПЯТЬДЕСЯТ ОЧКОВ ──────────────────────────────
	# Ослабление нужно, чтобы заслон был настоящей помехой, а не мишенью.
	# Здесь стояла ПЯТЬДЕСЯТ — половина от прежних семидесяти. После
	# балансного захода база 180, и пятьдесят стали ЧЕТВЕРТЬЮ: шестнадцать
	# нападающих гибли все до одного, не досняв заслон (замер: живых 0 из 16,
	# заслон 2 из 8). Доля от базы едет за балансом сама.
	#
	# ЧЕГО ЭТА СТРОКА БОЛЬШЕ НЕ ДАЁТ: прежний комментарий обещал, что заслон
	# выбьет часть отряда и тем проверит смыкание рядов ПО ГИБЕЛИ. С новым TTK
	# шестнадцать против восьми — размен настолько односторонний, что при любой
	# доле либо не гибнет никто, либо гибнут все (0.35 дало «заслон 2 из 8, свои
	# 16 из 16», 0.28 — «свои 0 из 16»). Подгонять долю до узкой щели нельзя:
	# это была бы проверка, держащаяся на удаче прогона. Смыкание по гибели
	# проверяет qa_combat_lock (блоки 2 и 4), где оно и есть предмет стенда
	for u in col:
		var half: float = (u as Unit).max_health * 0.5
		(u as Unit).max_health = half
		(u as Unit).current_health = half
	...
	# ── СРОК ПОДНЯТ ВСЛЕД ЗА TTK ───────────────────────────────────────────
	# Здесь стояло 45 секунд. Запас жизни поднят почти вдвое (балансный заход,
	# см. docs/BALANCE_MATH.md): восемь бойцов заслона теперь и сносятся вдвое
	# дольше. Срок — это НЕ проверяемое свойство, а бюджет ожидания, и он
	# обязан ехать за длительностью боя
	...
	# ПОРОГ ПОДНЯТ ВСЛЕД ЗА ДЛИТЕЛЬНОСТЬЮ БОЯ. Откат копится ВСЮ схватку
	# (толчки, расталкивание, смыкание), а схватка стала вдвое длиннее —
	# прежние шесть метров судили бы уже не «возню», а её продолжительность
	verdict("L3 откат от максимума в пределах строевой возни (<9 м)", worst_regress < 9.0,
