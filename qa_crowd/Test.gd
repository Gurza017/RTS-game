...
func _test_tree_escape() -> void:
	...
	# ── ЖДЁМ ФИЗКАДРЫ, А НЕ СЕКУНДЫ ─────────────────────────────────────────
	# Здесь стоял бюджет в 8000 мс по стенным часам, и это ошибка того же рода,
	# от которой предостерегает правило проекта: физика идёт фиксированные
	# 60 Гц, а headless крутит кадры без ограничения — сколько ФИЗИЧЕСКИХ тиков
	# уместится в восемь секунд, зависит от того, чем ещё занят кадр и насколько
	# загружена машина. Стенд краснел не от поломки обхода препятствия, а от
	# того, что боец не успевал доехать: «дошёл: false», остаток то 0.61, то
	# 0.85 м — то есть его застали НА ХОДУ.
	# Восемь секунд при 60 Гц — это 480 физических тиков; их и ждём
	var arrived := false
	for _i in range(480):
		await get_tree().physics_frame
