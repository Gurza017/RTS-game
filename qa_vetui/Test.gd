...
func _b_axis() -> void:
	...
	# ── КАРТОЧКА КОМПАКТНАЯ (ТЗ-B 19.09.2026, п. 3) — разворот «одной ширины»:
	# таблица статов уже панели отряда (STAT_COMPACT_W) и стоит над её левым
	# краем; правый край нижней панели за карточку выступать вправе
	verdict("B4 карточка статов не шире панели отряда (компактная, STAT_COMPACT_W)",
		stats.size.x <= panel.size.x + 0.51 and absf(stats.size.x - float(hud.STAT_COMPACT_W)) < 0.51,
